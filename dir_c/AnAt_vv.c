...
void *get_AnAt_vv__run(void *vp)
{
  /* This function serves as a module which calculates the matrix-matrix product AnAt, where both An and Zt==An are treated as +1/-1 values ;
  ...
  */
  ...
  struct L_handle *output_AnAt = (struct L_handle *)(vpra[ip++]);
  ...
  int mx_j=0,mx_chunk=0,ma_a=0,ma_b=0,ma_j=0,mz_a=0,mz_b=0,mz_j=0,nc=0,tab_An_ajdk=0,tab_r=0,tab_r_stride=0,tab_Zn_ajdk=0,tab_c=0,tab_c_stride=0,ta2_r=0,ta2_c=0;
  ...
  if (verbose>1){ printf(" %% calculating output_AnAt\n");}
  output_AnAt->spacing_row = output_spacing_r; output_AnAt->row_stride = tab_r_stride;
  output_AnAt->spacing_col = output_spacing_c; output_AnAt->col_stride = tab_c_stride;
  fill_uchar_zero((unsigned char *)(output_AnAt->lf),tab_r_stride*tab_c_stride*sizeof(double));
  if (strstr(GLOBAL_skip,"AnAt_vv")){ goto skip_AnAt_vv;}
  ...
  if (GLOBAL_omp_type==GLOBAL_omp_off){
	...
	switch (output_spacing_c){ case SPACING_j: ta2_c=ma_j; break; case SPACING_b: ta2_c=ma_b; break; case SPACING_a: ta2_c=ma_a; break; default: break; /* switch (output_spacing_r){ } */}
	mz_j=mz_block*zper;
	while (mz_j<minimum(M_Zn->rpop_j,mz_block*zper+zper)){
	  ...
	  if (ma_j<=mz_j){
	    ...
	    switch (output_spacing_r){ case SPACING_j: ta2_r=mz_j; break; case SPACING_b: ta2_r=mz_b; break; case SPACING_a: ta2_r=mz_a; break; default: break; /* switch (output_spacing_c){ } */}
	    ...
	    output_AnAt->lf[tab_r + tab_c*tab_r_stride] = output_tmp;
	    output_AnAt->lf[ta2_r + ta2_c*tab_r_stride] = output_tmp;
	    /* if (ma_j<=mz_j){ } */}
...
#pragma omp parallel private(mz_block,ma_j,ma_a,ma_b,mz_j,mz_a,mz_b,tab_An_ajdk,tab_Zn_ajdk,tab_r,tab_c,ta2_r,ta2_c,wA_tag,wZ_tag,mc_tag,mc_end,dinp,dtmp,output_tmp)
    { /* begin omp parallel */
...
#pragma omp for schedule(dynamic)
    ...
    /* else if (GLOBAL_omp_type==GLOBAL_omp__on){ } */}
 skip_AnAt_vv:
  if (verbose>1){ printf(" %% [finished get_AnAt_vv__run] tidx %d \n",tidx);}
  return NULL;
}
...
{
  /* This function uses the M_handles M_A and M_Zn to run a series of parallel calls to get_AnAt_vv__run ; 
  ...
  if (verbose){ printf(" %% [entering wrap_AnAt_vv__run] tidx %d type_flag %d\n",*tidx,type_flag);}
  ...
  if (*output_AnAt_p==NULL){ if (verbose){ printf(" %% allocating output size %llu*%d\n",length,(int)sizeof(double));} *output_AnAt_p = L_handle_make(length);}
  ...
  length = length_r*length_c; if ((*output_AnAt_p)->length<length){ printf(" %% Warning! length %llu<%llu in wrap_AnAt_vv__run\n",(*output_AnAt_p)->length,length);} memset((*output_AnAt_p)->lf,0,length*sizeof(double));
  ip=0; vpra[ip++] = tidx; vpra[ip++] = M_An; vpra[ip++] = M_Zn; vpra[ip++] = A_ajdk; vpra[ip++] = lf_An_ajdk; vpra[ip++] = lf_Zn_ajdk; vpra[ip++] = *output_AnAt_p;
  ...
  if (*tidx>0){ if (pthread_create(thread_in,NULL,&get_AnAt_vv__run,vpra)){ printf("Warning! cannot create thread %d in wrap_AnAt_vv__run\n",*tidx);}}
  else /* if (*tidx<=0) */{ get_AnAt_vv__run(vpra);} /* must join threads later */;
  if (verbose>2){ raprintf((*output_AnAt_p)->lf,"double",length_r,length_c," %% (*output_AnAt_p)->lf: ");}
  if (verbose){ printf(" %% [finished wrap_AnAt_vv__run] tidx %d\n",*tidx);}
  return length;
}  
...
{
  /* test for errors with input file: AnAt_vv_error.in ;
     GLOBAL_verbose= 1;
     GLOBAL_thread_count= 8;
     GLOBAL_omp_type= 0;
     GLOBAL_TEST_TYPE= AnAt_vv;
     ...
     END= 0;
  */
  /* test for speed with input file: AnAt_vv_speed.in ;
     GLOBAL_verbose= 1;
     GLOBAL_thread_count= 8;
     GLOBAL_omp_type= 0;
     GLOBAL_TEST_TYPE= AnAt_vv;
  ...
  */
  ...
  struct L_handle **lf_AnAt_vv=NULL,**lf_ZnZt_vv=NULL; int *length_AnAt_vv=NULL,*length_ZnZt_vv=NULL;
  struct L_handle **lf_AnAt_uu=NULL,**lf_ZnZt_uu=NULL; int *length_AnAt_uu=NULL,*length_ZnZt_uu=NULL;
  if (verbose){ printf(" %% [entering wrap_AnAt_vv_test]\n");}
  ...
  lf_AnAt_vv = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_AnAt_vv = (int *)wkspace_all0c(sizeof(int)*nbins);
  lf_ZnZt_vv = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_ZnZt_vv = (int *)wkspace_all0c(sizeof(int)*nbins);
  for (nb=0;nb<nbins;nb++){
    lf_AnAt_vv[nb] = L_handle_make((unsigned long long int)M_An[nb]->nrows*(unsigned long long int)M_An[nb]->nrows);
    lf_ZnZt_vv[nb] = L_handle_make((unsigned long long int)M_Zn[nb]->nrows*(unsigned long long int)M_Zn[nb]->nrows);
    /* for (nb=0;nb<nbins;nb++){ } */}
  if (error_check){  
    lf_AnAt_uu = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_AnAt_uu = (int *)wkspace_all0c(sizeof(int)*nbins);
    lf_ZnZt_uu = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_ZnZt_uu = (int *)wkspace_all0c(sizeof(int)*nbins);
    for (nb=0;nb<nbins;nb++){
      lf_AnAt_uu[nb] = L_handle_make((unsigned long long int)M_An[nb]->nrows*(unsigned long long int)M_An[nb]->nrows);
      lf_ZnZt_uu[nb] = L_handle_make((unsigned long long int)M_Zn[nb]->nrows*(unsigned long long int)M_Zn[nb]->nrows);
      /* for (nb=0;nb<nbins;nb++){ } */}
    /* if (error_check){ } */}
  for (n_type=1;n_type<=2;n_type++){ for (n_spacing_B=0;n_spacing_B<=2;n_spacing_B++){ for (n_spacing_A=0;n_spacing_A<=2;n_spacing_A++){
	...
	GLOBAL_nf_cur=0; GLOBAL_nf_opn=0;
  	for (nl=0;nl<iteration_max;nl++){ for (nb=0;nb<nbins;nb++){
  	  GLOBAL_pthread_tic(); 
  	  length_AnAt_vv[nb] = wrap_AnAt_vv__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_type,n_spacing_B,n_spacing_B,M_An[nb],M_An[nb],A_ajdk,lf_An_ajdk[nb],lf_An_ajdk[nb],&(lf_AnAt_vv[nb]));
  	  GLOBAL_pthread_toc();
  	  GLOBAL_pthread_tic(); 
  	  length_ZnZt_vv[nb] = wrap_AnAt_vv__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_type,n_spacing_B,n_spacing_B,M_Zn[nb],M_Zn[nb],A_ajdk,lf_Zn_ajdk[nb],lf_Zn_ajdk[nb],&(lf_ZnZt_vv[nb]));
  	  GLOBAL_pthread_toc();
  	  /* for (nl=0;nl<iteration_max;nl++){ for (nb=0;nb<nbins;nb++){ }} */}}
  	GLOBAL_pthread_tuc(); 
	GLOBAL_ops_addup_all(); GLOBAL_ops_printf_all(verbose && !error_check," %% AnAt_vv: ");
	...
	if (error_check){
	  ...
	  for (nb=0;nb<nbins;nb++){
	    GLOBAL_pthread_tic(); 
	    length_AnAt_uu[nb] = wrap_AnZt_uu__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_type,n_spacing_B,n_spacing_B,M_An[nb],M_An[nb],A_ajdk,&(lf_AnAt_uu[nb]));
	    GLOBAL_pthread_toc();
	    GLOBAL_pthread_tic(); 
	    length_ZnZt_uu[nb] = wrap_AnZt_uu__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_type,n_spacing_B,n_spacing_B,M_Zn[nb],M_Zn[nb],A_ajdk,&(lf_ZnZt_uu[nb]));
	    GLOBAL_pthread_toc();
	    /* for (nb=0;nb<nbins;nb++){ } */}
	  GLOBAL_pthread_tuc(); 
	  GLOBAL_ops_addup_all(); GLOBAL_ops_printf_all(verbose && !error_check," %% AnAt_uu: ");
	  GLOBAL_ops_toc(-1,0,verbose && !error_check," %% total time: ");
	  /*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
	  for (nb=0;nb<nbins;nb++){
	    printf(" %% lf_AnAt_vv[%d] error %0.16f\n",nb,dra_diff(lf_AnAt_vv[nb]->lf,lf_AnAt_uu[nb]->lf,length_AnAt_vv[nb],1));
	    printf(" %% lf_ZnZt_vv[%d] error %0.16f\n",nb,dra_diff(lf_ZnZt_vv[nb]->lf,lf_ZnZt_uu[nb]->lf,length_ZnZt_vv[nb],1));
	    /* for (nb=0;nb<nbins;nb++){ } */}
	  /* if (error_check){ } */}
  	/* for (n_type=1;n_type<=2;n_type++){ for (n_spacing_B=0;n_spacing_B<=2;n_spacing_B++){ for (n_spacing_A=0;n_spacing_A<=2;n_spacing_A++){ }}} */}}}
  wkspace_printf();
  if (verbose){ printf(" %% [finished wrap_AnAt_vv_test]\n");}
}
