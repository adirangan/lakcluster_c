...
void wrap_AttYn____WtsZn_vv_test()
{
  ...
  */
  ...
  for (nb=0;nb<nbins;nb++){
    lf_ZtsWn[nb] = L_handle_make(M_Zt[nb]->nrows*M_Wt[nb]->nrows);
    lf_AttYn[nb] = L_handle_make(M_At[nb]->nrows*M_Yt[nb]->nrows);
    lf_ZtsZn[nb] = L_handle_make(M_Zt[nb]->nrows*M_Zt[nb]->nrows);
    lf_AttAn[nb] = L_handle_make(M_At[nb]->nrows*M_At[nb]->nrows);
    /* for (nb=0;nb<nbins;nb++){ } */}
