struct L_handle *L_handle_make(int length)
{
...
void L_zero(struct L_handle *L){ int nl=0; for (nl=0;nl<L->length;nl++){ L->lf[nl]=0;}}
...
double *L3_get(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_get\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_get\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_get\n",tab_l,L->lyr_stride);}
  return &(L->lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride]);
}

double *L2_get(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a)
{
  int tab_r=0,tab_c=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L2_get: nr: %d,%d,%d \n",nr_j,nr_b,nr_a);}
  if (nc_j>nc_b || nc_j>nc_a || nc_b>nc_a){ printf(" %% Warning! improper indices in L2_get: nc: %d,%d,%d \n",nc_j,nc_b,nc_a);}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_get\n",tab_r,L->row_stride); exit(1);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_get\n",tab_c,L->col_stride); exit(1);}
  return &(L->lf[tab_r + tab_c*L->row_stride]);
}

double *L1_get(struct L_handle *L,int nr_j,int nr_b,int nr_a)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_get\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_get\n",tab_r,L->row_stride);}
  return &(L->lf[tab_r]);
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

void L3_set(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a,double val)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  switch (L->spacing_lyr){ case SPACING_j: tab_l = nl_j; break; case SPACING_b: tab_l = nl_b; break; case SPACING_a: tab_l = nl_a; break; default: break; /* switch (L->spacing_lyr){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_set\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_set\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_set\n",tab_l,L->lyr_stride);}
  L->lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride] = val;
}

void L2_set(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,double val)
{
  int tab_r=0,tab_c=0;
  ...
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_set\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_set\n",tab_c,L->col_stride);}
  L->lf[tab_r + tab_c*L->row_stride] = val;
}

void L1_set(struct L_handle *L,int nr_j,int nr_b,int nr_a,double val)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_set\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_set\n",tab_r,L->row_stride);}
  L->lf[tab_r] = val;
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

void L3_plusequals(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a,double val)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  switch (L->spacing_lyr){ case SPACING_j: tab_l = nl_j; break; case SPACING_b: tab_l = nl_b; break; case SPACING_a: tab_l = nl_a; break; default: break; /* switch (L->spacing_lyr){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_plusequals\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_plusequals\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_plusequals\n",tab_l,L->lyr_stride);}
  L->lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride] += val;
}

void L2_plusequals(struct L_handle *L,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,double val)
{
  int tab_r=0,tab_c=0;
  ...
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_plusequals\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_plusequals\n",tab_c,L->col_stride);}
  L->lf[tab_r + tab_c*L->row_stride] += val;
}

void L1_plusequals(struct L_handle *L,int nr_j,int nr_b,int nr_a,double val)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_plusequals\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_plusequals\n",tab_r,L->row_stride);}
  L->lf[tab_r] += val;
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

double *L3_lf_get(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  switch (L->spacing_lyr){ case SPACING_j: tab_l = nl_j; break; case SPACING_b: tab_l = nl_b; break; case SPACING_a: tab_l = nl_a; break; default: break; /* switch (L->spacing_lyr){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_lf_get\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_lf_get\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_lf_get\n",tab_l,L->lyr_stride);}
  return &(lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride]);
}

double *L2_lf_get(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a)
{
  int tab_r=0,tab_c=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L2_lf_get: nr: %d,%d,%d \n",nr_j,nr_b,nr_a);}
  if (nc_j>nc_b || nc_j>nc_a || nc_b>nc_a){ printf(" %% Warning! improper indices in L2_lf_get: nc: %d,%d,%d \n",nc_j,nc_b,nc_a);}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_lf_get\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_lf_get\n",tab_c,L->col_stride);}
  return &(lf[tab_r + tab_c*L->row_stride]);
}

double *L1_lf_get(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_lf_get\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_lf_get\n",tab_r,L->row_stride);}
  return &(lf[tab_r]);
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

void L3_lf_set(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a,double val)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  switch (L->spacing_lyr){ case SPACING_j: tab_l = nl_j; break; case SPACING_b: tab_l = nl_b; break; case SPACING_a: tab_l = nl_a; break; default: break; /* switch (L->spacing_lyr){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_lf_set\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_lf_set\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_lf_set\n",tab_l,L->lyr_stride);}
  lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride] = val;
}

void L2_lf_set(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,double val)
{
  int tab_r=0,tab_c=0;
  ...
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_lf_set\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_lf_set\n",tab_c,L->col_stride);}
  lf[tab_r + tab_c*L->row_stride] = val;
}

void L1_lf_set(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,double val)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_lf_set\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_lf_set\n",tab_r,L->row_stride);}
  lf[tab_r] = val;
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

void L3_lf_plusequals(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,int nl_j,int nl_b,int nl_a,double val)
{
  int tab_r=0,tab_c=0,tab_l=0;
  ...
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  switch (L->spacing_lyr){ case SPACING_j: tab_l = nl_j; break; case SPACING_b: tab_l = nl_b; break; case SPACING_a: tab_l = nl_a; break; default: break; /* switch (L->spacing_lyr){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L3_lf_plusequals\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L3_lf_plusequals\n",tab_c,L->col_stride);}
  if (tab_l<0 || tab_l>=L->lyr_stride){ printf(" %% Warning! tab_l %d/%d in L3_lf_plusequals\n",tab_l,L->lyr_stride);}
  lf[tab_r + tab_c*L->row_stride + tab_l*L->row_stride*L->col_stride] += val;
}

void L2_lf_plusequals(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,int nc_j,int nc_b,int nc_a,double val)
{
  int tab_r=0,tab_c=0;
  ...
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  switch (L->spacing_col){ case SPACING_j: tab_c = nc_j; break; case SPACING_b: tab_c = nc_b; break; case SPACING_a: tab_c = nc_a; break; default: break; /* switch (L->spacing_col){ } */}  
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L2_lf_plusequals\n",tab_r,L->row_stride);}
  if (tab_c<0 || tab_c>=L->col_stride){ printf(" %% Warning! tab_c %d/%d in L2_lf_plusequals\n",tab_c,L->col_stride);}
  lf[tab_r + tab_c*L->row_stride] += val;
}

void L1_lf_plusequals(struct L_handle *L,double *lf,int nr_j,int nr_b,int nr_a,double val)
{
  int tab_r=0;
  if (nr_j>nr_b || nr_j>nr_a || nr_b>nr_a){ printf(" %% Warning! improper indices in L1_lf_plusequals\n");}
  switch (L->spacing_row){ case SPACING_j: tab_r = nr_j; break; case SPACING_b: tab_r = nr_b; break; case SPACING_a: tab_r = nr_a; break; default: break; /* switch (L->spacing_row){ } */}
  if (tab_r<0 || tab_r>=L->row_stride){ printf(" %% Warning! tab_r %d/%d in L1_lf_plusequals\n",tab_r,L->row_stride);}
  lf[tab_r] += val;
}

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */

void L2_clean(struct L_handle *L,int nrows,unsigned char *mr_b,unsigned char *mr_j,int ncols,unsigned char *mc_b,unsigned char *mc_j)
{
  int nr_j=0,nr_b=0,nr_a=0; int nc_j=0,nc_b=0,nc_a=0; int tab_r=0,tab_c=0,tab_x=0;
...
void LL2_plustimesequals(struct L_handle *LA,struct L_handle *LB,double dtmp,int nrows,unsigned char *mr_b,unsigned char *mr_j,int ncols,unsigned char *mc_b,unsigned char *mc_j)
{
  int nr_a=0,nr_b=0,nr_j=0,nc_a=0,nc_b=0,nc_j=0;
  nc_j=0;nc_b=0;nc_a=0;
  while (nc_a<ncols){
    if (bget__on(mc_b,nc_a)){
      if (bget__on(mc_j,nc_a)){
...
/* void LL2_plustimesequals(struct L_handle *LA,struct L_handle *LB,int rpop_j,unsigned int *m_a_,unsigned int m_b_,int cpop_j,unsigned int *n_a_,unsigned int *n_b_,double dtmp) */
/* { */
/*   int nr_a=0,nr_b=0,nr_j=0,nc_a=0,nc_b=0,nc_j=0; */
/*   nc_j=0; */
/*   while (nc_j<cpop_j){ */
/*     nc_a = n_a_[nc_j]; nc_b = n_b_[nc_j]; */
/*     nr_j=0; */
/*     while (nr_j<rpop_j){ */
/*       nr_a = m_a_[nr_j]; nr_b = m_b_[nr_j]; */
/*       L2_plusequals(LA , nr_j,nr_b,nr_a , nc_j,nc_b,nc_a , dtmp*(*L2_get(LB , nr_j,nr_b,nr_a , nc_j,nc_b,nc_a)); */
/*       nr_j++; /\* while (nr_j<rpop_j){ } *\/} */
/*     nc_j++; /\* while (nc_j<cpop_j){ } *\/} */
/* } */
