...
{
  /* This function takes in M_At,M_Tt,M_Yt and the T-column-number ns_j and calculates :
     output_AttYn[na+ny*A_n_cols] = (At(na,:)-a_An*e_At)*diag(T(:,ns))*(Yn(:,ny)-e_An*a_Yn) = ;
  ...
  */
  ...
  int mx_j=0,mx_chunk=0,nt_j=0,nt_b=0,nt_a=0,tab_x=0;
  ...
  fill_uchar_zero((unsigned char *)(output_AttYn->lf),tab_a_stride*tab_y_stride*sizeof(double));
  ...
  if (GLOBAL_omp_type==GLOBAL_omp_off){
    ...
    GLOBAL_ops_count_one(tidx,M_At->rpop_j*3,M_At->rpop_j*M_At->mc_length*BIT8);
    /* if (GLOBAL_omp_type==GLOBAL_omp_off){ } */}
  if (GLOBAL_omp_type==GLOBAL_omp__on){
    ny_block_max = rup(M_Yt->rpop_j,yper)/yper;
    nt_j = ns_j; nt_a = nt_a_[nt_j]; nt_b = nt_b_[nt_j];
#pragma omp parallel private(mx_j,ny_block,na_j,na_a,na_b,ny_j,ny_a,ny_b,tab_y,tab_a,tab_x,hhay,hhaY,hhAy,hhAY,wAt_tag,wTt_tag,mctx_tag,mctx_end,wYt_tag,mcay_tag,mcay_end)
    { /* begin omp parallel */
...
int wrap_AttYn_vv__run(int *tidx,void **vpra,pthread_t *thread_in,int output_spacing_a,int output_spacing_y,struct M_handle *M_At,struct M_handle *M_Tt,int ns_j,struct M_handle *M_Yt,double *A_ajdk,double *Y_ajdk,struct L_handle *lf_AtTn,struct L_handle *lf_YtTn,struct L_handle **output_AttYn_p)
{
  ...
  length = length_a*length_y; if (*output_AttYn_p==NULL){ if (verbose){ printf(" %% allocating output size %llu*%d\n",length,(int)sizeof(double));} *output_AttYn_p = L_handle_make(length);}
  ...
  length = length_a*length_y; if ((*output_AttYn_p)->length<length){ printf(" %% Warning! length %llu<%llu in wrap_AttYn_vv__run\n",(*output_AttYn_p)->length,length);} memset((*output_AttYn_p)->lf,0,length*sizeof(double));
  if (ns_j<0 || ns_j>addressable_int_length-1){ printf(" %% Warning! ns_j %d in wrap_AttYn_vv_run\n",ns_j);}
  ip=0;
  vpra[ip++] = tidx; vpra[ip++] = M_At; vpra[ip++] = M_Tt; vpra[ip++] = &(addressable_int[ns_j]); vpra[ip++] = M_Yt; vpra[ip++] = A_ajdk; vpra[ip++] = Y_ajdk; vpra[ip++] = lf_AtTn; vpra[ip++] = lf_YtTn; vpra[ip++] = *output_AttYn_p; 
