...
{
  /* This function takes in M_At,M_Tt,M_Yt==M_At and calculates :
  ...
  */
  ...
  int mx_j=0,mx_chunk=0,nt_j=0,nt_b=0,nt_a=0,tab_t_stride=0,tab_t=0,tab_x=0,ta2_a=0,ta2_y=0,ta2_x=0;
  ...
  output_AtTAn->spacing_lyr = output_spacing_t; output_AtTAn->lyr_stride = tab_t_stride;
  fill_uchar_zero((unsigned char *)(output_AtTAn->lf),tab_a_stride*tab_y_stride*tab_t_stride*sizeof(double));
	...
	while (na_j<M_At->rpop_j){
	  ...
	  while (ny_j<minimum(M_Yt->rpop_j,ny_block*yper+yper)){
	    ...
	    if (na_j<=ny_j){
	      ...
	      ta2_x = ta2_a + ta2_y*tab_a_stride + tab_t*tab_a_stride*tab_y_stride;
  ...
  if (GLOBAL_omp_type==GLOBAL_omp__on){
    ny_block_max = rup(M_Yt->rpop_j,yper)/yper;
#pragma omp parallel private(mx_j,ny_block,nt_j,nt_a,nt_b,na_j,na_a,na_b,ny_j,ny_a,ny_b,tab_t,tab_y,tab_a,tab_x,ta2_y,ta2_a,ta2_x,hhay,hhaY,hhAy,hhAY,wAt_tag,wTt_tag,mctx_tag,mctx_end,wYt_tag,mcay_tag,mcay_end,output_tmp)
    { /* begin omp parallel */
...
{
  /* This function uses the M_handles M_At, M_Tt and M_Yt to run a series of parallel calls to get_AtTAn_vv ; 
  ...
  length = length_a*length_y*length_t; if (*output_AtTAn_p==NULL){ if (verbose){ printf(" %% allocating output size %llu*%d\n",length,(int)sizeof(double));} *output_AtTAn_p = L_handle_make(length);}
  ...
  length = length_a*length_y*length_t; if ((*output_AtTAn_p)->length<length){ printf(" %% Warning! length %llu<%llu in wrap_AtTAn_vv__run\n",(*output_AtTAn_p)->length,length);} memset((*output_AtTAn_p)->lf,0,length*sizeof(double));
  ip=0;
  vpra[ip++] = tidx; vpra[ip++] = M_At; vpra[ip++] = M_Tt; vpra[ip++] = M_Yt; vpra[ip++] = A_ajdk; vpra[ip++] = Y_ajdk; vpra[ip++] = lf_AtTn; vpra[ip++] = lf_YtTn; vpra[ip++] = *output_AtTAn_p; 
...
{
  /* test for errors with input file: AtTAn_vv_error.in ;
     GLOBAL_verbose= 1;
     GLOBAL_thread_count= 8;
     GLOBAL_omp_type= 0;
     GLOBAL_TEST_TYPE= AtTAn_vv;
     ...
     END= 0;
  */
  /* test for speed with input file: AtTAn_vv_speed.in ;
     GLOBAL_verbose= 1;
     GLOBAL_thread_count= 8;
     GLOBAL_omp_type= 0;
     GLOBAL_TEST_TYPE= AtTAn_vv;
  ...
  */
  ...
  struct L_handle **lf_AtTAn_vv=NULL,**lf_ZtSZn_vv=NULL; int *length_AtTAn_vv=NULL,*length_ZtSZn_vv=NULL;
  struct L_handle **lf_AtTAn_uu=NULL,**lf_ZtSZn_uu=NULL; int *length_AtTAn_uu=NULL,*length_ZtSZn_uu=NULL;
  if (verbose){ printf(" %% [entering wrap_AtTAn_vv_test]\n");}
  ...
  for (nb=0;nb<nbins;nb++){
    ...
    /* for (nb=0;nb<nbins;nb++){ } */}
  lf_AtTAn_vv = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_AtTAn_vv = (int *)wkspace_all0c(sizeof(int)*nbins);
  lf_ZtSZn_vv = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_ZtSZn_vv = (int *)wkspace_all0c(sizeof(int)*nbins);
  for (nb=0;nb<nbins;nb++){
    lf_AtTAn_vv[nb] = L_handle_make((unsigned long long int)M_An[nb]->ncols*(unsigned long long int)M_Yn[nb]->ncols*(unsigned long long int)M_Tn[nb]->ncols);
    lf_ZtSZn_vv[nb] = L_handle_make((unsigned long long int)M_Zn[nb]->ncols*(unsigned long long int)M_Wn[nb]->ncols*(unsigned long long int)M_Sn[nb]->ncols);
    /* for (nb=0;nb<nbins;nb++){ } */}
  if (error_check){ 
    lf_AtTAn_uu = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_AtTAn_uu = (int *)wkspace_all0c(sizeof(int)*nbins);
    lf_ZtSZn_uu = (struct L_handle **)wkspace_all0c(sizeof(struct L_handle *)*nbins); length_ZtSZn_uu = (int *)wkspace_all0c(sizeof(int)*nbins);
    for (nb=0;nb<nbins;nb++){
      lf_AtTAn_uu[nb] = L_handle_make((unsigned long long int)M_An[nb]->ncols*(unsigned long long int)M_Yn[nb]->ncols*(unsigned long long int)M_Tn[nb]->ncols);
      lf_ZtSZn_uu[nb] = L_handle_make((unsigned long long int)M_Zn[nb]->ncols*(unsigned long long int)M_Wn[nb]->ncols*(unsigned long long int)M_Sn[nb]->ncols);
      /* for (nb=0;nb<nbins;nb++){ } */}
    /* if (error_check){ } */}
  for (n_type=1;n_type<=1;n_type++){ for (n_spacing_B=0;n_spacing_B<=2;n_spacing_B++){ for (n_spacing_A=0;n_spacing_A<=2;n_spacing_A++){
	...
	for (nl=0;nl<iteration_max;nl++){ for (nb=0;nb<nbins;nb++){
	  GLOBAL_pthread_tic();
	  length_AtTAn_vv[nb] = wrap_AtTAn_vv__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_spacing_B,n_spacing_B,n_spacing_B,M_At[nb],M_Tt[nb],M_At[nb],A_ajdk,A_ajdk,lf_AtTn[nb],lf_AtTn[nb],&(lf_AtTAn_vv[nb]));
	  GLOBAL_pthread_toc();
	  GLOBAL_pthread_tic();
	  length_ZtSZn_vv[nb] = wrap_AtTAn_vv__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_spacing_B,n_spacing_B,n_spacing_B,M_Zt[nb],M_St[nb],M_Zt[nb],A_ajdk,A_ajdk,lf_ZtSn[nb],lf_ZtSn[nb],&(lf_ZtSZn_vv[nb]));
	  GLOBAL_pthread_toc();
	  /* for (nl=0;nl<iteration_max;nl++){ for (nb=0;nb<nbins;nb++){ }} */}}
	GLOBAL_pthread_tuc(); 
	GLOBAL_ops_addup_all(); GLOBAL_ops_printf_all(verbose && !error_check," %% AtTAn_vv: ");
	...
	if (error_check){	
	  ...
	  for (nb=0;nb<nbins;nb++){
	      GLOBAL_pthread_tic();
	      length_AtTAn_uu[nb] = wrap_AtTYn_uu__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_spacing_B,n_spacing_B,n_spacing_B,M_At[nb],M_Tt[nb],M_At[nb],A_ajdk,A_ajdk,&(lf_AtTAn_uu[nb]));
	      GLOBAL_pthread_toc();
	      GLOBAL_pthread_tic();
	      length_ZtSZn_uu[nb] = wrap_AtTYn_uu__run(&(GLOBAL_tint[GLOBAL_nf_cur]),GLOBAL_tvp[GLOBAL_nf_cur],&(GLOBAL_threads[GLOBAL_nf_cur]),n_spacing_B,n_spacing_B,n_spacing_B,M_Zt[nb],M_St[nb],M_Zt[nb],A_ajdk,A_ajdk,&(lf_ZtSZn_uu[nb]));
	      GLOBAL_pthread_toc();
	      /* for (nb=0;nb<nbins;nb++){ } */}
	  GLOBAL_pthread_tuc(); 
	  GLOBAL_ops_addup_all(); GLOBAL_ops_printf_all(verbose && !error_check," %% AtTAn_uu: ");
	  GLOBAL_ops_toc(-1,0,verbose && !error_check," %% total time: ");  
	  /*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
	  for (nb=0;nb<nbins;nb++){
	    printf(" %% lf_AtTAn_vv[%d] error %0.16f\n",nb,dra_diff(lf_AtTAn_vv[nb]->lf,lf_AtTAn_uu[nb]->lf,length_AtTAn_vv[nb],1));
	    printf(" %% lf_ZtSZn_vv[%d] error %0.16f\n",nb,dra_diff(lf_ZtSZn_vv[nb]->lf,lf_ZtSZn_uu[nb]->lf,length_ZtSZn_vv[nb],1));
	    /* for (nb=0;nb<nbins;nb++){ } */}	  
	  /* if (error_check){ } */}
	/* for (n_type=1;n_type<=1;n_type++){ for (n_spacing_B=0;n_spacing_B<=2;n_spacing_B++){ for (n_spacing_A=0;n_spacing_A<=2;n_spacing_A++){ }}} */}}}
  wkspace_printf();
  if (verbose){ printf(" %% [finished wrap_AtTAn_vv_test]\n");}
}
