...
int wrap_AtTAn_vv__run(int *tidx,void **vpra,pthread_t *thread_in,int output_spacing_a,int output_spacing_y,int output_spacing_t,struct M_handle *M_At,struct M_handle *M_Tt,struct M_handle *M_Yt,double *A_ajdk,double *Y_ajdk,struct L_handle *lf_AtTn,struct L_handle *lf_YtTn,struct L_handle **output_AtTAn_p)
{
  ...
  length = length_a*length_y*length_t; if (*output_AtTAn_p==NULL){ if (verbose){ printf(" %% allocating output size %d*%d\n",length,(int)sizeof(double));} *output_AtTAn_p = L_handle_make(length);}
  ...
  length = length_a*length_y*length_t; if ((*output_AtTAn_p)->length<length){ printf(" %% Warning! length %d<%d in wrap_AtTAn_vv__run\n",(*output_AtTAn_p)->length,length);} memset((*output_AtTAn_p)->lf,0,length*sizeof(double));
  ip=0;
...
void wrap_AtTAn_vv_test()
{
  ...
  */
  ...
  for (nb=0;nb<nbins;nb++){
    lf_AtTAn_vv[nb] = L_handle_make(M_An[nb]->ncols*M_Yn[nb]->ncols*M_Tn[nb]->ncols);
    lf_ZtSZn_vv[nb] = L_handle_make(M_Zn[nb]->ncols*M_Wn[nb]->ncols*M_Sn[nb]->ncols);
    /* for (nb=0;nb<nbins;nb++){ } */}
  if (error_check){ 
    ...
    for (nb=0;nb<nbins;nb++){
      lf_AtTAn_uu[nb] = L_handle_make(M_An[nb]->ncols*M_Yn[nb]->ncols*M_Tn[nb]->ncols);
      lf_ZtSZn_uu[nb] = L_handle_make(M_Zn[nb]->ncols*M_Wn[nb]->ncols*M_Sn[nb]->ncols);
      /* for (nb=0;nb<nbins;nb++){ } */}
    /* if (error_check){ } */}
