void *get_TAnZtS_ww_stage_0(void *vp)
{
  ...
  length = 1; lf_tmp = E->lf_a0d1; lf_tmp->row_stride = length; lf_tmp->spacing_row = SPACING_a; L_zero(lf_tmp);
  length = 1; lf_tmp = E->lf_a2d1; lf_tmp->row_stride = length; lf_tmp->spacing_row = SPACING_a; L_zero(lf_tmp);
  ...
  __m128i *wA1_tag=NULL; __m128i *wA2_tag=NULL; __m128i *mc_tag=NULL; __m128i *mc_end=NULL;
  int nc=0; double A_lftmp=0,A_lftmp_a0d1=0,A_lftmp_a2d1=0;
  ...
  L1_set(E->lf_a0d1, 0,0,0, A_lftmp_a0d1);
  L1_set(E->lf_a2d1, 0,0,0, A_lftmp_a2d1);
...
void dcc_TAnZtS_ww_stage_1(struct dcc_ajdk *D)
{
  ...
  GLOBAL_pthread_tuc();
  GLOBAL_ops_addup_all(); GLOBAL_ops_printf_all(verbose," %% F->lf_An_a0d1_ZtSn F->lf_An_a0d1_AtTn: ");
  GLOBAL_ops_toc(-1,0,verbose," %% total time: ");
  /*%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%*/
...
void *get_TAnZtS_ww_stage_2(void *vp)
{
  ...
  double *A_ajdk = D->A_ajdk;
  struct L_handle *lf_tmp=NULL;
  int A_ncols = D->A_ncols, T_ncols = D->T_ncols, A_nrows = E_nb1->A_nrows;
  ...
  if (QR_TAnZtS_bother || QR_TAnAtT_bother){
	...
	ma_j=0; while (ma_j<E_nb1->M_An->rpop_j){
	  ...
	  lftmp = (*L2_get(F->lf_An_a0d1_ZtSn , ma_j,ma_b,ma_a , ns_j,ns_b,ns_a)) - (*L1_get(E_nb2->lf_a1d1_ZtSn, ns_j,ns_b,ns_a)) - (*L2_get(E_nb1->lf_An_ajdk, ma_j,ma_b,ma_a , 0,0,AJDK_1_1))*(*L1_get(E_nb2->lf_et_Sn, ns_j,ns_b,ns_a)) + (*L1_get(E_nb1->lf_a2d1, 0,0,0))*(*L1_get(E_nb2->lf_et_Sn, ns_j,ns_b,ns_a));
	...
	ma_j=0; while (ma_j<E_nb1->M_An->rpop_j){
	  ma_a = E_nb1->M_An->m_a_[ma_j]; ma_b = E_nb1->M_An->m_b_[ma_j];
	  lftmp = (*L2_get(F->lf_An_a0d1_AtTn , ma_j,ma_b,ma_a , ns_j,ns_b,ns_a)) - (*L1_get(E_nb2->lf_a1d1_AtTn, ns_j,ns_b,ns_a)) - (*L2_get(E_nb1->lf_An_ajdk, ma_j,ma_b,ma_a , 0,0,AJDK_1_1))*(*L1_get(E_nb2->lf_et_Tn, ns_j,ns_b,ns_a)) + (*L1_get(E_nb1->lf_a2d1, 0,0,0))*(*L1_get(E_nb2->lf_et_Tn, ns_j,ns_b,ns_a));
  ...
  if (QR_TAnAtT_bother && F->nb1==F->nb2){
    ns_j=0; while (ns_j<E_nb1->M_Tt->rpop_j){
  	...
  	ma_a = E_nb1->M_An->m_a_[ma_j]; ma_b = E_nb1->M_An->m_b_[ma_j];
  	vT = bget____(Tt_tag,ma_a); vM = bget__on(mct_tag,ma_a); if(!vM){ printf(" %% Warning! bit %d,%d,%d off in M_Tt->mc_j\n",ma_j,ma_b,ma_a);}
  	lftmp = vT * ( (*L1_get(E_nb1->lf_a0d1,0,0,0)) - 2*(*L2_get(E_nb1->lf_An_ajdk , ma_j,ma_b,ma_a , 0,0,AJDK_1_1)) + (*L1_get(E_nb1->lf_a2d1,0,0,0)) );
  	L2_plusequals(F->lf_TAnAtT_ww, ma_j,ma_b,ma_a , ns_j,ns_b,ns_a , -lftmp);
  	GLOBAL_ops_count_one(tidx,1,0);
  	ma_j++; /* while (ma_j<E_nb1->M_An->rpop_j){ } */}
      ns_j++; /* while (ns_j<E_nb1->M_Tt->rpop_j){ } */}
