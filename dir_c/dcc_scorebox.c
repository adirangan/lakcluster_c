...
{
  /* Calculates D->QR_svalue and D->QC_svalue from E->QR_TAnAtT_nrm and D->QC_TAnAtT_nrm, respectively. */
  int verbose=0; 
  ...
  int Irow=0,Icol=0;
  if (verbose){ printf(" %% [entering dcc_scorebox_svalue]\n");}
  ...
  if (D->A_rpop_b_total && D->A_cbother){
    ...
    if (verbose>1){
      ...
      /* if (verbose){ } */}
    /* if (D->A_rpop_b_total && D->A_cbother){ } */}
  if (verbose){ printf(" %% [finished dcc_scorebox_svalue]\n");}
}

void dcc_scorebox_srt(struct dcc_ajdk *D,int nrows,int *mr_index_local_nb,int *mr_index_local_mr,int ncols,int *mc_index_sort)
{
