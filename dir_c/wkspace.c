unsigned char *wkspace_alloc_nocheck(unsigned long long int size) {
  ...
  if (wkspace_left < size) { printf(" %% Warning! out of memory (%lld < %lld)\n",wkspace_left,size); exit(RET_NOMEM); return NULL;}
...
unsigned char* wkspace_alloc(unsigned long long int size) {
  ...
  if (verbose){ printf(" %% wkspace_alloc: size %lld --> ",size);}
  size = CACHEALIGN(size); if (verbose){ printf(" size %lld\n",size);}
...
void wkspace_printf(){ 
  printf(" %% CHECK:%lld used/left %lld/%lld (%.2f/%.2f MB) (%.2f/%.2f GB)\n",wkspace_check_point(wkspace_point_0),wkspace_used,wkspace_left,(double)wkspace_used/1048576,(double)wkspace_left/1048576,(double)wkspace_used/1073741824,(double)wkspace_left/1073741824);
}
