void dcc_scorebox_mxA(struct dcc_ajdk *D,int nrows,int *out_xdrop_nb,int *out_xdrop_umr,int nr_final,int ncols,int *out_xdrop_umc,int nc_final)
{
  /* writes to E->A_umr_j and D->A_umc_j ;
     drops rows out_xdrop_umr[0] to out_xdrop_umr[nr_final-1] (using out_xdrop_nb to determine nb) ;
     drops cols out_xdrop_umc[0] to out_xdrop_umc[nc_final-1] ;
  ...
  if (verbose){ printf(" %% [entering dcc_scorebox_mxA] nrows %d, nr_final %d, ncols %d, nc_final %d\n",nrows,nr_final,ncols,nc_final);}
  cdrop=0; for (nc=0;nc<nc_final;nc++){ D->A_umc_j[out_xdrop_umc[nc]]=0; cdrop += D->A_umc_b[out_xdrop_umc[nc]];} 
  ckept=0; for (nc=nc_final;nc<ncols;nc++){ D->A_umc_j[out_xdrop_umc[nc]]=D->A_umc_b[out_xdrop_umc[nc]]; ckept += D->A_umc_b[out_xdrop_umc[nc]];}
  ...
  rdrop=0; for (nr=0;nr<nr_final;nr++){ E_[out_xdrop_nb[nr]]->A_umr_j[out_xdrop_umr[nr]]=0; rdrop += E_[out_xdrop_nb[nr]]->A_umr_b[out_xdrop_umr[nr]];}
  rkept=0; for (nr=nr_final;nr<nrows;nr++){ E_[out_xdrop_nb[nr]]->A_umr_j[out_xdrop_umr[nr]]=E_[out_xdrop_nb[nr]]->A_umr_b[out_xdrop_umr[nr]]; rkept += E_[out_xdrop_nb[nr]]->A_umr_b[out_xdrop_umr[nr]];}
  for (nb=0;nb<nbins;nb++){ 
    ...
    /* for (nb=0;nb<nbins;nb++){ } */}
  if (verbose){ printf(" %% [finished dcc_scorebox_mxA], rdrop %d, rkept %d, cdrop %d, ckept %d\n",rdrop,rkept,cdrop,ckept);}
}
...
void dcc_scorebox_mxB(struct dcc_ajdk *D,int nrows,int *out_xdrop_nb,int *out_xdrop_umr,int nr_final,int ncols,int *out_xdrop_umc,int nc_final)
{
  /* reads from D->A_bmc_j and writes to D->A_umc_j ; 
     uses nc_final and D->A_umc_j to define D->A_umc_j_rtn and D->A_umc_rmv ;
     reads from E->A_bmr_j and writes to E->A_umr_j ;
     uses nr_final and E->A_umr_j to define E->A_umr_j_rtn and E->A_umr_rmv ;
   */
  int verbose=0;
  int nbins = D->nbins; struct dcc_single **E_ = D->E_;
  int nb1=0,nr=0,nc=0; struct dcc_single *E=NULL;
  if (verbose){ printf(" %% [entering dcc_scorebox_mxB]\n");}
  ...
  for (nc=0;nc<nc_final;nc++){ if (D->A_umc_j_rtn[out_xdrop_umc[nc]]==1){ D->A_umc_j_rmv[out_xdrop_umc[nc]]=1; D->A_umc_j_rtn[out_xdrop_umc[nc]]=0;}}
  for (nb1=0;nb1<nbins;nb1++){ E = E_[nb1];
    ...
    /* for (nb1=0;nb1<nbins;nb1++){ } */}
  for (nr=0;nr<nr_final;nr++){ nb1 = out_xdrop_nb[nr]; E = E_[nb1]; if (E->A_umr_j_rtn[out_xdrop_umr[nr]]==1){ E->A_umr_j_rmv[out_xdrop_umr[nr]]=1; E->A_umr_j_rtn[out_xdrop_umr[nr]]=0;}}
  dcc_sumscores_mxB(D);
  if (verbose){ printf(" %% [finished dcc_scorebox_mxB]\n");}
}
...
/* void dcc_out_xdrop_lkp(int nrows,int *mr_a_srt,int nbins,struct dcc_single **E_,int **out_xdrop_nb_,int **out_xdrop_umr_) */
/* { */
...
/*   if (*out_xdrop_umr_==NULL){ (*out_xdrop_umr_) = (int *) wkspace_all0c(nrows*sizeof(int));} */
/*   for (nl=0;nl<nrows;nl++){   */
/*     tmpd = mr_a_srt[nl]; */
/*     nb=0; while (tmpd>=E_[nb]->A_nrows){ tmpd-=E_[nb]->A_nrows; nb++;} */
/*     (*out_xdrop_nb_)[nl] = nb; (*out_xdrop_umr_)[nl] = tmpd; */
/*     /\* for (nl=0;nl<nrows;nl++){ } *\/} */
/*   if (verbose>1){ raprintf(*out_xdrop_nb_,"int",1,nrows," %% out_xdrop_nb: ");} */
/*   if (verbose>1){ raprintf(*out_xdrop_umr_,"int",1,nrows," %% out_xdrop_umr: ");} */
/*   if (verbose){ printf(" %% [finished dcc_out_xdrop_lkp]\n");} */
/* } */
...
void dcc_sumscores_ifT(struct dcc_ajdk *D)
{
  ...
  if (verbose){ printf(" %% [entering dcc_sumscores_ifT], nmds %d\n",nmds);}
  /*
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  */
  ...
  if (nmds<=0){
	...
	if (E->A_rbother && D->A_cbother && E->Z_rbother && D->A_cbother){ 
	  dra_mdspow_s___m_m(&(E->QR_TAnZtS_nrm[nb2*E->A_nrows*D->T_ncols]),E->A_nrows,D->T_ncols,nmds*mds_scale_factor,E->A_bmr_j,D->T_bmc_j); 
	  if (verbose){ raprintf(&(E->QR_TAnZtS_nrm[nb2*E->A_nrows*D->T_ncols]),"double_trn",E->A_nrows,D->T_ncols," %%%% E->QR_TAnZtS_nrm: ");}
	  /* if (E->A_rbother && D->A_cbother && E->Z_rbother && D->A_cbother){ } */}
	if (E->A_rbother && D->A_cbother && E->A_rbother && D->A_cbother){ 
	  dra_mdspow_s___m_m(&(E->QR_TAnAtT_nrm[nb2*E->A_nrows*D->T_ncols]),E->A_nrows,D->T_ncols,nmds*mds_scale_factor,E->A_bmr_j,D->T_bmc_j); 
	  if (verbose){ raprintf(&(E->QR_TAnAtT_nrm[nb2*E->A_nrows*D->T_ncols]),"double_trn",E->A_nrows,D->T_ncols," %%%% E->QR_TAnAtT_nrm: ");}
	  /* if (E->A_rbother && D->A_cbother && E->A_rbother && D->A_cbother){ } */}
	if (D->A_rpop_b_total && D->A_cbother && D->A_cbother && D->Z_rpop_b_total){ 
	  dra_mdsnrm_s___m_m(&(D->QC_TAnZtS_nrm[nbx*D->A_ncols*D->T_ncols]),D->A_ncols,D->T_ncols,nmds*mds_scale_factor,D->A_bmc_j,D->T_bmc_j); 
	  if (verbose){ raprintf(&(D->QC_TAnZtS_nrm[nbx*D->A_ncols*D->T_ncols]),"double_trn",D->A_ncols,D->T_ncols," %%%% D->QC_TAnZtS_nrm: ");}
	  /* if (D->A_rpop_b_total && D->A_cbother && D->A_cbother && D->Z_rpop_b_total){ } */}
	if (D->A_cbother && D->A_rpop_b_total){ 
	  dra_mdsnrm_s___m_m(&(D->QC_TAnAtT_nrm[nbx*D->A_ncols*D->T_ncols]),D->A_ncols,D->T_ncols,nmds*mds_scale_factor,D->A_bmc_j,D->T_bmc_j); 
