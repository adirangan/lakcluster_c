...
void dcc_sumscores_mxA(struct dcc_ajdk *D)
{
  ...
  for (nc=0;nc<minimum(cdrop,lcij);nc++){ D->A_umc_j_rmv[D->QC_index_local_mc_a[nc]]=1; D->A_umc_j_rtn[D->QC_index_local_mc_a[nc]]=0;}
  for (nb1=0;nb1<nbins;nb1++){ E = E_[nb1];
    ...
    /* for (nb1=0;nb1<nbins;nb1++){ } */}
  for (nr=0;nr<minimum(rdrop,lrij);nr++){ E_[D->QR_index_local_nb[nr]]->A_umr_j_rmv[D->QR_index_local_mr_a[nr]]=1; E_[D->QR_index_local_nb[nr]]->A_umr_j_rtn[D->QR_index_local_mr_a[nr]]=0;}
...
void dcc_sumscores_xij(struct dcc_ajdk *D)
{
  /* Sorts scores in increasing order across rows and columns.
     Then sets D->QC_index_local_mc, as well as D->QR_index_local_mr and D->QR_index_local_nb.
  ...
  */
  ...
  if (D->A_cbother && D->A_rpop_b_total){
	...
	if (bget__on(D->A_bmc_j,na_a)){
	  D->QC_svalue[na_j] = D->QC_TAnAtT_nrm[na_a + 0*D->A_ncols + Icol*D->A_ncols*D->T_ncols];
  ...
  if (D->A_rpop_b_total && D->A_cbother){
	...
	if (bget__on(E->A_bmr_b,ma_a)){
	  if (bget__on(E->A_bmr_j,ma_a)){
	    D->QR_svalue[D->A_rpop_j_total] = E->QR_TAnAtT_nrm[ma_a + 0*E->A_nrows + Irow*E->A_nrows*D->T_ncols];
...
void dcc_time_sumscores_test()
{
  ...
  GLOBAL_tic(4); dcc_wrap_dcc_halfloop(D); GLOBAL_toc(4,1+verbose," %% halfloop: ");
