function lakcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num,walltime,row_factor,col_factor,verbose_flag)
% set up to allow walltime as input ; using lakcluster_ver18 ; no replication ; reverse-search ;
% try: ;
%{
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/?* ./ ;
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/lakcluster_PGC_ADZSZDA_ver6.m ./ ;

  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
  % test ;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
  dir_code = sprintf('/home/arangan/dir_code_080517');dir_trunk = sprintf('%s',pwd); 
  p_threshold_ = [0.01 , 0.05 , 0.10 , 0.25]; gamma=0.004;B_MLT=34;
  walltime=0;row_factor=1.0;col_factor=1.0; 
  ...
  rev_flag=0; mds_tab=1; nmds = 20; if mds_tab==1; mds_used = [1:2]; else mds_used = []; end; 
  shuffle_num=0; lakcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag);
  ...
  dir_code = sprintf('/home/arangan/dir_code_080517');dir_trunk = sprintf('%s',pwd); 
  %p_threshold_ = [0.01 , 0.05 , 0.10 , 0.25]; gamma=0.004;B_MLT=34;
  walltime_ = [28 , 26 , 23 , 17];
  p_threshold_ = [0.10]; gamma=0.004;B_MLT=34; walltime_ = [23];
  ...
  p_threshold = p_threshold_(n_p); walltime = walltime_(n_p);
  for rev_flag = 1; for mds_tab = [1];
  nmds = 20; if mds_tab==1; mds_used = [1:2]; else mds_used = []; end; 
  shuffle_num=0; lakcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag);
  for shuffle_num=1:16; lakcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag); end;
...
test_string = sprintf('%s_%s',prefix,PGC_xfix_gen_ver6(p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num));
...
found_trace_flag = 0; 
tmpchar_trn = sprintf('%s/out_trace.txt',dir_out);
if exist(tmpchar_trn,'file');
tmp_trn = textread(tmpchar_trn);
if length(tmp_trn)> 6; disp(sprintf(' %% found %s of length %d, not rerunning.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 1; end;
%if length(tmp_trn)> 6; disp(sprintf(' %% found %s of length %d, actually, rerunning anyway.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 0; end;
if length(tmp_trn)<=6; disp(sprintf(' %% found %s of length %d,     rerunning.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 0; end;
...
fname_mc_A_pre = sprintf('%s_bim_p%.2d.ext',d_inpre,floor(100*p_threshold));
...
mc_A_bim = lakcluster_PGC_getbim_ver0(fname_bim,p_threshold);
...
fname__in_trn = sprintf('%s.in',d_oupre);
if ~found_trace_flag;
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_PGC_ADZSZDA_ver6.m on %s;\n',date);
fclose(fp);
end;%if ~found_trace_flag;
...
if call_flag;
if ~found_trace_flag;
disp(sprintf('%s/lakcluster_ver18 < %s',dir_code,fname__in_trn));
system(sprintf('%s/lakcluster_ver18 < %s',dir_code,fname__in_trn));
end;%if ~found_trace_flag;
end;%if call_flag;
...
function [output_string,gamma_d] = PGC_xfix_gen_ver6(p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num);
...
output_string = sprintf('lak_ADZSZDA_%s_%s_m%d_g%.3d_B%.2d_n%d_s%.4d',p_str,rev_str,mds_code,gamma_d,B_MLT,Ireq,shuffle_num);
