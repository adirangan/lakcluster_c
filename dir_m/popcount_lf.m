function output = popcount_lf(mem1_,mask_,mask_end,din_,popcount_,POPLENGTH);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
%
% function output = popcount_lf(mem1_,mask_,mask_end,din_,popcount_,POPLENGTH);
%
% This function calculates: ;
% \sum__{j=0}^{mask_end-1} din_(1+j/POPLENGTH)*popcount(bitand(mem1_(1+j),mask_(1+j))) ;
% which accumulates the mem1_ from mask_(1+0) to mask_(1+mask_end-1), ;
% multiplied by din_. ;
% ;
% We assume that mem1_ and mask_ are of type uint8. ;
% ;
...
for nc1=0:An_ncols-1;
mem1_ = uAt_(:,1+nc1); mask_ = uAt_mr_; mask_end = An_brows;
X1_(1+nc1) = popcount_lf(mem1_,mask_,mask_end,din_,popcount_,POPLENGTH);
tmp_X2 = 0;
for nr1=0:An_nrows-1;
tmp_iAt = iAt_(1+nr1,1+nc1);
tmp_X2 = tmp_X2 + din_(1 + floor(nr1/POPLENGTH)) * (tmp_iAt & An_mr_(1+nr1)) ;
end;%for nr1=0:An_nrows-1;
X2_(1+nc1) = tmp_X2;
end;%for nc1=0:An_ncols-1;
...
output = sum(din__(1:brows).*double(intlut(bitand(mem1_(1:mask_end),mask_(1:mask_end)),popcount_)));
