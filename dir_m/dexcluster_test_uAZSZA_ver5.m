function dexcluster_test_uAZSZA_ver5(dir_trunk,N,X_factor,X_esm,gamma,B_MLT,rng_num,pt_num)
...
%{
  ...
  N=1024*1;X_factor=0.575;X_esm=0.0;gamma=0.25;B_MLT=32;rng_num=1;dexcluster_test_uAZSZA_ver5(pwd,N,X_factor,X_esm,gamma,B_MLT,rng_num);
...
test_string = 'dexcluster_test_uAZSZA';
...
% X0 is balanced across covariate categories and T, and is exclusive to the cases ;
X0 = 2*(rand(nrows_X,ncols_X)<X_pp)-1; X0(:,floor(end/2):end) = 1-X0(:,floor(end/2):end);
...
X4 = 2*(rand(3*nrows_XX,ncols_XX)<X_pp)-1; X4(:,floor(end/2):end) = 1-X4(:,floor(end/2):end);
for nb1=0:nbins-1; 
...
X1 = 2*(rand(nrows_XX,ncols_XX)<X_pp)-1; X1(:,floor(end/2):end) = 1-X1(:,floor(end/2):end);
...
X2 = 2*(rand(nrows_XX,ncols_XX)<X_pp)-1; X2(:,floor(end/2):end) = 1-X2(:,floor(end/2):end);
...
X3 = 2*(rand(nrows_XX,ncols_XX)<X_pp)-1; X3(:,floor(end/2):end) = 1-X3(:,floor(end/2):end);
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by dexcluster_test_uAZSZA_ver5.m on %s;\n',date);
fclose(fp);
