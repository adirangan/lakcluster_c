...
xlim_ = prctile(AZnV_DvX_nix_driver_pd__(:,1+0),[ 1,99]); xlim_ = mean(xlim_) + diff(xlim_)*0.5*1.25*[-1,+1];
ylim_ = prctile(AZnV_DvX_nix_driver_pd__(:,1+1),[ 1,99]); ylim_ = mean(ylim_) + diff(ylim_)*0.5*1.25*[-1,+1];
subplot(p_row,p_col,1+np);np=np+1;
hold on;
scatter(AZnV_DvX_nix_driver_pd__(:,1+0),AZnV_DvX_nix_driver_pd__(:,1+1),markersize_med,mr_dvx_,'filled','MarkerEdgeColor','k');
scatter(AZnV_DvX_nix_driver_pd__(tmp_xdrop_ij_rkeep_,1+0),AZnV_DvX_nix_driver_pd__(tmp_xdrop_ij_rkeep_,1+1),markersize_big,mr_dvx_(tmp_xdrop_ij_rkeep_),'MarkerEdgeColor','k');
hold off;
xlim(xlim_);ylim(ylim_); axis square;
...
xdrop_ij_rkeep_ = xdrop_.ij_rkeep_;
xdrop_ij_ckeep_ = xdrop_.ij_ckeep_;
index_r_case_ = efind(famext_dvx_==2); n_case = numel(index_r_case_);
index_r_ctrl_ = efind(famext_dvx_==1); n_ctrl = numel(index_r_ctrl_);
%%%%%%%%;
for nniter_use=0:n_niter_use-1;
niter_use = niter_use_(1+nniter_use);
r_rem_use = r_rem_(1+niter_use);
...
xlim_ = prctile(AZnV_DvX_nix_driver_pd__(:,1+0),[ 1,99]); xlim_ = mean(xlim_) + diff(xlim_)*0.5*1.25*[-1,+1];
ylim_ = prctile(AZnV_DvX_nix_driver_pd__(:,1+1),[ 1,99]); ylim_ = mean(ylim_) + diff(ylim_)*0.5*1.25*[-1,+1];
h2d_X__ = hist2d_0(AZnV_DvX_nix_driver_pd__(1+index_r_ctrl_,1+0),AZnV_DvX_nix_driver_pd__(1+index_r_ctrl_,1+1),n_h,n_h,xlim_,ylim_);
h2d_D__ = hist2d_0(AZnV_DvX_nix_driver_pd__(1+index_r_case_,1+0),AZnV_DvX_nix_driver_pd__(1+index_r_case_,1+1),n_h,n_h,xlim_,ylim_);
h2d_DvX__ = h2d_D__/n_case - h2d_X__/n_ctrl;
hlim_ = 0 + std(h2d_DvX__,1,'all') * 2.50 * [-1,+1]; 
subplot(p_row,p_col,1+np);np=np+1;
hold on;
imagesc(h2d_DvX__,hlim_); colorbar;
set(gca,'ydir','normal');
hold off;
axis image; axisnotick;
title(sprintf('niter %d r_rem %d c_rem %d',niter_use,r_rem_use,c_rem_use),'Interpreter','none'); xlabel('PC1'); ylabel('PC2');
end;%for nniter_use=0:n_niter_use-1;
end;%if flag_disp;
