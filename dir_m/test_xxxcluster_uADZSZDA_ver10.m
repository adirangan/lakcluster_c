...
test_xxxcluster_uADZSZDA_ver10( ...
 parameter ...
);
...
%{
  ...
  test_xxxcluster_uADZSZDA_ver10(parameter);
  %}
...
if ~isfield(parameter,'flag_compare_with_dr_4'); parameter.flag_compare_with_dr_4 = 0; end;
...
flag_compare_with_dr_4 = parameter.flag_compare_with_dr_4;
%%%%%%%%;
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by test_xxxcluster_uADZSZDA_ver10.m on %s;\n',date);
fclose(fp);
...
title('original (test_xxxcluster_uADZSZDA_ver10)','Interpreter','none');
subplot(1,3,2);
M_all = [M_n_{1} ; M_n_{2} ; M_n_{3}];
imagesc(M_all(1+r_index_B(end:-1:1) , 1 + c_index_B(end:-1:1)));
title('compare (xxxcluster_uADZSZDA_dr_4)','Interpreter','none');
subplot(2,3,3);
...
fname_V = sprintf('%s/pca_k2_B44_V_.mda',dir_out);
fname_AnV = sprintf('%s/pca_proj_k2_B44_AnV_.mda',dir_out);
fname_ZnV = sprintf('%s/pca_proj_k2_B44_ZnV_.mda',dir_out);
flag_call = 1; if ( exist(fname_V,'file') & exist(fname_AnV,'file') & exist(fname_ZnV,'file') ); flag_call=0; end;
flag_call = flag_force_create | flag_call;
if flag_call;
