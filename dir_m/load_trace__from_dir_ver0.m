...
%%%%;
PR_is__ = zeros(n_iter,1+n_shuffle);
for niter=0:n_iter-1;
[~,tmp_ij_ori_from_srt_] = sort(QR_is__(1+niter,:),'ascend'); [~,tmp_ij_srt_from_ori_] = sort(tmp_ij_ori_from_srt_,'ascend');
PR_is__(1+niter,:) = tmp_ij_srt_from_ori_;
end;%for niter=0:n_iter-1;
%%%%;
PC_is__ = zeros(n_iter,1+n_shuffle);
for niter=0:n_iter-1;
[~,tmp_ij_ori_from_srt_] = sort(QC_is__(1+niter,:),'ascend'); [~,tmp_ij_srt_from_ori_] = sort(tmp_ij_ori_from_srt_,'ascend');
PC_is__(1+niter,:) = tmp_ij_srt_from_ori_;
end;%for niter=0:n_iter-1;
%%%%;
...
trace__.PR_is__ = PR_is__;
trace__.PC_is__ = PC_is__;
trace__.PR_s0000_ = PR_is__(:,1+0);
trace__.PC_s0000_ = PC_is__(:,1+0);
%%%%;
