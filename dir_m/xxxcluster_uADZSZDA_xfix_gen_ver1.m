function [output_string,gamma_d] = xxxcluster_uADZSZDA_xfix_gen_ver1(prefix,rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num);
...
if (B_MLT>0); B_str = sprintf('_B%.2d',B_MLT); else; B_str = ''; end;
...
output_string = sprintf('%s%s%s%s%s%s%s',prefix,type_str,rev_str,gamma_str,B_str,I_str,shuffle_str);
