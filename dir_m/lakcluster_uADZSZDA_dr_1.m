function lakcluster_uADZSZDA_dr_1(dir_code,dir_trunk,prefix,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,T_n_,T_n_cind,gamma,B_MLT,Ireq,shuffle_num,walltime,verbose_flag,force_create_flag)
% using lakcluster_ver18 ; 
% test with: ;
%{
  lakcluster_uADZSZDA_dr_1();
  %}
...
dir_trunk = pwd;
prefix = 'test';
nrows_A = 1920*1; A_n_cols = 1920*2;
rng(1);
M_n_{1} = 2*(randn(nrows_A,A_n_cols)>0)-1; 
...
p_c = randperm(A_n_cols); [~,q_c] = sort(p_c); [~,q_c] = sort(p_c);
...
verbose_flag = 0;
force_create_flag = 1;
lakcluster_uADZSZDA_dr_1(dir_code,dir_trunk,prefix,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,T_n_,T_n_cind,gamma,B_MLT,Ireq,shuffle_num,walltime,verbose_flag,force_create_flag);
n_shuffles = 8;
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_uADZSZDA_dr_1.m on %s;\n',date);
fclose(fp);
...
slurm_flag = 1*(walltime>0);
if slurm_flag;
fname_slurm = sprintf('%s.slurm',d_oupre);
fp = fopen(fname_slurm,'w');
fprintf(fp,'#!/bin/sh \n');
fprintf(fp,'#\n');
fprintf(fp,'#SBATCH --verbose\n');
fprintf(fp,'#SBATCH --job-name=%s\n',fname__in);
fprintf(fp,'#SBATCH --output=%s_output.log\n',d_oupre);
fprintf(fp,'#SBATCH --error=%s_error.log\n',d_opre);
fprintf(fp,'#SBATCH --time=%d:00:00\n',walltime);
fprintf(fp,'#SBATCH --nodes=%d --ntasks-per-node=%d\n',nnodes,tpn);
fprintf(fp,'#SBATCH --mem=%dGB\n',memdecl);
fprintf(fp,'\n');
fprintf(fp,'/bin/hostname\n');
fprintf(fp,'/bin/pwd\n');
fprintf(fp,'module load matlab/2016b\n');
fprintf(fp,'%s/lakcluster_ver18 < %s\n',dir_code,fname__in);
fclose(fp);
type(fname_slurm);
