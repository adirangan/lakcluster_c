function lakcluster_test_uADZSZDA_ver8(dir_trunk,N,X_factor,X_esm,gamma,B_MLT,rng_num,pt_num,verbose_flag)
...
%{
  ...
  N=1024*2;X_factor=0.65;X_esm=0.01;gamma=0.05;B_MLT=32;rng_num=1;pt_num=-1;verbose_flag=0;lakcluster_test_uADZSZDA_ver8(pwd,N,X_factor,X_esm,gamma,B_MLT,rng_num);
...
nbins = 3;
nrows_A_ = ceil(N/nbins*ones(1,nbins) + 5*(1:nbins));
nrows_Z_ = ceil(2*N/nbins*ones(1,nbins) + 7*(1:nbins));
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_test_uADZSZDA_ver8.m on %s;\n',date);
fclose(fp);

call_flag=~exist(sprintf('%s/out_xdrop_a.txt',dir_out),'file');;%call_flag = input(' call? 1=yes (default), 0=no:'); if isempty(call_flag); call_flag=1; end;
if call_flag;
...
A_n_rind_{1+nb1} = intersect(1+r_ij(end:-1:1) - sum(nrows_A_(1:1+nb1-1)),1:nrows_A_(1+nb1),'stable');
Z_n_rind_{1+nb1} = nrows_A_(1+nb1) + (1:nrows_Z_(1+nb1)) ;
T_n_{1+nb1} = [ Tn{1+nb1} ; Sn{1+nb1} ];
end;%for nb1=0:nbins-1;
A_n_cind = intersect(1+c_ij(end:-1:1),1:ncols_A,'stable');
T_n_cind = 1:ncols_T;
rev_flag = 0;
pgap = 32; ggap = 0;
hmap_autumn_1(M_n_,A_n_rind_,A_n_cind,Z_n_rind_,[],rev_flag,pgap,ggap);
end;%if disp_flag;
...
%%%%%%%%%%%%%%%% ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
% calculating scorebox ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;

dir_code = '/data/rangan/dir_bcc/dir_lakcluster_c_dev';
fname_scorebox__in = sprintf('%s/%s_scorebox.in',dir__in,in__name);
fp = fopen(fname_scorebox__in,'w');
fprintf(fp,'GLOBAL_verbose= 0;\n');
fprintf(fp,'GLOBAL_thread_count= 8;\n');
fprintf(fp,'GLOBAL_omp_type= 1;\n');
fprintf(fp,'GLOBAL_TEST_TYPE= %s;\n','lakcluster_scorebox');
...
call_flag=1;
if call_flag;
disp(sprintf('%s/lakcluster_ver18 < %s',dir_code,fname_scorebox__in));
system(sprintf('%s/lakcluster_ver18 < %s',dir_code,fname_scorebox__in));
end;%if call_flag;

%%%%%%%%%%%%%%%% ;
end;%if scorebox_flag;
%%%%%%%%%%%%%%%% ;
...
%%%%%%%%%%%%%%%% ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
% Comparing with lakcluster_uADZSZDA_dr_1.m ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
if call_flag;
dir_code = '/data/rangan/dir_bcc/dir_lakcluster_c_dev';
lakcluster_uADZSZDA_dr_1(dir_code,dir_trunk,prefix_B,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,T_n_,T_n_cind,gamma,B_MLT,Ireq,shuffle_num,walltime,verbose_flag,force_create_flag);
end;%if call_flag;

test_string_B = sprintf('%s_%s',prefix_B,lakcluster_uADZSZDA_xfix_gen_ver0(rev_flag,gamma,B_MLT,Ireq,shuffle_num));
dir__in_B = sprintf('%s/dir_%s',dir_trunk,prefix_B);
dir_out_B = sprintf('%s/dir_%s',dir__in_B,test_string_B); 
...
r_ij_B = out_xdrop_a_B(find(out_xdrop_a_B(:,1)>-1),1); c_ij_B = out_xdrop_a_B(find(out_xdrop_a_B(:,2)>-1),2);
[r_ij_ord_B,r_ij_i_B] = sort(1+r_ij_B); [c_ij_ord_B,c_ij_i_B] = sort(1+c_ij_B);
% Note that c_ij_B is the same as c_ij from above. ;
% The same is true for r_ij and r_ij_B, provided we shift the row-indices appropriately. ;
save('tmp.mat');
 disp_flag=1;
 if disp_flag;
 A_all = [An{1} ; An{2} ; An{3}];figure(1);imagesc(A_all(1+r_ij(end:-1:1),1+c_ij(end:-1:1)));
 M_all = [M_n_{1} ; M_n_{2} ; M_n_{3}];figure(2);imagesc(M_all(1+r_ij_B(end:-1:1) , 1 + c_ij_B(end:-1:1)));
 end;%if disp_flag;
%%%%%%%%%%%%%%%% ;
end;%if compare_flag;
%%%%%%%%%%%%%%%% ;
...
% Comparing with lakcluster_uADZSZDA_dr_1.m ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
scorebox_out_xdrop = sprintf('%s/out_xdrop_a.txt',dir_out_B);
scorebox_row_max = 512; scorebox_row_num = 48;
scorebox_col_max = 2048; scorebox_col_num = 64;
lakcluster_uADZSZDA_scorebox_2(dir_code,dir_trunk,prefix_B,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,T_n_,T_n_cind,1,gamma,B_MLT,Ireq,shuffle_num,verbose_flag,force_create_flag,scorebox_out_xdrop,scorebox_row_max,scorebox_row_num,scorebox_col_max,scorebox_col_num,0,0,0,0);
%%%%%%%%%%%%%%%% ;
end;% if scorebox_flag;
%%%%%%%%%%%%%%%% ;
...
pca_tolerance = 1e-3;
pca_rank = 2;
fname_pca__in = sprintf('%s/%s_pca.in',dir__in,in__name);
fp = fopen(fname_pca__in,'w');
fprintf(fp,'GLOBAL_verbose= 0;\n');
fprintf(fp,'GLOBAL_thread_count= 8;\n');
fprintf(fp,'GLOBAL_omp_type= 1;\n');
fprintf(fp,'GLOBAL_TEST_TYPE= %s;\n','pca_driver');
...
call_flag=1;
if call_flag;
disp(sprintf('%s/lakcluster_ver18_dev < %s',dir_code,fname_pca__in));
system(sprintf('%s/lakcluster_ver18_dev < %s',dir_code,fname_pca__in));
end;%if call_flag;
...
pca_iteration_min = 2;
pca_iteration_max = 6;
fname_pca_proj__in = sprintf('%s/%s_pca_proj.in',dir__in,in__name);
fp = fopen(fname_pca_proj__in,'w');
fprintf(fp,'GLOBAL_verbose= 0;\n');
fprintf(fp,'GLOBAL_thread_count= 8;\n');
fprintf(fp,'GLOBAL_omp_type= 1;\n');
fprintf(fp,'GLOBAL_TEST_TYPE= %s;\n','pca_proj_driver');
...
call_flag=1;
if call_flag;
disp(sprintf('%s/lakcluster_ver18_dev < %s',dir_code,fname_pca_proj__in));
system(sprintf('%s/lakcluster_ver18_dev < %s',dir_code,fname_pca_proj__in));
end;%if call_flag;
...
subplot(1,pca_iteration_num,nl); hold on;
c_ = colormap('jet'); n_c = size(c_,1);
for nr=1:length(r_ij);
nb = max(1,min(n_c,floor(n_c*nr/length(r_ij))));
plot(AnV_(1+r_ij(nr),1,nl),AnV_(1+r_ij(nr),2,nl),'.','Color',c_(nb,:),'MarkerSize',25);
end;%for nr=1:length(r_ij);
plot(ZnV_(:,1,nl),ZnV_(:,2,nl),'ko','MarkerSize',5);
hold off;
xlabel('PC1');ylabel('PC2');title(sprintf('Iteration %d (A_cpop_j %d/%d)',nl,A_cpop_j(nl),ncols_A),'Interpreter','none');
