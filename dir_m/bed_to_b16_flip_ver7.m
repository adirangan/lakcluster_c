...
if (nargin<1+na); study_trunk_s_=[]; end; na=na+1;
if (nargin<1+na); study_name_s_=[]; end; na=na+1;
...
parameter.study_trunk_s_ = study_trunk_s_;
parameter.study_name_s_ = study_name_s_;
parameter.n_mds_p = n_mds_p;
parameter.n_mds_v = n_mds_v;
parameter.n_famex = n_famex;

if (verbose>0); disp(sprintf(' %% [entering_bed_to_b16_flip_ver7]')); end;
...
tmpchar = sprintf('%s/%sbim.ext',dir_out,str_output_prefix_local);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
tmpchar = sprintf('%s/%sfam.ext',dir_out,str_output_prefix_local);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
tmpchar = sprintf('%s/%sA_full_n.b16',dir_out,str_output_prefix_local);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
tmpchar = sprintf('%s/%sA_full_t.b16',dir_out,str_output_prefix_local);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
for nstudy=0:n_study-1;
tmpchar = sprintf('%s/%sA_%.2d_n.b16',dir_out,str_output_prefix_local,1+nstudy);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
tmpchar = sprintf('%s/%sA_%.2d_t.b16',dir_out,str_output_prefix_local,1+nstudy);
flag_exist_all = flag_exist_all & exist(tmpchar,'file');
end;%for nstudy=0:n_study-1;

if ( flag_exist_all);
if (verbose>0); disp(sprintf(' %% all files found in bed_to_b16_flip_ver7;')); end;
end;%if ( flag_exist_all);
if (~flag_exist_all);
if (verbose>0); disp(sprintf(' %% some files missing in bed_to_b16_flip_ver7;')); end;
end;%if (~flag_exist_all);
...
[~,rij_keep_s_{1+ns}] = setdiff(fam_fidandiid_s_{1+ns},famex_fidandiid_p_,'stable');
...
khrdist_s_{1+ns} = tmp;
if (verbose>0); disp(sprintf(' %% n_snp_s_(%d) --> chromosome distribution: [%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d]',n_snp_s_(1+ns),khrdist_s_{1+ns})); end;
end;%for ns=0:n_study-1;
...
if (verbose>0); disp(sprintf(' %% n_snp_cap %d',n_snp_cap)); end;
disp_flag=0;
if disp_flag;
figure;cla; 
for ns1=0:n_study-1;
...
if (verbose>0); disp(sprintf(' %% norm(sg1+sg2) %0.16f',norm(snp_sg1_ls__+snp_sg2_ls__,'fro'))); end;
...
set(gca,'XTick',1:n_study,'XTickLabel',study_name_s_,'ticklabelinterpreter','none'); xtickangle(90);
set(gca,'YTick',1:n_study,'YTickLabel',study_name_s_,'ticklabelinterpreter','none');
end;%if flag_disp;
if (verbose>0); disp(sprintf(' %% fraction of snps with mismatch: %d/%d = %0.4f',numel(find(abs(sum(snp_sg1_ls__,2))~=n_study)),numel(snp_sg1_ls__),numel(find(abs(sum(snp_sg1_ls__,2))~=n_study))/numel(snp_sg1_ls__))); end;
if (verbose>0); disp(sprintf(' %% fraction of mismatches (within %d-x-%d data-array) %d/%d = %0.4f',n_snp_cap,n_study,sum((n_study-abs(sum(snp_sg1_ls__(:,:),2)))/2),(numel(snp_sg1_ls__(:,1))*n_study),sum((n_study-abs(sum(snp_sg1_ls__(:,:),2)))/2)/(numel(snp_sg1_ls__(:,1))*n_study))); end;
if (verbose>0); disp(sprintf(' %% number of sign flips per study: ')); disp(num2str(sum(snp_sg1_ls__==-1))); end;
...
flag_ver=0;
if flag_ver==0;
if (verbose>0); disp(sprintf(' %% version 0: simultaneous read (assuming sufficient ram)...')); end; tic;
...
t_tmp = toc; if (verbose>0);disp(sprintf(' %% read input: time %0.2f seconds',t_tmp)); end; 
if (verbose>0); disp(sprintf(' %% reading 4 allele pairs per byte... ')); end; tic;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
...
t_tmp = toc; if (verbose>0);disp(sprintf(' %% bitshift: time %0.2f seconds',t_tmp)); end; 
if (verbose>0); disp(sprintf(' %% reading 4 tags per allele pair... ')); end; tic;
%%%%%%%%%%%%%%%%;
...
t_tmp = toc; if (verbose>0);disp(sprintf(' %% sums: time %0.2f seconds',t_tmp)); end; 
if (verbose>0); disp(sprintf(' %% totalling sum2...')); end; tic;
...
t_tmp = toc; if (verbose>0);disp(sprintf(' %% n_xxx: time %0.2f seconds',t_tmp)); end; 
if (verbose>0); disp(sprintf(' %% totalling sum1...')); end; tic;
...
t_tmp = toc; if (verbose>0);disp(sprintf(' %% m_xxx: time %0.2f seconds',t_tmp)); end; tic;
...
if flag_ver==1;
if (verbose>0); disp(sprintf(' %% Warning! version 1: sequential read not implemented')); end;
...
if (verbose>0); disp(sprintf(' %% ns %d: [05 25 50 75 95] percentile: ',ns)); end;
...
if (verbose>0); disp(sprintf(' %% p_opt_s_ [%0.2f,%0.2f,%0.2f,%0.2f,%0.2f]',tmp)); end;
...
if (verbose>0); disp(sprintf(' %% q_opt_s_ [%0.2f,%0.2f,%0.2f,%0.2f,%0.2f]',tmp)); end;
...
if (verbose>0); disp(sprintf(' %% I_opt_s_ [%0.2f,%0.2f,%0.2f,%0.2f,%0.2f]',tmp)); end;
end;%for ns=0:n_study-1;
...
if (verbose>0); disp(sprintf(' %% all studies: [05 25 50 75 95] percentile: ')); end;
...
if (verbose>0); disp(sprintf(' %% p_opt_tot_l_ [%0.2f,%0.2f,%0.2f,%0.2f,%0.2f]',tmp)); end;
...
if (verbose>0); disp(sprintf(' %% q_opt_tot_l_ [%0.2f,%0.2f,%0.2f,%0.2f,%0.2f]',tmp)); end;
...
if (verbose>0); disp(sprintf(' %% ns %d: retaining %d/%d/%d patients',ns,n_rij_cap_s_(1+ns),n_patient_keep_s_(1+ns),n_patient_fam_s_(1+ns))); end;
end;%for ns=0:n_study-1;
clear rij_tmp_ ;
...
if (verbose>0); disp(sprintf(' %% Warning! patients not found in mds_fidandiid_p_: ns %d, tmp_ij_r_ length %d/%d',ns,numel(tmp_ij_r_),n_rij_cap_s_(1+ns))); end; 
rij_cap_sp__{1+ns} = rij_cap_sp__{1+ns}(tmp_ij_r_);
rindex_cap_sp__{1+ns} = rindex_cap_sp__{1+ns}(tmp_ij_r_);
else; if (verbose>0); disp(sprintf(' %% ns %d, tmp_ij_r_ %d == n_rij_cap_s_(1+ns) %d',ns,numel(tmp_ij_r_),n_rij_cap_s_(1+ns))); end;
...
tmpchar = sprintf('%s/%smc_T.b16',dir_out,str_output_prefix_local); 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mc_T(:)>0,tmpchar); 
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_full_n.b16',dir_out,str_output_prefix_local);
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(nrows_T_sum,1) , (T_full_>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_full_t.b16',dir_out,str_output_prefix_local);
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,transpose([ones(nrows_T_sum,1) , (T_full_>0)]),tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
for ns=0:n_study-1;
tmpchar = sprintf('%s/%sT_%.2d_n.b16',dir_out,str_output_prefix_local,1+ns);
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(n_rij_cap_s_(1+ns),1) , (T_{1+ns}>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_%.2d_t.b16',dir_out,str_output_prefix_local,1+ns);
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,transpose([ones(n_rij_cap_s_(1+ns),1) , (T_{1+ns}>0)]),tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
end;%for ns=0:n_study-1;
...
tmpchar = sprintf('%s/%smc_T_m2r%d.b16',dir_out,str_output_prefix_local,nz); if (verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mc_T_m2rx),sum(mc_T_m2rx>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mc_T_m2rx(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_full_n.b16',dir_out,str_output_prefix_local,nz); if (verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,size(T_m2rx_full_,1),1+size(T_m2rx_full_,2))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(nrows_T_sum,1) , (T_m2rx_full_>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_full_t.b16',dir_out,str_output_prefix_local,nz); if (verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,1+size(T_m2rx_full_,2),size(T_m2rx_full_,1))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,transpose([ones(nrows_T_sum,1) , (T_m2rx_full_>0)]),tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
save(sprintf('%s/%sT_m2r%d_full_n.mat',dir_out,str_output_prefix_local,nz),'T_m2rz_med_','T_m2rz_full_','T_m2rx_full_','w_tmp','R_tmp');
for ns=0:n_study-1;
tmpchar = sprintf('%s/%sT_m2r%d_%.2d_n.b16',dir_out,str_output_prefix_local,nz,1+ns); if (verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,size(T_m2rx_{1+ns},1),1+size(T_m2rx_{1+ns},2))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(n_rij_cap_s_(1+ns),1) , (T_m2rx_{1+ns}>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_%.2d_t.b16',dir_out,str_output_prefix_local,nz,1+ns); if (verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,1+size(T_m2rx_{1+ns},2),size(T_m2rx_{1+ns},1))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,transpose([ones(n_rij_cap_s_(1+ns),1) , (T_m2rx_{1+ns}>0)]),tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
end;%for ns=0:n_study-1;
...
if (verbose>0); disp(sprintf(' %% nx %d: kappa^2 = %0.6f (versus %0.6f) --> writing to %s',nz,tmp_kappa_squared,tmp_d,tmpchar)); end;
end;%if (flag_force_create | ~exist(tmpchar,'file'));
end;%for nz=1:n_z;
...
for ns=0:n_study-1;
nrows_An_s_(1+ns) = n_rij_cap_s_(1+ns); nrows_An_extend_s_(1+ns) = mod(bitj - mod(nrows_An_s_(1+ns),bitj),bitj); lrows_An_s_(1+ns) = (nrows_An_s_(1+ns) + nrows_An_extend_s_(1+ns))/bit8;
if (verbose>0); disp(sprintf(' %% nrows_An_s_(1+%d): %d',ns,nrows_An_s_(1+ns))); end;
end;%for ns=0:n_study-1;
nrows_An_full = sum(nrows_An_s_); nrows_An_full_extend = mod(bitj - mod(nrows_An_full,bitj),bitj); lrows_An_full = (nrows_An_full + nrows_An_full_extend)/bit8;
if (verbose>0); disp(sprintf(' %% nrows_An_full: %d',nrows_An_full)); end;
...
tmp_fam{1+6}(1+nr) = {sprintf('%s/%s/%s.fam',dir_trunk,study_trunk_s_{1+ns},study_name_s_{1+ns})};
%{
if (verbose>0); 
disp(sprintf('%s %s %s %s %d %d %s' ...
,fam_s_{1+ns}{1+0}{1+np1} ...
,fam_s_{1+ns}{1+1}{1+np1} ...
,fam_s_{1+ns}{1+2}{1+np1} ...
,fam_s_{1+ns}{1+3}{1+np1} ...
,fam_s_{1+ns}{1+4}(1+np1) ...
,fam_s_{1+ns}{1+5}(1+np1) ...
,sprintf('%s/%s/%s' ...
,dir_trunk ...
,study_trunk_s_{1+ns} ...
,study_name_s_{1+ns} ...
...
tmpchar = sprintf('%s/%smc_A.b16',dir_out,str_output_prefix_local); 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mc_A(:)>0,tmpchar); 
end;%if (flag_force_create | ~exist(tmpchar,'file'));
...
tmpchar = sprintf('%s/%smr_A_full.b16',dir_out,str_output_prefix_local);if (verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_A_full),sum(mr_A_full>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_A_full(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
tmpchar = sprintf('%s/%smr_Z_full.b16',dir_out,str_output_prefix_local);if (verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_Z_full),sum(mr_Z_full>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_Z_full(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
for ns=0:n_study-1;
tmpchar = sprintf('%s/%smr_A_%.2d.b16',dir_out,str_output_prefix_local,1+ns);if (verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_A_{1+ns}),sum(mr_A_{1+ns}>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_A_{1+ns}(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
tmpchar = sprintf('%s/%smr_Z_%.2d.b16',dir_out,str_output_prefix_local,1+ns);if (verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_Z_{1+ns}),sum(mr_Z_{1+ns}>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_Z_{1+ns}(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
end;%for ns=0:n_study-1;
...
for ns=0:n_study-1;
An_{1+ns} = zeros(lrows_An_s_(1+ns),ncols_An,'uint8');
At_{1+ns} = zeros(lcols_An,nrows_An_s_(1+ns),'uint8');
end;%for ns=0:n_study-1;
...
stride3 = 3:4:4*n_patient_bed_s_(1+ns); [~,tmp_ij] = setdiff(stride3,1+rindex_keep_s_{1+ns},'stable'); uchar_00110000(:,tmp_ij)=snp_not_tag; clear tmp_ij;
stride4 = 4:4:4*n_patient_bed_s_(1+ns); [~,tmp_ij] = setdiff(stride4,1+rindex_keep_s_{1+ns},'stable'); uchar_11000000(:,tmp_ij)=snp_not_tag; clear tmp_ij;
if (verbose>0); disp(sprintf(' %% writing binary files... ')); end; t_start = tic; t_sub = tic;
...
if (verbose>0); disp(sprintf(' %% nl3 %d/%d nl2 %d/%d nl1 %d/%d; time %0.2f seconds; estimated total time %0.2f seconds (%0.2f hours)',nl3,n_snp_cap_sub,nl2,n_snp_cap,nl1,n_snp_s_(1+ns),t_tmp,t_tot_tmp,t_tot_tmp/3600)); end; 
...
t_tmp = toc(t_start); if (verbose>0); disp(sprintf(' %% bin: time %0.2f seconds',t_tmp)); end; 
clear uchar_00000011;clear uchar_00001100;clear uchar_00110000;clear uchar_11000000;
clear stride1 stride2 stride3 stride4;
end;%if flag_ver==0;
end;%for ns=0:n_study-1;
...
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpAnchar)); end;
...
tmpAtchar = sprintf('%s/%sA_full_t.b16',dir_out,str_output_prefix_local);
if (flag_force_create | ~exist(tmpAtchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpAtchar)); end;
...
for ns=0:n_study-1;
tmpAnchar = sprintf('%s/%sA_%.2d_n.b16',dir_out,str_output_prefix_local,1+ns);
if (flag_force_create | ~exist(tmpAnchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpAnchar)); end;
fid = fopen(tmpAnchar,'w');
fwrite(fid,bitj,'int');
...
tmpAtchar = sprintf('%s/%sA_%.2d_t.b16',dir_out,str_output_prefix_local,1+ns);
if (flag_force_create | ~exist(tmpAtchar,'file'));
if (verbose>0); disp(sprintf(' %% %s not found, creating',tmpAtchar)); end;
fid = fopen(tmpAtchar,'w');
fwrite(fid,bitj,'int');
...
t_tmp = toc; if (verbose>0); disp(sprintf(' %% b16: time %0.2f seconds',t_tmp)); end; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tmpAnchar = sprintf('%s/%sA_full_n.b16',dir_out,str_output_prefix_local);
Bn_full = binary_uncompress(tmpAnchar,1:nrows_An_full,1:ncols_An);
tmpAtchar = sprintf('%s/%sA_full_t.b16',dir_out,str_output_prefix_local);
Bt_full = binary_uncompress(tmpAtchar,1:ncols_An,1:nrows_An_full);
if (verbose>0); disp(sprintf(' %% ns all: error %f',norm(Bn_full-transpose(Bt_full),'fro'))); end;
disp_flag=0;
if disp_flag;
...
for ns=0:n_study-1;
tmpAnchar = sprintf('%s/%sA_%.2d_n.b16',dir_out,str_output_prefix_local,1+ns);
Bn_{1+ns} = binary_uncompress(tmpAnchar,1:nrows_An_s_(1+ns),1:ncols_An);
tmpAtchar = sprintf('%s/%sA_%.2d_t.b16',dir_out,str_output_prefix_local,1+ns);
Bt_{1+ns} = binary_uncompress(tmpAtchar,1:ncols_An,1:nrows_An_s_(1+ns));
if (verbose>0); disp(sprintf(' %% ns %d: error %f',ns,norm(Bn_{1+ns}-transpose(Bt_{1+ns}),'fro'))); end;
disp_flag=0;
if disp_flag;
...
if (verbose>0); disp(sprintf(' %% [finished_bed_to_b16_flip_ver7]')); end;
return; 
end;%if flag_stop>0;
