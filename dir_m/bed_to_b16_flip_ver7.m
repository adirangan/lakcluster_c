...
if ( flag_exist_all);
if (flag_verbose>0); disp(sprintf(' %% all files found in bed_to_b16_flip_ver7;')); end;
end;%if ( flag_exist_all);
if (~flag_exist_all);
if (flag_verbose>0); disp(sprintf(' %% some files missing in bed_to_b16_flip_ver7;')); end;
end;%if (~flag_exist_all);
...
if (size(mds_pv__,1)<max(tmp_ij_m_)); disp(sprintf(' %% Warning, mds_pv__ is size (%d,%d), too small in bed_to_b16_flip_ver7, returning',size(mds_pv__))); return; end;
...
tmpchar = sprintf('%s/%smc_T_m2r%d.b16',dir_out,str_output_prefix_local,nz); if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mc_T_m2rx),sum(mc_T_m2rx>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mc_T_m2rx(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_full_n.b16',dir_out,str_output_prefix_local,nz); if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,size(T_m2rx_full_,1),1+size(T_m2rx_full_,2))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(nrows_T_sum,1) , (T_m2rx_full_>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_full_t.b16',dir_out,str_output_prefix_local,nz); if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,1+size(T_m2rx_full_,2),size(T_m2rx_full_,1))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
...
for ns=0:n_study-1;
tmpchar = sprintf('%s/%sT_m2r%d_%.2d_n.b16',dir_out,str_output_prefix_local,nz,1+ns); if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,size(T_m2rx_{1+ns},1),1+size(T_m2rx_{1+ns},2))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,[ones(n_rij_cap_s_(1+ns),1) , (T_m2rx_{1+ns}>0)],tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
tmpchar = sprintf('%s/%sT_m2r%d_%.2d_t.b16',dir_out,str_output_prefix_local,nz,1+ns); if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d-x%d',tmpchar,1+size(T_m2rx_{1+ns},2),size(T_m2rx_{1+ns},1))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (flag_verbose>0); disp(sprintf(' %% ; ')); end;
if (flag_verbose>0); disp(sprintf(' %% Writing extended bim and fam files (i.e., bim.ext and fam.ext). ; ')); end;
if (flag_verbose>0); disp(sprintf(' %% ; ')); end;
...
tmpchar = sprintf('%s/%smr_A_full.b16',dir_out,str_output_prefix_local);if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_A_full),sum(mr_A_full>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_A_full(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
tmpchar = sprintf('%s/%smr_Z_full.b16',dir_out,str_output_prefix_local);if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_Z_full),sum(mr_Z_full>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
...
pause(1);
for ns=0:n_study-1;
tmpchar = sprintf('%s/%smr_A_%.2d.b16',dir_out,str_output_prefix_local,1+ns);if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_A_{1+ns}),sum(mr_A_{1+ns}>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
binary_compress(bitj,mr_A_{1+ns}(:)>0,tmpchar);
end;%if (flag_force_create | ~exist(tmpchar,'file'));
pause(1);
tmpchar = sprintf('%s/%smr_Z_%.2d.b16',dir_out,str_output_prefix_local,1+ns);if (flag_verbose>0); disp(sprintf(' %% Writing %s of size %d sum %d',tmpchar,numel(mr_Z_{1+ns}),sum(mr_Z_{1+ns}>0))); end; 
if (flag_force_create | ~exist(tmpchar,'file'));
if (flag_verbose>0); disp(sprintf(' %% %s not found, creating',tmpchar)); end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (flag_verbose>0); disp(sprintf(' %% ; ')); end;
if (flag_verbose>0); disp(sprintf(' %% writing A_n.b16 and A_t.b16 ; ')); end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic;
...
t_tmp = toc; if (flag_verbose>0); disp(sprintf(' %% b16: time %0.2f seconds',t_tmp)); end; 

flag_check = flag_crop;
if flag_check;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (flag_verbose>0); disp(sprintf(' %% checking .b16 files for consistency ;')); end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tmpAnchar = sprintf('%s/%sA_full_n.b16',dir_out,str_output_prefix_local);
...
if (flag_verbose>0); disp(sprintf(' %% [finished_bed_to_b16_flip_ver7]')); end;
return; 
end;%if flag_stop>0;
