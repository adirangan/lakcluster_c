...
parameter_pca.pca_mc_T = zeros(size(mx__.mc_T_default_)); parameter_pca.pca_mc_T(1)=1;
parameter_pca.pca_mc_A = zeros(size(mx__.mc_A_default_)); parameter_pca.pca_mc_A(xdrop_.ij_ckeep_(1:n_c_tmp))=1;
parameter_pca.pca_mr_A_ = cell(n_study,1);
parameter_pca.pca_mr_Z_ = cell(n_study,1);
for nstudy=0:n_study-1;
parameter_pca.pca_mr_A_{1+nstudy} = zeros(size(mx__.mr_A_default__{1+nstudy}));
parameter_pca.pca_mr_Z_{1+nstudy} = zeros(size(mx__.mr_Z_default__{1+nstudy}));
if (flag_reverse==0);
parameter_pca.pca_mr_A_{1+nstudy}(xdrop_.ij_rkeep_(1:n_r_tmp))=1;
parameter_pca.pca_mr_Z_{1+nstudy} = mx__.mr_Z_default__{1+nstudy};
end;%if (flag_reverse==0);
if (flag_reverse==1);
parameter_pca.pca_mr_Z_{1+nstudy}(xdrop_.ij_rkeep_(1:n_r_tmp))=1;
parameter_pca.pca_mr_A_{1+nstudy} = mx__.mr_A_default__{1+nstudy};
...
parameter_pca_proj.pca_mc_T = zeros(size(mx__.mc_T_default_)); parameter_pca_proj.pca_mc_T(1)=1;
parameter_pca_proj.pca_mc_A = zeros(size(mx__.mc_A_default_)); parameter_pca_proj.pca_mc_A(xdrop_.ij_ckeep_(1:n_c_tmp))=1;
parameter_pca_proj.pca_mr_A_ = cell(n_study,1);
parameter_pca_proj.pca_mr_Z_ = cell(n_study,1);
for nstudy=0:n_study-1;
parameter_pca_proj.pca_mr_A_{1+nstudy} = mx__.mr_A_default__{1+nstudy};
parameter_pca_proj.pca_mr_Z_{1+nstudy} = mx__.mr_Z_default__{1+nstudy};
end;%for nstudy=0:n_study-1;
