function lakcluster_uADZSWDY_dr_2(dir_code,dir_trunk,prefix,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,Y_n_cind,T_n_,T_n_cind,GLOBAL_TEST_sparse,GLOBAL_kappa_squared,gamma,B_MLT,Ireq,shuffle_type,shuffle_num,verbose_flag,force_create_flag,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl);
% using lakcluster_ver18 and slurm ; 
% test with: ;
%{
  lakcluster_uADZSWDY_dr_2();
  %}

if (nargin<1);
disp(sprintf(' %% testing lakcluster_uADZSWDY_dr_2'));
%%%%%%%%%%%%%%%%
...
T_n_cind = 1:n_T;
X_n = 2*(tutorial_makelr(16*nbins,128,1,0.01)>0)-1;
X_n_ = cell(nbins,1);
M_n_ = cell(nbins,1);
for nb=1:nbins;
M_n_{nb} = 2*(randn(M_n_rows_(nb),M_n_cols)>0.3)-1;
X_n_{nb} = X_n((nb-1)*16 + (1:16),:);
M_n_{nb}(A_n_rind_{nb}(1:16),A_n_cind(1:128)) = X_n_{nb};
...
if disp_flag;
figure(1);
subplot(2,2,1); imagesc(M_tmp_1,[-1,1]); title('M A');
subplot(2,2,2); imagesc(M_tmp_2,[-1,1]); title('M A full');
subplot(2,2,3); imagesc(M_tmp_3,[-1,1]); title('M Z');
subplot(2,2,4); imagesc(M_tmp_4,[-1,1]); title('M Z full');
drawnow();
end %if disp_flag;
...
disp_flag=0;
if disp_flag;
subplot(2,2,1); imagesc(T_tmp_1,[-1,1]); title('T A');
subplot(2,2,2); imagesc(T_tmp_2,[-1,1]); title('T A full');
subplot(2,2,3); imagesc(T_tmp_3,[-1,1]); title('T Z');
subplot(2,2,4); imagesc(T_tmp_4,[-1,1]); title('T Z full');
end %if disp_flag;
...
lakcluster_uADZSWDY_dr_2(dir_code,dir_trunk,prefix,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,Y_n_cind,T_n_,T_n_cind,GLOBAL_TEST_sparse,GLOBAL_kappa_squared,gamma,B_MLT,Ireq,shuffle_type,shuffle_num,verbose_flag,force_create_flag,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl);
...
r_ij = r_ij(end:-1:1); c_ij = c_ij(end:-1:1);
figure(2); hmap_autumn_1(M_n_full_,{[1+r_ij]},1+c_ij,Z_n_rind_full_,[],rev_flag,16,32); title(test_string,'Interpreter','none');
%%%%%%%%%%%%%%%%
disp(sprintf(' %% testing Ireq=%d',floor(nbins/2)));
Ireq = 0;
prefix = sprintf('test_lak_Ireq%d',Ireq);
...
r_ij = out_xdrop_a(find(out_xdrop_a(:,1)>-1),1); c_ij = out_xdrop_a(find(out_xdrop_a(:,2)>-1),2);
r_ij = r_ij(end:-1:1); c_ij = c_ij(end:-1:1);
figure(3); hmap_autumn_1(M_n_full_,{[1+r_ij]},1+c_ij,Z_n_rind_full_,[],rev_flag,16,32); title(test_string,'Interpreter','none');
%%%%%%%%%%%%%%%%
disp(sprintf(' %% testing full'));
Ireq = 0;
prefix = sprintf('test_lak_full');
lakcluster_uADZSWDY_dr_2(dir_code,dir_trunk,prefix,M_n_full_,rev_flag,A_n_rind_full_,A_n_cind,Z_n_rind_full_,Y_n_cind,T_n_full_,T_n_cind,GLOBAL_TEST_sparse,GLOBAL_kappa_squared,gamma,B_MLT,Ireq,shuffle_type,shuffle_num,verbose_flag,force_create_flag,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl);
test_string = sprintf('%s_%s',prefix,lakcluster_uADZSWDY_xfix_gen_ver0(rev_flag,gamma,B_MLT,Ireq,shuffle_num));
dir__in = sprintf('%s/dir_%s',dir_trunk,prefix);
dir_out = sprintf('%s/dir_%s',dir__in,test_string); 
out_xdrop_a = textread(sprintf('%s/out_xdrop_a.txt',dir_out)); out_xdrop_b = textread(sprintf('%s/out_xdrop_b.txt',dir_out));
r_ij = out_xdrop_a(find(out_xdrop_a(:,1)>-1),1); c_ij = out_xdrop_a(find(out_xdrop_a(:,2)>-1),2);
r_ij = r_ij(end:-1:1); c_ij = c_ij(end:-1:1);
figure(4); hmap_autumn_1(M_n_full_,{[1+r_ij]},1+c_ij,Z_n_rind_full_,[],rev_flag,16,32); title(test_string,'Interpreter','none');
%%%%%%%%%%%%%%%%
...
if (nargin<na); shuffle_type = 'AZWY'; end; na=na+1;
...
mc_A_prm = mc_A; mc_Y_prm = mc_Y;
end;%if (strcmp(shuffle_type,'AZ'));
if (strcmp(shuffle_type,'AZWY'));
disp(sprintf(' %% Note: We are shuffling across rows and columns'));
...
end;%if (strcmp(shuffle_type,'AZWY'));
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_uADZSWDY_dr_2.m on %s;\n',date);
fclose(fp);
