function dexcluster_uADZSZDA_scorebox_2(dir_code,dir_trunk,prefix,M_n_,rev_flag,A_n_rind_,A_n_cind,Z_n_rind_,T_n_,T_n_cind,GLOBAL_TEST_sparse,GLOBAL_kappa_squared,gamma,B_MLT,Ireq,shuffle_num,verbose_flag,force_create_flag,scorebox_out_xdrop,scorebox_row_max,scorebox_row_num,scorebox_col_max,scorebox_col_num,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl)
% using dexcluster_ver18 with slurm ; 
% test with: ;
%{
  % first run dexcluster_uADZSZDA_dr_2();
  dexcluster_uADZSZDA_scorebox_2();
  %}

if (nargin<1);
disp(sprintf(' %% testing dexcluster_uADZSZDA_scorebox_2'));
disp(sprintf(' %% Note: first run dexcluster_uADZSZDA_dr_2'));
%%%%%%%%%%%%%%%%
...
slurm_memdecl=24;
%%%%%%%%%%%%%%%%
disp(sprintf(' %% recreating inputs used in previous run of dexcluster_uADZSZDA_dr_2()'));
rng(0); mrnd=0.1;
nbins = 3; n_T = 3;
...
dir_out = sprintf('%s/dir_%s',dir__in,test_string); 
scorebox_out_xdrop = sprintf('%s/out_xdrop_a.txt',dir_out);
dexcluster_uADZSZDA_scorebox_2(dir_code,dir_trunk,prefix,M_n_full_,rev_flag,A_n_rind_full_,A_n_cind,Z_n_rind_full_,T_n_full_,T_n_cind,GLOBAL_TEST_sparse,GLOBAL_kappa_squared,gamma,B_MLT,Ireq,shuffle_num,verbose_flag,force_create_flag,scorebox_out_xdrop,scorebox_row_max,scorebox_row_num,scorebox_col_max,scorebox_col_num,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl);
%%%%%%%%%%%%%%%%
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by dexcluster_uADZSZDA_scorebox_2.m on %s;\n',date);
fclose(fp);
