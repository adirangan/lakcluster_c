function lakcluster_test_AZZA_ver2(dir_trunk,N,X_factor,X_esm,gamma,B_MLT,rng_num,pt_num)
...
%{
  ...
  N=1024;X_factor=0.55;X_esm=0.1;gamma=0.05;B_MLT=0;rng_num=1;lakcluster_test_AZZA_ver2(pwd,N,X_factor,X_esm,gamma,B_MLT,rng_num);
  ...
  lakcluster_test_AZZA_ver2(pwd,N,X_factor,X_esm,gamma,B_MLT,rng_num);
  xfix_del(pwd,'lakcluster_test_AZZA',N,X_factor,X_esm,gamma,B_MLT,rng_num,nbins);
...
test_string = 'lakcluster_test_AZZA';
...
XAZ = tutorial_binary_makelr2(3*nrows_X,ncols_X,X_pp);
XA_ = XAZ(1:nrows_X,:);
X_Z = XAZ(nrows_X+1:3*nrows_X,:);
...
pr3_f{1+nb1} = randperm(nrows_Z_(1+nb1)); [tmp,pr3_i{1+nb1}] = sort(pr3_f{1+nb1}); [tmp,pr3_i{1+nb1}] = sort(pr3_i{1+nb1});
Zn{1+nb1}(pr3_f{1+nb1}(1:2*nrows_X),pc2_f{1+nb1}(1:ncols_X)) = X_Z;
...
disp_flag=0;
if disp_flag;
figure;imagesc([ An{1+0}(pr1_i{1+0},pc1_i{1+0}) ; zeros(8,ncols_A) ; Zn{1+0}(         :,pc1_i{1+0}) ],[-1,1]); title('X00');
figure;imagesc([ An{1+0}(pr2_i{1+0},pc2_i{1+0}) ; zeros(8,ncols_A) ; Zn{1+0}(pr3_i{1+0},pc2_i{1+0}) ],[-1,1]); title('XAZ');
return;
end;%if disp_flag;
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_test_AZZA_ver2.m on %s;\n',date);
fclose(fp);
