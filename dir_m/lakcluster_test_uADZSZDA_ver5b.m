function lakcluster_test_uADZSZDA_ver5b(dir_trunk,N,X_factor,X_esm,gamma,B_MLT,rng_num,pt_num,verbose_flag)
...
%{
  ...
  N=16;X_factor=0.55;X_esm=0.1;gamma=0.25;B_MLT=32;rng_num=1;lakcluster_test_uADZSZDA_ver5b(pwd,N,X_factor,X_esm,gamma,B_MLT,rng_num);
...
mrnd = 0;
use_An_XtXXn_Xt_flag = 0;
use_AnXt_X_XnXt_flag = 1;
use_At_T_XnXt_X_Xn_flag = 1;
use_AtTXn____XtXXn_flag = 0;
...
fprintf(fp,'GLOBAL_thread_count= 12;\n');
fprintf(fp,'GLOBAL_omp_type= 0;\n');
fprintf(fp,'GLOBAL_NTEST= 1;\n');
fprintf(fp,'GLOBAL_VERSUS= 0;\n');
fprintf(fp,'GLOBAL_LRUP= 0;\n');
fprintf(fp,'GLOBAL_nb_srt= %d;\n',nb_srt);
fprintf(fp,'GLOBAL_gamma_type= 0;\n');
fprintf(fp,'GLOBAL_gamma= %0.4f;\n',gamma);
fprintf(fp,'GLOBAL_use_An_XtXXn_Xt_flag= %d;\n',use_An_XtXXn_Xt_flag);
fprintf(fp,'GLOBAL_use_AnXt_X_XnXt_flag= %d;\n',use_AnXt_X_XnXt_flag);
fprintf(fp,'GLOBAL_use_At_T_XnXt_X_Xn_flag= %d;\n',use_At_T_XnXt_X_Xn_flag);
fprintf(fp,'GLOBAL_use_AtTXn____XtXXn_flag= %d;\n',use_AtTXn____XtXXn_flag);
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by lakcluster_test_uADZSZDA_ver5b.m on %s;\n',date);
fclose(fp);

call_flag=1;%call_flag = input(' call? 1=yes (default), 0=no:'); if isempty(call_flag); call_flag=1; end;
if call_flag;
disp(sprintf('%s/../lakcluster_ver16 < %s/%s.in',dir_trunk,dir__in,in__name));
system(sprintf('%s/../lakcluster_ver16 < %s/%s.in',dir_trunk,dir__in,in__name));
end;%if call_flag;
