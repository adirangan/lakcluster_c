function [annot] = annot_struct_make_ver0(fname_annot);
...
rs__ = cell(n_lines,1);
for nl=1:n_lines;
if (verbose>0); if (mod(nl,500)==0); disp(sprintf(' %% nl %d/%d',nl,n_lines)); end; end;
tline = fgetl(fid);
tmp_ = strsplit(tline);
tmp_len = length(tmp_);
EZ_{nl} = tmp_{1};
...
[rs_sort_,rs_all_to_rs_sort_,rs_sort_to_rs_all_] = unique(rs_all_);
...
assert(strcmp(rs_all_{nrs_all},rs_sort_{rs_sort_to_rs_all_(nrs_all)}));
ln_(rs_sort_to_rs_all_(nrs_all)) = ln_(rs_sort_to_rs_all_(nrs_all)) + 1;
EZ__{rs_sort_to_rs_all_(nrs_all)}(ln_(rs_sort_to_rs_all_(nrs_all))) = {0};
EZ__{rs_sort_to_rs_all_(nrs_all)}{ln_(rs_sort_to_rs_all_(nrs_all))} = EZ_{nl};
...
annot = struct(...
	       'verbose',0 ...
	       ,'fname_annot',fname_annot...
	       ,'n_header',n_header...
	       ,'n_lines',n_lines...
	       );
annot.ru_ = rs_sort_;
annot.n_ru = n_rs_sort;
annot.ln_ = ln_;
annot.EZ__ = EZ__;
