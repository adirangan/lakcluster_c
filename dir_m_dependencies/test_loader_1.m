...
fname_base = sprintf('%s/label_count_C',dir_trunk);
...
title('histogram of cluster label counts');
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/label_count_I',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
end;%if flag_plot;
...
flag_disp=0;
if flag_disp;
imagedisp_0(XX_rank_(XX_rank_U_ij_,XX_rank_V_ij_));
end;%if flag_disp;
...
% Need to vectorize sorting above. ;
