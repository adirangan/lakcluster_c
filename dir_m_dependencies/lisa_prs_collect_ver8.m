function lisa_prs_collect_ver8(rdrop_prefix,cl_num_arm1,cl_num_arm2,flag_reverse_use,flag_rerun);
% Assuming that arm1 is cl4. ;
% Using data from /data/rangan/dir_bcc/dir_BIP32_loo_data/dir_icuk_scores ;
% try: ;
%{
  flag_rerun = 0;
  rdrop_prefix_ = {'469','319'};%rdrop_prefix_ = {'756','469','319'};
  for nd=1:length(rdrop_prefix_); for cl_num_arm1=[4]; for cl_num_arm2=[1,2,3]; for flag_reverse_use=[0:1];
  lisa_prs_collect_ver8(rdrop_prefix_{nd},cl_num_arm1,cl_num_arm2,flag_reverse_use,flag_rerun);
...
%rdrop_prefix = '756'; flag_rerun=0;

%cl_num_arm1 = 4; cl_num_arm2 = 1; flag_reverse_use = 0;
...
fname = sprintf('%s/icuk_geno_ex_bicl1_%s_on_dos_%s.S%s.profile',dir_icuk_ex,rdrop_prefix,tmp_study_name,str_prs_{nprs}); fcheck(fname); 
fid = fopen(fname);
...
%%%%%%%%;
%flag_rerun = 0;
fname = sprintf('%s/prs_collect_ver8_%s_trn%d_tst%d_ni%.3d_ni%.3d.jpg',dir_out_s0000_pca_r0_arm1,rdrop_prefix,cl_num_arm1,cl_num_arm2,ni_r0_arm2,ni_r1_arm2);
if (~flag_rerun & exist(fname,'file')); disp(sprintf(' %% %s found, not creating',fname)); end;
if ( flag_rerun | ~exist(fname,'file')); disp(sprintf(' %% %s not found, creating',fname)); 
%%%%%%%%;
