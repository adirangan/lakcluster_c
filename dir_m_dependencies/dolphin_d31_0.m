...
flag_drift0=0;
flag_center=1;
fname_infix = sprintf('aid31d%dc%d',flag_drift0,flag_center);
...
%%%%%%%%;
if flag_drift0==0; a_null_avg_ = zeros(n_var,1); end;
A_null_avg__ = zeros(n_var,n_var);
BB_null_avg__ = zeros(n_var,n_var);
CC_null_avg__ = zeros(n_var,n_var);
if flag_drift0==0; a_null_std_ = zeros(n_var,1); end;
...
if flag_drift0==0; a_null_avg_ = a_null_avg_ + tmp_.a__{1+tmp_index}; end;
...
if flag_drift0==0; a_null_std_ = a_null_std_ + tmp_.a__{1+tmp_index}.^2; end;
...
if flag_drift0==0; a_null_avg_ = a_null_avg_ / n_shuffle; end;
A_null_avg__ = A_null_avg__ / n_shuffle;
BB_null_avg__ = BB_null_avg__ / n_shuffle;
CC_null_avg__ = CC_null_avg__ / n_shuffle;
if flag_drift0==0; a_null_std_ = sqrt(a_null_std_/n_shuffle - a_null_avg_.^2); end;
...
if flag_drift0==0; a_ = tmp_.a__{1+tmp_index}; end;
A__ = tmp_.A___{1+tmp_index};
BB__ = tmp_.BB___{1+tmp_index};
CC__ = tmp_.CC___{1+tmp_index};
clear tmp_;
if flag_drift0==0; a_Z_ = real((a_ - a_null_avg_)./a_null_std_); end;
...
if flag_drift0==0; a_nlp_ = -z_to_lp(a_Z_); end;
...
disp(sprintf(' %% %s not found, creating',fname_mat));
%%%%%%%%;
if flag_drift0==0; pair_a_cmb_ = zeros(n_var,1); end;
...
if flag_drift0==1;
[pair_A__,pair_BB__,pair_CC__,pair_L_,pair_niteration,pair_A___,pair_BB___,pair_CC___] = dolphin_estimate_ABC_0(aid_,age_,pair_dat__,n_step,n_iteration);end;%if flag_drift0==1;
[~,tmp_index] = min(pair_L_(1:1+pair_niteration)); tmp_index = tmp_index-1;
if flag_drift0==0; pair_a_cmb_(1+nvar_0) = pair_a_cmb_(1+nvar_0) + pair_a__{1+tmp_index}(1+0)/n_pair; end;
if flag_drift0==0; pair_a_cmb_(1+nvar_1) = pair_a_cmb_(1+nvar_1) + pair_a__{1+tmp_index}(1+1)/n_pair; end;
...
if flag_drift0==0; pair_a_cmb_null_avg_ = pair_a_cmb_null_avg_ + tmp_.pair_a_cmb_; end;
...
if flag_drift0==0; pair_a_cmb_null_std_ = pair_a_cmb_null_std_ + tmp_.pair_a_cmb_.^2; end;
...
if flag_drift0==0; pair_a_cmb_null_avg_ = pair_a_cmb_null_avg_ / n_shuffle_use; end;
...
if flag_drift0==0; pair_a_cmb_null_std_ = sqrt(pair_a_cmb_null_std_/n_shuffle_use - pair_a_cmb_null_avg_.^2); end;
...
fname_mat = sprintf('%s/dolphin_%s_pair_cmb_s0000.mat',dir_mat,fname_infix);
tmp_ = load(fname_mat);
if flag_drift0==0; pair_a_cmb_ = tmp_.pair_a_cmb_; end;
pair_A_cmb__ = tmp_.pair_A_cmb__;
pair_BB_cmb__ = tmp_.pair_BB_cmb__;
pair_CC_cmb__ = tmp_.pair_CC_cmb__;
clear tmp_;
if flag_drift0==0; pair_a_cmb_Z_ = real((pair_a_cmb_ - pair_a_cmb_null_avg_)./pair_a_cmb_null_std_); end;
...
if flag_drift0==0; pair_a_cmb_nlp_ = -z_to_lp(pair_a_cmb_Z_); end;
