function lisa = lisa_struct_prs_scatterplot_ver4(specification);
...
%{
  ...
  %ni_r0_arm2 = 450; ni_r1_arm2 = 425;
  %ni_r0_arm2 = 175; ni_r1_arm2 = 163;
  %ni_r0_arm2 = 400; ni_r1_arm2 = 300;
  ni_r0_arm2 = 250; ni_r1_arm2 = 225;
  ...
  lisa_struct_prs_scatterplot_ver4(specification) ;
  ...
  %%%%%%%%;
  % D: trn4_tst1, ;
  % construct prs score while keeping only the patients in bc? ni???. ;
  %%%%%%%%;
  clear;
  dir_bip32 = '/data/rangan/dir_bcc/dir_BIP32_loo_data';
  dir_icuk = sprintf('%s/dir_icuk_scores',dir_bip32);
  ...
  flag_dex_vs_lak = 'dex'; %<-- differentially expressed clustering. ;
  cl_num_arm1 = 4; 
  for cl_num_arm2 = 1:3; %<-- train on platform 4, replicate on platform 2,3. ;
  cl_num_arm1_mc_A = []; cl_num_arm1_mr_A_full_ = []; cl_num_arm1_mr_Z_full_ = []; cl_num_arm1_string = []; 
  cl_num_arm2_mc_A = []; cl_num_arm2_mr_A_full_ = []; cl_num_arm2_mr_Z_full_ = []; cl_num_arm2_string = []; 
  flag_reverse = 0; %<-- forward bicluster (i.e., case-specific). ;
  gamma = [0.004]; %<-- gamma is the fraction eliminated per iteration. 000 implies a single patient eliminated per iteration. ;
  ...
  specification.prs_prefix = prs_prefix;
  lisa_struct_prs_scatterplot_ver4(specification) ;
  end;% for cl_num_arm2 = 2:3; %<-- train on platform 4, replicate on platform 2,3. ;
  ...
  pca_b_mlt = 44; pca_tolerance = 1e-2; pca_rank = 2;
  for nprefix=1:3;
  if nprefix==1; prs_prefix = 'keep_bc1_ni175'; end;
  if nprefix==2; prs_prefix = 'keep_bc1_ni375'; end;
  if nprefix==3; prs_prefix = 'keep_bc2_ni163'; end;
  n_iteration_stride = 25/2;
  ni_r0_arm2 = 175; ni_r1_arm2 = 163;
  specification = struct();
  specification.dir_bip32 = dir_bip32;
...
if (~exist(lisa_arm1_r0.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver4',lisa_arm1_r0.dir_out_s0_pca)); end;
...
if (~exist(lisa_arm1_r1.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver4',lisa_arm1_r1.dir_out_s0_pca)); end;
...
%%%%%%%%%%%%%%%%;
fname_mat = sprintf('%s/prs_scatterplot_ver4_%s_trn%d%s_tst%d%s_ni%.3d_ni%.3d.mat',dir_out_s0_pca_r0_arm1,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2,ni_r1_arm2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
fname_fig = sprintf('%s/prs_scatterplot_ver4_%s_trn%d%s_tst%d%s_ni%.3d_ni%.3d.jpg',lisa_arm1_r0.dir_out_s0_prs_jpg,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2,ni_r1_arm2);
