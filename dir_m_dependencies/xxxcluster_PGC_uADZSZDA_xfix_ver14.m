function [output_string,gamma_d] = xxxcluster_PGC_uADZSZDA_xfix_ver14(prefix,maf_lo_threshold,maf_hi_threshold,mc_string,rev_flag,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,nshuffle);

if nargin<12;
n_mds = 2; mds_used_ = [1:2];
for maf_lo_threshold = [0.01,0.25];
for maf_hi_threshold = [0.15,0.50];
for mc_string_ = {'','m02_p85'};
for rev_flag = [0:1];
for mds_repl = [0,3];
...
for n_scramble = [0,1];
for nshuffle = [0,8];
disp(xxxcluster_PGC_uADZSZDA_xfix_ver14('dex',maf_lo_threshold,maf_hi_threshold,mc_string_{1},rev_flag,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,nshuffle));
disp(xxxcluster_PGC_uADZSZDA_xfix_ver14('lak',maf_lo_threshold,maf_hi_threshold,mc_string_{1},rev_flag,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,nshuffle));
...
output_string = sprintf('%s%s%s%s%s%s%s%s%s%s',prefix,maf_str,mc_str,rev_str,mds_str,gamma_str,B_str,Ireq_str,scramble_str,shuffle_str);
