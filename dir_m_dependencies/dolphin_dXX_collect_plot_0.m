...
dXX_ = setdiff([0:143],[27]); n_dXX = numel(dXX_);

fname_mat = sprintf('%s/dolphin_dXX_collect_0.mat',dir_mat);
...
xlim([-4,+4]); ylim([-4,+4]); xlabel('all'); ylabel('pair');
end;%if flag_plot;e
...
% There is only a 0.29 correlation between the all-dolphin and single-dolphin fullwise-interactions. ;
%%%%%%%%;
flag_plot=0;
if flag_plot;
figure(1);clf;
