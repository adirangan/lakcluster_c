function dolphin_dXX_cmb_6_collect();
%{
...
n_aid_found_ = [];
for tmp_dXX=1:160;
tmp_fname_infix_0099 = sprintf('aid%.2dd%dc%dC%d_cmb',tmp_dXX,flag_drift0,flag_center,flag_constrain_CC);
...
a_0099_o__{1+n_found_dXX} = tmp_.a_cmb_;
A_0099_o___{1+n_found_dXX} = tmp_.A_cmb__;
BB_0099_o___{1+n_found_dXX} = tmp_.BB_cmb__;
CC_0099_o___{1+n_found_dXX} = tmp_.CC_cmb__;
%%%%%%%%;
tmp_ = load(tmp_fname_mat_0099_s);
n_found_0099_{1+n_found_dXX} = size(tmp_.L_cmb_prm_,1);
a_avg_0099__{1+n_found_dXX} = mean(tmp_.a_cmb_prm__(:,2:end),2); a_std_0099__{1+n_found_dXX} = std(tmp_.a_cmb_prm__(:,2:end),1,2);
A_avg_0099___{1+n_found_dXX} = mean(tmp_.A_cmb_prm___(:,:,2:end),3); A_std_0099___{1+n_found_dXX} = std(tmp_.A_cmb_prm___(:,:,2:end),1,3);
BB_avg_0099___{1+n_found_dXX} = mean(tmp_.BB_cmb_prm___(:,:,2:end),3); BB_std_0099___{1+n_found_dXX} = std(tmp_.BB_cmb_prm___(:,:,2:end),1,3);
CC_avg_0099___{1+n_found_dXX} = mean(tmp_.CC_cmb_prm___(:,:,2:end),3); CC_std_0099___{1+n_found_dXX} = std(tmp_.CC_cmb_prm___(:,:,2:end),1,3);
%%%%%%%%;
...
fname_fig = sprintf('%s/dolphin_dXX_cmb_6_collect_FIGAB_snlp_corr',dir_jpg);
...
fname_fig = sprintf('%s/dolphin_dXX_cmb_6_collect_FIGAB_o_corr',dir_jpg);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig)); figure(1);
figure(1);clf;figbig;markersize_use = 64;A_lim_ = [-1,+1];
%%%%%%%%;
...
fname_fig = sprintf('%s/dolphin_dXX_cmb_6_collect_FIGAB_o_5099_corr',dir_jpg);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig)); figure(1);
figure(1);clf;figbig;markersize_use = 64;A_lim_ = [-1,+1];
%%%%%%%%;
