...
%%%%%%%%;
rank_estimate_E = max(find(E_svd_nlpZ__(:,1)> -log(0.01)));ZR
...
%%%%%%%%;
fname_mat = sprintf('%s/dir_mat/test_loader_cluster_C_rank_AB.mat');
...
[tmp_UE_liXf_clogr_,tmp_SE_liXf_clogr_,tmp_VE_liXf_clogr_] = svds(E_liXf_clogr_,n_u); %<-- use full rank of E_liXf_clogr_. ;
UE_liXf_clogr_C_ = inv(tmp_SE_liXf_clogr_)*transpose(tmp_UE_liXf_clogr_)*mean_center_0(C_rank_); %<-- explicit mean-centering not required for dexcluster, but perhaps for others. ;
%UE_liXf_clogr_C_ = inv(tmp_SE_liXf_clogr_)*transpose(tmp_UE_liXf_clogr_)*(C_rank_);
[tmp_U_UE_liXf_clogr_C_,tmp_S_UE_liXf_clogr_C_,tmp_V_UE_liXf_clogr_C_] = svds(UE_liXf_clogr_C_,n_CCOV);
%%%%%%%%;
% spectral-->isosplit5;
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/UE_liXf_clogr_C_spectral_scatter_FIGA',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
n_rank_CC = 6;
for nrank_CC=1:n_rank_CC;
opts_isosplit5 = struct('K_init',200,'isocut_threshold',1.0);
label_C__{nrank_CC} = transpose(isosplit5(transpose(tmp_V_UE_liXf_clogr_C_(:,1:nrank_CC)*tmp_S_UE_liXf_clogr_C_(1:nrank_CC,1:nrank_CC)),opts_isosplit5));
end;%for nrank_CC=1:n_rank_CC;
for nrank_CC=1:n_rank_CC;
subplot(2,3,nrank_CC); scatter(tmp_V_UE_liXf_clogr_C_(:,1),tmp_V_UE_liXf_clogr_C_(:,2),15,label_C__{nrank_CC}); colormap('lines');
end;%for nrank_CC=1:n_rank_CC;
figbig;
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
...
% dexnb;
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/UE_liXf_clogr_C_dexnb_scatter_FIGA',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
gamma = 0.00; n_shuffle = 64; p_set = 0.05; n_member_lob = 2;
gamma_d = floor(gamma*1000); str_g = sprintf('_g%.3d',gamma_d);
p_set_d = floor(p_set*1000); str_p = sprintf('_p%.3d',p_set_d);
str_nml = sprintf('_nml%d',n_member_lob);
str_xfix = sprintf('dexcluster_nonbinary_trace_ZRimax%s%s%s',str_g,str_p,str_nml);
prefix_base = sprintf('UE_liXf_clogr_C_%s',str_xfix); 
dir_out = []; E_array_base_ = transpose(UE_liXf_clogr_C_); E_array_r_ij_ = []; E_array_c_ij_ = [];
test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3(dir_cluster,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
%ZRimax_label_ = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3(dir_cluster,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
p_set_ = [0.05]*0.5.^[0:5];
for np=1:length(p_set_);
...
tmp_ZRimax_label_ = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3(dir_cluster,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,tmp_p_set,n_member_lob);
subplot(2,3,np);
scatter(tmp_V_UE_liXf_clogr_C_(:,1),tmp_V_UE_liXf_clogr_C_(:,2),15,label_str_to_enum_0(tmp_ZRimax_label_),'filled');
colormap('lines');
title(sprintf('p %0.6f #=%d',tmp_p_set,length(unique(tmp_ZRimax_label_))));
axisnotick;
end;%for np=1:length(p_set_);
...
%%%%%%%%;
p_set = 0.05; 
[ZRimax_output_label_,ZRimax_lpFmax_label_] = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3(dir_cluster,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
ZRimax_enum_ = label_str_to_enum_0(ZRimax_output_label_);
n_QCluster = length(unique(ZRimax_enum_));
%%%%%%%%;
% plot QC-clusters in PCA-space;
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/UE_liXf_clogr_C_dexnb_scatter_FIGA1',dir_trunk);
if (~exist(sprintf('%s.jpg',fname_fig),'file'));
clf;
tmp_Legend_ = {1:n_QCluster};
subplot(1,2,1);
colormap('lines');
hold on;
for nQCluster=0:n_QCluster-1;
...
for nQCluster=0:n_QCluster-1;
tmp_ij_ = find(ZRimax_enum_==1+nQCluster);
scatter(tmp_V_UE_liXf_clogr_C_(tmp_ij_,1),tmp_V_UE_liXf_clogr_C_(tmp_ij_,2),65,ZRimax_enum_(tmp_ij_),'filled','MarkerEdgeColor','k');
end;%for nQCluster=0:n_QCluster-1;
title(sprintf('QC clusters (UE_liXf_clogr_C)'),'Interpreter','none');
axisnotick;
...
%%%%%%%%;
% Identify numerical rank of each QC-cluster. ;
%%%%%%%%;
QCluster_rank_estimate_ = zeros(n_QCluster,1);
for nQCluster=0:n_QCluster-1;
tmp_c_index_ = efind(ZRimax_enum_==1+nQCluster);
tmp_C_ = C_rank_(:,1+tmp_c_index_);
tmp_UEC_ = inv(tmp_SE_liXf_clogr_)*transpose(tmp_UE_liXf_clogr_)*mean_center_0(tmp_C_);
tmp_n_CCOV = numel(tmp_c_index_);
tmp_fname_mat = sprintf('%s/dir_mat/S_c%d_UE_liXf_clogr_x__.mat',dir_trunk,1+nQCluster);
%%%%%%%%;
...
rank_estimate_sample_0(tmp_UEC_);
save(tmp_fname_mat ...
,'nQCluster' ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
...
QCluster_rank_estimate_(1+nQCluster) = tmp_.rank_estimate_sample;
clear tmp_;
end;%for nQCluster=0:n_QCluster-1;
%%%%%%%%;
...
for nQCluster=0:n_QCluster-1;
disp(sprintf(' %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'));
tmp_infix = sprintf('c%d_UE_liXf_clogr__',1+nQCluster);
tmp_c_index_ = efind(ZRimax_enum_==1+nQCluster);
tmp_n_CCOV = numel(tmp_c_index_);
disp(sprintf(' %% nQCluster %d/%d --> %s(%d)',nQCluster,n_QCluster,tmp_infix,tmp_n_CCOV));
disp(sprintf(' QCluster #%d (rank %d):',1+nQCluster,QCluster_rank_estimate_(1+nQCluster)));
...
%%%%%%%%;
Table_UE_liXf_clogr_C_ = zeros(n_QCluster,n_Sample_Label);
for nQCluster=0:n_QCluster-1;
QCluster_ij_ = find(ZRimax_enum_==1+nQCluster);
n_l = length(QCluster_ij_);
for nl=1:n_l;
...
for nSample_Label_B=1:n_Sample_Label;
tmp_lp = z_to_p_0(AB_.AB_C_absZ_(1+ns,QC_ij)); tmp_lp = -tmp_lp/(n_l*n_Sample_Label);
Table_UE_liXf_clogr_C_(1+nQCluster,nSample_Label_A) = Table_UE_liXf_clogr_C_(1+nQCluster,nSample_Label_A) + tmp_lp;
Table_UE_liXf_clogr_C_(1+nQCluster,nSample_Label_B) = Table_UE_liXf_clogr_C_(1+nQCluster,nSample_Label_B) + tmp_lp;
ns=ns+1;
end;%for nSample_Label_B=1:n_Sample_Label;
end;%for nSample_Label_A=1:n_Sample_Label;
end;%for nl=1:n_l;
...
subplot(1,1,1);
colormap(colormap_beach());
imagesc(Table_UE_liXf_clogr_C_); colorbar; figbig;
set(gca,'XTick',1:n_Sample_Label,'XTickLabel',unique(str_Sample_Label_)); xtickangle(90);
set(gca,'YTick',1:n_QCluster,'YTickLabel',1:n_QCluster);
title('QC-cluster vs sample-cluster -log(p-value)');
fname_fig = sprintf('%s/dir_jpg/Table_UE_liXf_clogr_C_dexnb_FIGB',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
%%%%%%%%;
...
%%%%%%%%;
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
UE_liXf_clogr_C_sub_ = fast_tsne_dr_0(transpose(UE_liXf_clogr_C_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.0));
opts_isosplit5 = struct('K_init',n_CCOV,'isocut_threshold',1.0);
label_UE_liXf_clogr_C_ = transpose(isosplit5(transpose(UE_liXf_clogr_C_sub_),opts_isosplit5));
subplot(1,1,1); scatter(UE_liXf_clogr_C_sub_(:,1),UE_liXf_clogr_C_sub_(:,2),15,label_UE_liXf_clogr_C_); colormap('lines');
fname_fig = sprintf('%s/dir_jpg/UE_liXf_clogr_C_umap00_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
%%%%%%%%;
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
UE_liXf_clogr_C_sub_ = fast_tsne_dr_0(transpose(UE_liXf_clogr_C_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.5));
opts_isosplit5 = struct('K_init',n_CCOV,'isocut_threshold',1.0);
label_UE_liXf_clogr_C_ = transpose(isosplit5(transpose(UE_liXf_clogr_C_sub_),opts_isosplit5));
subplot(1,1,1); scatter(UE_liXf_clogr_C_sub_(:,1),UE_liXf_clogr_C_sub_(:,2),15,label_UE_liXf_clogr_C_); colormap('lines');
fname_fig = sprintf('%s/dir_jpg/UE_liXf_clogr_C_umap50_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
tmp_rank_avg_ = mean(E_liXf_rankn_,1); tmp_rank_std_ = max(1,std(E_liXf_rankn_,1,1));
tmp_E_ = (E_liXf_rankn_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
...
tmp_rank_avg_ = mean(C_rank_,1); tmp_rank_std_ = max(1,std(C_rank_,1,1));
tmp_C_ = (C_rank_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
tmp_rank_avg_ = mean(I_liXf_rankn_,1); tmp_rank_std_ = max(1,std(I_liXf_rankn_,1,1));
tmp_I_ = (I_liXf_rankn_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
clear tmp_rank_avg_ tmp_rank_std_;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Fit linear model to log of imputed data (liXf) ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
n_rank_xeta = 16;
...
% cUE1 ;
%%%%%%%%;
[xeta_RRR_fill_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE1');
[xeta_LSQ_fill_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_fill_,I_fill_,dir_trunk,'fill_cUE1');
[xeta_RRR_liXf_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE1');
[xeta_LSQ_liXf_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE1');
[xeta_RRR_liXf_clogr_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE1');
[xeta_LSQ_liXf_clogr_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE1');
[xeta_RRR_liXf_rankn_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE1');
[xeta_LSQ_liXf_rankn_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE1');
%%%%%%%%;
% cUE2 ;
%%%%%%%%;
[xeta_RRR_fill_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE2');
[xeta_LSQ_fill_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_fill_,I_fill_,dir_trunk,'fill_cUE2');
[xeta_RRR_liXf_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE2');
[xeta_LSQ_liXf_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE2');
[xeta_RRR_liXf_clogr_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE2');
[xeta_LSQ_liXf_clogr_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE2');
[xeta_RRR_liXf_rankn_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE2');
[xeta_LSQ_liXf_rankn_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE2');
%%%%%%%%;
% cUE3 ;
%%%%%%%%;
[xeta_RRR_fill_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE3');
[xeta_LSQ_fill_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_fill_,I_fill_,dir_trunk,'fill_cUE3');
[xeta_RRR_liXf_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE3');
[xeta_LSQ_liXf_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE3');
[xeta_RRR_liXf_clogr_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE3');
[xeta_LSQ_liXf_clogr_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE3');
[xeta_RRR_liXf_rankn_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE3');
[xeta_LSQ_liXf_rankn_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE3');
%%%%%%%%;
% cUE4 ;
%%%%%%%%;
[xeta_RRR_fill_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE4');
[xeta_LSQ_fill_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_fill_,I_fill_,dir_trunk,'fill_cUE4');
[xeta_RRR_liXf_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE4');
[xeta_LSQ_liXf_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE4');
[xeta_RRR_liXf_clogr_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE4');
[xeta_LSQ_liXf_clogr_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE4');
[xeta_RRR_liXf_rankn_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE4');
[xeta_LSQ_liXf_rankn_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE4');
%%%%%%%%;
% cUE5 ;
%%%%%%%%;
[xeta_RRR_fill_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE5');
[xeta_LSQ_fill_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_fill_,I_fill_,dir_trunk,'fill_cUE5');
[xeta_RRR_liXf_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE5');
[xeta_LSQ_liXf_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE5');
[xeta_RRR_liXf_clogr_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE5');
[xeta_LSQ_liXf_clogr_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE5');
[xeta_RRR_liXf_rankn_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE5');
[xeta_LSQ_liXf_rankn_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE5');
%%%%%%%%;
% cUE6 ;
%%%%%%%%;
[xeta_RRR_fill_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cUE6');
[xeta_LSQ_fill_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_fill_,I_fill_,dir_trunk,'fill_cUE6');
[xeta_RRR_liXf_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cUE6');
[xeta_LSQ_liXf_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_liXf_,I_liXf_,dir_trunk,'liXf_cUE6');
[xeta_RRR_liXf_clogr_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cUE6');
[xeta_LSQ_liXf_clogr_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cUE6');
[xeta_RRR_liXf_rankn_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cUE6');
[xeta_LSQ_liXf_rankn_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cUE6');
%%%%%%%%;
% cPhr ;
%%%%%%%%;
[xeta_RRR_fill_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cPhr');
[xeta_LSQ_fill_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_fill_,I_fill_,dir_trunk,'fill_cPhr');
[xeta_RRR_liXf_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cPhr');
[xeta_LSQ_liXf_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_liXf_,I_liXf_,dir_trunk,'liXf_cPhr');
[xeta_RRR_liXf_clogr_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cPhr');
[xeta_LSQ_liXf_clogr_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cPhr');
[xeta_RRR_liXf_rankn_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cPhr');
[xeta_LSQ_liXf_rankn_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cPhr');
%%%%%%%%;
% c127 ;
%%%%%%%%;
[xeta_RRR_fill_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_c127');
[xeta_LSQ_fill_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_fill_,I_fill_,dir_trunk,'fill_c127');
[xeta_RRR_liXf_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_c127');
[xeta_LSQ_liXf_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_liXf_,I_liXf_,dir_trunk,'liXf_c127');
[xeta_RRR_liXf_clogr_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_c127');
[xeta_LSQ_liXf_clogr_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_c127');
[xeta_RRR_liXf_rankn_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_c127');
[xeta_LSQ_liXf_rankn_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_c127');
%%%%%%%%;
% c012 ;
%%%%%%%%;
[xeta_RRR_fill_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_c012');
[xeta_LSQ_fill_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_fill_,I_fill_,dir_trunk,'fill_c012');
[xeta_RRR_liXf_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_c012');
[xeta_LSQ_liXf_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_liXf_,I_liXf_,dir_trunk,'liXf_c012');
[xeta_RRR_liXf_clogr_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_c012');
[xeta_LSQ_liXf_clogr_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_c012');
[xeta_RRR_liXf_rankn_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_c012');
[xeta_LSQ_liXf_rankn_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_c012');
%%%%%%%%;
% cRRR ;
%%%%%%%%;
[xeta_RRR_fill_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_fill_,I_fill_,n_rank_xeta,dir_trunk,'fill_cRRR');
[xeta_LSQ_fill_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_fill_,I_fill_,dir_trunk,'fill_cRRR');
[xeta_RRR_liXf_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_liXf_,I_liXf_,n_rank_xeta,dir_trunk,'liXf_cRRR');
[xeta_LSQ_liXf_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_liXf_,I_liXf_,dir_trunk,'liXf_cRRR');
[xeta_RRR_liXf_clogr_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_liXf_clogr_,I_liXf_clogr_,n_rank_xeta,dir_trunk,'liXf_clogr_cRRR');
[xeta_LSQ_liXf_clogr_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_liXf_clogr_,I_liXf_clogr_,dir_trunk,'liXf_clogr_cRRR');
[xeta_RRR_liXf_rankn_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_liXf_rankn_,I_liXf_rankn_,n_rank_xeta,dir_trunk,'liXf_rankn_cRRR');
[xeta_LSQ_liXf_rankn_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_liXf_rankn_,I_liXf_rankn_,dir_trunk,'liXf_rankn_cRRR');
...
%%%%%%%%;
% Note that ranks of each UEx are roughly: [1,5,8,1,1,8] ;
%%%%%%%%;
n_iteration_wrap = 128;n_rank_wrap = 1;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE1_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 5;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE2_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 8;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE3_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 1;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE4_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 1;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE5_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 8;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cUE6_,n_rank_wrap);
%%%%%%%%;
n_iteration_wrap = 128;n_rank_wrap = 12;
%%%%%%%%;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cPhr_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_c127_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_c012_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_liXf_rankn_,I_liXf_rankn_,xeta_RRR_liXf_rankn_cRRR_,n_rank_wrap);
...
fprintf(fp_readme,'E_liXf_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns).\n');
fprintf(fp_readme,'I_liXf_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns).\n');
fprintf(fp_readme,'E_liXf_clogr_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio).\n');
fprintf(fp_readme,'I_liXf_clogr_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio).\n');
fprintf(fp_readme,'E_liXf_rankn_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio) followed by rank-normalization.\n');
fprintf(fp_readme,'I_liXf_rankn_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio) followed by rank-normalization.\n');
fprintf(fp_readme,'C_rank_c012_.tsv: tab delimited array of samples (rows) by covariate ranks (columns).\n');
fprintf(fp_readme,'E_liXf_c012_zeta_un_.tsv and E_liXf_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_c012_zeta_] := [E_liXf_c012_zeta_un_]*[E_liXf_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-LiXf] = [1 , Covariate-Rank]*[E_liXf_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_liXf_c012_beta_un_.tsv and E_liXf_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_c012_beta_] := [E_liXf_c012_beta_un_]*[E_liXf_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-LiXf]*[E_liXf_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_c012_zeta_un_.tsv and I_liXf_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_c012_zeta_] := [I_liXf_c012_zeta_un_]*[I_liXf_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-LiXf] = [1 , Covariate-Rank]*[I_liXf_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_c012_beta_un_.tsv and I_liXf_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_c012_beta_] := [I_liXf_c012_beta_un_]*[I_liXf_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-LiXf]*[I_liXf_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_liXf_clogr_c012_zeta_un_.tsv and E_liXf_clogr_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_clogr_c012_zeta_] := [E_liXf_clogr_c012_zeta_un_]*[E_liXf_clogr_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-LiXf_Clogr] = [1 , Covariate-Rank]*[E_liXf_clogr_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_liXf_clogr_c012_beta_un_.tsv and E_liXf_clogr_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_clogr_c012_beta_] := [E_liXf_clogr_c012_beta_un_]*[E_liXf_clogr_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-LiXf_Clogr]*[E_liXf_clogr_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_clogr_c012_zeta_un_.tsv and I_liXf_clogr_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_clogr_c012_zeta_] := [I_liXf_clogr_c012_zeta_un_]*[I_liXf_clogr_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-LiXf_Clogr] = [1 , Covariate-Rank]*[I_liXf_clogr_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_clogr_c012_beta_un_.tsv and I_liXf_clogr_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_clogr_c012_beta_] := [I_liXf_clogr_c012_beta_un_]*[I_liXf_clogr_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-LiXf_Clogr]*[I_liXf_clogr_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_liXf_rankn_c012_zeta_un_.tsv and E_liXf_rankn_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_rankn_c012_zeta_] := [E_liXf_rankn_c012_zeta_un_]*[E_liXf_rankn_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-LiXf_Rankn] = [1 , Covariate-Rank]*[E_liXf_rankn_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_liXf_rankn_c012_beta_un_.tsv and E_liXf_rankn_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_liXf_rankn_c012_beta_] := [E_liXf_rankn_c012_beta_un_]*[E_liXf_rankn_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-LiXf_Rankn]*[E_liXf_rankn_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_rankn_c012_zeta_un_.tsv and I_liXf_rankn_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_rankn_c012_zeta_] := [I_liXf_rankn_c012_zeta_un_]*[I_liXf_rankn_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-LiXf_Rankn] = [1 , Covariate-Rank]*[I_liXf_rankn_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_liXf_rankn_c012_beta_un_.tsv and I_liXf_rankn_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_liXf_rankn_c012_beta_] := [I_liXf_rankn_c012_beta_un_]*[I_liXf_rankn_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-LiXf_Rankn]*[I_liXf_rankn_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_C_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_K_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the E gene-ranks.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_Z_absZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_H_absZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_C_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_K_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the I gene-ranks.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_Z_absZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_H_absZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_C_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_K_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the E gene-ranks.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_Z_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_liXf_rankn_c012_H_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_C_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_K_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the I gene-ranks.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_Z_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_liXf_rankn_c012_H_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fclose(fp_readme);

%%%%%%%%;
str_X_ = {'E','I','EI'}; n_X = numel(str_X_);
str_Y_ = {'liXf_rankn'}; n_Y = numel(str_Y_);
str_Z_ = {'127','012','RRR','UE1','UE2','UE3','UE4','UE5','UE6'}; n_Z = numel(str_Z_);
n_Z_rank_ = [12,12,12,1,5,8,1,1,8]; assert(numel(n_Z_rank_)==n_Z);
n_Z_rank_sum = sum(n_Z_rank_);
cp_markergene___ = zeros(n_X,n_Z_rank_sum,1+n_Sample_Label);
for nX=1:n_X;
str_X = str_X_{nX};
for nY=1:n_Y;
str_Y = str_Y_{nY};
...
str_Z = str_Z_{nZ};
%%%%%%%%;
n_Z_rank = n_Z_rank_(nZ);
for nZ_rank=1:n_Z_rank;
nrank = nZ_rank;
str_prefix = sprintf('%s_%s_c%s_r%d',str_X,str_Y,str_Z,nrank);
str_file = sprintf('%s/dir_mat/dir_AB/AB_%s_.mat',dir_trunk,str_prefix);
disp(sprintf(' %% nX %d/%d nY %d/%d nZ %d/%d nZ_rank %d/%d %s',nX,n_X,nY,n_Y,nZ,n_Z,nZ_rank,n_Z_rank,str_file));
if (~exist(str_file,'file')); disp(sprintf(' %% %s not found, skipping',str_file)); end;
if ( exist(str_file,'file')); 
disp(sprintf(' %% %s found, not skipping',str_file));
tmp_AB_ = load(str_file,'AB_');
cp_markergene___(nX,1+na,:) = tmp_AB_.AB_.cp_markergene_;
end;%if ( exist(str_file,'file')); 
na=na+1;
end;%for nZ_rank=1:n_Z_rank;
%%%%%%%%;
end;%for nZ=1:n_Z;
assert(na==n_Z_rank_sum);
end;%for nY=1:n_Y;
end;%for nX=1:n_X;
...
str_prefix = sprintf('%s_%s_c%s_r%d',str_X,str_Y,str_Z,nrank);
str_file = sprintf('%s/dir_mat/dir_AB/AB_%s_.mat',dir_trunk,str_prefix);
str_out_ = sprintf(' %% %s\t:\t MarkerGene Correlation: \t ALL: %+0.2f\t EACH: ',str_prefix,cp_markergene___(nX,1+na,1));
for nSample_Label=1:n_Sample_Label;
str_out_ = sprintf('%s %+0.2f',str_out_,cp_markergene___(nX,1+na,1+nSample_Label));
end;%for nSample_Label=1:n_Sample_Label;
...
% Generate figures. ;
%%%%%%%%;
str_ticklabel_ = cell(1+n_Sample_Label,1);
str_ticklabel_{1} = 'all';
for nSample_Label=1:n_Sample_Label;
str_ticklabel_{1+nSample_Label} = sprintf('%d: ''%s''',nSample_Label,u_Sample_Label_{nSample_Label});
end;%for nSample_Label=1:n_Sample_Label;
for nX=1:n_X;
str_X = str_X_{nX};
for nY=1:n_Y;
str_Y = str_Y_{nY};
figure(nX);clf;
title(sprintf('%s %s',str_X,str_Y));
str_ylabel_ = cell(n_Z_rank_sum,1);
na=0;
for nZ=1:n_Z;
str_Z = str_Z_{nZ};
n_Z_rank = n_Z_rank_(nZ);
...
nrank = nZ_rank;
str_prefix = sprintf('%s_%s_c%s_r%d',str_X,str_Y,str_Z,nrank);
str_ylabel_{1+na} = str_prefix;
na=na+1;
end;%for nZ_rank=1:n_Z_rank;
end;%for nZ=1:n_Z;
assert(na==n_Z_rank_sum);
colormap(colormap_beach());
imagesc(squeeze(cp_markergene___(nX,:,:)),[0,1]);
set(gca,'TickLength',[0;0]);
set(gca,'XTick',1:1+n_Sample_Label,'XTickLabel',str_ticklabel_);
set(gca,'YTick',1:n_Z_rank_sum,'YTickLabel',str_ylabel_);
set(gca,'TickLabelInterpreter','none');
xtickangle(90);
set(gca,'FontSize',7);
%colorbar();
figbig;
fname_base = sprintf('%s/dir_jpg/cp_markergene_%s_%s_FIGA',dir_trunk,str_X,str_Y);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
close(gcf);
...
figbig;
fname = sprintf('%s/test_loader_27_cp_markergene_%s_FIGA',dir_jpg,str_X);
...
title(sprintf('%s',str_X));
figbig;
fname = sprintf('%s/test_loader_27_cp_markergene_%s_FIGB',dir_jpg,str_X);
disp(sprintf(' %% writing %s',fname));
print('-djpeg',sprintf('%s.jpg',fname));
print('-depsc',sprintf('%s.eps',fname));
