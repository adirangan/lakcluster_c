%function test_loader_cluster_collect_13();
% collect results from test_loader_cluster_wrap_driver_3.m ;
...
%%%%%%%%;
str_C_tsv_ = sprintf('%s/C_rank_.tsv',dir_mat);
C_rank_ = textread(str_C_tsv_); C_rank_ = C_rank_(:,1:end-1);
str_C_nsv_ = sprintf('%s/C_VariableName_.nsv',dir_mat);
...
str_E_tsv_ = sprintf('%s/E_%s_.tsv',dir_mat,prefix_normalization);
fp = fopen(str_E_tsv_,'r'); tmp_ = fgetl(fp); fclose(fp);
n_E_GENE = numel(str2num(tmp_)); clear tmp_;
str_I_tsv_ = sprintf('%s/I_%s_.tsv',dir_mat,prefix_normalization);
...
str_Y_ = {'fill','fill_c127_r12','fill_cRRR_r12','fill_c012_r12','fill_cUE1_r1','fill_cUE2_r5','fill_cUE3_r8','fill_cUE4_r1','fill_cUE5_r1','fill_cUE6_r8'}; n_Y = numel(str_Y_);
%str_Y_ = {'fill','fill_c127_r12'}; n_Y = numel(str_Y_);
...
str_Y = str_Y_{nY};
%%%%%%%%;
str_mat = sprintf('%s/dir_%s_%s_cluster/%s.mat',dir_cluster,str_X,str_Y,str_Z);
...
%%%%%%%%;
str_each_pre = sprintf('%s/dir_%s_%s_cluster/%s_each',dir_cluster,str_X,str_Y,str_Z);
...
ylabel('-log(p-value)');
figbig;
fname = sprintf('%s/test_loader_cluster_collect_13_%s_PYZ',dir_jpg,str_X);
...
ylabel('-log(p-value)');
figbig;
fname = sprintf('%s/test_loader_cluster_collect_13_%s_PZY',dir_jpg,str_X);
disp(sprintf(' %% writing %s',fname));
print('-djpeg',sprintf('%s.jpg',fname));
print('-depsc',sprintf('%s.eps',fname));
end;%for nX=1:n_X;
...
%{
  ...
  str_mat = sprintf('%s/dir_E_fill_cluster/dexcluster_nonbinary_trace_ZRimax_g010_p050_nml3.mat',dir_loader);
  tmp_mat_ = load(str_mat);
