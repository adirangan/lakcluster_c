function [A__,L] = dolphin_estimate_A_from_BC_2(aid_,age_,dat__,BB__,CC__,dt_lim_,n_step);
% Assuming a_=0, estimates A__, given BB__ and CC__;
% uses midpoint Y_{j+1/2}. ;
...
Y_mid_ = 0.5*(Y_pre_ + Y_pos_);
Y_mid_all_vt__(:,1+ntotal) = Y_mid_(:);
dY_ = (Y_pos_ - Y_pre_);
dY_all_vt__(:,1+ntotal) = dY_;
%YY_pre__ = Y_pre_*transpose(Y_pre_);
%YY_pre_all_vvt__(:,1+ntotal) = YY_pre__(:);
...
tmp_t = tic();
E01_bkp_nnm__ = DDinv_vvt__*(sparse(1:n_total,1:n_total,dt2_all_,n_total,n_total)*Y_mid_all_tv__);
...
tmp_t = tic();
E11_bkp_nnmm__ = DDinv_vvt__*(sparse(1:n_total,1:n_total,dt2_all_,n_total,n_total)*YY_mid_all_tvv__);
...
dtDDinvdYY__ = dtDDinvdY_*transpose(Y_mid_);
...
tmp_L = transpose(dY_ - tmp_dt*(A__*Y_mid_)) * DD__ * (dY_ - tmp_dt*(A__*Y_mid_));
