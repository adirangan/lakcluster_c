...
nb0 = tmp_b0_(nl0);
disp(sprintf(' %% nb0 %.9d: b0_{nb0} %s (%s)',nb0,b0_{nb0},bim_arm1_{7}{nb0}));
...
[e1_,e1_to_b1_,e1_to_d1_] = intersect(b1_,d1_,'stable');
...
nb0 = tmp_b0_(nl1);
disp(sprintf(' %% nb0 %.9d: b0_{nb0} %s (%s) (%0.6f %0.6f)',nb0,b0_{nb0},bim_arm1_{7}{nb0},V_r0_base_(nb0,:)));
assert(strcmp(b0_{nb0},e1_{ne1}));
end;%for nl0=0:length(tmp_b0_);
end;%if flag_check;

% Ordering V_r0_base_xxx_ to align with dosage file. ;
V_r0_perm_and_ = zeros(size(V_r0_base_and_)); V_r0_perm_xor_ = zeros(size(V_r0_base_xor_)); V_r0_perm_nor_ = zeros(size(V_r0_base_nor_));
...
[h1_,h1_to_f1_,h1_to_g1_] = intersect(f1_,g1_,'stable');
