function test_loader_lakcluster_nonbinary_dr_0(E_array_,gamma,n_shuffle,prefix);
...
str_trace = sprintf('%s/out_trace_laknb_s%.4d.txt',dir_trace,shuffle_num);
str_xdrop = sprintf('%s/out_xdrop_a_laknb.txt',dir_out);
...
[out_xdrop_E_0_,out_trace_E_0_] = lakcluster_nonbinary_AAAA_ver0(E_array_,gamma);
...
trace_{1+nshuffle} = textread(sprintf('%s/out_trace_laknb_s%0.4d.txt',dir_trace,nshuffle));
end;%for nshuffle=1:n_shuffle;
...
fname_base = sprintf('%s/%s_PCA_laknb_ni%.4d_r%d_c%d_scatterplot',dir_jpg,prefix,niteration,tmp_r_rem_cut,tmp_c_rem_cut);
...
flag_plot=1;
fname_base = sprintf('%s/%s_QR_QC_ZR_ZC_laknb',dir_jpg,prefix);
if flag_plot & (~exist(sprintf('%s.jpg',fname_base),'file') | flag_force_fig);
figure;clf;
...
flag_plot=1;
fname_base = sprintf('%s/%s_A_n_sorted_laknb',dir_jpg,prefix);
if flag_plot & (~exist(sprintf('%s.jpg',fname_base),'file') | flag_force_fig);
figure;clf;
subplot(1,2,1);colormap(colormap_beach()); imagesc(E_array_(rkeep_E_0_,ckeep_E_0_)); colorbar; title('lakcluster'); 
subplot(1,2,2);colormap(colormap_beach()); imagesc(E_array_(rkeep_E_0_,ckeep_E_0_)>0,[0,1]); colorbar; title('lakcluster'); 
figbig;
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
