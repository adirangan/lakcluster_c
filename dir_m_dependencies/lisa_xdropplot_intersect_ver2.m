function lisa_xdropplot_intersect_ver2(verbose,...
string_name_1_s0000,n_iteration_1,r_tot_1_,n_r_rem_1,r_rem_1_,c_tot_1_,n_c_rem_1,c_rem_1_,mr_D_1_full,mc_A_1,rdrop_a_1_,cdrop_a_1_,...
string_name_2_s0000,n_iteration_2,r_tot_2_,n_r_rem_2,r_rem_2_,c_tot_2_,n_c_rem_2,c_rem_2_,mr_D_2_full,mc_A_2,rdrop_a_2_,cdrop_a_2_);

%%%%%%%%%%%%%%%%;
% reconstruct r_tot and c_tot (total remaining) ;
%%%%%%%%%%%%%%%%;
%r_tot_1_ = sum(n_r_rem_1_)-cumsum([0;n_r_rem_1_(1:end-1)]);
%c_tot_1_ = sum(n_c_rem_1_)-cumsum([0;n_c_rem_1_(1:end-1)]);
%r_tot_2_ = sum(n_r_rem_2_)-cumsum([0;n_r_rem_2_(1:end-1)]);
%c_tot_2_ = sum(n_c_rem_2_)-cumsum([0;n_c_rem_2_(1:end-1)]);
...
c_rtn_max__ = n_c_full - max(c_rmv_1__,c_rmv_2__);
c_rtn_f__ = c_rtn__ ./ c_rtn_max__ ;
%%%%%%%%%%%%%%%%;
...
%%%%%%%%;
p_c_1_ = c_tot_1_./c_tot_1_(1); p_c_2_ = c_tot_2_./c_tot_2_(1);
c_mu__ = p_c_1_ * transpose(p_c_2_) * c_rtn__(1,1);
c_s2__ = (p_c_1_ * transpose(p_c_2_)) .* (1 - (p_c_1_ * transpose(p_c_2_))) .* c_rtn__(1,1);
c_sg__ = sqrt(c_s2__);
c_pv__ = 0.5*(1 - erf((c_rtn__(1:end-1,1:end-1) - c_mu__)./sqrt(2)./c_sg__));
...
figure;clf;
colormap('hot');
subplot(2,3,1); imagesc(r_rtn_f__,[0,1]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
...
subplot(2,3,4); imagesc(c_rtn_f__,[0,1]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
xlabel(string_name_2_s0000,'Interpreter','none'); 
ylabel(string_name_1_s0000,'Interpreter','none'); 
...
subplot(2,3,2); imagesc(log(r_rtn_f__./r_rtn_avg__),[-2,+2]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
xlabel(string_name_2_s0000,'Interpreter','none'); 
ylabel(string_name_1_s0000,'Interpreter','none'); 
title('log(row retain / expected)');
subplot(2,3,5); imagesc(log(c_rtn_f__./c_rtn_avg__),[-2,+2]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
xlabel(string_name_2_s0000,'Interpreter','none'); 
ylabel(string_name_1_s0000,'Interpreter','none'); 
title('log(col retain / expected)');
subplot(2,3,3); imagesc(-log10(r_pv__),[0,5]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
xlabel(string_name_2_s0000,'Interpreter','none'); 
ylabel(string_name_1_s0000,'Interpreter','none'); 
title('log10(row pvalue)');
subplot(2,3,6); imagesc(-log10(c_pv__),[0,5]); colorbar;
axis square;
%set(gca,'Xtick',[],'Ytick',[]); 
xlabel(string_name_2_s0000,'Interpreter','none'); 
ylabel(string_name_1_s0000,'Interpreter','none'); 
title('log10(col pvalue)');
