function test_loader_dexcluster_nonbinary_scorebox_recursive_0(dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set);
%{
  ...
  test_loader_dexcluster_nonbinary_scorebox_recursive_0([],prefix_base,E_xxxx_,[],[],gamma,n_shuffle,[]);
  ...
  test_loader_dexcluster_nonbinary_scorebox_recursive_0([],prefix_base,E_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear E_xxxx_cxxx_ ;
  ...
  test_loader_dexcluster_nonbinary_scorebox_recursive_0([],prefix_base,I_xxxx_,[],[],gamma,n_shuffle,[]);
  end;%if ( strcmp(str_ctype,'000'));
  if (~strcmp(str_ctype,'000'));
  C_rank_cxxx_ = textread(sprintf('%s/dir_mat/C_rank_c%s_.tsv',dir_trunk,str_ctype)); C_rank_cxxx_ = C_rank_cxxx_(:,1:end-1);
  ...
  test_loader_dexcluster_nonbinary_scorebox_recursive_0([],prefix_base,I_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear I_xxxx_cxxx_ ;
  end;%for ncol_=1:length(n_col_);
  end;%if (~strcmp(str_ctype,'000'));
  %clear I_xxxx_ ;
...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('dexnb_scorebox',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering test_loader_dexcluster_nonbinary_scorebox_recursive_0]: dir_out %s',dir_out)); end;
...
str_qsub = sprintf('%s/out_qsub_s%.4d.txt',dir_trace,shuffle_num);
if ( (exist(str_trace,'file') & exist(str_qsub,'file')) & ~flag_force_create);
if (verbose>1); disp(sprintf(' %% %s found, not creating',str_trace)); end;
out_trace_E_0_ = textread(str_trace);
end;%if ( (exist(str_trace,'file') & exist(str_qsub,'file')) & ~flag_force_create);
if (~(exist(str_trace,'file') & exist(str_qsub,'file')) |  flag_force_create);
...
[Q_] = dexcluster_nonbinary_AAAA_scorebox_ver0(L_array_(rkeep_E_0_,ckeep_E_0_));
Z_sub_ = (rkeep_E_0_ij_.^2 - rkeep_E_0_ij_) * transpose(ckeep_E_0_ij_) ;
Q_sub_ = Q_(rkeep_E_0_ij_,ckeep_E_0_ij_)./Z_sub_;
clear L_array Q_;
...
fclose(fp);
save(str_qsub,'Q_sub_','-ascii','-tabs');
end;%if (~(exist(str_trace,'file') & exist(str_qsub,'file')) |  flag_force_create);
%%%%%%%%;
...
Q_p_s0000 = 1;
if n_iteration>1;
Q_sub_ = cell(1+n_shuffle,1);
for nshuffle=0:n_shuffle;
str_qsub = sprintf('%s/out_qsub_s%0.4d.txt',dir_trace,nshuffle);
tmp_Q_sub_ = textread(str_qsub); tmp_Q_sub_ = tmp_Q_sub_(:,1:end-1);
Q_sub_{1+nshuffle} = flipud(fliplr(tmp_Q_sub_));
clear tmp_Q_sub_;
end;%for nshuffle=0:n_shuffle;
%%%%%%%%;
figure(1);clf;
[Q_p_s0000,Q_p_s0000_] = test_loader_xxxcluster_Q_to_p_0(gamma,Q_sub_,n_iter_,r_rem_,c_rem_);
str_fig = sprintf('%s/Q_p',dir_jpg);
figbig;
...
%%%%%%%%;
flag_split=1;
Q_x_ij_ = []; Q_r_ij_ = []; Q_c_ij_ = []; p_branch = 1;
%%%%%%%%;
if (Q_p_s0000> p_set); 
if (verbose>1); disp(sprintf(' %% Q_p_s0000 %0.6f; not significant. ;',Q_p_s0000));  end;
end;%if (Q_p_s0000> p_set); 
%%%%%%%%;
if (Q_p_s0000<=p_set); 
%%%%%%%%;
Q_p_s0000_max_ = ones(1+2*(n_iteration-1),1); 
Q_r_ij_ = zeros(1+2*(n_iteration-1),1); 
Q_c_ij_ = zeros(1+2*(n_iteration-1),1);
for ni_r=1:n_iteration;for ni_c=1:n_iteration;
ni_x = 1 + (ni_r-1) + (ni_c-1);
ni_y = ni_r - ni_c;
flag_p_01 = (Q_p_s0000_max_(ni_x)==1);
flag_p_lt = Q_p_s0000_(ni_r,ni_c) <  Q_p_s0000_max_(ni_x);
flag_p_eq = Q_p_s0000_(ni_r,ni_c) == Q_p_s0000_max_(ni_x);
flag_p_le = Q_p_s0000_(ni_r,ni_c) <= Q_p_s0000_max_(ni_x);
flag_p_lm = Q_p_s0000_(ni_r,ni_c) <= p_set;
flag_y_lt = abs(ni_y) <  abs(Q_r_ij_(ni_x)-Q_c_ij_(ni_x));
flag_y_ge = abs(ni_y) >= abs(Q_r_ij_(ni_x)-Q_c_ij_(ni_x));
%if ((flag_p_lt) | (flag_p_eq & flag_y_lt));
if ((flag_p_lm & flag_p_01) | (flag_p_lm & flag_y_lt));
Q_p_s0000_max_(ni_x) = Q_p_s0000_(ni_r,ni_c);
Q_r_ij_(ni_x) = ni_r; Q_c_ij_(ni_x) = ni_c;
end;%if ((flag_p_lm & flag_p_01) | (flag_p_lm & flag_y_lt));
%end;%if ((flag_p_lt) | (flag_p_eq & flag_y_lt));
end;end;%for ni_r=1:n_iteration;for ni_c=1:n_iteration;
[Q_x_ij_] = find(Q_p_s0000_max_<=p_set); %<-- imagesc(sparse(Q_r_ij_(Q_x_ij_),Q_c_ij_(Q_x_ij_),1,n_iteration,n_iteration));
n_Q_x_ij = length(Q_x_ij_);
if (verbose>1); disp(sprintf(' %% Q_p_s0000 %0.6f; statistically significant, n_Q_x_ij %d. ;',Q_p_s0000,n_Q_x_ij)); end;
E1_gamma_z_ = zeros(n_Q_x_ij,1);
E1_gsplit_llr_ = zeros(n_Q_x_ij,1);
E1_gsplit_label_ = cell(n_Q_x_ij,1);
E1_gsplit_llr__ = cell(n_Q_x_ij,1);
n_gamma_z = 16;%n_gamma_z = 64;
gamma_z_ = linspace(0,pi,n_gamma_z+1); gamma_z_ = gamma_z_(1:end-1);
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
for nQ_x_ij=1:n_Q_x_ij;
Q_x_ij = Q_x_ij_(nQ_x_ij);
Q_r_ij = Q_r_ij_(Q_x_ij);
Q_c_ij = Q_c_ij_(Q_x_ij);
if (verbose>1); disp(sprintf(' %% nQ_x_ij %d/%d Q_x_ij %d/%d Q_r_ij %d/%d Q_c_ij %d/%d Q_p_ %0.6f',nQ_x_ij,n_Q_x_ij,Q_x_ij,1+2*(n_iteration-1),Q_r_ij,n_iteration,Q_c_ij,n_iteration,Q_p_s0000_(Q_r_ij,Q_c_ij))); end;
tmp_r_rem_cut = r_rem_(Q_r_ij); tmp_c_rem_cut = c_rem_(Q_c_ij); n_rank = 2;
...
[E1_gamma_z_(nQ_x_ij),E1_gsplit_llr_(nQ_x_ij),E1_gsplit_label_{nQ_x_ij},~,E1_gsplit_llr__{nQ_x_ij}] = gsplit_gamma_z(tmp_E1_pos_,n_gamma_z);
clear tmp_r_rem_cut tmp_c_rem_cut tmp_U_ tmp_V_ tmp_E1_pos_;
end;%for nQ_x_ij=1:n_Q_x_ij;
clear L_array_;
%%%%%%%%;
[~,nQ_x_ij] = max(E1_gsplit_llr_);
Q_x_ij = Q_x_ij_(nQ_x_ij);
Q_r_ij = Q_r_ij_(Q_x_ij);
Q_c_ij = Q_c_ij_(Q_x_ij);
p_branch = Q_p_s0000_(Q_r_ij,Q_c_ij);
if (verbose>1); disp(sprintf(' %% max E1_gsplit_llr_: Q_r_ij %d/%d Q_c_ij %d/%d p %0.6f',Q_r_ij,n_iteration,Q_c_ij,n_iteration,Q_p_s0000_(Q_r_ij,Q_c_ij))); end;
tmp_r_rem_cut = r_rem_(Q_r_ij); tmp_c_rem_cut = c_rem_(Q_c_ij); n_rank = 2;
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
[tmp_U_,~,tmp_V_] = svds( L_array_(rkeep_E_0_(1:tmp_r_rem_cut),ckeep_E_0_(1:tmp_c_rem_cut)) ,n_rank);
tmp_E1_pos_ = L_array_(:,ckeep_E_0_(1:tmp_c_rem_cut))*tmp_V_; %<-- original. ;
tmp_data_ = tmp_E1_pos_(:,[1,2]);
tmp_label_ = E1_gsplit_label_{nQ_x_ij};
tmp_gamma_z = E1_gamma_z_(nQ_x_ij);
tmp_llr = E1_gsplit_llr_(nQ_x_ij);
tmp_data_0_ = tmp_data_(:,1)*cos(tmp_gamma_z) + tmp_data_(:,2)*sin(tmp_gamma_z);
...
save(str_branch,'trace_','QR_','QC_','ZR_','ZC_','Q_p_s0000','Q_p_s0000_','p_set','p_branch','type_code','nQ_x_ij','Q_x_ij','Q_x_ij_','Q_r_ij','Q_r_ij_','Q_c_ij','Q_c_ij_','branch_A_r_ij_','branch_B_r_ij_','E1_gamma_z_','E1_gsplit_llr_','E1_gsplit_label_','E1_gsplit_llr__','n_gamma_z','gamma_z_');
%%%%%%%%;
...
subplot(1,2,1);
scatter(tmp_data_(:,1),tmp_data_(:,2),25,E1_gsplit_label_{nQ_x_ij}); xlabel('pc1'); ylabel('pc2');
title(sprintf('%s branch original <-- %0.6f',dir_out,p_branch),'Interpreter','none');
subplot(3,2,2);
hold on;
plot(gamma_z_,E1_gsplit_llr__{nQ_x_ij},'k-');
plot(tmp_gamma_z,tmp_llr,'ro');
hold off;
xlim([0,pi]); xlabel('gamma_z'); ylabel('log-likelihood-ratio');
title(sprintf('gamma vs llr for Q_r_ij %d Q_c_ij %d',Q_r_ij,Q_c_ij));
subplot(3,2,[4,6]);
hold on;
...
%%%%%%%%;
if ((Q_p_s0000>p_set) | ~flag_split);
type_code = 'stop';
branch_A_r_ij_ = [];
branch_B_r_ij_ = [];
save(str_branch,'trace_','QR_','QC_','ZR_','ZC_','Q_p_s0000','Q_p_s0000_','p_set','p_branch','type_code','Q_x_ij_','Q_r_ij_','Q_c_ij_','branch_A_r_ij_','branch_B_r_ij_');
end;%if ((Q_p_s0000>p_set) | ~flag_split);
%%%%%%%%;
end;%if (~exist(str_branch,'file'));
...
test_loader_dexcluster_nonbinary_scorebox_recursive_0(dir_A_out,prefix_base,E_array_base_,E_array_A_r_ij_,E_array_A_c_ij_,gamma,n_shuffle,p_set);
...
test_loader_dexcluster_nonbinary_scorebox_recursive_0(dir_B_out,prefix_base,E_array_base_,E_array_B_r_ij_,E_array_B_c_ij_,gamma,n_shuffle,p_set);
