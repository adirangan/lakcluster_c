...
rank_estimate_sample_0( [ ones(n_u,1) , UX_C_ ] );
save(tmp_fname_mat ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
,'svd_sample__' ...
...
scatter(tmp_SC_(1,1)*tmp_VC_(tmp_ij_,1),tmp_SC_(2,2)*tmp_VC_(tmp_ij_,2),65,ZRimax_enum_(tmp_ij_),tmp_symbol,'filled','MarkerEdgeColor','k');
end;%for nQCluster=0:n_QCluster-1;
...
tmp_ij_ = find(ZRimax_enum_==1+nQCluster);
tmp_symbol = tmp_symbol_{1+nQCluster};
scatter(tmp_V_UX_C_(tmp_ij_,1),tmp_V_UX_C_(tmp_ij_,2),65,ZRimax_enum_(tmp_ij_),tmp_symbol,'filled','MarkerEdgeColor','k');
end;%for nQCluster=0:n_QCluster-1;
title(sprintf('QC clusters (U%s_C)',X_infix),'Interpreter','none');
axis equal; axisnotick;
legend(tmp_Legend_,'Location','SouthEast');
subplot(1,3,3);
[tmp_AUL_n_] = label_pca_plot_0(transpose(UX_C_),label_str_to_enum_0(ZRimax_output_label_));
colormap(colormap_beach());
hold on;
for nQCluster=0:n_QCluster-1;
tmp_ij_ = find(ZRimax_enum_==1+nQCluster);
...
title(sprintf('QC clusters (labeled pca)',X_infix),'Interpreter','none');
axis equal; axisnotick;
legend(tmp_Legend_,'Location','SouthEast');
...
rank_estimate_sample_0( [ ones(n_u,1) ,  tmp_UXC_ ] );
save(tmp_fname_mat ...
,'nQCluster' ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
...
rank_estimate_sample_0( [ ones(n_u,1) , mean_center_0(tmp_C_) ] );
save(tmp_fname_mat ...
,'nQCluster' ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
