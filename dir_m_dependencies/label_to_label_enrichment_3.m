function [lpv,lP_0,flag_method,cap_,cup_] = label_to_label_enrichment_3(label_A_,label_B_);
...
if (nargin<1);
disp(sprintf(' %% [testing label_to_label_enrichment_3]'));
setup_OptiPlex;
fname_mat = sprintf('~/dir_bcc/dir_jamison/dir_mat/label_to_label_enrichment_3.mat');
...
u_nlP0_ = unique(nlP0__(:)); 
u_nlP0_ = u_nlP0_(find(isfinite(u_nlP0_)));
...
%        = 1/sqrt(2*pi)^n_d * sqrt(2*pi)^n_d * \int_{R}^{\infty}  * exp(-0.5 * r^2) r^(n_d-1)dr ;
...
hold off;
legend({'mcis','true','quad','mc','phat'},'Location','SouthEast');
xlim([min(u_nlP0_),max(u_nlP0_)/2]);
ylim(ylim_);
ylabel('-log(p)');
xlabel('-log(phat)');
title(sprintf('-log(p)'));
%%%%%%%%;
figbig;
disp(sprintf(' %% writing %s',sprintf('~/dir_bcc/dir_jamison/dir_jpg/label_to_label_enrichment_cartoon_3.eps')));
print('-depsc',sprintf('~/dir_bcc/dir_jamison/dir_jpg/label_to_label_enrichment_cartoon_3.eps'));
print('-djpeg',sprintf('~/dir_bcc/dir_jamison/dir_jpg/label_to_label_enrichment_cartoon_3.jpg'));
close(gcf);
...
%%%%%%%%;
verbose=0;
if (verbose); disp(sprintf(' %% [entering label_to_label_enrichment_3]')); end;
...
if (verbose); disp(sprintf(' %% [finished label_to_label_enrichment_3]')); end;
