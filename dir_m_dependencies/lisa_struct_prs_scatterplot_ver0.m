function lisa = lisa_struct_prs_scatterplot_ver0(specification);
% try: ;
%{
...
specification.mc_string = mc_string;
  ...
  lisa_struct_prs_scatterplot_ver0(specification) ;
...
if (~exist(lisa_arm1_r0.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver0',lisa_arm1_r0.dir_out_s0_pca)); end;
...
if (~exist(lisa_arm1_r1.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver0',lisa_arm1_r1.dir_out_s0_pca)); end;
...
%%%%%%%%%%%%%%%%;
fname_mat = sprintf('%s/prs_scatterplot_ver0_%s_trn%d%s_tst%d%s.mat',dir_out_s0_pca_r0_arm1,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string);
...
load(fname_mat,...
     ...
     'auc19_prs_t_',...
     'p_x_rem_','p_d_rem_',...
     'r0_min','r0_max','r0_t_','r1_min','r1_max','r1_t_',...
     'lp_auc00_');
...
auc19_prs_t_ = zeros(1+n_r0_t,1+n_r1_t,n_prs);
p_x_rem_ = zeros(1+n_r0_t,1+n_r1_t);
p_d_rem_ = zeros(1+n_r0_t,1+n_r1_t);
r0_min = prctile(DnV_r0_arm2_(find_d_,1), 0);
r0_max = prctile(DnV_r0_arm2_(find_d_,1), 95);
r0_t_ = prctile(DnV_r0_arm2_(find_d_,1), linspace(0,100,1+2*n_r0_t) );
r1_min = prctile(DnV_r1_arm2_(find_d_,1), 0);
r1_max = prctile(DnV_r1_arm2_(find_d_,1), 95);
r1_t_ = prctile(DnV_r1_arm2_(find_d_,1), linspace(0,100,1+2*n_r1_t) );
%%%%%%%%;
for nr0_t=0:n_r0_t; for nr1_t=0:n_r1_t;
r0_t_min = r0_t_(1 + n_r0_t - nr0_t); 
r0_t_max = r0_t_(1 + n_r0_t + nr0_t); 
r1_t_min = r1_t_(1 + n_r1_t - nr1_t); 
r1_t_max = r1_t_(1 + n_r1_t + nr1_t); 
find_x_t_ = find( ( (XnV_r0_arm2_(find_x_,1)<=r0_t_min) | (XnV_r0_arm2_(find_x_,1)>=r0_t_max) ) & ( (XnV_r1_arm2_(find_x_,1)<=r1_t_min) | (XnV_r1_arm2_(find_x_,1)>=r1_t_max) ) ); 
find_d_t_ = find( ( (DnV_r0_arm2_(find_d_,1)<=r0_t_min) | (DnV_r0_arm2_(find_d_,1)>=r0_t_max) ) & ( (DnV_r1_arm2_(find_d_,1)<=r1_t_min) | (DnV_r1_arm2_(find_d_,1)>=r1_t_max) ) ); 
p_x_rem_(1+nr0_t,1+nr1_t) = length(find_x_t_)/length(find_x_);
p_d_rem_(1+nr0_t,1+nr1_t) = length(find_d_t_)/length(find_d_);
for nprs=1:n_prs;
...
if (length(find_x_t_)==0 & length(find_d_t_)==0); auc19_prs_t_(1+nr0_t,1+nr1_t,nprs) = 0.5; end;
if (length(find_x_t_)==0 & length(find_d_t_)> 0); auc19_prs_t_(1+nr0_t,1+nr1_t,nprs) = 1.0; end;
if (length(find_x_t_)> 0 & length(find_d_t_)==0); auc19_prs_t_(1+nr0_t,1+nr1_t,nprs) = 0.0; end;
if (length(find_x_t_)> 0 & length(find_d_t_)> 0); auc19_prs_t_(1+nr0_t,1+nr1_t,nprs) = cauc_0(tmp_prs_x_t_,tmp_prs_d_t_,tmp_mds_x_t_,tmp_mds_d_t_); end;
end;%for nprs=1:n_prs;
end;end;%for nr0_t=0:n_r0_t;for nr1_t=0:n_r1_t;
%%%%%%%%;
lp_auc00_ = zeros(1+n_r0_t,1+n_r1_t,n_prs);
for nr0_t=0:n_r0_t; for nr1_t=0:n_r1_t;
r0_t_min = r0_t_(1 + n_r0_t - nr0_t); 
r0_t_max = r0_t_(1 + n_r0_t + nr0_t); 
r1_t_min = r1_t_(1 + n_r1_t - nr1_t); 
r1_t_max = r1_t_(1 + n_r1_t + nr1_t); 
for nprs=1:n_prs;
tmp_n = min(p_x_rem_(1+nr0_t,1+nr1_t)*length(find_x_),p_d_rem_(1+nr0_t,1+nr1_t)*length(find_d_));
tmp_a = auc19_prs_t_(1+nr0_t,1+nr1_t,nprs);
lp_auc00_(1+nr0_t,1+nr1_t,nprs) = logp_auc_0(tmp_a,tmp_n);
end;%for nprs=1:n_prs;
end;end;%for nr0_t=0:n_r0_t;for nr1_t=0:n_r1_t;
%%%%%%%%;
...
fname_fig = sprintf('%s/prs_tbthreshold_ver0_%s_trn%d%s_tst%d%s_ni%.3d_ni%.3d.jpg',dir_out_s0_pca_r0_arm1,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2,ni_r1_arm2);
...
imagesc(squeeze(auc19_prs_t_(:,:,nprs)),cauclim_);
tmp_tick_r1_ = round(linspace(1,1+n_r1_t,n_tick)); xlabel('r1 threshold (p_rem)'); set(gca,'XTick',tmp_tick_r1_,'XTickLabel',0.01*round(100*p_d_rem_(1,tmp_tick_r1_)));
tmp_tick_r0_ = round(linspace(1,1+n_r0_t,n_tick)); ylabel('r0 threshold (p_rem)'); set(gca,'YTick',tmp_tick_r0_,'YTickLabel',0.01*round(100*p_d_rem_(tmp_tick_r0_,1)));
xtickangle(90);
...
hold on;
plot(0:n_r0_t,squeeze(auc19_prs_t_(1,:,nprs)),'b.-','MarkerSize',25,'LineWidth',2); 
plot(0:n_r1_t,squeeze(auc19_prs_t_(:,1,nprs)),'r.-','MarkerSize',25,'LineWidth',2); 
xlim([0,n_r0_t]); ylim(cauclim_); legend('bc2','bc1','Location','NorthWest');
tmp_tick_ = round(linspace(1,1+n_r0_t,n_tick)); %tmp_tick_ = tmp_tick_(end:-1:1);
tmp_tick_val_ = tmp_tick_; %tmp_tick_val_ = p_d_rem_(tmp_tick_,1);
tmp_tick_label_ = num2str(0.01*round(100*p_d_rem_(tmp_tick_,1)),'%0.2f');
...
%imagesc(squeeze(p_d_rem_(:,:)),[0.0,1.0]);
%tmp_tick_r1_ = round(linspace(1,1+n_r1_t,n_tick)); xlabel('r1 threshold (p_rem)'); set(gca,'XTick',tmp_tick_r1_,'XTickLabel',0.01*round(100*p_d_rem_(1,tmp_tick_r1_)));
%tmp_tick_r0_ = round(linspace(1,1+n_r0_t,n_tick)); ylabel('r0 threshold (p_rem)'); set(gca,'YTick',tmp_tick_r0_,'YTickLabel',0.01*round(100*p_d_rem_(tmp_tick_r0_,1)));
%xtickangle(90);
...
%imagesc(-(1/log(10))*squeeze(lp_auc00_(:,:,nprs)),[0,10]);
%tmp_tick_r1_ = round(linspace(1,1+n_r1_t,n_tick)); xlabel('r1 threshold (p_rem)'); set(gca,'XTick',tmp_tick_r1_,'XTickLabel',0.01*round(100*p_d_rem_(1,tmp_tick_r1_)));
%tmp_tick_r0_ = round(linspace(1,1+n_r0_t,n_tick)); ylabel('r0 threshold (p_rem)'); set(gca,'YTick',tmp_tick_r0_,'YTickLabel',0.01*round(100*p_d_rem_(tmp_tick_r0_,1)));
%xtickangle(90);
