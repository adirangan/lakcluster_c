function output_label_ = label_rearrange_1(input_label_,data_);
% rearranges numeric labels; 
% 0. calculates the intra-cluster mean (avg) and maximum-likelihood std for isotropic-gaussian ( std^2 = 1/d * mean(squared-distance) ). ;
...
%%%%%%%%;
verbose=0;
if (verbose); disp(sprintf(' %% [entering label_rearrange_1]')); end;
%%%%%%%%;
...
tmp_d2_ = zeros(n_u_(nu1),1);
for nu2=1:n_u_(nu1);
tmp_d2_(nu2) = fnorm(tmp_data_(nu2,:) - avg_(nu1,:)).^2;
end;%for nu2=1:n_u_(nu1);
std_(nu1) = sqrt( mean(tmp_d2_) / size(data_,2) );
if (std_(nu1)<=1e-12); std_(nu1) = 1.0d0 ; end;
clear tmp_data_ tmp_d2_ ;
end;%for nu1=1:n_u;
...
%%%%%%%%;
if (verbose); disp(sprintf(' %% [finished label_rearrange_1]')); end;
