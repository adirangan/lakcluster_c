function test_loader_cluster_collect_5();
...
n_index = n_index_(nZ);
%%%%%%%%;
for str_Y_ = {'li16f','li16f_c127_r9','li16f_cRRR_r9','li16f_c012_r9'};%for str_Y_ = {'li16f','li16f_c127_r3','li16f_c127_r6','li16f_c127_r9'};
str_Y = str_Y_{1};
%%%%%%%%;
...
lpv_each_ = zeros(n_label_A,n_index);
lp0_each_ = zeros(n_label_A,n_index);
flag_method_each_ = zeros(n_label_A,n_index);
for nindex=1:n_index;
for nlabel_A=1:n_label_A;
disp(sprintf(' %% %s %s %s: each for nindex %d nlabel_A %d (str %s)',str_X,str_Y,str_Z,nindex,nlabel_A,u_label_A_{nlabel_A}));
[lpv_each_(nlabel_A,nindex),lP0_each_(nlabel_A,nindex),flag_method_each_(nlabel_A,nindex)] = label_to_label_enrichment_2(label_A_each__(:,nlabel_A),tmp_mat_.label_B__{nindex});
disp(sprintf(' %% lpv exp(%0.6f) lp0 exp(%0.6f) flag_method %d',lpv_each_(nlabel_A,nindex),lP0_each_(nlabel_A,nindex),flag_method_each_(nlabel_A,nindex)));
end;%for nlabel_A=1:n_label_A;
end;%for nindex=1:n_index;
...
tmp_str2_ = sprintf('%s %.3d',tmp_str2_,round(-lpv_each_(nlabel_A,nindex)));
end;%for nlabel_A=1:n_label_A;
