...
dir_trunk = pwd;
dir_out = [];
prefix_base = "test_d"; system('rm -rf /home/rangan/dir_bcc/dir_jamison/dir_m/dir_test_d/dir_test_d_dexnbrtZRgumb_g020');
...
for nr=0:n_r-1;
disp(sprintf(' %% %s --> %s --> %s',output_label_{1+nr},nlpbra_label_{1+nr},nlpnex_label_{1+nr}));
end;%for nr=0:n_r-1;

%%%%%%%%;
% compare outputs. ;
%%%%%%%%;
c_out_xdrop__ = transpose(c_MDA_read_i4('/home/rangan/dir_bcc/dir_halfloop_dev/dir_test_r0/dir_test_r0_g020_n03/xdrop_F__.mda'));
m_out_xdrop__ = textread('./dir_test_d/dir_test_d_dexnbrtZRgumb_g020/out_xdrop_a.txt');
disp(sprintf(' %% m_out_xdrop__ vs c_out_xdrop__: %0.16f',fnorm(m_out_xdrop__ - c_out_xdrop__)/fnorm(m_out_xdrop__)));
c_QR_F__ = c_MDA_read_r8('/home/rangan/dir_bcc/dir_halfloop_dev/dir_test_r0/dir_test_r0_g020_n03/QR_F__.mda');
m_out_trace_F_ = load('./dir_test_d/dir_test_d_dexnbrtZRgumb_g020/out_trace_.mat');
m_QR_F_ = m_out_trace_F_.trace_E_{1}(:,1+3);
n_iteration = numel(m_QR_F_);
m_QR_F__ = zeros(n_iteration,1+n_shuffle);
m_QR_F__(:,1) = m_QR_F_;
for nshuffle=0:n_shuffle-1;
m_QR_F__(:,1+nshuffle+1) = m_out_trace_F_.trace_E_{1+nshuffle+1}(:,1+3);
end;%for nshuffle=0:n_shuffle-1;
disp(sprintf(' %% m_QR_F__ vs c_QR_F__: %0.16f',fnorm(m_QR_F__ - c_QR_F__)/fnorm(m_QR_F__)));
%%%%%%%%;

%%%%%%%%;
c_out_xdrop__ = transpose(c_MDA_read_i4('/home/rangan/dir_bcc/dir_halfloop_dev/dir_test_r0/dir_test_r0_g020_n03/A/xdrop_F__.mda'));
m_out_xdrop__ = textread('./dir_test_d/dir_test_d_dexnbrtZRgumb_g020/dir_A/out_xdrop_a.txt');
disp(sprintf(' %% m_out_xdrop__ vs c_out_xdrop__: %0.16f',fnorm(m_out_xdrop__ - c_out_xdrop__)/fnorm(m_out_xdrop__)));
c_QR_F__ = c_MDA_read_r8('/home/rangan/dir_bcc/dir_halfloop_dev/dir_test_r0/dir_test_r0_g020_n03/A/QR_F__.mda');
m_out_trace_F_ = load('./dir_test_d/dir_test_d_dexnbrtZRgumb_g020/dir_A/out_trace_.mat');
m_QR_F_ = m_out_trace_F_.trace_E_{1}(:,1+3);
n_iteration = numel(m_QR_F_);
m_QR_F__ = zeros(n_iteration,1+n_shuffle);
m_QR_F__(:,1) = m_QR_F_;
