function lisa_dosage_to_b16_ver0(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
...
%{
  ...
  flag_fam = 0;
  for cl_num=1:4;
  lisa_dosage_to_b16_ver0(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
  end;% for cl_num=1:4;
  %}
...
% clear; cl_num = 6; flag_stop = 0; flag_crop = 0; flag_bim=1; flag_fam=1; 
...
[study_dosage_trunk_,study_dosage_name_,study_fam_name_,study_bim_name_,n_study] = lisa_define_study_dosage_ver0(cl_num);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% reading dosage header ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
n_dosage_patient_ = zeros(n_study,1);
dosage_fam_name_ = cell(n_study,1);
for ns=1:n_study;
fn_dsg_{ns} = sprintf('%s',study_dosage_name_{ns}); fcheck(fn_dsg_{ns});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Reading bim ; %<-- this is the cropped bim file taken from the first 3 fields of the dosage data. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
snp__id_ = cell(n_study,1); snp_al1_ = cell(n_study,1); snp_al2_ = cell(n_study,1);
for ns=1:n_study;
disp(sprintf('%% ns %d: Reading bim file %s (snp names) ;',ns,study_bim_name_{ns}));
n_snp_(ns) = wc_0(study_bim_name_{ns})-1;
snp__id_{ns} = cell(n_snp_(ns),1);
snp_al1_{ns} = zeros(n_snp_(ns),1);
snp_al2_{ns} = zeros(n_snp_(ns),1);
fn_bim_{ns} = sprintf('%s',study_bim_name_{ns}); fcheck(fn_bim_{ns});
fp_bim{ns} = fopen(fn_bim_{ns}); 
tmp_line_ = fgetl(fp_bim{ns}); %<-- skip header line. ;
for nsnp=1:n_snp_(ns);
tmp_line_ = fgetl(fp_bim{ns});
...
for ns=1:n_study;
tic;
fn_dsg_{ns} = sprintf('%s',study_dosage_name_{ns});
fp_dsg_{ns} = fopen(fn_dsg_{ns}); 
