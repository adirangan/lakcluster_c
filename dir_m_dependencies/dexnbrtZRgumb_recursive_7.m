...
%%%%%%%%;
% Fitting sample distribution with gumbel. ;
%%%%%%%%;
...
if (isempty(n_member_lob)); n_member_lob = 2; end; %<-- match c. ;
...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('dexnbrtZRgumb',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering dexnbrtZRgumb_recursive_6]: dir_out %s',dir_out)); end;
...
binary_label_ = cast(zeros(n_u,1),'uint64'); for nu=1:n_u; binary_label_(nu) = cast(0,'uint64'); end;
output_label_ = cell(n_u,1); for nu=1:n_u; output_label_{nu} = '0'; end;
nlpbra_label_ = cell(n_u,1); for nu=1:n_u; nlpbra_label_{nu} = ''; end;
nlpnex_label_ = cell(n_u,1); for nu=1:n_u; nlpnex_label_{nu} = ''; end;
...
r_rem_E_ = trace_E_{1}(:,1+1); tmp_E_ij_ = find( (r_rem_E_>=n_member_lob) & ((n_u - r_rem_E_)>=n_member_lob) );
[~,~,~,tmp_p_opt,tmp_p_emp] = gumbel_fit(max(ZR_E_(tmp_E_ij_,2:end),[],1),max(ZR_E_(tmp_E_ij_,1)));
p_gbranch = tmp_p_emp; if (p_gbranch<1/n_shuffle); p_gbranch = tmp_p_opt; end;
nlpbra = -log(p_gbranch);
...
if (tmp_binary_label_(nu)>4.295e9); disp(sprintf(' %% Warning, too many clusters, increase label length')); end;
binary_label_(tmp_ij) = binary_label_(tmp_ij) + ( cast(0,'uint64') + cast(2,'uint64')*tmp_binary_label_(nu) );
output_label_{tmp_ij} = sprintf('%s%s',sprintf('A'),tmp_output_label_{nu});
nlpbra_label_{tmp_ij} = sprintf('%s%s',sprintf('%0.2f ',nlpbra),tmp_nlpbra_label_{nu}); %<-- match c. ;
nlpnex_label_{tmp_ij} = sprintf('%s%s',sprintf('%0.2f ',nlpnex),tmp_nlpnex_label_{nu}); %<-- match c. ;
...
if (tmp_binary_label_(nu)>4.295e9); disp(sprintf(' %% Warning, too many clusters, increase label length')); end;
binary_label_(tmp_ij) = binary_label_(tmp_ij) + ( cast(1,'uint64') + cast(2,'uint64')*tmp_binary_label_(nu) );
...
[out_xdrop_E_0_,out_trace_E_0_] = dexcluster_nonbinary_AAAA_rdrop_ver0(L_array_,gamma);
...
rseed = cast(nshuffle,'uint32'); rseed = calllib('halfloop_lib','RSEED_adv8',rseed);
tmp_Q_ = zeros(n_u); [tmp_Q_,rseed] = calllib('halfloop_lib','array_orth_f',n_u,n_u,tmp_Q_,rseed); tmp_Q_ = transpose(tmp_Q_);
...
cdrop_E_0_ = 1+out_xdrop_E_0_(find(out_xdrop_E_0_(:,2)>-1),2); ckeep_E_0_ = cdrop_E_0_(end:-1:1); ckeep_E_0_ = transpose(1:numel(E_array_c_ij_));
%%%%%%%%;
load(fname_trace_E_);
trace_E_s0000_ = trace_E_{1};
QR_E_ = zeros(size(trace_E_s0000_,1),1+n_shuffle);
QC_E_ = zeros(size(trace_E_s0000_,1),1+n_shuffle);
for nshuffle=0:n_shuffle;
tmp_d = min(length(trace_E_s0000_(:,1)),length(trace_E_{1+nshuffle}(:,1)));
QR_E_(1:tmp_d,1+nshuffle) = trace_E_{1+nshuffle}(1:tmp_d,4);
QC_E_(1:tmp_d,1+nshuffle) = trace_E_{1+nshuffle}(1:tmp_d,5);
end;%for nshuffle=1:n_shuffle;
n_iter_E_ = trace_E_s0000_(:,1); n_iteration_E = n_iter_E_(end);
r_rem_E_ = trace_E_s0000_(:,2);
c_rem_E_ = trace_E_s0000_(:,3);
%%%%%%%%;
QR_E_avg_ = mean(QR_E_(:,2:end),2); QR_E_std_ = std(QR_E_(:,2:end),1,2); %<-- match c. ;
ZR_E_ = (QR_E_ - repmat(QR_E_avg_,1,1+n_shuffle))./max(1e-12,repmat(QR_E_std_,1,1+n_shuffle)); %<-- match c. ;
QC_E_avg_ = mean(QC_E_(:,2:end),2); QC_E_std_ = std(QC_E_(:,2:end),1,2); %<-- match c. ;
ZC_E_ = (QC_E_ - repmat(QC_E_avg_,1,1+n_shuffle))./max(1e-12,repmat(QC_E_std_,1,1+n_shuffle)); %<-- match c. ;
%%%%%%%%;
lp_R_ = z_to_p_0(ZR_E_); lp_C_ = z_to_p_0(ZC_E_);
[lp_R_max,ZR_E_ij] = Z_imax(0,-lp_R_(:,1),0);
r_rem_E_cut = r_rem_E_(ZR_E_ij); c_rem_E_cut = c_rem_E_(ZR_E_ij); n_rank = min(c_rem_E_cut,2);
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
...
plot(n_iter_E_,-lp_R_(:,2:end),'k-','LineWidth',1);
plot(n_iter_E_,-lp_R_(:,1),'r-','LineWidth',2);
plot(n_iter_E_(ZR_E_ij),-lp_R_(ZR_E_ij,1),'go');
hold off;
xlim([0,n_iteration_E]);
xlabel('iteration'); ylabel('-log(p(ZR_E))'); title(sprintf('%s: -log(p(ZR_E)) vs iteration',dir_out),'Interpreter','none');
...
clf;
subplot(1,2,1); hold on;
plot(n_iter_E_,ZR_E_(:,2:end),'k-','LineWidth',1);
plot(n_iter_E_,ZR_E_(:,1),'r-','LineWidth',2);
hold off;
xlim([0,n_iteration_E]);
xlabel('iteration'); ylabel('ZR_E'); title(sprintf('%s: ZR_E vs iteration',dir_out),'Interpreter','none');
subplot(1,2,2); hold on;
plot(n_u-r_rem_E_,ZR_E_(:,2:end),'k-','LineWidth',1);
plot(n_u-r_rem_E_,ZR_E_(:,1),'r-','LineWidth',2);
hold off;
xlim([0,n_u]);
xlabel('samples remaining'); ylabel('ZR_E'); title(sprintf('%s: ZR_E vs sample',dir_out),'Interpreter','none');
figbig;
print('-depsc',sprintf('%s.eps',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
close(gcf);
...
%%%%%%%%;
if (lp_R_max>=-log(p_use)); 
if (verbose>1); disp(sprintf(' %% lp_R_max %0.6f; statistically significant, ZR_E_ij %d. ;',lp_R_max,ZR_E_ij)); end;
[lp_R_max,ZR_E_ij] = Z_imax(0,-lp_R_(:,1),-log(p_use));
if (verbose>1); disp(sprintf(' %% redefining: lp_R_max %0.6f; statistically significant, ZR_E_ij %d. ;',lp_R_max,ZR_E_ij)); end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% No need to recalculate traces, since we only dropped rows above. ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
%%%%%%%%;
% Now extract out_xdrop_F. ;
%%%%%%%%;
out_xdrop_F_0_ = out_xdrop_E_0_;
...
r_rem_F_ = r_rem_E_;
n_iter_F_ = n_iter_E_;
n_iteration_F = n_iteration_E;
%%%%%%%%;
% Now define F1_ZRmax_label_. ;
%%%%%%%%;
lp_R_F_ = z_to_p_0(ZR_F_); lp_R_F_ij_ = find( (r_rem_F_>=n_member_lob) & ((n_u - r_rem_F_)>=n_member_lob) ); %<-- match c. ;
...
clear L_array_;
%%%%%%%%;
if ~isempty(ZR_F_ij) & ~isempty(ZR_E_ij) & n_iteration_E>1;
fname_fig = sprintf('%s/base_ZRmax_F',dir_jpg);
if (flag_plot & ~exist(sprintf('%s.jpg',fname_fig),'file') |  flag_force_create);
if (verbose); disp(sprintf(' %% %s not found, creating',sprintf('%s.jpg',fname_fig))); end;
figure(1);
clf;
...
end;%if if ~isempty(ZR_F_ij) & ~isempty(ZR_E_ij) & n_iteration_E>1;
%%%%%%%%;
p_brach = 1;
...
if (flag_split); flag_split = flag_split & length(find(F1_ZRmax_label_==tmp_u_(1)))>=n_member_lob; end; %<-- match c. ;
if (flag_split); flag_split = flag_split & length(find(F1_ZRmax_label_==tmp_u_(2)))>=n_member_lob; end; %<-- match c. ;
%%%%%%%%;
if ~flag_split;
type_code = 'stop';
branch_A_r_ij_ = [];
...
type_code = 'split';
branch_A_r_ij_ = find(F1_ZRmax_label_==tmp_u_(2)); %<-- match c. ;
branch_B_r_ij_ = find(F1_ZRmax_label_==tmp_u_(1)); %<-- match c. ;
end;%if flag_split;
%%%%%%%%;
save(fname_branch ...
,'trace_E_' ...
