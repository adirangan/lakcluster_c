function lisa_bed_to_b16_flip_ver6(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
...
% scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20190129/mds.mat /home/arangan/dir_PGC_20190129/ ;
% scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20190129/dir_m/?*.m /home/arangan/dir_PGC_20190129/dir_m/ ;
% scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20190129/dir_m/lisa_define_study_ver0.m /home/arangan/dir_PGC_20190129/dir_m/lisa_define_study_ver0.m ;
% scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20190129/dir_m/lisa_bed_to_b16_flip_ver6.m /home/arangan/dir_PGC_20190129/dir_m/lisa_bed_to_b16_flip_ver6.m ;
%
%{
  flag_stop = 1;
  flag_crop = 0;
  ...
  flag_fam = 0;
  for cl_num=1:4;
  lisa_bed_to_b16_flip_ver6(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
  end;% for cl_num=1:4;
  %}
...
% cl_num = 3; flag_stop = 0; flag_crop = 1; flag_bim=1; flag_fam=1; 
...
% 5: not <-- do not use. ;
%%%%%%%%%%%%%%%%;
snp_and_tag=3; snp_xor_tag=2; snp_nor_tag=0; snp_mss_tag=1; snp_not_tag=5;
...
dir_trunk = sprintf('%s','/home/arangan/dir_PGC_20190129');
%dir_trunk = sprintf('%s','/data/rangan/dir_bcc/dir_PGC_20190129');
dir_pre = sprintf('%s','/home/arangan/dir_PGC_20190129');
%dir_pre = sprintf('%s','/data/rangan/dir_bcc/dir_PGC_20190129');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
disp(sprintf('%% Reading fam ; '));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for ns=1:n_study;
disp(sprintf('%% ns %d: Reading fam file %s/%s.fam (patients) ;',ns,study_trunk_{ns},study_name_{ns}));
fn_fam_{ns} = sprintf('%s/%s/%s.fam',dir_trunk,study_trunk_{ns},study_name_{ns});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp(sprintf('%% Reading bim ; '));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ns=1:n_study;
disp(sprintf('%% ns %d: Reading bim file %s/%s.bim (snp names) ;',ns,study_trunk_{ns},study_name_{ns}));
fn_bim_{ns} = sprintf('%s/%s/%s.bim',dir_trunk,study_trunk_{ns},study_name_{ns});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
snp__id_ = cell(n_study,1); snp_pos_ = cell(n_study,1); 
for ns=1:n_study; snp__id_{ns} = bim_{ns}{2}; snp_pos_{ns} = bim_{ns}{4}; end;
snp_al1_ = cell(n_study,1); snp_al2_ = cell(n_study,1);
for ns=1:n_study; snp_al1_{ns} = bim_{ns}{5}; snp_al2_{ns} = bim_{ns}{6}; end;%for ns=1:n_study;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
disp(sprintf('%% now checking to see if the snps in each study are unique or appear multiple times. ; '));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for ns=1:n_study; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
disp(sprintf('%% now checking to see if the snps in the intersection are consistent across studies. ; '));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
snp_sg1_ = zeros(n_snp_cap,n_study); snp_sg2_ = zeros(n_snp_cap,n_study);
for ns=1:n_study;
tmp_sg1_ = cast(cell2mat(snp_al1_{ns}(snp_cap__{ns})),'double')-'A'+1;
tmp_sg2_ = cast(cell2mat(snp_al2_{ns}(snp_cap__{ns})),'double')-'A'+1;
% convert T to A;
tmp_ij = find(tmp_sg1_==20); tmp_sg1_(tmp_ij) = 1;
% convert C to G;
tmp_ij = find(tmp_sg1_==3); tmp_sg1_(tmp_ij) = 7;
% convert A to -1 and G to +1;
tmp_sg1_ = 2*(tmp_sg1_==7)-1;
snp_sg1_(:,ns) = tmp_sg1_;
% convert T to A;
tmp_ij = find(tmp_sg2_==20); tmp_sg2_(tmp_ij) = 1;
% convert C to G;
tmp_ij = find(tmp_sg2_==3); tmp_sg2_(tmp_ij) = 7;
% convert A to -1 and G to +1;
tmp_sg2_ = 2*(tmp_sg2_==7)-1;
snp_sg2_(:,ns) = tmp_sg2_;
end;%for ns=1:n_study;
% Now assert that al1 and al2 are complements. ;
disp(sprintf(' %% norm(sg1-sg2) %0.16f',norm(snp_sg1_+snp_sg2_,'fro')));
% Now compute hamming distance;
flag_disp=0;
if flag_disp;
disp((transpose(snp_sg1_)*snp_sg1_ + n_snp_cap)/2/n_snp_cap);
imagesc(((transpose(snp_sg1_)*snp_sg1_ + n_snp_cap)/2/n_snp_cap),[0,1]); colorbar;
set(gca,'XTick',1:n_study,'XTickLabel',study_name_,'ticklabelinterpreter','none'); xtickangle(90);
set(gca,'YTick',1:n_study,'YTickLabel',study_name_,'ticklabelinterpreter','none');
end;%if flag_disp;
disp(sprintf(' %% fraction of snps with mismatch: %d/%d = %0.4f',length(find(abs(sum(snp_sg1_,2))~=n_study)),length(snp_sg1_),length(find(abs(sum(snp_sg1_,2))~=n_study))/length(snp_sg1_)));
disp(sprintf(' %% fraction of mismatches (within %d-x-%d data-array) %d/%d = %0.4f',n_snp_cap,n_study,sum((n_study-abs(sum(snp_sg1_(:,:),2)))/2),(length(snp_sg1_(:,1))*n_study),sum((n_study-abs(sum(snp_sg1_(:,:),2)))/2)/(length(snp_sg1_(:,1))*n_study)));
disp(sprintf(' %% number of sign flips per study: ')); disp(num2str(sum(snp_sg1_==-1)));
disp(sprintf(' %% number of sign keeps per study: ')); disp(num2str(sum(snp_sg1_==+1)));
 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp(sprintf('%% Reading bed. ;'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ns=1:n_study;
disp(sprintf('%% ns %d: Reading bed file %s/%s.bed (binarized snp data) ;',ns,study_trunk_{ns},study_name_{ns}));
fn_bed_{ns} = sprintf('%s/%s/%s.bed',dir_trunk,study_trunk_{ns},study_name_{ns});
...
% These are the sums -- over snps -- of the and, xor, nor and mss symbols for each patient. ; only m_mss_{ns} is used. ;
%m_and_{ns} = zeros(1,4*n_patient_rup_(ns));m_xor_{ns} = zeros(1,4*n_patient_rup_(ns));m_nor_{ns} = zeros(1,4*n_patient_rup_(ns));
m_mss_{ns} = zeros(1,4*n_patient_rup_(ns));
%%%%%%%%%%%%%%%%;
...
if ver_flag==1;
disp(sprintf(' %% Warning! version 1: sequential read not implemented'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp(sprintf('%% ; '));
disp(sprintf('%% Calculating relative-entropy (i.e., KL-divergence) for each snp within each study. ;'));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for ns=1:n_study;
frq_and_ = n_and_{ns}/n_patient_keep_(ns); frq_xor_ = n_xor_{ns}/n_patient_keep_(ns); frq_nor_ = n_nor_{ns}/n_patient_keep_(ns);
p_opt_{ns} = frq_and_ + 0.5*frq_xor_; q_opt_{ns} = frq_nor_ + 0.5*frq_xor_;
tmp_and_ = frq_and_.*log(frq_and_./(p_opt_{ns}.^2)); tmp_and_(find(~isfinite(tmp_and_)))=0;
tmp_xor_ = frq_xor_.*log(frq_xor_./(2*p_opt_{ns}.*q_opt_{ns})); tmp_xor_(find(~isfinite(tmp_xor_)))=0;
tmp_nor_ = frq_nor_.*log(frq_nor_./(q_opt_{ns}.^2)); tmp_nor_(find(~isfinite(tmp_nor_)))=0;
I_opt_{ns} = tmp_and_ + tmp_xor_ + tmp_nor_ ;
...
disp(sprintf('%% Note that we must ''flip'' the and-vs-nor values for snp-study combinations that have snp_sg1_(nsnp,ns)==-1. ;'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
n_and_tot_ = zeros(n_snp_cap,1); n_xor_tot_ = zeros(n_snp_cap,1); n_nor_tot_ = zeros(n_snp_cap,1); n_mss_tot_ = zeros(n_snp_cap,1); I_opt_max_ = zeros(n_snp_cap,1);
for ns=1:n_study; 
n_and_tot_ = n_and_tot_ + n_and_{ns}(snp_cap__{ns}).*(snp_sg1_(:,ns)==+1) + n_nor_{ns}(snp_cap__{ns}).*(snp_sg1_(:,ns)==-1) ;
n_xor_tot_ = n_xor_tot_ + n_xor_{ns}(snp_cap__{ns}); 
n_nor_tot_ = n_nor_tot_ + n_nor_{ns}(snp_cap__{ns}).*(snp_sg1_(:,ns)==+1) + n_and_{ns}(snp_cap__{ns}).*(snp_sg1_(:,ns)==-1) ;
n_mss_tot_ = n_mss_tot_ + n_mss_{ns}(snp_cap__{ns}); 
...
tmp_nor_ = fr2_nor_tot_.*log(fr2_nor_tot_./(q_opt_tot_.^2)); tmp_nor_(find(~isfinite(tmp_nor_)))=0;
tmp_xor_ = fr2_xor_tot_.*log(fr2_xor_tot_./(2*p_opt_tot_.*q_opt_tot_)); tmp_xor_(find(~isfinite(tmp_xor_)))=0;
tmp_and_ = fr2_and_tot_.*log(fr2_and_tot_./(p_opt_tot_.^2)); tmp_and_(find(~isfinite(tmp_and_)))=0;
I_opt_tot_ = tmp_nor_ + tmp_xor_ + tmp_and_ ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
snp_cap_sub_ = find(I_opt_tot_ < ent_cutoff & I_opt_max_ < ent_cutoff & min(p_opt_tot_,q_opt_tot_) > frq_cutoff & frq_mss_tot_ < mss_cutoff); n_snp_cap_sub = length(snp_cap_sub_);
...
%%%%%%%%%%%%%%%%;
disp(sprintf('%% ; '));
disp(sprintf('%% Counting frequencies of each column (i.e., each retained snp across studies) ; '));
disp(sprintf('%% Note that this calculation involves fr2_and_tot_ and fr2_nor_tot_, ; '));
disp(sprintf('%% both of which have been ''flipped'' based on the modification of n_and_tot_ ; '));
disp(sprintf('%% and n_nor_tot_ using snp_sg1_. ; '));
%%%%%%%%%%%%%%%%;
...
disp(sprintf(' %% ns %d: retaining %d/%d/%d patients',ns,n_rij_cap_(ns),n_patient_keep_(ns),n_patient_(ns)));
end;%for ns=1:n_study;
...
if flag_stop>2;
disp('returning at line 529'); return; 
end;%if flag_stop>2;
...
tmp_bim_{ 7}(cij_and_rpl_(nl3)) = {'and(AA)'};
...
tmp_bim_{ 7}(cij_xor_rpl_(nl3)) = {'xor(AG)'};
...
tmp_bim_{ 7}(cij_nor_rpl_(nl3)) = {'nor(GG)'};
...
tmp_fam{7}(nr) = {sprintf('%s/%s/%s.fam',dir_trunk,study_trunk_{ns},study_name_{ns})};
%disp(sprintf('%s %s %s %s %d %d %s',fam_{ns}{1}{np1},fam_{ns}{2}{np1},fam_{ns}{3}{np1},fam_{ns}{4}{np1},fam_{ns}{5}(np1),fam_{ns}{6}(np1),sprintf('%s/%s/%s',dir_trunk,study_trunk_{ns},study_name_{ns})));
nr = nr+1;
...
for nl3=1:n_snp_cap_sub; 
% nl3 refers to the index within 1:n_snp_cap_sub. ;
...
% data_snp holds the tags for the patients (columns) associated with snp nl1. ;
...
% truncate to exactly n_patient_(ns) entries. ;
data_snp = data_snp(1,1:n_patient_(ns));
% We replace the missing data with the mode below, stored in fr2_mode_(nl2). ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
if (snp_sg1_(nl2,ns)==+1);
data_snp_and = ( (data_snp==snp_and_tag) | ( (fr2_mode_(nl2)==snp_and_tag) & (data_snp==snp_mss_tag) ) ) ; 
end;%if (snp_sg1_(nl2,ns)==+1);
if (snp_sg1_(nl2,ns)==-1);
data_snp_and = ( (data_snp==snp_nor_tag) | ( (fr2_mode_(nl2)==snp_and_tag) & (data_snp==snp_mss_tag) ) ) ; 
end;%if (snp_sg1_(nl2,ns)==-1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
data_snp_xor = ( (data_snp==snp_xor_tag) | ( (fr2_mode_(nl2)==snp_xor_tag) & (data_snp==snp_mss_tag) ) ) ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
if (snp_sg1_(nl2,ns)==+1);
data_snp_nor = ( (data_snp==snp_nor_tag) | ( (fr2_mode_(nl2)==snp_nor_tag) & (data_snp==snp_mss_tag) ) ) ; 
end;%if (snp_sg1_(nl2,ns)==+1);
if (snp_sg1_(nl2,ns)==-1);
data_snp_nor = ( (data_snp==snp_and_tag) | ( (fr2_mode_(nl2)==snp_nor_tag) & (data_snp==snp_mss_tag) ) ) ; 
end;%if (snp_sg1_(nl2,ns)==-1);
