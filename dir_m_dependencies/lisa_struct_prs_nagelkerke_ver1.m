function lisa = lisa_struct_prs_nagelkerke_ver1(specification);
% try: ;
%{
  ...
  lisa_struct_prs_nagelkerke_ver1(specification) ;
...
if (~exist(lisa_arm1_r0.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_nagelkerke_ver1',lisa_arm1_r0.dir_out_s0_pca)); end;
...
if (~exist(lisa_arm1_r1.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_nagelkerke_ver1',lisa_arm1_r1.dir_out_s0_pca)); end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
fname_mat = sprintf('%s/prs_nagelkerke_ver1_%s_trn%d%s_tst%d%s.mat',dir_out_s0_pca_r0_arm1,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string);
if ( flag_rerun==0 &  exist(fname_mat,'file')); 
disp(sprintf(' %% found %s, loading',fname_mat));
load(fname_mat,...
     'r2_000m_','r2_00zm_','r2_000m__','r2_00zm__','r2_xwzm__','r2_0wzm__','r2_0w0m__',...
     'n_permutation',...
     'r2_xwzm_rand__','r2_0wzm_rand__','r2_0w0m_rand__',...
     'n_omega','omega_');
...
%%%%%%%%;
n_permutation = 1024;
r2_0w0m_rand__ = zeros(n_permutation,n_prs);
r2_0wzm_rand__ = zeros(n_permutation,n_prs);
r2_xwzm_rand__ = zeros(n_permutation,n_prs);
...
tmp_r2_0w0m_rand_ = zeros(1,n_prs);
tmp_r2_0wzm_rand_ = zeros(1,n_prs);
tmp_r2_xwzm_rand_ = zeros(1,n_prs);
for nprs=1:n_prs;
...
tmp_Xa_ = tmp_Xz_.*tmp_Xw_; tmp_Da_ = tmp_Dz_.*tmp_Dw_;
tmp_r2_0w0m_rand_(nprs) = nagelkerke_r2_ver0([tmp_Dw_ , tmp_mds_d_ ; tmp_Xw_ , tmp_mds_x_ ] , tmp_outcome_);
tmp_r2_0wzm_rand_(nprs) = nagelkerke_r2_ver0([tmp_Dw_ , tmp_Dz_ , tmp_mds_d_ ; tmp_Xw_ , tmp_Xz_ , tmp_mds_x_ ] , tmp_outcome_);
tmp_r2_xwzm_rand_(nprs) = nagelkerke_r2_ver0([tmp_Da_ , tmp_Dw_ , tmp_Dz_ , tmp_mds_d_ ; tmp_Xa_ , tmp_Xw_ , tmp_Xz_ , tmp_mds_x_ ] , tmp_outcome_);
end;%for nprs=1:n_prs;
r2_0w0m_rand__(npermutation,:) = tmp_r2_0w0m_rand_;
r2_0wzm_rand__(npermutation,:) = tmp_r2_0wzm_rand_;
r2_xwzm_rand__(npermutation,:) = tmp_r2_xwzm_rand_;
end;%parfor npermutation=1:n_permutation;
%%%%%%%%;
r2_000m_ = zeros(n_prs,1);
r2_00zm_ = zeros(n_prs,1);
for nprs=1:n_prs;
prs_min = prs_min_(nprs); prs_max = prs_max_(nprs);
tmp_Xz_ = transpose(prs_sco_{nprs}(iprs_{nprs}(find_x_)));
tmp_Dz_ = transpose(prs_sco_{nprs}(iprs_{nprs}(find_d_)));
r2_000m_(nprs) = nagelkerke_r2_ver0([tmp_mds_d_ ; tmp_mds_x_ ] , tmp_outcome_);
r2_00zm_(nprs) = nagelkerke_r2_ver0([tmp_Dz_ , tmp_mds_d_ ; tmp_Xz_ , tmp_mds_x_ ] , tmp_outcome_);
end;%for nprs=1:n_prs;
r2_000m__ = repmat(transpose(r2_000m_),n_omega,1);
r2_00zm__ = repmat(transpose(r2_00zm_),n_omega,1);
%%%%%%%%;
r2_0w0m__ = zeros(n_omega,n_prs);
r2_0wzm__ = zeros(n_omega,n_prs);
r2_xwzm__ = zeros(n_omega,n_prs);
...
tmp_r2_0w0m_ = zeros(1,n_prs);
tmp_r2_0wzm_ = zeros(1,n_prs);
tmp_r2_xwzm_ = zeros(1,n_prs);
for nprs=1:n_prs;
tmp_Xz_ = transpose(prs_sco_{nprs}(iprs_{nprs}(find_x_)));
tmp_Dz_ = transpose(prs_sco_{nprs}(iprs_{nprs}(find_d_)));
tmp_Xa_ = tmp_Xz_.*tmp_Xw_; tmp_Da_ = tmp_Dz_.*tmp_Dw_;
tmp_r2_0w0m_(nprs) = nagelkerke_r2_ver0([tmp_Dw_ , tmp_mds_d_ ; tmp_Xw_ , tmp_mds_x_ ] , tmp_outcome_);
tmp_r2_0wzm_(nprs) = nagelkerke_r2_ver0([tmp_Dw_ , tmp_Dz_ , tmp_mds_d_ ; tmp_Xw_ , tmp_Xz_ , tmp_mds_x_ ] , tmp_outcome_);
tmp_r2_xwzm_(nprs) = nagelkerke_r2_ver0([tmp_Da_ , tmp_Dw_ , tmp_Dz_ , tmp_mds_d_ ; tmp_Xa_ , tmp_Xw_ , tmp_Xz_ , tmp_mds_x_ ] , tmp_outcome_);
end;%for nprs=1:n_prs;
r2_0w0m__(nomega,:) = tmp_r2_0w0m_;
r2_0wzm__(nomega,:) = tmp_r2_0wzm_;
r2_xwzm__(nomega,:) = tmp_r2_xwzm_;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
flag_replot=1;
fname_fig = sprintf('%s/prs_nagelkerke_stack_ver1_%s_trn%d%s_tst%d%s_ni%.3d_ni%.3d.jpg',lisa_arm1_r0.dir_out_s0_prs_jpg,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2,ni_r1_arm2);
...
%%%%%%%%;
subplot(2,3,1);
bar(transpose([r2_000m__(1,:);(r2_00zm__(1,:)-r2_000m__(1,:));(r2_0wzm__(1,:)-r2_00zm__(1,:));(r2_xwzm__(1,:)-r2_0wzm__(1,:))]),'stacked');
ylim_ = r2_000m__(1,1) + max(r2_xwzm__(1,:)-r2_000m__(1,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
legend('covariates only','+ prs','+ BC1','+ prs*BC1','Location','NorthWest');
title('Nagelkerke pseudo-r^2 for bicluster 1');
xlabel('prs cutoff');
ylabel('pseudo-r^2');
%%%%%%%%;
subplot(2,3,2);
bar(transpose([r2_000m__(end,:);(r2_00zm__(end,:)-r2_000m__(end,:));(r2_0wzm__(end,:)-r2_00zm__(end,:));(r2_xwzm__(end,:)-r2_0wzm__(end,:))]),'stacked');
%ylim_ = r2_000m__(end,1) + max(r2_0wzm__(end,:)-r2_000m__(end,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
legend('covariates only','+ prs','+ BC2','+ prs*BC2','Location','NorthWest');
title('Nagelkerke pseudo-r^2 for bicluster 2');
xlabel('prs cutoff');
ylabel('pseudo-r^2');
%%%%%%%%;
subplot(2,3,3); nomega = 6; omega = omega_(nomega);
bar(transpose([r2_000m__(nomega,:);(r2_00zm__(nomega,:)-r2_000m__(nomega,:));(r2_0wzm__(nomega,:)-r2_00zm__(nomega,:));(r2_xwzm__(nomega,:)-r2_0wzm__(nomega,:))]),'stacked');
%ylim_ = r2_000m__(nomega,1) + max(r2_0wzm__(nomega,:)-r2_000m__(nomega,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
legend('covariates only','+ prs',sprintf('+ BC(\\omega=%d)',round(180*omega/pi)),sprintf('+ prs*BC(\\omega=%d)',round(180*omega/pi)),'Location','NorthWest');
title(sprintf('Nagelkerke pseudo-r^2 for bicluster \\omega=%d',round(180*omega/pi)));
xlabel('prs cutoff');
ylabel('pseudo-r^2');
%%%%%%%%;
subplot(2,3,4);
bar(transpose([r2_000m__(1,:);(r2_0wzm__(1,:)-r2_0w0m__(1,:));(r2_0w0m__(1,:)-r2_000m__(1,:));(r2_xwzm__(1,:)-r2_0wzm__(1,:))]),'stacked');
ylim_ = r2_000m__(1,1) + max(r2_xwzm__(1,:)-r2_000m__(1,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
...
%%%%%%%%;
subplot(2,3,5);
bar(transpose([r2_000m__(end,:);(r2_0wzm__(end,:)-r2_0w0m__(end,:));(r2_0w0m__(end,:)-r2_000m__(end,:));(r2_xwzm__(end,:)-r2_0wzm__(end,:))]),'stacked');
%ylim_ = r2_000m__(end,1) + max(r2_0wzm__(end,:)-r2_000m__(end,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
...
ylabel('pseudo-r^2');
%%%%%%%%;
subplot(2,3,6); nomega = 6; omega = omega_(nomega);
bar(transpose([r2_000m__(nomega,:);(r2_0wzm__(nomega,:)-r2_0w0m__(nomega,:));(r2_0w0m__(nomega,:)-r2_000m__(nomega,:));(r2_xwzm__(nomega,:)-r2_0wzm__(nomega,:))]),'stacked');
%ylim_ = r2_000m__(nomega,1) + max(r2_0wzm__(nomega,:)-r2_000m__(nomega,:))*[-0.25,+1.25];
ylim(ylim_);
set(gca,'XTick',1:n_prs,'XTickLabel',str_prs_cutoff_);
xtickangle(90);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
flag_replot=1;
fname_fig = sprintf('%s/prs_nagelkerke_ver1_%s_trn%d%s_tst%d%s_ni%.3d_ni%.3d.jpg',lisa_arm1_r0.dir_out_s0_prs_jpg,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2,ni_r1_arm2);
if (~flag_replot & exist(fname_fig,'file')); disp(sprintf(' %% %s found, not creating',fname_fig)); end;
if ( flag_replot | ~exist(fname_fig,'file')); disp(sprintf(' %% %s not found, creating',fname_fig)); 
%%%%%%%%;
%dlim_ = [0,1e-3];
%subplot(1,2,1);
%r2_diff_ = (r2_0wzm__ - r2_00zm__);
...
rlim_ = [0,0.25];
subplot(1,2,1);
r2_ratio_ = (r2_0wzm__ - r2_00zm__)./(r2_00zm__ - r2_000m__);
...
tmp_p = max(1,length(find(r2_0wzm_rand__(:,nprs)>=r2_0wzm__(nomega,nprs))))/n_permutation;
