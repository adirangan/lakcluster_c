...
corr_A_est_ofd = corr(tmp_A_tru_ofd__(:),tmp_A_est_ofd__(:));
...
okay_est_sn__ = sum( corr_A_est_ofd_snr___(:,:,1+index_rseed_).*(nlpvalue_est_snr___(:,:,1+index_rseed_)>3.0) , 3)./found_flag_sn__;
okay_irg_sn__ = sum( corr_A_irg_ofd_snr___(:,:,1+index_rseed_).*(nlpvalue_irg_snr___(:,:,1+index_rseed_)>3.0) , 3)./found_flag_sn__;
...
hold on;
plot(log2(snr_),okay_est_sn__(:,1+nn_T_0in),'ro-','LineWidth',2);
