...
tmp_rseed = 2; %<-- different from tmp_rseed = 1, which is used in test_loader_29.m ;
tmp_n_iteration = 256;
tmp_tolerance = 1e-3;
if flag_load==0;
%%%%%%%%;
...
E_li16b_prefix_ = sprintf('%s/dir_jpg/E_li16b_impute_fit',dir_trunk);
[~,E_li16bf_] = svd_impute_fit_3(log(max(1e-12,E_val_)),E_ij_missing_,n_rank_imputation,E_li16b_prefix_,tmp_rseed,tmp_n_iteration,tmp_tolerance);
%save('/data/rangan/dir_bcc/dir_jamison/dir_mat/E_li16b_.mat','E_li16b_');
save('/data/rangan/dir_bcc/dir_jamison/dir_mat/E_li16bf_.mat','E_li16bf_');
%tmp_ = E_li16b_; fname_tsv = sprintf('%s/dir_mat/E_li16b_.tsv',dir_trunk); 
%disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs');
tmp_ = E_li16bf_; fname_tsv = sprintf('%s/dir_mat/E_li16bf_.tsv',dir_trunk); 
disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs');
%%%%%%%%;
...
I_li16b_prefix_ = sprintf('%s/dir_jpg/I_li16b_impute_fit',dir_trunk);
[~,I_li16bf_] = svd_impute_fit_3(log(max(1e-12,I_val_)),I_ij_missing_,n_rank_imputation,I_li16b_prefix_,tmp_rseed,tmp_n_iteration,tmp_tolerance);
%save('/data/rangan/dir_bcc/dir_jamison/dir_mat/I_li16b_.mat','I_li16b_');
save('/data/rangan/dir_bcc/dir_jamison/dir_mat/I_li16bf_.mat','I_li16bf_');
%tmp_ = I_li16b_; fname_tsv = sprintf('%s/dir_mat/I_li16b_.tsv',dir_trunk); 
%disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs');
tmp_ = I_li16bf_; fname_tsv = sprintf('%s/dir_mat/I_li16bf_.tsv',dir_trunk); 
disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs');
%%%%%%%%;
% demonstrate structure imposed by typical imputation. ;
%%%%%%%%;
...
if flag_load==1;
%load('/data/rangan/dir_bcc/dir_jamison/dir_mat/E_li16b_.mat','E_li16b_');
%load('/data/rangan/dir_bcc/dir_jamison/dir_mat/I_li16b_.mat','I_li16b_');
load('/data/rangan/dir_bcc/dir_jamison/dir_mat/E_li16bf_.mat','E_li16bf_');
load('/data/rangan/dir_bcc/dir_jamison/dir_mat/I_li16bf_.mat','I_li16bf_');
end;%if flag_load==1;
...
%%%%%%%%;
if flag_E; 
tmp_99 = prctile(E_li16bf_(:),99); tmp_01 = prctile(E_li16bf_(:), 1);
E_li16bf_clogr_ = mean_center_0(max(tmp_01,min(tmp_99,E_li16bf_)),'col');
tmp_ = E_li16bf_clogr_; fname_tsv = sprintf('%s/dir_mat/E_li16bf_clogr_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
end;%if flag_E; 
if flag_I; 
tmp_99 = prctile(I_li16bf_(:),99); tmp_01 = prctile(I_li16bf_(:), 1);
I_li16bf_clogr_ = mean_center_0(max(tmp_01,min(tmp_99,I_li16bf_)),'col');
tmp_ = I_li16bf_clogr_; fname_tsv = sprintf('%s/dir_mat/I_li16bf_clogr_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
end;%if flag_I; 
...
%%%%%%%%;
if flag_E; 
E_li16bf_rankn_ = rank_normalize_0(E_li16bf_clogr_,'row'); 
tmp_ = E_li16bf_rankn_; fname_tsv = sprintf('%s/dir_mat/E_li16bf_rankn_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
end;%if flag_E; 
if flag_I; 
I_li16bf_rankn_ = rank_normalize_0(I_li16bf_clogr_,'row'); 
tmp_ = I_li16bf_rankn_; fname_tsv = sprintf('%s/dir_mat/I_li16bf_rankn_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
end;%if flag_I;
...
%%%%%%%%;
% spectral clustering of covariates after mahalanobization via E_li16bf_. ;
% Verdict: Covariates are significantly clustered with respect to transcript data. ;
%%%%%%%%;
flag_plot=0;
if flag_plot;
%%%%%%%%;
[tmp_UE_,tmp_SE_,tmp_VE_] = svds(E_li16bf_,n_u);
...
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
[tmp_UX_,tmp_SX_,tmp_VX_] = svds(UEC_*tmp_Q_,n_CCOV);
%%%%%%%%;
% spectral-->isosplit5;
%%%%%%%%;
n_rank_CC = 6;
...
label_C__{nrank_CC} = transpose(isosplit5(transpose(tmp_VX_(:,1:nrank_CC)*tmp_SX_(1:nrank_CC,1:nrank_CC)),opts_isosplit5));
end;%for nrank_CC=1:n_rank_CC;
for nrank_CC=1:n_rank_CC;
subplot(2,3,nrank_CC); scatter(tmp_VX_(:,1),tmp_VX_(:,2),15,label_C__{nrank_CC}); colormap('lines');
end;%for nrank_CC=1:n_rank_CC;
figbig;
...
tmp_ZRimax_label_ = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_2(dir_base,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,tmp_p_set,n_member_lob);
subplot(2,3,np);
scatter(tmp_VX_(:,1),tmp_VX_(:,2),15,label_str_to_enum_0(tmp_ZRimax_label_),'filled');
colormap('lines');
title(sprintf('p %0.6f #=%d',tmp_p_set,length(unique(tmp_ZRimax_label_))));
axisnotick;
end;%for np=1:length(p_set_);
...
%%%%%%%%;
clf;
%tmp_Legend_ = {'Cluster GC Percentage','Batch, Depth of Sample Capture','Population-Wide Predictors of Sample Quality','Sequencing Quality (across the length of the read)','Sequencing Quality (across the full span of the read)','Depth of Sequencing (including ERCC ladder sequence)'};
tmp_Legend_ = {'Cluster-GC%','Batch+Depth (capture)','Pop.-Wide Quality','Seq. Quality (length)','Seq. Quality (span)','Depth (ERCC ladder)'};
subplot(1,2,1);
colormap('lines');
hold on;
for nQCluster=1:n_QCluster;
...
for nQCluster=1:n_QCluster;
tmp_ij_ = find(ZRimax_enum_==nQCluster);
scatter(tmp_VX_(tmp_ij_,1),tmp_VX_(tmp_ij_,2),65,ZRimax_enum_(tmp_ij_),'filled','MarkerEdgeColor','k');
end;%for nQCluster=1:n_QCluster;
...
tmp_S_cUEx_ = svds(mean_center_0(C_rank_cUEx_),n_CCOV_cUEx);
subplot(2,3,nQCluster); plot(tmp_S_cUEx_,'ko-'); ylim([0,max(tmp_S_cUEx_)*1.25]);
end;%for nQCluster=1:n_QCluster;
figbig;
end;%if flag_plot;
%%%%%%%%;
...
%%%%%%%%;
% spectral clustering of covariates after mahalanobization via I_li16bf_. ;
% Verdict: ? ;
%%%%%%%%;
flag_plot=0;
if flag_plot;
%%%%%%%%;
[tmp_UI_,tmp_SI_,tmp_VI_] = svds(I_li16bf_,n_u);
%UIC_ = inv(tmp_SI_)*transpose(tmp_UI_)*mean_center_0(C_rank_);
UIC_ = inv(tmp_SI_)*transpose(tmp_UI_)*(C_rank_);
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
[tmp_UX_,tmp_SX_,tmp_VX_] = svds(UIC_*tmp_Q_,n_CCOV);
%%%%%%%%;
% spectral-->isosplit5;
%%%%%%%%;
n_rank_CC = 6;
...
subplot(2,2,np);
scatter(tmp_VX_(:,1),tmp_VX_(:,2),15,label_str_to_enum_0(tmp_ZRimax_label_));
colormap('lines');
title(sprintf('p %0.6f',tmp_p_set));
axisnotick;
end;%for np=1:length(p_set_);
...
%%%%%%%%;
% spectral clustering of covariates after mahalanobization via EI_li16bf_. ;
% Verdict: ? ;
%%%%%%%%;
flag_plot=0;
if flag_plot;
%%%%%%%%;
[tmp_UEI_,tmp_SEI_,tmp_VEI_] = svds([E_li16bf_ , I_li16bf_],n_u);
%UEIC_ = inv(tmp_SEI_)*transpose(tmp_UEI_)*mean_center_0(C_rank_);
UEIC_ = inv(tmp_SEI_)*transpose(tmp_UEI_)*(C_rank_);
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
[tmp_UX_,tmp_SX_,tmp_VX_] = svds(UEIC_*tmp_Q_,n_CCOV);
%%%%%%%%;
% spectral-->isosplit5;
%%%%%%%%;
n_rank_CC = 6;
...
%%%%%%%%;
% spectral clustering of covariates after accounting for both E_li16bf_ as well as AIBS-clusters. ;
...
[kld___,n_rank_,kld__,DVS__,SAD2__] = SampleSVD_5(E_li16bf_,Sample_Label_enum_,alpha_);
...
DVSX_ = pinv(DVS__{1}(:,1:n_rank_use),1e-6*norm(DVS__{1}(:,1:n_rank_use),2))*(C_rank_);
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
[tmp_UX_,tmp_SX_,tmp_VX_] = svds(DVSX_*tmp_Q_,n_CCOV);
%%%%%%%%;
% spectral-->isosplit5;
%%%%%%%%;
n_rank_CC = 6;
...
figbig;
fname_base = sprintf('%s/dir_jpg/DVSX_spectral_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
%%%%%%%%;
...
%%%%%%%%;
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
DVSX_sub_ = fast_tsne_dr_0(transpose(DVSX_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.0));
opts_isosplit5 = struct('K_init',n_CCOV,'isocut_threshold',1.0);
label_DVSX_ = transpose(isosplit5(transpose(DVSX_sub_),opts_isosplit5));
subplot(1,1,1); scatter(DVSX_sub_(:,1),DVSX_sub_(:,2),15,label_DVSX_); colormap('lines');
fname_base = sprintf('%s/dir_jpg/DVSX_umap00_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
%%%%%%%%;
...
%%%%%%%%;
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
DVSX_sub_ = fast_tsne_dr_0(transpose(DVSX_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.5));
opts_isosplit5 = struct('K_init',n_CCOV,'isocut_threshold',1.0);
label_DVSX_ = transpose(isosplit5(transpose(DVSX_sub_),opts_isosplit5));
subplot(1,1,1); scatter(DVSX_sub_(:,1),DVSX_sub_(:,2),15,label_DVSX_); colormap('lines');
fname_base = sprintf('%s/dir_jpg/DVSX_umap50_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
%%%%%%%%;
...
str_nml = sprintf('_nml%d',n_member_lob);
str_xfix = sprintf('dexcluster_nonbinary_trace_ZRimax%s%s%s',str_g,str_p,str_nml);
prefix_base = sprintf('DVSX_%s',str_xfix); 
dir_out = []; EI_array_base_ = transpose(DVSX_); EI_array_r_ij_ = []; EI_array_c_ij_ = [];
test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_2(dir_base,dir_out,prefix_base,EI_array_base_,EI_array_r_ij_,EI_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
%ZRimax_label_ = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_2(dir_base,dir_out,prefix_base,EI_array_base_,EI_array_r_ij_,EI_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
p_set_ = [0.05]*0.5.^[0:3];
for np=1:length(p_set_);
...
figbig;
fname_base = sprintf('%s/dir_jpg/DVSX_dexnb_scatter_FIGA',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
%%%%%%%%;
...
tmp_rank_avg_ = mean(E_li16bf_rankn_,1); tmp_rank_std_ = max(1,std(E_li16bf_rankn_,1,1));
tmp_E_ = (E_li16bf_rankn_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
...
tmp_rank_avg_ = mean(C_rank_,1); tmp_rank_std_ = max(1,std(C_rank_,1,1));
tmp_C_ = (C_rank_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
tmp_rank_avg_ = mean(I_li16bf_rankn_,1); tmp_rank_std_ = max(1,std(I_li16bf_rankn_,1,1));
tmp_I_ = (I_li16bf_rankn_ - repmat(tmp_rank_avg_,n_u,1))./repmat(tmp_rank_std_,n_u,1);
clear tmp_rank_avg_ tmp_rank_std_;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% Fit linear model to log of imputed data (li16bf) ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
n_rank_xeta = 16;
...
% cUE1 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE1');
[xeta_LSQ_li16bf_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE1');
[xeta_RRR_li16bf_clogr_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE1');
[xeta_LSQ_li16bf_clogr_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE1');
[xeta_RRR_li16bf_rankn_cUE1_] = test_loader_lm_RRR_1(C_rank_cUE1_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE1');
[xeta_LSQ_li16bf_rankn_cUE1_] = test_loader_lm_LSQ_1(C_rank_cUE1_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE1');
%%%%%%%%;
% cUE2 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE2');
[xeta_LSQ_li16bf_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE2');
[xeta_RRR_li16bf_clogr_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE2');
[xeta_LSQ_li16bf_clogr_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE2');
[xeta_RRR_li16bf_rankn_cUE2_] = test_loader_lm_RRR_1(C_rank_cUE2_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE2');
[xeta_LSQ_li16bf_rankn_cUE2_] = test_loader_lm_LSQ_1(C_rank_cUE2_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE2');
%%%%%%%%;
% cUE3 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE3');
[xeta_LSQ_li16bf_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE3');
[xeta_RRR_li16bf_clogr_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE3');
[xeta_LSQ_li16bf_clogr_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE3');
[xeta_RRR_li16bf_rankn_cUE3_] = test_loader_lm_RRR_1(C_rank_cUE3_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE3');
[xeta_LSQ_li16bf_rankn_cUE3_] = test_loader_lm_LSQ_1(C_rank_cUE3_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE3');
%%%%%%%%;
% cUE4 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE4');
[xeta_LSQ_li16bf_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE4');
[xeta_RRR_li16bf_clogr_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE4');
[xeta_LSQ_li16bf_clogr_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE4');
[xeta_RRR_li16bf_rankn_cUE4_] = test_loader_lm_RRR_1(C_rank_cUE4_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE4');
[xeta_LSQ_li16bf_rankn_cUE4_] = test_loader_lm_LSQ_1(C_rank_cUE4_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE4');
%%%%%%%%;
% cUE5 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE5');
[xeta_LSQ_li16bf_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE5');
[xeta_RRR_li16bf_clogr_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE5');
[xeta_LSQ_li16bf_clogr_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE5');
[xeta_RRR_li16bf_rankn_cUE5_] = test_loader_lm_RRR_1(C_rank_cUE5_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE5');
[xeta_LSQ_li16bf_rankn_cUE5_] = test_loader_lm_LSQ_1(C_rank_cUE5_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE5');
%%%%%%%%;
% cUE6 ;
%%%%%%%%;
[xeta_RRR_li16bf_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cUE6');
[xeta_LSQ_li16bf_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cUE6');
[xeta_RRR_li16bf_clogr_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cUE6');
[xeta_LSQ_li16bf_clogr_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cUE6');
[xeta_RRR_li16bf_rankn_cUE6_] = test_loader_lm_RRR_1(C_rank_cUE6_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cUE6');
[xeta_LSQ_li16bf_rankn_cUE6_] = test_loader_lm_LSQ_1(C_rank_cUE6_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cUE6');
%%%%%%%%;
% cPhr ;
%%%%%%%%;
[xeta_RRR_li16bf_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cPhr');
[xeta_LSQ_li16bf_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cPhr');
[xeta_RRR_li16bf_clogr_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cPhr');
[xeta_LSQ_li16bf_clogr_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cPhr');
[xeta_RRR_li16bf_rankn_cPhr_] = test_loader_lm_RRR_1(C_rank_cPhr_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cPhr');
[xeta_LSQ_li16bf_rankn_cPhr_] = test_loader_lm_LSQ_1(C_rank_cPhr_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cPhr');
%%%%%%%%;
% c127 ;
%%%%%%%%;
[xeta_RRR_li16bf_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_c127');
[xeta_LSQ_li16bf_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_c127');
[xeta_RRR_li16bf_clogr_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_c127');
[xeta_LSQ_li16bf_clogr_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_c127');
[xeta_RRR_li16bf_rankn_c127_] = test_loader_lm_RRR_1(C_rank_c127_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_c127');
[xeta_LSQ_li16bf_rankn_c127_] = test_loader_lm_LSQ_1(C_rank_c127_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_c127');
%%%%%%%%;
% c012 ;
%%%%%%%%;
[xeta_RRR_li16bf_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_c012');
[xeta_LSQ_li16bf_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_c012');
[xeta_RRR_li16bf_clogr_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_c012');
[xeta_LSQ_li16bf_clogr_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_c012');
[xeta_RRR_li16bf_rankn_c012_] = test_loader_lm_RRR_1(C_rank_c012_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_c012');
[xeta_LSQ_li16bf_rankn_c012_] = test_loader_lm_LSQ_1(C_rank_c012_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_c012');
%%%%%%%%;
% cRRR ;
%%%%%%%%;
[xeta_RRR_li16bf_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_li16bf_,I_li16bf_,n_rank_xeta,dir_trunk,'li16bf_cRRR');
[xeta_LSQ_li16bf_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_li16bf_,I_li16bf_,dir_trunk,'li16bf_cRRR');
[xeta_RRR_li16bf_clogr_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_li16bf_clogr_,I_li16bf_clogr_,n_rank_xeta,dir_trunk,'li16bf_clogr_cRRR');
[xeta_LSQ_li16bf_clogr_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_li16bf_clogr_,I_li16bf_clogr_,dir_trunk,'li16bf_clogr_cRRR');
[xeta_RRR_li16bf_rankn_cRRR_] = test_loader_lm_RRR_1(C_rank_cRRR_,E_li16bf_rankn_,I_li16bf_rankn_,n_rank_xeta,dir_trunk,'li16bf_rankn_cRRR');
[xeta_LSQ_li16bf_rankn_cRRR_] = test_loader_lm_LSQ_1(C_rank_cRRR_,E_li16bf_rankn_,I_li16bf_rankn_,dir_trunk,'li16bf_rankn_cRRR');
...
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE1_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 5;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE2_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 8;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE3_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 1;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE4_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 1;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE5_,n_rank_wrap);
n_iteration_wrap = 128;n_rank_wrap = 8;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cUE6_,n_rank_wrap);
%%%%%%%%;
n_iteration_wrap = 128;n_rank_wrap = 12;
%%%%%%%%;
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cPhr_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_c127_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_c012_,n_rank_wrap);
test_loader_helper_wrap_2(dir_trunk,n_iteration_wrap,n_Sample_Label,str_Sample_Label_,C_VariableName_LC_,E_li16bf_rankn_,I_li16bf_rankn_,xeta_RRR_li16bf_rankn_cRRR_,n_rank_wrap);
...
fprintf(fp_readme,'E_li16bf_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns).\n');
fprintf(fp_readme,'I_li16bf_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns).\n');
fprintf(fp_readme,'E_li16bf_clogr_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio).\n');
fprintf(fp_readme,'I_li16bf_clogr_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio).\n');
fprintf(fp_readme,'E_li16bf_rankn_.tsv: tab delimited array of samples (rows) by exon gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio) followed by rank-normalization.\n');
fprintf(fp_readme,'I_li16bf_rankn_.tsv: tab delimited array of smaples (rows) by intron gene log-expression-values (rank-6 linear imputation) (columns) followed by mean-centering (centered-log-ratio) followed by rank-normalization.\n');
fprintf(fp_readme,'C_rank_c012_.tsv: tab delimited array of samples (rows) by covariate ranks (columns).\n');
fprintf(fp_readme,'E_li16bf_c012_zeta_un_.tsv and E_li16bf_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_c012_zeta_] := [E_li16bf_c012_zeta_un_]*[E_li16bf_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-Li16bf] = [1 , Covariate-Rank]*[E_li16bf_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_li16bf_c012_beta_un_.tsv and E_li16bf_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_c012_beta_] := [E_li16bf_c012_beta_un_]*[E_li16bf_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-Li16bf]*[E_li16bf_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_c012_zeta_un_.tsv and I_li16bf_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_c012_zeta_] := [I_li16bf_c012_zeta_un_]*[I_li16bf_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-Li16bf] = [1 , Covariate-Rank]*[I_li16bf_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_c012_beta_un_.tsv and I_li16bf_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_c012_beta_] := [I_li16bf_c012_beta_un_]*[I_li16bf_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-Li16bf]*[I_li16bf_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_li16bf_clogr_c012_zeta_un_.tsv and E_li16bf_clogr_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_clogr_c012_zeta_] := [E_li16bf_clogr_c012_zeta_un_]*[E_li16bf_clogr_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-Li16bf_Clogr] = [1 , Covariate-Rank]*[E_li16bf_clogr_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_li16bf_clogr_c012_beta_un_.tsv and E_li16bf_clogr_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_clogr_c012_beta_] := [E_li16bf_clogr_c012_beta_un_]*[E_li16bf_clogr_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-Li16bf_Clogr]*[E_li16bf_clogr_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_clogr_c012_zeta_un_.tsv and I_li16bf_clogr_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_clogr_c012_zeta_] := [I_li16bf_clogr_c012_zeta_un_]*[I_li16bf_clogr_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-Li16bf_Clogr] = [1 , Covariate-Rank]*[I_li16bf_clogr_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_clogr_c012_beta_un_.tsv and I_li16bf_clogr_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_clogr_c012_beta_] := [I_li16bf_clogr_c012_beta_un_]*[I_li16bf_clogr_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-Li16bf_Clogr]*[I_li16bf_clogr_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_li16bf_rankn_c012_zeta_un_.tsv and E_li16bf_rankn_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_rankn_c012_zeta_] := [E_li16bf_rankn_c012_zeta_un_]*[E_li16bf_rankn_c012_zeta_vn_] is a coefficient matrix for the linear model [Exon-Gene-Li16bf_Rankn] = [1 , Covariate-Rank]*[E_li16bf_rankn_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'E_li16bf_rankn_c012_beta_un_.tsv and E_li16bf_rankn_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [E_li16bf_rankn_c012_beta_] := [E_li16bf_rankn_c012_beta_un_]*[E_li16bf_rankn_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Exon-Gene-Li16bf_Rankn]*[E_li16bf_rankn_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_rankn_c012_zeta_un_.tsv and I_li16bf_rankn_c012_zeta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_rankn_c012_zeta_] := [I_li16bf_rankn_c012_zeta_un_]*[I_li16bf_rankn_c012_zeta_vn_] is a coefficient matrix for the linear model [Intron-Gene-Li16bf_Rankn] = [1 , Covariate-Rank]*[I_li16bf_rankn_c012_zeta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'I_li16bf_rankn_c012_beta_un_.tsv and I_li16bf_rankn_c012_beta_vn_.tsv: tab delimited arrays of coefficients. The outer-product [I_li16bf_rankn_c012_beta_] := [I_li16bf_rankn_c012_beta_un_]*[I_li16bf_rankn_c012_beta_vn_] is a coefficient matrix for the linear model [Covariate-Rank] = [1 , Intron-Gene-Li16bf_Rankn]*[I_li16bf_rankn_c012_beta_]. The ''1'' in the previous expression refers to a vector of all ones (i.e., the constant term in the linear model).\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_C_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_K_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the E gene-ranks.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_Z_absZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_H_absZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_C_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_K_absZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the I gene-ranks.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_Z_absZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_H_absZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the absolute-value of the z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_C_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_K_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the E gene-ranks.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_Z_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_E_li16bf_rankn_c012_H_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by exon genes (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_C_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular covariate-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_K_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by covariate (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular covariate-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the I gene-ranks.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_Z_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the raw z-score associated with the (raw) differential-expression of that particular gene-rank over that cluster pair.\n');
fprintf(fp_readme,'AB_I_li16bf_rankn_c012_H_rawZ_.tsv: tab delimited array of cluster-pairs (rows) by intron genes (columns). Each array entry indicates the raw z-score associated with the (relative) differential-expression of that particular gene-rank over that cluster pair. In this context ''relative'' means corrected for the linear effect of the covariate-rank.\n');
fclose(fp_readme);

%%%%%%%%;
str_X_ = {'E','I','EI'}; n_X = numel(str_X_);
str_Y_ = {'li16bf_rankn'}; n_Y = numel(str_Y_);
