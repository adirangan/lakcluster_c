...
if (flag_hnb);
local_fname_hnb_pre = sprintf('%s/%s_postprocess',dir_local_mat,tmp_infix_hnb_1);
...
[tmp_lpv_hnb] = label_to_label_enrichment_quad_4(label_xxxx_enum_,label_str_to_enum_0(local_output_label__));
%%%%%%%%;
tmp_label_tree_hnb ...
= ...
label_to_tree_2( ...
...
%%%%%%%%;
if (strcmp(str_local_method,'hnbrtZRgumb'));
index_louvain = efind(strcmp(global_prefix_method_,'louvain00pr_default'));
index_umap = efind(strcmp(global_prefix_method_,'umap00pr_default'));
end;%if (strcmp(str_local_method,'hnbrtZRgumb'));
if (strcmp(str_local_method,'hnbtZRgumb'));
index_louvain = efind(strcmp(global_prefix_method_,'louvain00_default'));
index_umap = efind(strcmp(global_prefix_method_,'umap00_default'));
end;%if (strcmp(str_local_method,'hnbtZRgumb'));
%%%%%%%%;
fname_fig = sprintf('%s/local_lpv_nlpnex_threshold_%s_FIGA',dir_local_jpg,tmp_infix_hnb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
%%%%%%%%;
...
plot(tmp_nlpnex_threshold_,-local_lpv_xxxx_(1+index_louvain)*ones(numel(tmp_nlpnex_threshold_),1),'rx-');
plot(tmp_nlpnex_threshold_,-local_lpv_xxxx_(1+index_umap)*ones(numel(tmp_nlpnex_threshold_),1),'k^-');
xlabel('nlp'); ylabel('entropy');
title('all','Interpreter','none');
%%%%%%%%;
sgtitle(sprintf('%s',dir_local_mat_hnb),'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
%%%%%%%%;
fname_fig = sprintf('%s/local_lpv_nlpnex_threshold_%s_FIGB',dir_local_jpg,tmp_infix_hnb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
%%%%%%%%;
...
plot(tmp_edge_,-local_lpv_xxxx_(1+index_louvain)*ones(numel(tmp_edge_),1),'r--','LineWidth',2);
plot(tmp_edge_,-local_lpv_xxxx_(1+index_umap)*ones(numel(tmp_edge_),1),'k--','LineWidth',2);
hold off;
ylabel('entropy');
%%%%%%%%;
sgtitle(sprintf('%s',dir_local_mat_hnb),'Interpreter','none');
