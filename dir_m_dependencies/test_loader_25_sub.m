...
tmp_Q_ = eye(n_CCOV); %[tmp_Q_,~] = qr(randn(n_CCOV));
n_rank_CC = 2;
C_n_sub_ = fast_tsne_dr_0(transpose(C_rank_*tmp_Q_),struct('rand_seed',1,'no_dims',nrank_CC,'theta',0.0));
opts_isosplit5 = struct('K_init',n_CCOV,'isocut_threshold',1.0);
...
[tmp_U_,tmp_S_,tmp_V_] = svds(C_rank_*tmp_Q_,n_CCOV); %<-- around 11 large principal values. ;
...
%%%%%%%%;
% Check for bias modes. ;
%%%%%%%%;
n_iteration = 128;
[AB_] = test_loader_cluster_C_AB_0(n_iteration,str_CLabel_,C_rank_);
Table_UEC_ = zeros(n_QCluster,n_CLabel);
for nQCluster=1:n_QCluster;
...
ns=0;
for nCLabel_A=1:n_CLabel;
for nCLabel_B=1:n_CLabel;
tmp_lp = z_to_p_0(AB_.AB_C_absZ_(1+ns,QC_ij)); tmp_lp = -tmp_lp/(n_l*n_CLabel);
Table_UEC_(nQCluster,nCLabel_A) = Table_UEC_(nQCluster,nCLabel_A) + tmp_lp;
Table_UEC_(nQCluster,nCLabel_B) = Table_UEC_(nQCluster,nCLabel_B) + tmp_lp;
ns=ns+1;
end;%for nCLabel_B=1:n_CLabel;
end;%for nCLabel_A=1:n_CLabel;
end;%for nl=1:n_l;
end;%for nQCluster=1:n_QCluster;
subplot(1,1,1);
colormap(colormap_beach());
imagesc(Table_UEC_); colorbar; figbig;
set(gca,'XTick',1:n_CLabel,'XTickLabel',unique(str_CLabel_)); xtickangle(90);
...
n_rank_wrap = 9;
%%%%%%%%;
test_loader_helper_wrap_1(dir_trunk,n_iteration_wrap,n_CLabel,str_CLabel_,C_VariableName_LC_,E_li16f_rankn_,I_li16f_rankn_,xeta_RRR_li16f_rankn_cPhr_,n_rank_wrap);
test_loader_helper_wrap_1(dir_trunk,n_iteration_wrap,n_CLabel,str_CLabel_,C_VariableName_LC_,E_li16f_rankn_,I_li16f_rankn_,xeta_RRR_li16f_rankn_c127_,n_rank_wrap);
test_loader_helper_wrap_1(dir_trunk,n_iteration_wrap,n_CLabel,str_CLabel_,C_VariableName_LC_,E_li16f_rankn_,I_li16f_rankn_,xeta_RRR_li16f_rankn_c012_,n_rank_wrap);
test_loader_helper_wrap_1(dir_trunk,n_iteration_wrap,n_CLabel,str_CLabel_,C_VariableName_LC_,E_li16f_rankn_,I_li16f_rankn_,xeta_RRR_li16f_rankn_cRRR_,n_rank_wrap);
