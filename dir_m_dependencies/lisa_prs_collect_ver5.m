function lisa_prs_collect_ver5(cl_num_arm1,cl_num_arm2,flag_reverse_use);
...
%{
  ...
  lisa_prs_collect_ver5(cl_num_arm1,cl_num_arm2,flag_reverse_use);
...
fname = sprintf('%s/icuk_dos_%s.hg19.ch.fl.out.dosage_imp_scores.txt.S%d.profile',dir_icuk,tmp_study_name,nprs); fcheck(fname); 
fid = fopen(fname);
...
%%%%%%%%;
fname = sprintf('%s/prs_collect_ver5_trn%d_tst%d_ni%.3d_ni%.3d.jpg',dir_out_s0000_pca_r0_arm1,cl_num_arm1,cl_num_arm2,ni_r0_arm2,ni_r1_arm2);
...
imagesc(squeeze(auc19_prs_t_(:,:,nprs,nir0_arm2_,nir1_arm2_)),[0.5,0.60]);
