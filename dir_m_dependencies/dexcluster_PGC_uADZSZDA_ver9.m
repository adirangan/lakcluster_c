function dexcluster_PGC_uADZSZDA_ver9(dir_code,dir_trunk,string_prefix,p_threshold,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose)
...
%{
  ...
  !scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20180304/dir_m/?*.m ~/dir_PGC_20180304/dir_m/ ;
  !scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20180304/dir_m/dexcluster_PGC_uADZSZDA_ver9.m ~/dir_PGC_20180304/dir_m/ ;
  !scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20180304/dir_m/dexcluster_PGC_uADZSZDA_excerpt_0.m ~/dir_PGC_20180304/dir_m/ ;
  !scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20180304/dir_m/xxxcluster_PGC_uADZSZDA_xfix_ver9.m ~/dir_PGC_20180304/dir_m/ ;
  ...
  dir_code = sprintf('%s/dir_lakcluster_c',string_platform);
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
  % test ;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
  ...
  p_threshold = 0.05; flag_reverse = 0; Ireq = 0; Icat = 1; n_mds = 20; mds_used_ = [1:2]; mds_repl = 3; nshuffle = 1;
  for p_threshold = p_threshold_;
  for flag_reverse=[0,1]; 
  for Ireq = [0,Ireq_half]; 
  if (Ireq==0); Icat = 1; mds_tab_max = 4; else; Icat = Icat_full; mds_tab_max = 2; end;
  for mds_tab=[0:mds_tab_max]; 
  n_mds = 20; 
  if mds_tab==0; mds_used_ = []; mds_repl = 0; end; 
  if mds_tab==1; mds_used_ = [1:2]; mds_repl = 1; end;
  if mds_tab==2; mds_used_ = [1:2]; mds_repl = 2; end;
  if mds_tab==3; mds_used_ = [1:2]; mds_repl = 3; end;
  if mds_tab==4; mds_used_ = [1:2]; mds_repl = 4; end;
  nshuffle=0; dexcluster_PGC_uADZSZDA_ver9(dir_code,dir_trunk,string_prefix,p_threshold,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose);
  %for nshuffle=1:8; dexcluster_PGC_uADZSZDA_ver9(dir_code,dir_trunk,string_prefix,p_threshold,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose); end;
  end;%for mds_tab=[0:mds_tab_max]; 
  end;%for flag_reverse=[0,1]; 
  end;%for Ireq = [0,Ireq_half];
  ...
  p_threshold_ = [0.01 , 0.05 , 0.10 , 0.25]; gamma=0.004;B_MLT=34;
  flag_rerun = 0;
  pbs_walltime_ = [4.50 , 4.25 , 4.00 , 3.75]*strcmp(string_platform,'/home/arangan');
  slurm_walltime_ = [4.50 , 4.25 , 4.00 , 3.75]*strcmp(string_platform,'/scratch/avr209');;
  slurm_nnodes = 1; slurm_tpn = 15; slurm_memdecl = 32;
  row_factor=1.0;col_factor=1.0; 
  flag_verbose=0;
  cl_num=3;%cl_num = input('cluster number? [1,2,3]');
  ...
  p_threshold = p_threshold_(n_p); pbs_walltime = pbs_walltime_(n_p); slurm_walltime = slurm_walltime_(n_p);
  for flag_reverse=[0,1]; 
  for Ireq = [0,Ireq_half]; 
  if (Ireq==0); Icat = 1; mds_tab_max = 4; else; Icat = Icat_full; mds_tab_max = 1; end;
  for mds_tab=[0:mds_tab_max]; 
  n_mds = 20; 
  if mds_tab==0; mds_used_ = []; mds_repl = 0; end; 
  if mds_tab==1; mds_used_ = [1:2]; mds_repl = 1; end;
  ...
  cl_num=3;%cl_num = input('cluster number? [1,2,3]');
  [study_trunk_,study_name_,n_study] = lisa_define_study_ver0(cl_num); Icat_full = n_study; Ireq_half = floor(Icat_full/2);
  for n_p=[1,4]; %for n_p=1:length(p_threshold_); 
  p_threshold = p_threshold_(n_p); pbs_walltime = pbs_walltime_(n_p); slurm_walltime = slurm_walltime_(n_p);
  for flag_reverse=[1]; %for flag_reverse=[0,1]; 
  for Ireq = [0,Ireq_half]; if (Ireq==0); Icat = 1; mds_tab = 2; else; Icat = Icat_full; mds_tab = 1; end;
  n_mds = 20; 
  if mds_tab==0; mds_used_ = []; mds_repl = 0; end; 
  if mds_tab==1; mds_used_ = [1:2]; mds_repl = 1; end;
  if mds_tab==2; mds_used_ = [1:2]; mds_repl = 2; end;
  ...
  if mds_tab==4; mds_used_ = [1:2]; mds_repl = 4; end;
  nshuffle=0; dexcluster_PGC_uADZSZDA_ver9(dir_code,dir_trunk,string_prefix,p_threshold,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose);
  for nshuffle=1:64; dexcluster_PGC_uADZSZDA_ver9(dir_code,dir_trunk,string_prefix,p_threshold,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose); end;
  end;%for Ireq = [0,Ireq_half];
  end;%for flag_reverse=[0,1]; 
  end;%for n_p = 1:length(p_threshold_);
...
string_name_s0000 = sprintf('%s_%s',string_prefix,xxxcluster_PGC_uADZSZDA_xfix_ver9('dex',p_threshold,flag_reverse,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,0));
...
string_name = sprintf('%s_%s',string_prefix,xxxcluster_PGC_uADZSZDA_xfix_ver9('dex',p_threshold,flag_reverse,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,nshuffle));
name_suffix = sprintf('%s','analyze');
...
if (flag_error); disp(sprintf(' %% Warning! incorrect dimensions in dexcluster_PGC_uADZSZDA_ver9')); return; end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
dexcluster_PGC_uADZSZDA_excerpt_0(fp,'GLOBAL_A_n_name_= ',Icat,d_inpre,'_A_','_n.b16');
dexcluster_PGC_uADZSZDA_excerpt_0(fp,'GLOBAL_A_t_name_= ',Icat,d_inpre,'_A_','_t.b16');
...
dexcluster_PGC_uADZSZDA_excerpt_0(fp,'GLOBAL_Z_n_name_= ',Icat,d_inpre,'_A_','_n.b16');
dexcluster_PGC_uADZSZDA_excerpt_0(fp,'GLOBAL_Z_t_name_= ',Icat,d_inpre,'_A_','_t.b16');
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by dexcluster_PGC_uADZSZDA_ver9.m on %s;\n',date);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
fclose(fp);
type(fname__in);
...
if (flag_timing); pbs_walltime_use = 1.5*elrt_s0000/3600; end;
...
fprintf(pbs_fp,'%s/lakcluster_ver18 < %s\n',dir_code,fname__in);
%%%%%%%%%%%%%%%%;
string_command = sprintf('scp -p %s/out_trace.txt %s/out_trace_s%0.4d.txt;',dir_out,dir_out_trace,nshuffle);
fprintf(pbs_fp,'%s\n',string_command);
if (nshuffle>0);
...
if (flag_timing); slurm_walltime_use = 1.5*elrt_s0000/3600; end;
