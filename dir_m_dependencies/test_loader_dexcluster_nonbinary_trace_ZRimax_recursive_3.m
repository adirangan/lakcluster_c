...
if (verbose>0); disp(sprintf(' %% [entering test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3]: dir_out %s',dir_out)); end;
...
[lp_R_F_max,ZR_F_ij] = Z_imax(0,-lp_R_F_(lp_R_F_ij_,1),-log(p_set)*1); ZR_F_ij = lp_R_F_ij_(ZR_F_ij); %<-- threshold at -log(p_set);
...
if strcmp(type_code,'split');
if (verbose>1); disp(sprintf(' %% %s --> split (lp_R_max %0.2f lp_R_F_max %0.2f --> p_branch %0.6f)',dir_out,lp_R_max,lp_R_F_max,exp(-lp_R_F_max))); end;
dir_A_out = sprintf('%s/dir_A',dir_out); if (~exist(dir_A_out,'dir')); disp(sprintf(' %% mkdir %s',dir_A_out)); mkdir(dir_A_out); end;
E_array_A_r_ij_ = E_array_r_ij_(branch_A_r_ij_); E_array_A_c_ij_ = E_array_c_ij_;
if (verbose>1); disp(sprintf(' %% %s <-- calling (n_sample = %d)',dir_A_out,length(E_array_A_r_ij_))); end;
test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_3( ...
