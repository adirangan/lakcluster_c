...
AB_H_sort_list_p__ = zeros(n_CLabel,n_CLabel,n_I_GENE);
AB_K_sort_list_p__ = zeros(n_CLabel,n_CLabel,n_CCOV);
I_AB_sort_list_p__ = zeros(n_CLabel.^2,n_I_GENE);
H_AB_sort_list_p__ = zeros(n_CLabel.^2,n_I_GENE);
...
tmp_H_aucX__ = zeros(n_CLabel,n_CLabel,n_I_GENE);
tmp_K_aucX__ = zeros(n_CLabel,n_CLabel,n_CCOV);
tmp_I_rank_avg_ = zeros(n_CLabel,n_I_GENE);
tmp_H_rank_avg_ = zeros(n_CLabel,n_I_GENE);
...
tmp_H_rank_avg_(nA,:) = mean((ones(length(tmp_Label_A_ij_),1)*zeta_I_un_(1,1:n_zeta_I) + C_rank_(tmp_Label_A_ij_,:)*zeta_I_un_(2:end,1:n_zeta_I))*transpose(zeta_I_vn_(:,1:n_zeta_I)),1);
tmp_K_rank_avg_(nA,:) = mean((ones(length(tmp_Label_A_ij_),1)*beta_I_un_(1,1:n_beta_I) + I_rank_(tmp_Label_A_ij_,:)*beta_I_un_(2:end,1:n_beta_I))*transpose(beta_I_vn_(:,1:n_beta_I)),1);
end;%if flag_I;
end;%for nA = 1:n_CLabel;
%%%%%%%%;
for nA = 1:n_CLabel;
...
tmp_H_aucX__(nA,nB,:) = [tmp_I_rank_avg_(nA,:) - tmp_I_rank_avg_(nB,:)] - [tmp_H_rank_avg_(nA,:) - tmp_H_rank_avg_(nB,:)];
...
[tmp_AB_H_sort_list__,tmp_AB_H_sort_ij__] = sort(tmp_H_aucX__,3,'ascend');
...
[tmp_H_AB_sort_list__,tmp_H_AB_sort_ij__] = sort(reshape(tmp_H_aucX__,n_CLabel.^2,n_I_GENE),2,'ascend');
...
AB_H_sort_list_avg__ = AB_H_sort_list_avg__ + tmp_AB_H_sort_list__;
...
H_AB_sort_list_avg__ = H_AB_sort_list_avg__ + tmp_H_AB_sort_list__;
...
AB_H_sort_list_std__ = AB_H_sort_list_std__ + tmp_AB_H_sort_list__.^2;
...
H_AB_sort_list_std__ = H_AB_sort_list_std__ + tmp_H_AB_sort_list__.^2;
K_AB_sort_list_std__ = K_AB_sort_list_std__ + tmp_K_AB_sort_list__.^2;
end;%if flag_I;
end;%if (niteration>0);
%%%%%%%%;
clear tmp_C_aucX__ tmp_C_rank_avg_ tmp_I_aucX__ tmp_I_aucX__ tmp_K_aucX__ tmp_I_rank_avg_ tmp_H_rank_avg_ tmp_K_rank_avg_;
clear tmp_Label_A_ij_ tmp_C_rank_avg_ tmp_n_A tmp_I_rank_avg_ tmp_H_rank_avg_ tmp_K_rank_avg_ ;
clear tmp_C_aucX__ tmp_I_aucX__ tmp_H_aucX__ tmp_K_aucX__ ;
clear tmp_AB_C_sort_list__ tmp_C_AB_sort_list__ tmp_AB_I_sort_list__ tmp_AB_H_sort_list__ tmp_AB_K_sort_list__ tmp_I_AB_sort_list__ tmp_H_AB_sort_list__ tmp_K_AB_sort_list__ ;
clear tmp_AB_C_sort_ij__ tmp_C_AB_sort_ij__ tmp_AB_I_sort_ij__ tmp_AB_H_sort_ij__ tmp_AB_K_sort_ij__ tmp_I_AB_sort_ij__ tmp_H_AB_sort_ij__ tmp_K_AB_sort_ij__ ;
...
AB_H_sort_list_avg__ = AB_H_sort_list_avg__/n_iteration;
AB_K_sort_list_avg__ = AB_K_sort_list_avg__/n_iteration;
I_AB_sort_list_avg__ = I_AB_sort_list_avg__/n_iteration;
H_AB_sort_list_avg__ = H_AB_sort_list_avg__/n_iteration;
...
AB_H_sort_list_std__ = sqrt(AB_H_sort_list_std__/n_iteration - AB_H_sort_list_avg__.^2);
...
H_AB_sort_list_std__ = sqrt(H_AB_sort_list_std__/n_iteration - H_AB_sort_list_avg__.^2);
...
AB_H_sort_list_p__ = erfc( abs(AB_H_sort_list_ori__ - AB_H_sort_list_avg__)./AB_H_sort_list_std__ / sqrt(2) );
...
H_AB_sort_list_p__ = erfc( abs(H_AB_sort_list_ori__ - H_AB_sort_list_avg__)./H_AB_sort_list_std__ / sqrt(2) );
...
AB_H_absZ_avg__ = zeros(n_I_GENE,1);
...
tmp_ij_ = squeeze(AB_H_sort_list_ij__(nA,nB,:)); tmp_z_ = squeeze( abs(AB_H_sort_list_ori__(nA,nB,:) - AB_H_sort_list_avg__(nA,nB,:))./AB_H_sort_list_std__(nA,nB,:) / sqrt(2) ); AB_H_absZ_avg__(tmp_ij_) = AB_H_absZ_avg__(tmp_ij_) + tmp_z_/tmp_s;
...
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/dir_jpg/AB_C_absZ_avg',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
end;%if flag_disp;
...
figure(1);clf;
subplot(2,1,1);
[~,tmp_ij_] = sort(AB_I_absZ_avg__,'ascend');
plot(1:n_I_GENE,sort(AB_I_absZ_avg__,'ascend'),'ko');
...
set(gca,'FontSize',7); %set(gca,'FontName','Courier');
subplot(2,1,2);
[~,tmp_ij_] = sort(AB_H_absZ_avg__,'ascend');
plot(1:n_I_GENE,sort(AB_H_absZ_avg__,'ascend'),'ko');
set(gca,'XTick',[],'XTickLabel',[]);
...
set(gca,'FontSize',7); %set(gca,'FontName','Courier');
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/dir_jpg/AB_I_absZ_avg',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
end;%if flag_I;
...
save(sprintf('%s/test_loader_8.mat',dir_data),'-v7.3'...
,'AB_C_absZ_avg__'...
