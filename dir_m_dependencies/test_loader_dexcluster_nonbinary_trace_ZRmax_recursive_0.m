function output_label_ = test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0(dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set);
%{
  ...
  test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0([],prefix_base,E_xxxx_,[],[],gamma,n_shuffle,[]);
  ...
  test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0([],prefix_base,E_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear E_xxxx_cxxx_ ;
  ...
  test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0([],prefix_base,I_xxxx_,[],[],gamma,n_shuffle,[]);
  end;%if ( strcmp(str_ctype,'000'));
  if (~strcmp(str_ctype,'000'));
  C_rank_cxxx_ = textread(sprintf('%s/dir_mat/C_rank_c%s_.tsv',dir_trunk,str_ctype)); C_rank_cxxx_ = C_rank_cxxx_(:,1:end-1);
  ...
  test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0([],prefix_base,I_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear I_xxxx_cxxx_ ;
  end;%for ncol_=1:length(n_col_);
  end;%if (~strcmp(str_ctype,'000'));
  %clear I_xxxx_ ;
...
if (verbose>0); disp(sprintf(' %% [entering test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0]: dir_out %s',dir_out)); end;
...
tmp_label_ = test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0(dir_A_out,prefix_base,E_array_base_,E_array_A_r_ij_,E_array_A_c_ij_,gamma,n_shuffle,p_set);
...
tmp_label_ = test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0(dir_B_out,prefix_base,E_array_base_,E_array_B_r_ij_,E_array_B_c_ij_,gamma,n_shuffle,p_set);
...
[lp_R_F_max,ZR_F_ij] = max(-lp_R_F_(:,1));
...
E_array_A_r_ij_ = E_array_r_ij_(branch_A_r_ij_); E_array_A_c_ij_ = E_array_c_ij_;
if (verbose>1); disp(sprintf(' %% %s <-- calling (n_sample = %d)',dir_A_out,length(E_array_A_r_ij_))); end;
test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0(dir_A_out,prefix_base,E_array_base_,E_array_A_r_ij_,E_array_A_c_ij_,gamma,n_shuffle,p_set);
dir_B_out = sprintf('%s/dir_B',dir_out); if (~exist(dir_B_out,'dir')); disp(sprintf(' %% mkdir %s',dir_B_out)); mkdir(dir_B_out); end;
E_array_B_r_ij_ = E_array_r_ij_(branch_B_r_ij_); E_array_B_c_ij_ = E_array_c_ij_;
if (verbose>1); disp(sprintf(' %% %s <-- calling (n_sample = %d)',dir_B_out,length(E_array_B_r_ij_))); end;
test_loader_dexcluster_nonbinary_trace_ZRmax_recursive_0(dir_B_out,prefix_base,E_array_base_,E_array_B_r_ij_,E_array_B_c_ij_,gamma,n_shuffle,p_set);
end;%if strcmp(type_code,'split');
