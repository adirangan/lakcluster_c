...
% Assuming a_=0, estimates a_ and A__, given BB_inv__ and CC_inv__;
...
tmp_t = toc(tmp_t); if (verbose_flag); disp(sprintf(' %% DD_vvt__: time %0.2fs',tmp_t)); end;

E00_bkp_nn_ = zeros(n_var^2,1);
tmp_t = tic();
E00_bkp_nn_ = DD_vvt__*dt2_all_;
tmp_t = toc(tmp_t); if (verbose_flag); disp(sprintf(' %% E00_bkp_nn_: time %0.2fs',tmp_t)); end;

E01_bkp_nnm__ = zeros(n_var^2,n_var^1);
tmp_t = tic();
E01_bkp_nnm__ = DD_vvt__*(sparse(1:n_total,1:n_total,dt2_all_,n_total,n_total)*Y_mid_all_tv__);
...
tmp_t = tic();
E11_bkp_nnmm__ = DD_vvt__*(sparse(1:n_total,1:n_total,dt2_all_,n_total,n_total)*YY_mid_all_tvv__);
...
dt_DD_dY_ = dt_all_(1+nt)*reshape(DD_vvt__(:,1+nt),[n_var,n_var])*dY_;
dt_DD_dYY__ = dt_DD_dY_*transpose(Y_mid_);
F0_bkp_n_ = F0_bkp_n_ + dt_DD_dY_;
F1_bkp_nm__ = F1_bkp_nm__ + dt_DD_dYY__;
...
%%%%%%%%;
tmp_t = tic();
chebfun_DD__ = chebfun(@(dt) chebout_dolphin_estimate_DD_0(dt,BB_inv__,CC_inv__) , reshape(dt_lim_,[1,2]) , 'splitting' , 'on' );
...
E01_nnm__ = E01_nnm__ + tmp_dt^2 * reshape(DD__,[n_var^2,1])*reshape(Y_mid_,[1,n_var]);
E10_mnn__ = E10_mnn__ + tmp_dt^2 * reshape(Y_mid_,[n_var,1])*reshape(DD__,[1,n_var^2]);
E11_nnmm__ = E11_nnmm__  + tmp_dt^2 * reshape(DD__,[n_var^2,1])*reshape(YY_mid__,[1,n_var^2]) ;
F0_n_ = F0_n_ + tmp_dt * (DD__*dY_) ;
F1_nm__ = F1_nm__ + tmp_dt * (DD__*dY_)*transpose(Y_mid_) ;
end;%for nage=2*(1+nstep):n_age-1;
end;%for nstep=0:n_step-1;
end;%for naid=0:n_aid-1;
...
DD__ = reshape(DD_vvt__(:,1+nt),[n_var,n_var]);
tmp_L = transpose(dY_ - tmp_dt*(a_ + A__*Y_mid_)) * DD__ * (dY_ - tmp_dt*(a_ + A__*Y_mid_));
det_DD = det_DD_t_(1+nt);
L_bkp = L_bkp + 0.5*n_var*log(2*pi) - 0.5*log(det_DD) + 0.5*tmp_L;
end;%for nt=0:n_total-1;
