function lisa_bed_to_b16_ver5(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
...
% scp -p rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_PGC_20180304/dir_m/lisa_bed_to_b16_ver5.m /home/arangan/dir_PGC_20180304/dir_m/lisa_bed_to_b16_ver5.m ;
%
%{
  flag_stop = 1;
  flag_crop = 0;
  ...
  flag_fam = 0;
  for cl_num=1:4;
  lisa_bed_to_b16_ver5(cl_num,flag_stop,flag_crop,flag_bim,flag_fam);
  end;% for cl_num=1:4;
  %}
...
dir_trunk = sprintf('%s','/data/rangan/dir_bcc/dir_PGC_20180304');
%dir_pre = sprintf('%s','/home/arangan/dir_PGC_20180304');
dir_pre = sprintf('%s','/data/rangan/dir_bcc/dir_PGC_20180304');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
snp_sgn_ = zeros(n_snp_cap,n_study);
for ns=1:n_study;
tmp_sgn_ = cast(cell2mat(snp_al1_{ns}(snp_cap__{ns})),'double')-'A'+1;
% convert T to A;
tmp_ij = find(tmp_sgn_==20); tmp_sgn_(tmp_ij) = 1;
% convert C to G;
tmp_ij = find(tmp_sgn_==3); tmp_sgn_(tmp_ij) = 7;
% convert A to -1 and G to +1;
tmp_sgn_ = 2*(tmp_sgn_==7)-1;
snp_sgn_(:,ns) = tmp_sgn_;
end;%for ns=1:n_study;
% Now compute hamming distance;
flag_disp=0;
if flag_disp;
disp((transpose(snp_sgn_)*snp_sgn_ + n_snp_cap)/2/n_snp_cap);
imagesc(((transpose(snp_sgn_)*snp_sgn_ + n_snp_cap)/2/n_snp_cap),[0,1]); colorbar;
...
disp(sprintf(' %% fraction of snps with mismatch: %d/%d = %0.4f',length(find(abs(sum(snp_sgn_,2))~=n_study)),length(snp_sgn_),length(find(abs(sum(snp_sgn_,2))~=n_study))/length(snp_sgn_)));
disp(sprintf(' %% fraction of mismatches (within %d-x-%d data-array) %d/%d = %0.4f',n_snp_cap,n_study,sum((n_study-abs(sum(snp_sgn_(:,:),2)))/2),(length(snp_sgn_(:,1))*n_study),sum((n_study-abs(sum(snp_sgn_(:,:),2)))/2)/(length(snp_sgn_(:,1))*n_study)));
disp(sprintf(' %% number of sign flips per study: ')); disp(num2str(sum(snp_sgn_==-1)));
disp(sprintf(' %% number of sign keeps per study: ')); disp(num2str(sum(snp_sgn_==+1)));
...
for ns=1:n_study; 
n_and_tot_ = n_and_tot_ + n_and_{ns}(snp_cap__{ns}); n_xor_tot_ = n_xor_tot_ + n_xor_{ns}(snp_cap__{ns}); n_nor_tot_ = n_nor_tot_ + n_nor_{ns}(snp_cap__{ns}); n_mss_tot_ = n_mss_tot_ + n_mss_{ns}(snp_cap__{ns}); 
...
%cij_index_array_ = zeros(2*n_snp_cap_sub,n_study+brk);
...
%cij_index_array_(cij_xor,col_fr2) = fr2_and_tot_(snp_cap_sub_) + fr2_xor_tot_(snp_cap_sub_);
...
if flag_stop>0;
disp('returning at line 847'); return; 
end;%if flag_stop>0;
