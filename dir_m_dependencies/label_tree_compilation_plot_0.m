...
verbose=1;
if (verbose); disp(sprintf(' %% [entering label_tree_compilation_plot_0]')); end;
...
if (~flag_exist);
disp(sprintf(' %% Warning, ~flag_exist in label_tree_compilation_plot_0'));
end;%if (~flag_exist);
if (flag_exist);
...
plot(nlpnex_threshold_,-lpv__(:,1+0),'m.-');
plot(nlpnex_threshold_,-lpv_rearrange_2b__(:,1+0),'mo-');
...
yyaxis right;
hold on;
stairs(nlpnex_threshold_,n_cluster_rearrange_2b_,'m-','LineWidth',2);
hold off;
...
lim_ = [0,max(-lpv_rearrange_2b__(:,1+0))];
p = stairfall_1(edge_,-lpv_rearrange_2b__(:,1+0),lim_,0.85*[1,1,1]); set(p,'EdgeColor','none');
...
yyaxis right;
hold on;
stairs(n_cluster_rearrange_2b_,-lpv_rearrange_2b__(:,1+0),'m-','LineWidth',2);
...
yyaxis left;
hold on;
edge_ = [n_cluster_rearrange_2b_;max(n_cluster_rearrange_2b_)+0];
...
hold on;
edge_ = [nlpnex_threshold_;3];
%stairs([0;-lpv_rearrange_2b__(:,1+0)],edge_,'m-','LineWidth',2);
tmp_x_ = [0;-lpv_rearrange_2b__(:,1+0)]; tmp_x_ = repmat(transpose(tmp_x_),[2,1]); tmp_x_ = tmp_x_(:);
...
if (verbose); disp(sprintf(' %% [finished label_tree_compilation_plot_0]')); end;
