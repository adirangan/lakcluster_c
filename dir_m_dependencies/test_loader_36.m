...
tmp_ = load(tmp_fname_mat); rank_estimate_H = tmp_.rank_estimate_sample; clear tmp_;
...
if (~isempty(strfind(prefix_normalization,'clogr'))); X_ = mean_center_0(X_,'col'); end;
rank_estimate_X = n_u;
[tmp_UH_,tmp_SH_,tmp_VH_] = svds( [ ones(n_u,1) , tmp_H_ ] , max(2,rank_estimate_H));
X_ = X_ - tmp_UH_*transpose(tmp_UH_)*X_; %<-- residual. ;
...
clear X_ tmp_H_ tmp_UH_ tmp_SH_ tmp_VH_ ;
...
tmp_ = load(tmp_fname_mat); rank_estimate_UXUXH = tmp_.rank_estimate_sample; clear tmp_;
...
[tmp_UUXH_,tmp_SUXH_,tmp_VUXH_] = svds( [ ones(n_u,1) , tmp_UXH_ ] ,rank_estimate_UXUXH);
X_ = (tmp_UX_*tmp_SX_)*(transpose(tmp_VX_) - tmp_UUXH_*(transpose(tmp_UUXH_)*transpose(tmp_VX_))); %<-- transpose(VX_) = UXH_*Z_  --> residual. ;
...
clear X_ tmp_H_ tmp_UX_ tmp_SX_ tmp_VX_ tmp_UXH_ tmp_UUXH_ tmp_SUXH_ tmp_VUXH_ ;
end;%if (~isempty(tmp_H_));
%%%%%%%%;
end;%for nQCluster=0:n_QCluster-1+1;
end;%for prefix_covariate_ = {'C','G'};
