...
% When estimating ranks, do not include the ones-vector. ;

[tmp_UH_,tmp_SH_,tmp_VH_] = svds(H_,n_HCOV);
%%%%%%%%;
% Identify original rank of H_. ;
%%%%%%%%;
...
rank_estimate_sample_0( [ mean_center_0(H_) ] );
save(tmp_fname_mat ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
,'svd_sample__' ...
...
% spectral-->isosplit5;
%%%%%%%%;
fname_fig = sprintf('%s/%s_spectral_scatter_FIGA',dir_jpg,H_infix);
...
label_H__{nrank_HC} = transpose(isosplit5(transpose(tmp_VH_(:,1:nrank_HC)*tmp_SH_(1:nrank_HC,1:nrank_HC)),opts_isosplit5));
end;%for nrank_HC=1:n_rank_HC;
for nrank_HC=1:n_rank_HC;
subplot(2,3,nrank_HC); scatter(tmp_VH_(:,1),tmp_VH_(:,2),15,label_H__{nrank_HC}); colormap(colormap_beach());
end;%for nrank_HC=1:n_rank_HC;
figbig;
...
prefix_base = sprintf('%s_%s',H_infix,str_xfix); 
dir_out = []; E_array_base_ = transpose(H_); E_array_r_ij_ = []; E_array_c_ij_ = [];
...
%%%%%%%%;
fname_fig = sprintf('%s/%s_dexnb_scatter_FIGA',dir_jpg,H_infix);
if (flag_force_create | flag_force_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
p_set_ = [0.05]*0.5.^[0:5];
...
subplot(2,3,np);
scatter(tmp_VH_(:,1),tmp_VH_(:,2),15,label_str_to_enum_0(tmp_ZRimax_label_),'filled');
...
fname_tsv = sprintf('%s/n_QCluster_%s_original.tsv',dir_mat,H_infix);
...
% plot QC-clusters. ;
%%%%%%%%;
fname_fig = sprintf('%s/%s_dexnb_scatter_FIGA0',dir_jpg,H_infix);
if (flag_force_create | flag_force_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
figure(1);clf;
label_plot_recursive_0(ZRimax_output_label_,ZRimax_lpFmax_label_,H_VariableName_);
title(sprintf('%s_dexnb',H_infix),'Interpreter','none');
figbig;
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
%%%%%%%%;
fname_fig = sprintf('%s/%s_dexnb_scatter_FIGA1',dir_jpg,H_infix);
if (flag_force_create | flag_force_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
figure(1);clf;
...
tmp_ij_ = find(ZRimax_enum_==1+nQCluster);
tmp_symbol = tmp_symbol_{1+nQCluster};
scatter(tmp_VH_(tmp_ij_,1),tmp_VH_(tmp_ij_,2),65,ZRimax_enum_(tmp_ij_),tmp_symbol,'filled','MarkerEdgeColor','k');
end;%for nQCluster=0:n_QCluster-1;
title(sprintf('QC clusters (%s)',H_infix),'Interpreter','none');
axis equal; axisnotick;
legend(tmp_Legend_,'Location','SouthEast');
subplot(1,3,3);
[tmp_AUL_n_] = label_pca_plot_0(transpose(H_),label_str_to_enum_0(ZRimax_output_label_));
colormap(colormap_beach());
hold on;
for nQCluster=0:n_QCluster-1;
tmp_ij_ = find(ZRimax_enum_==1+nQCluster);
...
title(sprintf('QC clusters (labeled pca) (%s)',H_infix),'Interpreter','none');
axis equal; axisnotick;
legend(tmp_Legend_,'Location','SouthEast');
...
rank_estimate_sample_0( [ mean_center_0(tmp_H_) ] );
save(tmp_fname_mat ...
,'nQCluster' ...
,'rank_estimate_sample' ...
,'s_B_nlp_' ...
...
for nQCluster=0:n_QCluster-1;
disp(sprintf(' %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'));
tmp_infix = sprintf('c%d_%s__',1+nQCluster,H_infix);
tmp_h_index_ = efind(ZRimax_enum_==1+nQCluster);
tmp_n_HCOV = numel(tmp_h_index_);
disp(sprintf(' %% nQCluster %d/%d --> %s(%d)',nQCluster,n_QCluster,tmp_infix,tmp_n_HCOV));
disp(sprintf(' %s: QCluster #%d:',H_infix,1+nQCluster));
disp(sprintf('\t[rank: original %d]',QCluster_rank_estimate_original_(1+nQCluster)));
...
% tsne00-->isosplit5;
%%%%%%%%;
fname_fig = sprintf('%s/%s_tsne00_scatter_FIGA',dir_jpg,H_infix);
if (flag_force_create | flag_force_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
figure(1);clf;
tmp_Q_ = eye(n_HCOV); %[tmp_Q_,~] = qr(randn(n_HCOV));
H_sub_ = fast_tsne_dr_0(transpose(H_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.0));
opts_isosplit5 = struct('K_init',n_HCOV,'isocut_threshold',1.0);
label_H_ = transpose(isosplit5(transpose(H_sub_),opts_isosplit5));
subplot(1,1,1); scatter(H_sub_(:,1),H_sub_(:,2),15,label_H_); colormap(colormap_beach());
disp(sprintf(' %% writing %s.jpg',fname_fig));
print('-djpeg',sprintf('%s.jpg',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
close(gcf);
...
% tsne50-->isosplit5;
%%%%%%%%;
fname_fig = sprintf('%s/%s_tsne50_scatter_FIGA',dir_jpg,H_infix);
if (flag_force_create | flag_force_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
figure(1);clf;
tmp_Q_ = eye(n_HCOV); %[tmp_Q_,~] = qr(randn(n_HCOV));
H_sub_ = fast_tsne_dr_0(transpose(H_*tmp_Q_),struct('rand_seed',1,'no_dims',2,'theta',0.5));
opts_isosplit5 = struct('K_init',n_HCOV,'isocut_threshold',1.0);
label_H_ = transpose(isosplit5(transpose(H_sub_),opts_isosplit5));
subplot(1,1,1); scatter(H_sub_(:,1),H_sub_(:,2),15,label_H_); colormap(colormap_beach());
disp(sprintf(' %% writing %s.jpg',fname_fig));
