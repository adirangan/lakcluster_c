function xxxcluster_PGC_uADZSZDA_ver15(dir_code,dir_trunk,string_prefix,maf_lo_threshold,maf_hi_threshold,mr_string,mc_string,flag_dex_vs_lak,flag_reverse,Icat,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,scramble_out_xdrop_,scramble_rseed_,nshuffle,flag_rerun,pbs_walltime,slurm_walltime,slurm_nnodes,slurm_tpn,slurm_memdecl,row_factor,col_factor,flag_verbose)
% set up to allow pbs_walltime as input ; using lakcluster_ver18 ; no replication ; reverse-search ;
% run with: xxxcluster_PGC_uADZSZDA_ver15_dr.m;
...
if (nargin<na); mr_string = mrtest; end; na=na+1;
if (nargin<na); mc_string = mctest; end; na=na+1;
...
if (nshuffle>0 & n_scramble>0); disp(sprintf(' %% Warning! n_scramble %d nshuffle %d in xxxcluster_PGC_uADZSZDA_ver15. Not yet implemented appropriately!',n_scramble,nshuffle)); end;
...
string_name_s0000 = sprintf('%s_%s',string_prefix,xxxcluster_PGC_uADZSZDA_xfix_ver15(flag_dex_vs_lak,maf_lo_threshold,maf_hi_threshold,mr_string,mc_string,flag_reverse,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,0));
...
string_name = sprintf('%s_%s',string_prefix,xxxcluster_PGC_uADZSZDA_xfix_ver15(flag_dex_vs_lak,maf_lo_threshold,maf_hi_threshold,mr_string,mc_string,flag_reverse,n_mds,mds_used_,mds_repl,gamma,B_MLT,Ireq,n_scramble,nshuffle));
name_suffix = sprintf('%s','analyze');
...
string_xdrop = sprintf('%s/out_xdrop_a_s%0.4d.txt',dir_out_trace,nshuffle);
flag_exist = exist(string_timing,'file') | (exist(string_trace,'file') & exist(string_xdrop,'file'));
if (~flag_rerun & flag_exist); 
if (exist(string_timing,'file')); disp(sprintf(' %% found %s, not rerunning.',string_timing)); end;
if (exist(string_trace,'file') & exist(string_xdrop,'file')) disp(sprintf(' %% found %s and\n%% found %s, not rerunning.',string_trace,string_xdrop)); end;
flag_found = 1; 
end; %if (~flag_rerun & flag_exist);
if (flag_rerun | ~flag_exist);
disp(sprintf(' %% could not find one of: \n %% %s\n %% %s\n %% %s, rerunning.',string_timing,string_trace,string_xdrop)); 
flag_found = 0; 
end;%if (flag_rerun | ~flag_exist);
...
if (Icat==1); tmpchar = sprintf('%s_mr_A_%s_full.b16',d_inpre,mr_string); else; tmpchar = sprintf('%s_mr_A_%s_%0.2d.b16',d_inpre,mr_string,1+nb); end;
 else;
...
if (length(mr_string)>0);
if (Icat==1); tmpchar = sprintf('%s_mr_Z_%s_full.b16',d_inpre,mr_string); else; tmpchar = sprintf('%s_mr_Z_%s_%0.2d.b16',d_inpre,mr_string,1+nb); end;
 else;
...
if (flag_error); disp(sprintf(' %% Warning! incorrect dimensions in xxxcluster_PGC_uADZSZDA_ver15')); return; end;
...
% writing col-mask ;
mc_A_use = mc_A_pre;
fname_bim = sprintf('%s/%s_bim.ex2',dir__in,string_prefix);
mc_A_bim = xxxcluster_PGC_getbim_ver4(fname_bim,maf_lo_threshold,maf_hi_threshold);
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by xxxcluster_PGC_uADZSZDA_ver15.m on %s;\n',date);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
fclose(fp);
type(fname__in);
...
string_command = sprintf('scp -p %s/out_trace.txt %s/out_trace_s%0.4d.txt; scp -p %s/out_xdrop_a.txt %s/out_xdrop_a_s%0.4d.txt;',dir_out,dir_out_trace,nshuffle,dir_out,dir_out_trace,nshuffle);
