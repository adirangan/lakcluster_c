...
dir_out_s0000_pca_arm1 = sprintf('%s/dir_pca_trn%d_tst%d',dir_out_s0000_arm1,cl_num_arm1,cl_num_arm2);
if (~exist(dir_out_s0000_pca_arm1,'dir')); disp(sprintf(' %% creating %s',dir_out_s0000_pca_arm1)); mkdir(dir_out_s0000_pca_arm1); end;
dir_out_s0000_pca_arm1_jpg = sprintf('%s/dir_pca_trn%d_tst%d/dir_jpg',dir_out_s0000_arm1,cl_num_arm1,cl_num_arm2);
if (~exist(dir_out_s0000_pca_arm1_jpg,'dir')); disp(sprintf(' %% creating %s',dir_out_s0000_pca_arm1_jpg)); mkdir(dir_out_s0000_pca_arm1_jpg); end;
...
pca_fname_fig = sprintf('%s/%s_pca_proj_ni%d_tst%d.jpg',dir_out_s0000_pca_arm1_jpg,string_name_s0000_arm1,niteration,cl_num_arm2); 
if (...
...
fname_bim_arm2 = sprintf('%s/%s_bim.ex2',dir__in_arm2,string_prefix_arm2); fcheck(fname_bim_arm2);
[n_snp_arm2,~,~,~,~,~,~,~,bim_name_arm2_,~] = lisa_bimex2_ver0(fname_bim_arm2);
%%%%%%%%%%%%%%%% ;
pca_fname_i = sprintf('%s/i_trn%d_tst%d.mat',dir_out_s0000_pca_arm1,cl_num_arm1,cl_num_arm2);
...
[~,i_arm1,i_arm2] = intersect(bim_name_arm1_,bim_name_arm2_,'stable');
save(pca_fname_i,'i_arm1','i_arm2');
end;%if (~exist(pca_fname_i,'file'));
disp(sprintf(' %% bim_name_arm1_ length %d, bim_name_arm2_ length %d, intersection length %d',length(bim_name_arm1_),length(bim_name_arm2_),length(i_arm1)));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (flag_reverse==1); mr_Z_arm1_tmp = mr_Z_arm1_full*0; mr_Z_arm1_tmp(rdrop_a_arm1_(length(rdrop_a_arm1_)-r_rem_arm1_(niteration)+1:length(rdrop_a_arm1_)))=1; end;
if (flag_reverse==0); mr_A_arm1_tmp = mr_A_arm1_full*0; mr_A_arm1_tmp(rdrop_a_arm1_(length(rdrop_a_arm1_)-r_rem_arm1_(niteration)+1:length(rdrop_a_arm1_)))=1; end;
...
for nc=1:length(ii); assert(strcmp(bim_name_arm2_{i_arm2(ii(nc))},bim_name_arm1_{i_arm1(ii(nc))})); end;%for nc=1:length(ii);
...
assert(length(intersect(bim_name_arm2_(j_arm2_),bim_name_arm1_(j_)))==length(j_));
%%%%%%%%%%%%%%%% ;
pca_infix = sprintf('pca_ni%d_tst%d',niteration,cl_num_arm2);
%%%%%%%%%%%%%%%% ;
% Assuming Icat==1 (i.e., full) for single row mask. ;
if (flag_reverse==1); 
tmpchar = sprintf('%s/%s_mr_Z_arm1_full_%s.b16',dir_out_s0000_pca_arm1,string_name_s0000_arm1,pca_infix); %delete(tmpchar);
...
%pca_rank = 2;
flag_T = 0;
if (mds_repl< 1 | length(mds_used_)~=2); flag_T = 0; T_n_crop_cols = 1+length(mds_used_); end;
if (mds_repl>=1 & length(mds_used_)==2); flag_T = 1; end;
mds_str = sprintf('m%dr%d',length(mds_used_),mds_repl);
mds_kappa_squared = 1.0;
if (length(mds_used_)>0); mds_kappa_squared = textread(sprintf('%s_T_%s_kappa.txt',d_inpre,mds_str)); end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ;
if (flag_reverse==1); 
mr_Z_arm1_tmp = mr_Z_arm1_full*0; mr_Z_arm1_tmp(rdrop_a_arm1_(length(rdrop_a_arm1_)-r_rem_arm1_(niteration)+1:length(rdrop_a_arm1_)))=1;
end;%if (flag_reverse==1); 
if (flag_reverse==0); 
...
if (flag_reverse==1);
plot(XnV_arm1_(find(mr_A_arm1_full),1),XnV_arm1_(find(mr_A_arm1_full),2),'k.','MarkerSize',15);
plot(DnV_arm1_(find(mr_Z_arm1_full-mr_Z_arm1_tmp),1),DnV_arm1_(find(mr_Z_arm1_full-mr_Z_arm1_tmp),2),'g.','MarkerSize',15);
plot(DnV_arm1_(find(mr_Z_arm1_tmp),1),DnV_arm1_(find(mr_Z_arm1_tmp),2),'r.','MarkerSize',25);
end;%if (flag_reverse==1);
if (flag_reverse==0);
plot(XnV_arm1_(find(mr_Z_arm1_full),1),XnV_arm1_(find(mr_Z_arm1_full),2),'k.','MarkerSize',15);
plot(DnV_arm1_(find(mr_A_arm1_full-mr_A_arm1_tmp),1),DnV_arm1_(find(mr_A_arm1_full-mr_A_arm1_tmp),2),'g.','MarkerSize',15);
...
if (flag_reverse==1);
h_A_full = hist(XnV_arm1_(find(mr_A_arm1_full),1),hbins); h_A_full = h_A_full/sum(mr_A_arm1_full);
h_Z_full = hist(DnV_arm1_(find(mr_Z_arm1_full),1),hbins); h_Z_full = h_Z_full/sum(mr_Z_arm1_full);
h_Z_exc = hist(DnV_arm1_(find(mr_Z_arm1_full-mr_Z_arm1_tmp),1),hbins); h_Z_exc = h_Z_exc/sum(mr_Z_arm1_full);
h_Z_tmp = hist(DnV_arm1_(find(mr_Z_arm1_tmp),1),hbins); h_Z_tmp = h_Z_tmp/sum(mr_Z_arm1_full);
...
if (flag_reverse==0);
h_Z_full = hist(XnV_arm1_(find(mr_Z_arm1_full),1),hbins); h_Z_full = h_Z_full/sum(mr_Z_arm1_full);
h_A_full = hist(DnV_arm1_(find(mr_A_arm1_full),1),hbins); h_A_full = h_A_full/sum(mr_A_arm1_full);
h_A_exc = hist(DnV_arm1_(find(mr_A_arm1_full-mr_A_arm1_tmp),1),hbins); h_A_exc = h_A_exc/sum(mr_A_arm1_full);
h_A_tmp = hist(DnV_arm1_(find(mr_A_arm1_tmp),1),hbins); h_A_tmp = h_A_tmp/sum(mr_A_arm1_full);
