function dexcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num,walltime,row_factor,col_factor,verbose_flag)
% set up to allow walltime as input ; using lakcluster_ver18 ; no replication ; reverse-search ;
% try: ;
%{
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/Makefile ./;
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/?*.[c] ./;
  ...
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dexcluster_PGC_ADZSZDA_ver6.m ./ ;
  ...
  shuffle_num=0; dexcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag);
  ...
  dir_code = sprintf('/home/grouther/dir_code_080517');dir_trunk = sprintf('%s',pwd); 
  p_threshold_ = [0.01 , 0.05 , 0.10 , 0.25]; gamma=0.004;B_MLT=34;
  %p_threshold_ = [0.10]; gamma=0.05;B_MLT=34;
  walltime_ = [2 , 2 , 2 , 2];
  ...
  shuffle_num=0; dexcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag);
  for shuffle_num=1:64; dexcluster_PGC_ADZSZDA_ver6(dir_code,dir_trunk,prefix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,0,shuffle_num,walltime,row_factor,col_factor,verbose_flag); end;
...
test_string = sprintf('%s_%s',prefix,PGC_dcc_xfix_gen_ver6(p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num));
...
%if length(tmp_trn)> 970; disp(sprintf(' %% found %s of length %d, not rerunning.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 1; end;
if length(tmp_trn)> 970; disp(sprintf(' %% found %s of length %d, actually, rerunning anyway.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 0; end;
if length(tmp_trn)<=970; disp(sprintf(' %% found %s of length %d,     rerunning.',tmpchar_trn,length(tmp_trn))); found_trace_flag = 0; end;
...
fprintf(fp,'END= 0;\n');
fprintf(fp,'%% generated by dexcluster_PGC_ADZSZDA_ver6.m on %s;\n',date);
fclose(fp);
end;%if ~found_trace_flag;
...
function [output_string,gamma_d] = PGC_dcc_xfix_gen_ver6(p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num);
...
output_string = sprintf('dex_ADZSZDA_%s_%s_m%d_g%.3d_B%.2d_n%d_s%.4d',p_str,rev_str,mds_code,gamma_d,B_MLT,Ireq,shuffle_num);
