function [r_fr__,r_pv__,r_rtn__,r_mu__,r_sg__,r_sum__,r_cap__] = cumulative_intersect_unique(r_remaining_A_,rdrop_A_,r_remaining_B_,rdrop_B_);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% function [r_fr__,r_pv__,r_rtn__,r_mu__,r_sg__,r_sum__,r_cap__] = cumulative_intersect_unique(r_remaining_A_,rdrop_A_,r_remaining_B_,rdrop_B_);
% ;
...
%
% Outputs: ;
% r_fr__   double array of size n_iteration_A-by-n_iteration_B. r_fr_(j,k) is the fraction of overlap associated with each entry in r_rtn__. ;
...
if (nargin<4);
disp(sprintf(' %% testing cumulative_intersect_unique'));
rng(1);
...
[r_fr__,r_pv__,r_rtn__,r_mu__,r_sg__,r_sum__,r_cap__] = cumulative_intersect_unique(r_remaining_A_,rdrop_A_,r_remaining_B_,rdrop_B_);
...
%%%%%%%%;
if (n_total_B~=n_total_A); disp(sprintf(' %% Warning! n_total_A %d ~= n_total_B %d in cumulative_intersect_unique',n_total_A,n_total_B)); end;
n_total = n_total_A;
%%%%%%%%;
[~,iA_] = sort(rdrop_A_); 
[~,iB_] = sort(rdrop_B_); 
...
r_sg__ = sqrt( (r_pA__.*r_pB__) .* (1 - r_pA__.*r_pB__) * n_total );
...
r_fr__ = r_rtn__./max(1,max(repmat(r_rtn__(:,1),1,n_iteration_B),repmat(r_rtn__(1,:),n_iteration_A,1)));
