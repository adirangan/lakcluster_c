function [rdrop,cdrop] = get_q3drop(lrij,lcij,gamma);
...
if (verbose); disp(sprintf(' %% [entering get_q3drop] gamma %0.3f, lrij %d lcij %d',gamma,lrij,lcij)); end;
...
ammag_tmp_col = ammag_tmp_row;  %<-- assuming linear balance. ;
gamma_tmp_col = dbl1-ammag_tmp_col;
rdrop = ceil(gamma_tmp_row*lrij); 
cdrop = round(gamma_tmp_col*lcij); 
...
if (verbose); disp(sprintf(' %% [finished get_q3drop] rdrop %d cdrop %d',rdrop,cdrop)); end;
