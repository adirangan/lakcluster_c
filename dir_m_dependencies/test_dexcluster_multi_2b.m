function test_dexcluster_multi_2b(M,N,snr,n_cluster,gamma,n_rank,n_iteration,n_shuffle,flag_rerun);
...
%{
  ...
  for nstep=1:n_step;
  snr = snr_(nstep);
  test_dexcluster_multi_2b(M,N,snr,n_cluster,gamma,n_rank,n_iteration,n_shuffle,flag_rerun);
...
[lpv_(ntab),lP0_(ntab),fla_(ntab)] = label_to_label_enrichment_quad_4(label_A_,label_B__{ntab});
...
str_louvain00_tmp = sprintf('%s/%s.tmp',dir_base,str_xfix);
if (~exist(str_louvain00_mat,'file') & ~exist(str_louvain00_tmp,'file')); 
disp(sprintf(' %% %s not found, creating',str_louvain00_mat)); 
save(str_louvain00_tmp,'str_louvain00_mat');
try;
%%%%%%%%;
lpv_ = zeros(1,1); lP0_ = zeros(1,1); fla_ = zeros(1,1); label_B__ = cell(1,1);
...
[lpv_(1),lP0_(1),fla_(1)] = label_to_label_enrichment_quad_4(label_A_,label_B__{1});
%%%%%%%%;
save(str_louvain00_mat,'lpv_','lP0_','fla_','label_B__');
clear A_n_sub_ lpv_ lP0_ fla_ label_B__ ;
catch; disp(sprintf(' %% WARNING: error generating %s',str_louvain00_mat)); end;%try;
delete(str_louvain00_tmp);
end;%if (~exist(str_louvain00,'file'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% louvainpr ;
...
str_louvainpr_tmp = sprintf('%s/%s.tmp',dir_base,str_xfix);
if (~exist(str_louvainpr_mat,'file') & ~exist(str_louvainpr_tmp,'file')); 
disp(sprintf(' %% %s not found, creating',str_louvainpr_mat)); 
save(str_louvainpr_tmp,'str_louvainpr_mat');
try;
%%%%%%%%;
lpv_ = zeros(1,1); lP0_ = zeros(1,1); fla_ = zeros(1,1); label_B__ = cell(1,1);
[tmp_U_,tmp_S_,tmp_V_] = svds(A_n_,n_cluster+1);
tmp_US_ = tmp_U_*tmp_S_;
label_B__{1} = GCModulMax1(tmp_US_*transpose(tmp_US_));
clear tmp_U_ tmp_S_ tmp_V_ tmp_US_;
[lpv_(1),lP0_(1),fla_(1)] = label_to_label_enrichment_quad_4(label_A_,label_B__{1});
%%%%%%%%;
save(str_louvainpr_mat,'lpv_','lP0_','fla_','label_B__');
clear A_n_sub_ lpv_ lP0_ fla_ label_B__ ;
catch; disp(sprintf(' %% WARNING: error generating %s',str_louvainpr_mat)); end;%try;
delete(str_louvainpr_tmp);
end;%if (~exist(str_louvainpr,'file'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
% dexcluster --> ZRimax ;
...
test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_4(dir_base,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
ZRimax_label_ = test_loader_dexcluster_nonbinary_trace_ZRimax_recursive_4(dir_base,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
...
prefix_base = sprintf('tmp_%s',str_xfix); 
dir_out = []; E_array_base_ = A_n_; E_array_r_ij_ = []; E_array_c_ij_ = []; p_set = []; n_member_lob = 2;
test_loader_lakcluster_nonbinary_trace_ZRimax_recursive_4(dir_base,dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set,n_member_lob);
