function [BB__,CC__,l2_R__,sum_1,sum_dt,sum_dtdt,sum_DDj__,sum_DDjdt__] = dolphin_estimate_BC_from_aA_1(aid_,age_,dat__,a_,A__,n_step,flag_constrain_CC);
% Estimates BB__ and CC__, given a_ and A__. ;
% if flag_constrain_CC then we constrain CC to be diagonal. ;
...
BB__(1+nvar0,1+nvar1) = sum_DDjdt__(1+nvar0,1+nvar1)/sum_dtdt;
CC__(1+nvar0,1+nvar1) = 0;
else;
tmp_DDj_ = [ sum_DDjdt__(1+nvar0,1+nvar1) ; sum_DDj__(1+nvar0,1+nvar1) ];
tmp_BC_ = tmp_AA__ \ tmp_DDj_;
BB__(1+nvar0,1+nvar1) = tmp_BC_(1+0);
CC__(1+nvar0,1+nvar1) = tmp_BC_(1+1);
end;%if;
BB__(1+nvar1,1+nvar0) = BB__(1+nvar0,1+nvar1);
CC__(1+nvar1,1+nvar0) = CC__(1+nvar0,1+nvar1);
end;%for nvar1=nvar0:n_var-1;
end;%for nvar0=0:n_var-1;

tmp_t = toc(tmp_t); if (verbose_flag); disp(sprintf(' %% BB__ and CC__: time %0.2fs',tmp_t)); end;
...
BB__ = real(V_BB__)*max(0,real(S_BB__))*transpose(real(V_BB__));
[V_CC__,S_CC__] = eig(CC__);
CC__ = real(V_CC__)*max(0,real(S_CC__))*transpose(real(V_CC__));
