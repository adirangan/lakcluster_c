...
flag_plot=1;
if flag_plot;
figure(2);subplot(3,5,nalpha)
K_50_ = zeros(n_rank,1);
for nrank=1:n_rank;
K_ =zeros(n_cluster,n_cluster);
for ncluster1=1:n_cluster;for ncluster2=1:n_cluster;
tmp_1_ = tmp_DVS_(Sample_Label_ij_{ncluster1},1:nrank);
tmp_2_ = tmp_DVS_(Sample_Label_ij_{ncluster2},1:nrank);
K_(ncluster1,ncluster2) = kld_iso_0(tmp_1_,tmp_2_);
clear tmp_1_ tmp_2_;
end;end;%for ncluster1=1:n_cluster;for ncluster2=1:n_cluster;
K_50_(nrank) = median(K_(:));
end;%for nrank=1:n_rank;
plot(1:n_rank,K_50_,'ko-');
xlabel('rank'); ylabel('median kd');
title(sprintf('alpha %0.2f',alpha));
drawnow();
end;%if flag_plot;
