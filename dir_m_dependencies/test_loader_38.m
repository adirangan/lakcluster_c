...
clear tmp_;
%%%%%%%%;
% define log-normalized data-sets. ;
%%%%%%%%;
val_min = min(min(E_val_(1+E_index_filled_)),min(I_val_(1+I_index_filled_)));
disp(sprintf(' %% val_min %0.16f',val_min)); %<-- counts as '1' for expression counts. ;
E_logn_ = log(val_min + E_val_);
I_logn_ = log(val_min + I_val_);
tmp_ = E_logn_; fname_tsv = sprintf('%s/dir_mat/E_logn_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
clear tmp_;
tmp_ = I_logn_; fname_tsv = sprintf('%s/dir_mat/I_logn_.tsv',dir_trunk); 
if (~exist(fname_tsv,'file')); disp(sprintf(' %% writing %s',fname_tsv)); save(fname_tsv,'tmp_','-ascii','-tabs'); end;
clear tmp_;
...
if (strcmp(str_X,'E') & ~isempty(strfind(prefix_normalization,'logn'))); X_ = E_logn_; end;
...
if (strcmp(str_X,'I') & ~isempty(strfind(prefix_normalization,'logn'))); X_ = I_logn_; end;
...
if (strcmp(str_X,'EI') & ~isempty(strfind(prefix_normalization,'logn'))); X_ = [E_logn_ , I_logn_]; end;
...
for str_X_ = {'E','I','EI'};
str_X = str_X_{1};
for prefix_normalization_ = {'logn','liXf','liXf_clogr','fill'};
...
for prefix_normalization_ = {'logn','liXf','liXf_clogr','fill'};
prefix_normalization = prefix_normalization_{1};
if (~isempty(strfind(prefix_normalization,'logn'))); E_xxxx_ = E_logn_ ; I_xxxx_ = I_logn_; end;%if (~isempty(strfind(prefix_normalization,'logn')));
