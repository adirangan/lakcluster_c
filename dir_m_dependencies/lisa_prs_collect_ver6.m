function lisa_prs_collect_ver6(cl_num_arm1,cl_num_arm2,flag_reverse_use);
% Assuming that arm1 is cl4. ;
% Using data from /data/rangan/dir_bcc/dir_BIP32_loo_data/dir_icuk_scores ;
% try: ;
%{
  for cl_num_arm1=[4]; for cl_num_arm2=[1,2,3]; for flag_reverse_use=[0:1];
  lisa_prs_collect_ver6(cl_num_arm1,cl_num_arm2,flag_reverse_use);
  end;end;end;%for cl_num_arm1=[4]; for cl_num_arm2=[1,2,3]; for flag_reverse_use=[0:1];
  %}
...
dir_GCTA = sprintf('%s/dir_GCTA_icuk_geno_on_dos_just_profile',dir_bip32);
...
if nprs==0; tmp_str = '25'; end;
if nprs==1; tmp_str = '10'; end;
if nprs==2; tmp_str = '01'; end;
fname = sprintf('%s/GCTA_icuk_on_dos_cl%d_%sto50.profile',dir_GCTA,cl_num_arm2,tmp_str); fcheck(fname);
fid = fopen(fname);
tmp_prs_ = textscan(fid,'%s%s%d%f','headerlines',0);
fclose(fid);
...
%%%%%%%%;
fname = sprintf('%s/prs_collect_ver6_trn%d_tst%d_ni%.3d_ni%.3d.jpg',dir_out_s0000_pca_r0_arm1,cl_num_arm1,cl_num_arm2,ni_r0_arm2,ni_r1_arm2);
if ( exist(fname,'file')); disp(sprintf(' %% %s found, not creating',fname)); end;
if (~exist(fname,'file')); disp(sprintf(' %% %s not found, creating',fname)); 
%%%%%%%%;
...
% plot output. ;
%%%%%%%%;
nprs_sub_ = [1:3]; n_tick = 11; cauclim_ = [0.500,0.625];
