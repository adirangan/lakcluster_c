...
test_loader_lakcluster_nonbinary_trace_ZRimax_recursive_3( ...
 dir_trunk ...
,dir_out ...
,prefix_base ...
,E_array_base_ ...
...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('laknb_trace_ZRmax',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering test_loader_lakcluster_nonbinary_trace_ZRimax_recursive_3]: dir_out %s',dir_out)); end;
...
if (p_branch>p_set);
if (verbose>1); disp(sprintf(' %% %s --> stop (p_branch %0.6f > p_set %0.6f)',dir_out,p_branch,p_set)); end;
end;%if (p_branch>p_set);
if (p_branch<p_set);
if (verbose>1); disp(sprintf(' %% %s --> split (p_branch %0.6f < p_set %0.6f)',dir_out,p_branch,p_set)); end;
...
if ( exist(tmp_fname_trace_E,'file') &  exist(tmp_fname_xdrop_E,'file'));
out_trace_E_0_ = textread(tmp_fname_trace_E);
out_xdrop_E_0_ = textread(tmp_fname_xdrop_E);
end;%if ( exist(tmp_fname_trace_E,'file') &  exist(tmp_fname_xdrop_E,'file'));
if (~exist(tmp_fname_trace_E,'file') | ~exist(tmp_fname_xdrop_E,'file'));
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
[out_xdrop_E_0_,out_trace_E_0_] = lakcluster_nonbinary_AAAA_ver0(L_array_,gamma);
...
tmp_fname_trace_E = sprintf('%s/out_trace_s%.4d.txt',dir_trace,nshuffle);
if ( exist(tmp_fname_trace_E,'file'));
out_trace_E_0_ = textread(tmp_fname_trace_E);
end;%if ( exist(tmp_fname_trace_E,'file'));
if (~exist(tmp_fname_trace_E,'file'));
if (verbose>1); disp(sprintf(' %% %d/%d: not found, creating',nshuffle,n_shuffle)); end;
...
load(fname_branch);
end;%if ( exist(fname_branch,'file'));
if (~exist(fname_branch,'file'));
...
[lp_R_max,ZR_ij] = Z_imax(0,-lp_R_(:,1),-log(p_set));
...
[out_xdrop_F_0_,out_trace_F_0_] = lakcluster_nonbinary_AAAA_rdrop_ver0(L_array_,gamma_rdrop);
...
[lp_R_F_max,ZR_F_ij] = Z_imax(0,-lp_R_F_(lp_R_F_ij_,1),0); ZR_F_ij = lp_R_F_ij_(ZR_F_ij);
if (lp_R_F_max<=-log(p_set)); if (verbose>0); disp(sprintf(' %% Warning: lp_R_F_max %0.2f <=-log(p_set) %0.2f',lp_R_F_max,-log(p_set))); end; end;
if (lp_R_F_max> -log(p_set)); 
[lp_R_F_max,ZR_F_ij] = Z_imax(0,-lp_R_F_(lp_R_F_ij_,1),-log(p_set)); ZR_F_ij = lp_R_F_ij_(ZR_F_ij);
if (verbose>1); disp(sprintf(' %% redefining: lp_R_F_max %0.6f; statistically significant, ZR_F_ij %d',lp_R_F_max,ZR_F_ij)); end;
end;%if (lp_R_F_max> -log(p_set)); 
