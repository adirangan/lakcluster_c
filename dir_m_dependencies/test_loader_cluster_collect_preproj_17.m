...
%%%%;
if (~isempty(strfind(prefix_method_{1+nmethod},'hnbt'))); 
if (~isempty(strfind(prefix_method_{1+nmethod},'preproj'))); 
...
tmp_US_X_sg__ = [];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
for str_local_method_ = {'hnbrtZRgumb','hnbtZRgumb'};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
tmp_infix_hnb_0 = sprintf('preproj_hnbtZRgumb_g000_p050_nml3'); 
tmp_infix_hnb_1 = sprintf('%s_n03',tmp_infix_hnb_0);
end;%if (strcmp(str_local_method,'hnbrtZRgumb'));
if (strcmp(str_local_method,'hnbtZRgumb'));
tmp_infix_hnb_0 = sprintf('hnbtZRgumb_g010_p050_nml3'); 
tmp_infix_hnb_1 = sprintf('%s_g010_n03',tmp_infix_hnb_0);
end;%if (strcmp(str_local_method,'hnbtZRgumb'));
tmp_dir_cluster_hnb = sprintf('%s/dir_tmp_%s/dir_tmp_%s',tmp_dir_cluster,tmp_infix_hnb_0,tmp_infix_hnb_1);
tmp_local_fname_output_label = sprintf('%s/output_label__.txt',tmp_dir_cluster_hnb);
tmp_local_fname_nlpbra_label = sprintf('%s/nlpbra_label__.txt',tmp_dir_cluster_hnb);
tmp_local_fname_nlpnex_label = sprintf('%s/nlpnex_label__.txt',tmp_dir_cluster_hnb);
flag_hnb = exist(tmp_local_fname_output_label,'file') & exist(tmp_local_fname_nlpbra_label,'file') & exist(tmp_local_fname_nlpnex_label,'file');
if (flag_hnb);
local_fname_hnb_pre = sprintf('%s/%s_postprocess',tmp_dir_cluster,tmp_infix_hnb_1);
[local_flag_hnb_skip,local_fname_hnb_mat] = open_fname_tmp(local_fname_hnb_pre,date_diff_threshold,flag_force_create_mat,flag_force_create_tmp);
if ~local_flag_hnb_skip;
...
[tmp_lpv_hnb] = label_to_label_enrichment_quad_4(label_A_,label_str_to_enum_0(local_output_label__));
%%%%%%%%;
tmp_label_tree_hnb ...
= ...
label_to_tree_1( ...
...
%%%%%%%%%%%%%%%%;
if isempty(tmp_US_X_sg__);
[tmp_U_,tmp_S_,tmp_V_] = svds(X_,rank_estimate_X);
tmp_US_X_sg__ = tmp_U_*tmp_S_; clear tmp_U_ tmp_S_ tmp_V_;
end;%if isempty(tmp_US_X_sg__);
[ ...
 tmp_nlpnex_threshold_ ...
 tmp_psplit_threshold_ ...
,~ ...
,tmp_lpv_hnb__ ...
,tmp_n_cluster_hnb_ ...
,tmp_lpv_hnb_rearrange_2b__ ...
,tmp_n_cluster_hnb_rearrange_2b_ ...
] = ...
...
close_fname_tmp(local_fname_hnb_pre);
end;%if ~local_flag_hnb_skip;
%%%%%%%%;
if exist(local_fname_hnb_mat,'file');
load(local_fname_hnb_mat);
%%%%%%%%;
if (strcmp(str_local_method,'hnbrtZRgumb'));
tmp_ = load(sprintf('%s/preproj_umap00_default.mat',tmp_dir_cluster));
tmp_label_B_ = tmp_.label_B__{1};
n_cluster_umap00_default = numel(unique(tmp_label_B_));
lpv_umap00_default = tmp_.lpv_(1);
%label_to_label_enrichment_quad_4(label_A_,tmp_.label_B__{1}); %<-- should equal lpv. ;
clear tmp_ tmp_label_B_;
tmp_ = load(sprintf('%s/preproj_louvain00_default.mat',tmp_dir_cluster));
tmp_label_B_ = tmp_.label_B__{1};
n_cluster_louvain00_default = numel(unique(tmp_label_B_));
lpv_louvain00_default = tmp_.lpv_(1);
%label_to_label_enrichment_quad_4(label_A_,tmp_.label_B__{1}); %<-- should equal lpv. ;
clear tmp_ tmp_label_B_;
end;%if (strcmp(str_local_method,'hnbrtZRgumb'));
%%%%%%%%;
if (strcmp(str_local_method,'hnbtZRgumb'));
tmp_ = load(sprintf('%s/umap00_default.mat',tmp_dir_cluster));
tmp_label_B_ = tmp_.label_B__{1};
n_cluster_umap00_default = numel(unique(tmp_label_B_));
lpv_umap00_default = tmp_.lpv_(1);
%label_to_label_enrichment_quad_4(label_A_,tmp_.label_B__{1}); %<-- should equal lpv. ;
clear tmp_ tmp_label_B_;
tmp_ = load(sprintf('%s/louvain00_default.mat',tmp_dir_cluster));
tmp_label_B_ = tmp_.label_B__{1};
n_cluster_louvain00_default = numel(unique(tmp_label_B_));
lpv_louvain00_default = tmp_.lpv_(1);
%label_to_label_enrichment_quad_4(label_A_,tmp_.label_B__{1}); %<-- should equal lpv. ;
...
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/local_lpv_nlpnex_threshold_%s_FIGA',tmp_dir_cluster,tmp_infix_hnb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
%%%%%%%%;
subplot(1,1,1); hold on;
plot(tmp_nlpnex_threshold_,-tmp_lpv_hnb__(:,1+0),'m.-');
plot(tmp_nlpnex_threshold_,-tmp_lpv_hnb_rearrange_2b__(:,1+0),'mo-');
plot(tmp_nlpnex_threshold_,-lpv_louvain00_default*ones(numel(tmp_nlpnex_threshold_),1),'rx-');
plot(tmp_nlpnex_threshold_,-lpv_umap00_default*ones(numel(tmp_nlpnex_threshold_),1),'k^-');
xlabel('nlp'); ylabel('entropy');
title('all','Interpreter','none');
%%%%%%%%;
sgtitle(sprintf('%s',tmp_dir_cluster_hnb),'Interpreter','none');
...
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/local_lpv_nlpnex_threshold_%s_FIGB',tmp_dir_cluster,tmp_infix_hnb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
%%%%%%%%;
...
yyaxis right;
hold on;
stairs(tmp_nlpnex_threshold_,tmp_n_cluster_hnb_rearrange_2b_,'m-','LineWidth',2);
hold off;
ylabel('n_cluster','Interpreter','none');
%%%%;
yyaxis left;
hold on;
tmp_edge_ = [tmp_nlpnex_threshold_;3];
tmp_lim_ = [0,max(-tmp_lpv_hnb_rearrange_2b__(:,1+0))];
p = stairfall_1(tmp_edge_,-tmp_lpv_hnb_rearrange_2b__(:,1+0),tmp_lim_,0.85*[1,1,1]); set(p,'EdgeColor','none');
plot(tmp_edge_,-lpv_louvain00_default*ones(numel(tmp_edge_),1),'r--','LineWidth',2);
plot(tmp_edge_,-lpv_umap00_default*ones(numel(tmp_edge_),1),'k--','LineWidth',2);
hold off;
ylabel('entropy');
%%%%%%%%;
sgtitle(sprintf('%s',tmp_dir_cluster_hnb),'Interpreter','none');
...
%%%%%%%%;
fname_fig = sprintf('%s/dir_jpg/local_lpv_nlpnex_threshold_%s_FIGC',tmp_dir_cluster,tmp_infix_hnb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
%%%%%%%%;
...
yyaxis right;
hold on;
stairs(tmp_n_cluster_hnb_rearrange_2b_,-tmp_lpv_hnb_rearrange_2b__(:,1+0),'m-','LineWidth',2);
...
hold off;
ylabel('entropy','Interpreter','none');
%%%%;
yyaxis left;
hold on;
tmp_edge_ = [tmp_n_cluster_hnb_rearrange_2b_;max(tmp_n_cluster_hnb_rearrange_2b_)+1];
tmp_lim_ = [0,max(tmp_nlpnex_threshold_)];
p = stairfall_1(tmp_edge_,tmp_nlpnex_threshold_,tmp_lim_,0.85*[1,1,1]); set(p,'EdgeColor','none');
hold off;
ylabel('nlp');
%%%%%%%%;
sgtitle(sprintf('%s',tmp_dir_cluster_hnb),'Interpreter','none');
...
%%%%%%%%;
end;%if exist(local_fname_hnb_mat,'file');
%%%%%%%%;
end;%if (flag_hnb);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%;
...
parameter.flag_force_create_mat = 1;
