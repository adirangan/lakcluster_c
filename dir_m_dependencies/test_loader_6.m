...
clear tmp_str ;
%%%%%%%%;
str_table_name = sprintf('%s/Homo_sapiens.GRCh38.cdna.all.fa.gene_length_faidx.tsv',dir_data);
fp = fopen(str_table_name); gene_length_ = textscan(fp,'%s %d'); fclose(fp);
for ng=1:length(gene_length_{1});
gene_length_{1}{ng}(4) = 'G'; %<-- change ENST to ENSG ;
...
clear tmp_cap_;
%%%%%%%%;
% Note that only a handful (53) of the E_ are found, and only 26 of the I_ are found. ;
%%%%%%%%;
 %}
...
n_iteration = 32;
XX_rank_avg_ = zeros(n_CCOV,n_I_GENE);
XX_rank_std_ = zeros(n_CCOV,n_I_GENE);
...
%%%%%%%%;
n_rank = 28;
beta_un_ = zeros(n_I_GENE,n_rank);
...
% We will use the first 10 terms (so that sorting is not an issue). ;
%%%%%%%%;
n_beta = 10;
...
subplot(2,2,2); imagesc((I_rank_*beta_un_)*transpose(beta_vn_)); title('In*betan');
subplot(2,2,3); imagesc(transpose(I_rank_)*C_rank_); title('It*Cn');
subplot(2,2,4); imagesc(transpose(I_rank_)*((I_rank_*beta_un_)*transpose(beta_vn_))); title('It*In*betan');
end;%if flag_disp;
...
I_aucX__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
C_aucX__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
D_aucX__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_rank_avg_ = zeros(n_Label_ID,n_I_GENE);
C_rank_avg_ = zeros(n_Label_ID,n_CCOV);
D_rank_avg_ = zeros(n_Label_ID,n_CCOV);
%%%%%%%%;
for nA = 1:n_Label_ID;
...
D_rank_avg_(nA,:) = mean((I_rank_(tmp_Label_A_ij_,:)*beta_un_(:,1:n_beta))*transpose(beta_vn_(:,1:n_beta)),1);
...
D_aucX__(nA,nB,:) = [C_rank_avg_(nA,:) - C_rank_avg_(nB,:)] - [D_rank_avg_(nA,:) - D_rank_avg_(nB,:)];
D_aucX__(nB,nA,:) = -D_aucX__(nA,nB,:);
...
%%%%%%%%;
AB_I_sort_list_ori__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
AB_C_sort_list_ori__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
AB_D_sort_list_ori__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_AB_sort_list_ori__ = zeros(n_Label_ID.^2,n_I_GENE);
C_AB_sort_list_ori__ = zeros(n_Label_ID.^2,n_CCOV);
D_AB_sort_list_ori__ = zeros(n_Label_ID.^2,n_CCOV);
AB_I_sort_list_ij__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
AB_C_sort_list_ij__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
AB_D_sort_list_ij__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_AB_sort_list_ij__ = zeros(n_Label_ID.^2,n_I_GENE);
C_AB_sort_list_ij__ = zeros(n_Label_ID.^2,n_CCOV);
D_AB_sort_list_ij__ = zeros(n_Label_ID.^2,n_CCOV);
n_iteration = 64;
AB_I_sort_list_avg__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
AB_C_sort_list_avg__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
AB_D_sort_list_avg__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_AB_sort_list_avg__ = zeros(n_Label_ID.^2,n_I_GENE);
C_AB_sort_list_avg__ = zeros(n_Label_ID.^2,n_CCOV);
D_AB_sort_list_avg__ = zeros(n_Label_ID.^2,n_CCOV);
AB_I_sort_list_std__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
AB_C_sort_list_std__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
AB_D_sort_list_std__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_AB_sort_list_std__ = zeros(n_Label_ID.^2,n_I_GENE);
C_AB_sort_list_std__ = zeros(n_Label_ID.^2,n_CCOV);
D_AB_sort_list_std__ = zeros(n_Label_ID.^2,n_CCOV);
%%%%%%%%;
AB_I_sort_list_p__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
AB_C_sort_list_p__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
AB_D_sort_list_p__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
I_AB_sort_list_p__ = zeros(n_Label_ID.^2,n_I_GENE);
C_AB_sort_list_p__ = zeros(n_Label_ID.^2,n_CCOV);
D_AB_sort_list_p__ = zeros(n_Label_ID.^2,n_CCOV);
%%%%%%%%;
I_Label_ = C_.Cluster_ID_20161007(I_to_C_ID_);
Label_ID_ = unique(I_Label_); n_Label_ID = length(Label_ID_);
rng(1);
...
tmp_I_aucX__ = zeros(n_Label_ID,n_Label_ID,n_I_GENE);
tmp_C_aucX__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
tmp_D_aucX__ = zeros(n_Label_ID,n_Label_ID,n_CCOV);
tmp_I_rank_avg_ = zeros(n_Label_ID,n_I_GENE);
tmp_C_rank_avg_ = zeros(n_Label_ID,n_CCOV);
tmp_D_rank_avg_ = zeros(n_Label_ID,n_CCOV);
%%%%%%%%;
for nA = 1:n_Label_ID;
...
tmp_D_rank_avg_(nA,:) = mean((I_rank_(tmp_Label_A_ij_,:)*beta_un_(:,1:n_beta))*transpose(beta_vn_(:,1:n_beta)),1);
end;%for nA = 1:n_Label_ID;
%%%%%%%%;
for nA = 1:n_Label_ID;
for nB = nA+1:n_Label_ID;
...
tmp_D_aucX__(nA,nB,:) = [tmp_C_rank_avg_(nA,:) - tmp_C_rank_avg_(nB,:)] - [tmp_D_rank_avg_(nA,:) - tmp_D_rank_avg_(nB,:)];
tmp_D_aucX__(nB,nA,:) = -tmp_D_aucX__(nA,nB,:);
end;%for nB = nA+1:n_Label_ID;
end;%for nA = 1:n_Label_ID;
%%%%%%%%;
...
[tmp_AB_D_sort_list__,tmp_AB_D_sort_ij__] = sort(tmp_D_aucX__,3,'ascend');
[tmp_I_AB_sort_list__,tmp_I_AB_sort_ij__] = sort(reshape(tmp_I_aucX__,n_Label_ID.^2,n_I_GENE),2,'ascend');
[tmp_C_AB_sort_list__,tmp_C_AB_sort_ij__] = sort(reshape(tmp_C_aucX__,n_Label_ID.^2,n_CCOV),2,'ascend');
[tmp_D_AB_sort_list__,tmp_D_AB_sort_ij__] = sort(reshape(tmp_D_aucX__,n_Label_ID.^2,n_CCOV),2,'ascend');
%%%%%%%%;
if (niteration==0); 
AB_I_sort_list_ori__ = tmp_AB_I_sort_list__;
AB_C_sort_list_ori__ = tmp_AB_C_sort_list__;
AB_D_sort_list_ori__ = tmp_AB_D_sort_list__;
I_AB_sort_list_ori__ = tmp_I_AB_sort_list__;
C_AB_sort_list_ori__ = tmp_C_AB_sort_list__;
D_AB_sort_list_ori__ = tmp_D_AB_sort_list__;
AB_I_sort_list_ij__ = tmp_AB_I_sort_ij__;
AB_C_sort_list_ij__ = tmp_AB_C_sort_ij__;
AB_D_sort_list_ij__ = tmp_AB_D_sort_ij__;
I_AB_sort_list_ij__ = tmp_I_AB_sort_ij__;
C_AB_sort_list_ij__ = tmp_C_AB_sort_ij__;
D_AB_sort_list_ij__ = tmp_D_AB_sort_ij__;
end; %if (niteration==0);
if (niteration>0);
...
AB_D_sort_list_avg__ = AB_D_sort_list_avg__ + tmp_AB_D_sort_list__;
...
D_AB_sort_list_avg__ = D_AB_sort_list_avg__ + tmp_D_AB_sort_list__;
...
AB_D_sort_list_std__ = AB_D_sort_list_std__ + tmp_AB_D_sort_list__.^2;
...
D_AB_sort_list_std__ = D_AB_sort_list_std__ + tmp_D_AB_sort_list__.^2;
end;%if (niteration>0);
%%%%%%%%;
clear tmp_I_aucX__ tmp_C_aucX__ tmp_D_aucX__ tmp_I_rank_avg_ tmp_C_rank_avg_ tmp_D_rank_avg_;
clear tmp_Label_A_ij_ tmp_n_A tmp_I_rank_avg_ tmp_C_rank_avg_ tmp_D_rank_avg_ ;
clear tmp_I_aucX__ tmp_C_aucX__ tmp_D_aucX__ ;
clear tmp_AB_I_sort_list__ tmp_AB_C_sort_list__ tmp_AB_D_sort_list__ tmp_I_AB_sort_list__ tmp_C_AB_sort_list__ tmp_D_AB_sort_list__ ;
clear tmp_AB_I_sort_ij__ tmp_AB_C_sort_ij__ tmp_AB_D_sort_ij__ tmp_I_AB_sort_ij__ tmp_C_AB_sort_ij__ tmp_D_AB_sort_ij__ ;
...
AB_D_sort_list_avg__ = AB_D_sort_list_avg__/n_iteration;
I_AB_sort_list_avg__ = I_AB_sort_list_avg__/n_iteration;
C_AB_sort_list_avg__ = C_AB_sort_list_avg__/n_iteration;
D_AB_sort_list_avg__ = D_AB_sort_list_avg__/n_iteration;
...
AB_D_sort_list_std__ = sqrt(AB_D_sort_list_std__/n_iteration - AB_D_sort_list_avg__.^2);
...
D_AB_sort_list_std__ = sqrt(D_AB_sort_list_std__/n_iteration - D_AB_sort_list_avg__.^2);
%%%%%%%%;
...
AB_D_sort_list_p__ = erfc( abs(AB_D_sort_list_ori__ - AB_D_sort_list_avg__)./AB_D_sort_list_std__ / sqrt(2) );
...
D_AB_sort_list_p__ = erfc( abs(D_AB_sort_list_ori__ - D_AB_sort_list_avg__)./D_AB_sort_list_std__ / sqrt(2) );

%%%%%%%%;
% Now run through the cluster pairs, ;
% accumulating the covariates which contribute most to the differences. ;
%%%%%%%%;
AB_I_absZ_sum__ = zeros(n_I_GENE,1);
AB_C_absZ_sum__ = zeros(n_CCOV,1);
AB_D_absZ_sum__ = zeros(n_CCOV,1);
for nA = 1:n_Label_ID;
for nB = nA+1:n_Label_ID;
tmp_ij_ = squeeze(AB_I_sort_list_ij__(nA,nB,:)); tmp_z_ = squeeze( abs(AB_I_sort_list_ori__(nA,nB,:) - AB_I_sort_list_avg__(nA,nB,:))./AB_I_sort_list_std__(nA,nB,:) / sqrt(2) ); AB_I_absZ_sum__(tmp_ij_) = AB_I_absZ_sum__(tmp_ij_) + tmp_z_;
tmp_ij_ = squeeze(AB_C_sort_list_ij__(nA,nB,:)); tmp_z_ = squeeze( abs(AB_C_sort_list_ori__(nA,nB,:) - AB_C_sort_list_avg__(nA,nB,:))./AB_C_sort_list_std__(nA,nB,:) / sqrt(2) ); AB_C_absZ_sum__(tmp_ij_) = AB_C_absZ_sum__(tmp_ij_) + tmp_z_;
tmp_ij_ = squeeze(AB_D_sort_list_ij__(nA,nB,:)); tmp_z_ = squeeze( abs(AB_D_sort_list_ori__(nA,nB,:) - AB_D_sort_list_avg__(nA,nB,:))./AB_D_sort_list_std__(nA,nB,:) / sqrt(2) ); AB_D_absZ_sum__(tmp_ij_) = AB_D_absZ_sum__(tmp_ij_) + tmp_z_;
end;%for nB = nA+1:n_Label_ID;
end;%for nA = 1:n_Label_ID;
...
figure(1);clf;
subplot(2,1,1);
[~,tmp_ij_] = sort(AB_C_absZ_sum__,'ascend');
plot(1:n_CCOV,sort(AB_C_absZ_sum__,'ascend'),'ko');
set(gca,'XTick',1:n_CCOV,'XTickLabel',C_VariableName_(tmp_ij_),'TickLabelInterpreter','none'); xtickangle(90);
xlim([1,n_CCOV]); ylabel('accumulated abs(z-score)'); title('Covariates sorted by raw relevance, accumulated across all label-pairs');
set(gca,'FontSize',7);
subplot(2,1,2);
[~,tmp_ij_] = sort(AB_D_absZ_sum__,'ascend');
plot(1:n_CCOV,sort(AB_D_absZ_sum__,'ascend'),'ko');
set(gca,'XTick',1:n_CCOV,'XTickLabel',C_VariableName_(tmp_ij_),'TickLabelInterpreter','none'); xtickangle(90);
...
set(gca,'FontSize',7);
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/dir_jpg/AB_X_absZ_sum',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
end;%if flag_disp;
...
%%%%%%%%;
clf; hold on;
plot(squeeze(AB_D_sort_list_avg__(nA,nB,:)+2.0*AB_D_sort_list_std__(nA,nB,:)),'k','LineWidth',0.5);
plot(squeeze(AB_D_sort_list_avg__(nA,nB,:)+0.0*AB_D_sort_list_std__(nA,nB,:)),'k','LineWidth',1.0);
plot(squeeze(AB_D_sort_list_avg__(nA,nB,:)-2.0*AB_D_sort_list_std__(nA,nB,:)),'k','LineWidth',0.5);
plot(squeeze(AB_D_sort_list_ori__(nA,nB,:)),'r','LineWidth',2.0);
xlim([1,n_CCOV]); xlabel('covariate'); ylabel('rank difference (auc)'); title(sprintf('sorted auc of covariates influencing labels %s-vs-%s',Label_ID_{nA},Label_ID_{nB}));
set(gca,'XTick',1:n_CCOV,'XTickLabel',C_VariableName_(AB_D_sort_list_ij__(nA,nB,:)),'TickLabelInterpreter','none'); xtickangle(90);
set(gca,'FontSize',7);
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/dir_jpg/AB_D_sort_list_ori_%s_vs_%s',dir_trunk,Label_ID_{nA},Label_ID_{nB});
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
hold off;
%%%%%%%%;
clf; hold on;
plot(-log10(squeeze(AB_D_sort_list_p__(nA,nB,:))),'r','LineWidth',2.0);
xlim([1,n_CCOV]); xlabel('covariate'); ylim([0,15]); ylabel('-log10(p)'); title(sprintf('-log10(p-value) for sorted auc of covariates influencing labels %s-vs-%s',Label_ID_{nA},Label_ID_{nB}));
set(gca,'XTick',1:n_CCOV,'XTickLabel',C_VariableName_(AB_D_sort_list_ij__(nA,nB,:)),'TickLabelInterpreter','none'); xtickangle(90);
set(gca,'FontSize',7);
set(gcf,'Position',1+[0,0,1024*2,1024]);
fname_base = sprintf('%s/dir_jpg/AB_D_sort_list_p_%s_vs_%s',dir_trunk,Label_ID_{nA},Label_ID_{nB});
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
hold off;
