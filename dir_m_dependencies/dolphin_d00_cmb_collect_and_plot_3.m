...
dolphin_d00_cmb_collect_and_plot_3( ...
 parameter ...
 ,string_dat_name_ ...
 ,dir_mat ...
 ,dir_shuffle_mat ...
...
verbose=1;
if (verbose); disp(sprintf(' %% [entering dolphin_d00_cmb_collect_and_plot_3]')); end;
...
n_shuffle = n_found_zzzz;
a_avg_zzzz_ = mean(tmp_aaaa_s_.a_cmb_prm__(:,2:end) - tmp_bbbb_s_.a_cmb_prm__(:,2:end),2);
a_std_zzzz_ = std(tmp_aaaa_s_.a_cmb_prm__(:,2:end) - tmp_bbbb_s_.a_cmb_prm__(:,2:end),1,2);
A_avg_zzzz__ = mean(tmp_aaaa_s_.A_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.A_cmb_prm___(:,:,2:end),3);
A_std_zzzz__ = std(tmp_aaaa_s_.A_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.A_cmb_prm___(:,:,2:end),1,3);
BB_inv_avg_zzzz__ = mean(tmp_aaaa_s_.BB_inv_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.BB_inv_cmb_prm___(:,:,2:end),3);
BB_inv_std_zzzz__ = std(tmp_aaaa_s_.BB_inv_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.BB_inv_cmb_prm___(:,:,2:end),1,3);
CC_inv_avg_zzzz__ = mean(tmp_aaaa_s_.CC_inv_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.CC_inv_cmb_prm___(:,:,2:end),3);
CC_inv_std_zzzz__ = std(tmp_aaaa_s_.CC_inv_cmb_prm___(:,:,2:end) - tmp_bbbb_s_.CC_inv_cmb_prm___(:,:,2:end),1,3);
end;%if ( exist(fname_mat_aaaa_s,'file') &  exist(fname_mat_bbbb_s,'file'));
%%%%;
tmp_zzzz_s_ = struct('type','tmp');
tmp_zzzz_s_.a_cmb_prm__ = tmp_aaaa_s_.a_cmb_prm__ - tmp_bbbb_s_.a_cmb_prm__;
tmp_zzzz_s_.A_cmb_prm___ = tmp_aaaa_s_.A_cmb_prm___ - tmp_bbbb_s_.A_cmb_prm___;
tmp_zzzz_s_.BB_inv_cmb_prm___ = tmp_aaaa_s_.BB_inv_cmb_prm___ - tmp_bbbb_s_.BB_inv_cmb_prm___;
tmp_zzzz_s_.CC_inv_cmb_prm___ = tmp_aaaa_s_.CC_inv_cmb_prm___ - tmp_bbbb_s_.CC_inv_cmb_prm___;
tmp_zzzz_s_.L_cmb_prm_ = tmp_aaaa_s_.L_cmb_prm_ - tmp_bbbb_s_.L_cmb_prm_;
%%%%;
a_nlp_zzzz_ = -z_to_p_twosided_0((a_zzzz_o_ - a_avg_zzzz_)./a_std_zzzz_);
A_nlp_zzzz__ = -z_to_p_twosided_0((A_zzzz_o__ - A_avg_zzzz__)./A_std_zzzz__);
BB_inv_nlp_zzzz__ = -z_to_p_twosided_0((BB_inv_zzzz_o__ - BB_inv_avg_zzzz__)./BB_inv_std_zzzz__);
...
%%%%%%%%;
fname_fig = sprintf('%s/dolphin_%s_shuffle_aholm_FIGa',dir_jpg,fname_infix_zzzz);
...
a_zzzz_prm__ = tmp_zzzz_s_.a_cmb_prm__;
...
xlim([0,n_retain]);
xlabel('entries (unsorted)'); ylabel('nlp (unsigned)');
subplot(3,1,2);
plot(1:n_retain,a_snlp_zzzz_,'ro',1:n_retain,a_sbnlp_zzzz_,'go');
xlim([0,n_retain]);
xlabel('entries (unsorted)'); ylabel('nlp (signed)');
subplot(3,1,3);
plot(1:n_retain,sort(a_zzzz_nlp__(:,1),'descend'),'ro',1:n_retain,sort(a_zzzz_nlp__(:,2:end),'descend'),'k.');
xlim([0,n_retain]);
xlabel('entries (sorted)'); ylabel('nlp (unsigned)');
%%%%%%%%;
% verdict: looks like elements of a_ are interdependent: bonferroni correction is an underestimate. ;
%%%%%%%%;
...
n_retain = numel(index_retain_);
tmp_zzzz_s_A_prm__ = reshape(tmp_zzzz_s_.A_cmb_prm___,[n_var^2,n_shuffle+1]);
A_zzzz_prm__ = tmp_zzzz_s_A_prm__(1+index_retain_,:);
A_zzzz_avg_ = mean(A_zzzz_prm__(:,2:end),2); A_zzzz_std_ = std(A_zzzz_prm__(:,2:end),1,2);
A_zzzz_nlp__ = -z_to_p_twosided_0((A_zzzz_prm__ - A_zzzz_avg_)./A_zzzz_std_);
A_zzzz_snlp__ = sign(A_zzzz_prm__ - A_zzzz_avg_).*A_zzzz_nlp__;
A_zzzz_sbnlp__ = sign(A_zzzz_prm__ - A_zzzz_avg_).*max(0,A_zzzz_nlp__-log(n_retain));
figbig;clf;p_row = 2; p_col = 2;
...
semilogx(1:n_retain,sort(A_zzzz_nlp__(:,1),'descend'),'ro',1:n_retain,sort(A_zzzz_nlp__(:,2:end),'descend'),'k.');
xlim([0,n_retain]);
xlabel('entries (sorted)'); ylabel('nlp (unsigned)');
%%%%%%%%;
...
fname_fig = sprintf('%s/dolphin_%s_shuffle_BBholm_FIGB',dir_jpg,fname_infix_zzzz);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig)); figure(1);
figure(1);clf;figbig;fontsize_use = 12;
index_retain_ = efind(ones(n_var,n_var)-tril(ones(n_var,n_var)));
n_retain = numel(index_retain_);
tmp_zzzz_s_BB_inv_prm__ = reshape(tmp_zzzz_s_.BB_inv_cmb_prm___,[n_var^2,n_shuffle+1]);
BB_inv_zzzz_prm__ = tmp_zzzz_s_BB_inv_prm__(1+index_retain_,:);
BB_inv_zzzz_avg_ = mean(BB_inv_zzzz_prm__(:,2:end),2); BB_inv_zzzz_std_ = std(BB_inv_zzzz_prm__(:,2:end),1,2);
BB_inv_zzzz_nlp__ = -z_to_p_twosided_0((BB_inv_zzzz_prm__ - BB_inv_zzzz_avg_)./BB_inv_zzzz_std_);
BB_inv_zzzz_snlp__ = sign(BB_inv_zzzz_prm__ - BB_inv_zzzz_avg_).*BB_inv_zzzz_nlp__;
BB_inv_zzzz_sbnlp__ = sign(BB_inv_zzzz_prm__ - BB_inv_zzzz_avg_).*max(0,BB_inv_zzzz_nlp__-log(n_retain));
figbig;clf;p_row = 2; p_col = 2;
subplot(p_row,p_col,1);
%plot(1:n_retain,BB_inv_zzzz_prm__(:,1),'ro',1:n_retain,BB_inv_zzzz_prm__(:,2:end),'kx');
plot(1:n_retain,BB_inv_zzzz_nlp__(:,1),'ro',1:n_retain,BB_inv_zzzz_nlp__(:,2:end),'kx');
xlim([0,n_retain]);
xlabel('entries (unsorted)'); ylabel('nlp (unsigned)');
subplot(p_row,p_col,2);
plot(1:n_retain,BB_inv_snlp_zzzz__(1+index_retain_),'ro',1:n_retain,BB_inv_sbnlp_zzzz__(1+index_retain_),'go');
xlim([0,n_retain]);
xlabel('entries (unsorted)'); ylabel('nlp (signed)');
subplot(p_row,p_col,3);
plot(1:n_retain,sort(BB_inv_zzzz_nlp__(:,1),'descend'),'ro',1:n_retain,sort(BB_inv_zzzz_nlp__(:,2:end),'descend'),'k.');
xlim([0,n_retain]);
xlabel('entries (sorted)'); ylabel('nlp (unsigned)');
subplot(p_row,p_col,4);
semilogx(1:n_retain,sort(BB_inv_zzzz_nlp__(:,1),'descend'),'ro',1:n_retain,sort(BB_inv_zzzz_nlp__(:,2:end),'descend'),'k.');
xlim([0,n_retain]);
xlabel('entries (sorted)'); ylabel('nlp (unsigned)');
%%%%%%%%;
% verdict: entries of BB_inv__ are quite interdependent: bonferroni correction is an underestimate. ;
%%%%%%%%;
sgtitle(sprintf('%s',fname_fig),'Interpreter','none');
disp(sprintf(' %% writing %s',fname_fig));
print('-depsc',sprintf('%s.eps',fname_fig));
...
if ( exist(fname_mat_zzzz_s,'file'));
n_found_zzzz = size(tmp_zzzz_s_.L_cmb_prm_,1) - 1;
a_zzzz__ = tmp_zzzz_s_.a_cmb_prm__(:,2:end);
A_zzzz__ = reshape(tmp_zzzz_s_.A_cmb_prm___(:,:,2:end),[n_var^2,n_found_zzzz]);
BB_inv_zzzz__ = reshape(tmp_zzzz_s_.BB_inv_cmb_prm___(:,:,2:end),[n_var^2,n_found_zzzz]);
CC_inv_zzzz__ = zeros(n_var,n_found_zzzz);
for nfound=0:n_found_zzzz-1;
CC_inv_zzzz__(:,1+nfound)= diag(tmp_zzzz_s_.CC_inv_cmb_prm___(:,:,1+1+nfound));
...
if (verbose); disp(sprintf(' %% [finished dolphin_d00_cmb_collect_and_plot_3]')); end;
