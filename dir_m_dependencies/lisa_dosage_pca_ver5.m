function lisa_dosage_pca_ver5(flag_stage);
...
save(fname_stage_0,...
     'dir__in_arm2','string_prefix_arm2','n_patient_F_arm2','study_name_arm2_','string_name_r0_arm1','niteration','bim_arm1_','b11_','b01_','b02_','b12_','b01_by_b11_xref_','b02_by_b12_xref_','V_r0_base_and_','V_r0_base_xor_','V_r0_base_nor_','atfr_arm1_','atfr_arm2_','atfr_arm1_and_','atfr_arm1_xor_','atfr_arm1_nor_','atfr_arm2_and_','atfr_arm2_xor_','atfr_arm2_nor_','fam_name_arm1_','fam_name_arm2_','allele1_arm1_','allele2_arm1_','alleletype_arm1_','allele1_arm2_','allele2_arm2_','alleletype_arm2_', 'B_MLT', 'cl_num_arm1', 'cl_num_arm2', 'DnV_r0_arm1_', 'DnV_r0_arm2_', 'flag_dex_vs_lak', 'flag_reverse', 'flag_reverse_use', 'gamma', 'Icat', 'Icat_full', 'Ireq', 'Ireq_half', 'maf_hi_threshold', 'maf_lo_threshold', 'mc_string', 'mds_repl', 'mr_D_arm2_', 'mr_X_arm2_', 'n_cov', 'n_maf', 'n_mds', 'n_scramble', 'n_shuffle', 'pca_b_mlt', 'pca_rank', 'XnV_r0_arm1_', 'XnV_r0_arm2_' );
...
fname_dosage_gz = sprintf('%s/%s_eur_sr-qc.hg19.ch.fl.out.dosage_%s_out_cdrop_ni%d.out.dosage.gz',dir_dosage,study_name_arm2_prefix,string_name_r0_arm1,niteration);
...
fname_dosage = sprintf('%s/%s_eur_sr-qc.hg19.ch.fl.out.dosage_%s_out_cdrop_ni%d.out.dosage',dir_dosage_unzip,study_name_arm2_prefix,string_name_r0_arm1,niteration);
...
if (mod(ne11,1000)==0); disp(sprintf(' %% ne11 %d/%d',ne11,length(e11_))); end;
tmp_d1d_ = find(sum(d1d_by_e11_xref_(:,ne11),2));
if (length(tmp_d1d_)>1); disp(sprintf(' %% Warning! tmp_d1d_ length %d in lisa_dosage_pca_ver5',length(tmp_d1d_))); end;
nd1d = tmp_d1d_(1);
tmp_b11_ = find(sum(b11_by_e11_xref_(:,ne11),2));
if (length(tmp_b11_)>1); disp(sprintf(' %% Warning! tmp_b11_ length %d in lisa_dosage_pca_ver5',length(tmp_b11_))); end;
nb11 = tmp_b11_(1);
...
if (mod(ne12,1000)==0); disp(sprintf(' %% ne12 %d/%d',ne12,length(e12_))); end;
tmp_d1d_ = find(sum(d1d_by_e12_xref_(:,ne12),2));
if (length(tmp_d1d_)>1); disp(sprintf(' %% Warning! tmp_d1d_ length %d in lisa_dosage_pca_ver5',length(tmp_d1d_))); end;
nd1d = tmp_d1d_(1);
tmp_b12_ = find(sum(b12_by_e12_xref_(:,ne12),2));
if (length(tmp_b12_)>1); disp(sprintf(' %% Warning! tmp_b12_ length %d in lisa_dosage_pca_ver5',length(tmp_b12_))); end;
nb12 = tmp_b12_(1);
...
if (length(tmp_f11_)>1); disp(sprintf(' %% Warning! tmp_f11_ length %d in lisa_dosage_pca_ver5',length(tmp_f11_))); end;
nf11 = tmp_f11_(1);
tmp_g1d_ = find(sum(g1d_by_h11_xref_(:,nh11),2));
if (length(tmp_g1d_)>1); disp(sprintf(' %% Warning! tmp_g1d_ length %d in lisa_dosage_pca_ver5',length(tmp_g1d_))); end;
ng1d = tmp_g1d_(1);
...
if (length(tmp_f12_)>1); disp(sprintf(' %% Warning! tmp_f12_ length %d in lisa_dosage_pca_ver5',length(tmp_f12_))); end;
nf12 = tmp_f12_(1);
...
%%%%%%%%;
tmp_b02_and = find(strcmp(b02_,d1d_{nd1d}) & strcmp(alleletype_arm2_,'and')); if (length(tmp_b02_and)>1); disp(sprintf(' %% Warning! tmp_b02_and length %d in lisa_dosage_pca_ver5.m',length(tmp_b02_and))); end;
...
for nh12=1:length(h12_);
tmp_f12_ = find(sum(f12_by_h12_xref_(:,nh12),2)); if (length(tmp_f12_)>1); disp(sprintf(' %% Warning! tmp_f12_ length %d in lisa_dosage_pca_ver5',length(tmp_f12_))); end; nf12 = tmp_f12_(1);
tmp_g1d_ = find(sum(g1d_by_h12_xref_(:,nh12),2)); if (length(tmp_g1d_)>1); disp(sprintf(' %% Warning! tmp_g1d_ length %d in lisa_dosage_pca_ver5',length(tmp_g1d_))); end; ng1d = tmp_g1d_(1);
assert(strcmp(f12_{nf12},g1d_{ng1d}));
...
%%%%%%%%;
tmp_b02_xor = find(strcmp(b02_,d1d_{nd1d}) & strcmp(alleletype_arm2_,'xor')); if (length(tmp_b02_xor)>1); disp(sprintf(' %% Warning! tmp_b02_xor length %d in lisa_dosage_pca_ver5.m',length(tmp_b02_xor))); end;
...
%%%%%%%%;
tmp_b02_nor = find(strcmp(b02_,d1d_{nd1d}) & strcmp(alleletype_arm2_,'nor')); if (length(tmp_b02_nor)>1); disp(sprintf(' %% Warning! tmp_b02_nor length %d in lisa_dosage_pca_ver5.m',length(tmp_b02_nor))); end;
...
fname_stage_1 = sprintf('%s/dir_misc/lisa_dosage_pca_stage_1.mat',dir_trunk); 
save(fname_stage_1,...
