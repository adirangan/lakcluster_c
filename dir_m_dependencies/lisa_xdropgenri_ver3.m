...
tmp2_ = cap_EZid_EZ_put_(tmp0_(tmp1_)); %<-- now tmp2_ allows us to reference the EZid within pathway_EZid_uni_ that are also within EZ_EZid_uni. ; That is, EZ_EZid_uni_(tmp2_) will list the actual EZids in this intersection. ;
tmp3_ = EZ_EZid_uni_(tmp2_); %<-- now tmp3_ stores only the EZid within EZ_EZid_uni_ and pathway_EZid_uni_. ;
tmp4_c01_ = (sum(EZ4_01__(:,tmp2_),2));%tmp4_c01_ = find(sum(EZ4_01__(:,tmp2_),2)); %<-- now tmp4_ indexes the first entry from cdrop_a_ associated with each of the genes in tmp3_(nl2). ;
tmp4_c25_ = (sum(EZ4_25__(:,tmp2_),2));%tmp4_c25_ = find(sum(EZ4_25__(:,tmp2_),2));
tmp4_c50_ = (sum(EZ4_50__(:,tmp2_),2));%tmp4_c50_ = find(sum(EZ4_50__(:,tmp2_),2));
tmp4_c75_ = (sum(EZ4_75__(:,tmp2_),2));%tmp4_c75_ = find(sum(EZ4_75__(:,tmp2_),2));
tmp4_c99_ = (sum(EZ4_99__(:,tmp2_),2));%tmp4_c99_ = find(sum(EZ4_99__(:,tmp2_),2)); %<-- now tmp4_ indexes the last entry from cdrop_a_ associated with each of the genes in tmp3_(nl2). ;
...
npathway = 1+floor(n_pathway*rand());
disp(sprintf(' %% npathway %d name %s size %d',npathway,pathway_name_{npathway},pathway_size_(npathway)));
tmp_rs_01_ = unique(bim_{2}(cdrop_a_(find(pathway_EZid_01_cdrop_a_get__{npathway}))));
tmp_rs_25_ = unique(bim_{2}(cdrop_a_(find(pathway_EZid_25_cdrop_a_get__{npathway}))));
tmp_rs_50_ = unique(bim_{2}(cdrop_a_(find(pathway_EZid_50_cdrop_a_get__{npathway}))));
tmp_rs_75_ = unique(bim_{2}(cdrop_a_(find(pathway_EZid_75_cdrop_a_get__{npathway}))));
tmp_rs_99_ = unique(bim_{2}(cdrop_a_(find(pathway_EZid_99_cdrop_a_get__{npathway}))));
...
%%%%%%%%;
pathway_EZid_01_cdrop_a_mc_ = sum(EZ4_01__,2);
pathway_EZid_25_cdrop_a_mc_ = sum(EZ4_25__,2);
pathway_EZid_50_cdrop_a_mc_ = sum(EZ4_50__,2);
pathway_EZid_75_cdrop_a_mc_ = sum(EZ4_75__,2);
pathway_EZid_99_cdrop_a_mc_ = sum(EZ4_99__,2);
...
%%%%%%%%;
flag_plot=0;
log_pval_up_99__ = cell(n_pathway,1); % upper integral. ;
log_pval_dn_99__ = cell(n_pathway,1); % lower integral. ;
...
tmp_N = sum(pathway_EZid_99_cdrop_a_mc_); 
...
tmp_K = pathway_EZid_99_size;
tmp_M = tmp_N-tmp_K; 
tmp_n_ = cumsum(pathway_EZid_99_cdrop_a_mc_,'reverse');
...
%%%%%%%%%%%%%%%%;
disp(sprintf(' %% %% %% npathway %d name %s original size %d snp_cdrop size %d',npathway,pathway_name_{npathway},length(pathway_EZid__{npathway}),full(sum(pathway_EZid_99_cdrop_a_get__{npathway}))));
tmp_k_mc_ = pathway_EZid_99_cdrop_a_get__{npathway};
tmp_k_ = cumsum(tmp_k_mc_,'reverse'); assert(min(tmp_k_)>=0 & max(tmp_k_)<=tmp_K);
tmp_m_ = tmp_n_ - tmp_k_; assert(min(tmp_m_)>=0 & max(tmp_m_)<=tmp_M);
tmp_pk_ = tmp_k_/tmp_K; tmp_qk_ = 1 - tmp_pk_;
tmp_pm_ = tmp_m_/tmp_M; tmp_qm_ = 1 - tmp_pm_; 
%%%%%%%%;
