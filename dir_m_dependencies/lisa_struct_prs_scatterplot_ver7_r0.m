function lisa = lisa_struct_prs_scatterplot_ver7_r0(specification);
...
%{
  ...
  for prs_suffix = [0];%for prs_suffix = [0,150,163,175,188,200,213,225,238,250,263,275,288,300,313,325];
  if prs_suffix==0; prs_prefix = sprintf('ori'); ni_r0_arm2_ = [150,163,175,188,200,213,225,238,250]; ni_r1_arm2 = [];
  else; prs_prefix = sprintf('tree_plt4'); ni_r0_arm2_ = [150,163,175,188,200,213,225,238,250]; ni_r1_arm2 = []; end;
  %else; prs_prefix = sprintf('tree_plt4_bicl1_ni%d',prs_suffix); ni_r0_arm2_ = prs_suffix; ni_r1_arm2 = []; end;
  for nl=0:numel(ni_r0_arm2_)-1;
  ni_r0_arm2 = ni_r0_arm2_(1+nl);
  ...
  lisa_struct_prs_scatterplot_ver7_r0(specification) ;
  end;%for nl=0:numel(ni_r0_arm2_)-1;
  end;%for prs_suffix = [0];
...
if (~exist(lisa_arm1_r0.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver7_r0',lisa_arm1_r0.dir_out_s0_pca)); end;
...
if (~exist(lisa_arm1_r1.dir_out_s0_pca,'dir')); disp(sprintf(' %% Warning! %s does not exist in lisa_struct_prs_scatterplot_ver7_r0',lisa_arm1_r1.dir_out_s0_pca)); end;
...
fname_mat = sprintf('%s/prs_scatterplot_ver7_r0_%s_trn%d%s_tst%d%s_ni%.3d.mat',lisa_arm1_r0.dir_out_s0_prs_mat,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2);
if ( flag_rerun==0 &  exist(fname_mat,'file')); 
disp(sprintf(' %% found %s, loading',fname_mat));
load(fname_mat,...
     'auc19_prs_t_',...
...
if ( 1 | flag_rerun==1 | ~exist(fname_mat,'file')); 
disp(sprintf(' %% could not find %s, creating',fname_mat));
%%%%%%%%;
n_omega = 1; omega_ = 0;
...
tmp_Xy_ = zeros(size(tmp_Xx_)); tmp_Dy_ = zeros(size(tmp_Dx_)); 
%%%%%%%%;
...
fname_fig = sprintf('%s/prs_tbthreshold_ver2_r0_%s_trn%d%s_tst%d%s_ni%.3d.jpg',lisa_arm1_r0.dir_out_s0_prs_jpg,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2);
...
hold off;
xlim([0,n_rx_t]); ylim(cauclim_); legend('bc1','Location','NorthWest');
...
fname_fig = sprintf('%s/prs_tbthreshold_ver2a_r0_%s_trn%d%s_tst%d%s_ni%.3d.jpg',lisa_arm1_r0.dir_out_s0_prs_jpg,prs_prefix,cl_num_arm1,cl_num_arm1_string,cl_num_arm2,cl_num_arm2_string,ni_r0_arm2);
if (~flag_replot & exist(fname_fig,'file')); disp(sprintf(' %% %s found, not creating',fname_fig)); end;
if ( flag_replot | ~exist(fname_fig,'file')); disp(sprintf(' %% %s not found, creating',fname_fig)); 
flag_disp=1;
if flag_disp;
...
hold off;
xlim([0,n_rx_t]); ylim(cauclim_); legend('bc1','p01','p05','p15','Location','NorthWest');
tmp_tick_ = round(linspace(1,1+n_rx_t,n_tick)); %tmp_tick_ = tmp_tick_(end:-1:1);
tmp_tick_val_ = tmp_tick_; 
tmp_tick_label_ = num2str(transpose(prctile_rem_(tmp_tick_)),'%d');
set(gca,'XTick',tmp_tick_val_,'XTickLabel',tmp_tick_label_);
