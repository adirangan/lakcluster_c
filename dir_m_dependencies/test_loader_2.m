...
imagesc(transpose(XX_rank_(XX_rank_U_ij_,XX_rank_V_ij_)),[-1,+1]); 
...
set(gcf,'Position',1+[0,0,1024*2,1024]);
colorbar;
fname_base = sprintf('%s/dir_jpg/gene_covariate_correlation',dir_trunk);
disp(sprintf(' %% writing %s.jpg',fname_base));
print('-djpeg',sprintf('%s.jpg',fname_base));
print('-depsc',sprintf('%s.eps',fname_base));
end;%if flag_disp;
...
XX_rank_Z_ = ( XX_rank_(XX_rank_U_ij_,XX_rank_V_ij_) - XX_rank_avg_ ) ./ XX_rank_std_ ;
...
set(gca,'XTick',1:n_COV,'XTickLabel',C_VariableName_(XX_rank_U_ij_),'TickLabelInterpreter','none'); xtickangle(90);
set(gca,'FontSize',7);
xlabel('covariates'); ylabel('genes'); title('log10(pvalue) of Correlation between Genes and Covariates');
set(gcf,'Position',1+[0,0,1024*2,1024]);
colorbar;
...
% Again, we can compare this to the distribution of sorted-lists obtained ina  (Label-Label) permutation-test. ;
%%%%%%%%;
n_iteration = 4;
AB_I_sort_list__ = zeros(n_Label_ID,n_Label_ID,n_GENE,1+n_iteration);
AB_C_sort_list__ = zeros(n_Label_ID,n_Label_ID,n_COV,1+n_iteration);
I_AB_sort_list__ = zeros(n_Label_ID.^2,n_GENE,1+n_iteration);
C_AB_sort_list__ = zeros(n_Label_ID.^2,n_COV,1+n_iteration);
I_Label_ = C_.Cluster_ID_20161007(I_to_C_ID_);
Label_ID_ = unique(I_Label_); n_Label_ID = length(Label_ID_);
rng(1);
...
%for nA = 1:n_Label_ID;
%for nB = nA+1:n_Label_ID;
%AB_I_sort_list__(nA,nB,:,1+niteration) = sort(tmp_I_aucX__(nA,nB,:),'ascend');
%AB_C_sort_list__(nA,nB,:,1+niteration) = sort(tmp_C_aucX__(nA,nB,:),'ascend');
%end;%for nB = nA+1:n_Label_ID;
%end;%for nA = 1:n_Label_ID;
AB_I_sort_list__(:,:,:,1+niteration) = sort(tmp_I_aucX__,3,'ascend');
AB_C_sort_list__(:,:,:,1+niteration) = sort(tmp_C_aucX__,3,'ascend');
%%%%%%%%;
%for ng=1:n_GENE;
%I_AB_sort_list__(:,n_Label_ID.^2,1+niteration) = sort(reshape(tmp_I_aucX__(:,:,ng),n_Label_ID.^2,1),'ascend');
%end;%for ng=1:n_GENE;
I_AB_sort_list__(:,:,1+niteration) = sort(reshape(tmp_I_aucX__,n_Label_ID.^2,n_GENE),3,'ascend');
%for nc=1:n_COV;
%C_AB_sort_list__(:,n_Label_ID.^2,1+niteration) = sort(reshape(tmp_C_aucX__(:,:,nc),n_Label_ID.^2,1),'ascend');
%end;%for nc=1:n_COV;
C_AB_sort_list__(:,:,1+niteration) = sort(reshape(tmp_C_aucX__,n_Label_ID.^2,n_COV),3,'ascend');
