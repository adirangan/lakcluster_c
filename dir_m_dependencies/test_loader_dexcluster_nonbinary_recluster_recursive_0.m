function test_loader_dexcluster_nonbinary_recluster_recursive_0(dir_out,prefix_base,E_array_base_,E_array_r_ij_,E_array_c_ij_,gamma,n_shuffle,p_set);
%{
  ...
  test_loader_dexcluster_nonbinary_recluster_recursive_0([],prefix_base,E_xxxx_,[],[],gamma,n_shuffle,[]);
  ...
  test_loader_dexcluster_nonbinary_recluster_recursive_0([],prefix_base,E_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear E_xxxx_cxxx_ ;
  ...
  test_loader_dexcluster_nonbinary_recluster_recursive_0([],prefix_base,I_xxxx_,[],[],gamma,n_shuffle,[]);
  end;%if ( strcmp(str_ctype,'000'));
  if (~strcmp(str_ctype,'000'));
  C_rank_cxxx_ = textread(sprintf('%s/dir_mat/C_rank_c%s_.tsv',dir_trunk,str_ctype)); C_rank_cxxx_ = C_rank_cxxx_(:,1:end-1);
  ...
  test_loader_dexcluster_nonbinary_recluster_recursive_0([],prefix_base,I_xxxx_cxxx_,[],[],gamma,n_shuffle,[]);
  %clear I_xxxx_cxxx_ ;
  end;%for ncol_=1:length(n_col_);
  end;%if (~strcmp(str_ctype,'000'));
  %clear I_xxxx_ ;
...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('dexnb_recluster',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering test_loader_dexcluster_nonbinary_recluster_recursive_0]: dir_out %s',dir_out)); end;
...
%%%%%%%%;
nZR_ij = ceil(n_ZR_ij/2); %<-- pick the median iteration for now. ;
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
ZR_ij = ZR_ij_(nZR_ij);
...
tmp_r_rem_cut = r_rem_(ZR_ij); tmp_c_rem_cut = c_rem_(ZR_ij); n_rank = 5;
...
label_orig_ = ones(n_u,1);
label_orig_(rkeep_E_0_(1:tmp_r_rem_cut)) = 2;
%scatter(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),25,label_orig_);
[out_xdrop_F_s0000_,out_trace_F_s0000_] = dexcluster_nonbinary_AAAA_rdrop_ver0(tmp_E1_pos_,gamma);
n_shuffle_F = 32;
out_xdrop_F_ = cell(1+n_shuffle_F,1); out_trace_F_ = cell(1+n_shuffle_F,1);
out_xdrop_F_{1+0} = out_xdrop_F_s0000_; out_trace_F_{1+0} = out_trace_F_s0000_;
for nshuffle_F = 1:n_shuffle_F;
if (mod(nshuffle_F,10)==0); disp(sprintf(' %% nshuffle_F %d/%d',nshuffle_F,n_shuffle_F)); end;
rng(nshuffle_F); [tmp_Q_,~] = qr(randn(n_u));
[out_xdrop_F_{1+nshuffle_F},out_trace_F_{1+nshuffle_F}] = dexcluster_nonbinary_AAAA_rdrop_ver0(mean_center_0(tmp_Q_*tmp_E1_pos_),gamma);
end;%for nshuffle_F = 1:n_shuffle_F;
clear tmp_Q_;
%%%%%%%%;
QR_F_ = zeros(size(out_trace_F_s0000_,1),1+n_shuffle_F);
QC_F_ = zeros(size(out_trace_F_s0000_,1),1+n_shuffle_F);
for nshuffle_F=0:n_shuffle_F;
tmp_d = min(length(out_trace_F_s0000_(:,1)),length(out_trace_F_{1+nshuffle_F}(:,1)));
QR_F_(1:tmp_d,1+nshuffle_F) = out_trace_F_{1+nshuffle_F}(1:tmp_d,4);
QC_F_(1:tmp_d,1+nshuffle_F) = out_trace_F_{1+nshuffle_F}(1:tmp_d,5);
end;%for nshuffle_F=1:n_shuffle_F;
n_iter_F_ = out_trace_F_s0000_(:,1); n_iteration_F = n_iter_F_(end);
r_rem_F_ = out_trace_F_s0000_(:,2);
c_rem_F_ = out_trace_F_s0000_(:,3);
%%%%%%%%;
QR_F_avg_ = mean(QR_F_(:,2:end),2); QR_F_std_ = std(QR_F_(:,2:end),[],2);
ZR_F_ = (QR_F_ - repmat(QR_F_avg_,1,1+n_shuffle_F))./repmat(QR_F_std_,1,1+n_shuffle_F);
QC_F_avg_ = mean(QC_F_(:,2:end),2); QC_F_std_ = std(QC_F_(:,2:end),[],2);
ZC_F = (QC_F_ - repmat(QC_F_avg_,1,1+n_shuffle_F))./repmat(QC_F_std_,1,1+n_shuffle_F);
%%%%%%%%;
lp_R_F_ = z_to_p_0(ZR_F_);
lp_C_F_ = z_to_p_0(ZC_F);
%%%%%%%%;
subplot(1,2,1);
hold on;
plot(QR_F_(:,2:end),'k-','LineWidth',1);
plot(QR_F_(:,1),'r-','LineWidth',2);
hold off;
xlim([0,n_iteration_F]);
xlabel('iteration'); ylabel('QR_F');
subplot(1,2,2);
hold on;
plot(ZR_F_(:,2:end),'k-','LineWidth',1);
plot(ZR_F_(:,1),'r-','LineWidth',2);
hold off;
xlim([0,n_iteration_F]);
xlabel('iteration'); ylabel('ZR_F');
figbig;
%%%%%%%%;
rdrop_F_0_ = 1+out_xdrop_F_s0000_(find(out_xdrop_F_s0000_(:,1)>-1),1); rkeep_F_0_ = rdrop_F_0_(end:-1:1);
cdrop_F_0_ = 1+out_xdrop_F_s0000_(find(out_xdrop_F_s0000_(:,2)>-1),2); ckeep_F_0_ = cdrop_F_0_(end:-1:1);
[~,ZR_F_ij] = max(ZR_F_(:,1));
tmp_r_rem_F_cut = r_rem_F_(ZR_F_ij); tmp_c_rem_F_cut = c_rem_F_(ZR_F_ij);
label_F_ = ones(n_u,1);
label_F_(rkeep_F_0_(1:tmp_r_rem_F_cut)) = 2;
%%%%%%%%;

clf;
colormap(colormap_beach());
subplot(1,2,1);
scatter(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),25,label_orig_); axis equal;
xlabel('pc1 orig');ylabel('pc2 orig'); title(sprintf('ZR_ij %d',ZR_ij),'Interpreter','none');
subplot(1,2,2);
scatter(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),25,label_F_); axis equal;
xlabel('pc1 F');ylabel('pc2 F'); title(sprintf('ZR_F_ij %d',ZR_F_ij),'Interpreter','none');
figbig;

clf;
colormap(colormap_beach());
subplot(2,2,1);
scatter3(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),tmp_E1_pos_(:,3),25,label_orig_); axis equal;
xlabel('pc1 orig');ylabel('pc2 orig'); title(sprintf('ZR_ij %d',ZR_ij),'Interpreter','none');
subplot(2,2,2);
scatter3(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),tmp_E1_pos_(:,3),25,label_F_); axis equal;
xlabel('pc1 F');ylabel('pc2 F'); title(sprintf('ZR_F_ij %d',ZR_F_ij),'Interpreter','none');
subplot(2,2,3);
opts_isosplit5 = struct('K_init',200,'isocut_threshold',1.0);
label_isosplit5_ = transpose(isosplit5(transpose(tmp_E1_pos_),opts_isosplit5));
scatter3(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),tmp_E1_pos_(:,3),25,label_isosplit5_); axis equal;
xlabel('pc1 F');ylabel('pc2 F'); title(sprintf('isosplit5 1.0'),'Interpreter','none');
subplot(2,2,4);
opts_isosplit5 = struct('K_init',200,'isocut_threshold',1.5);
label_isosplit5_ = transpose(isosplit5(transpose(tmp_E1_pos_),opts_isosplit5));
scatter3(tmp_E1_pos_(:,1),tmp_E1_pos_(:,2),tmp_E1_pos_(:,3),25,label_isosplit5_); axis equal;
xlabel('pc1 F');ylabel('pc2 F'); title(sprintf('isosplit5 1.5'),'Interpreter','none');
figbig;
...
E1_gplit_label_ = cell(n_ZR_ij,1);
E1_gplit_llr__ = cell(n_ZR_ij,1);
...
[tmp_U_,~,tmp_V_] = svds( L_array_(rkeep_E_0_(1:tmp_r_rem_cut),ckeep_E_0_(1:tmp_c_rem_cut)) ,n_rank);
tmp_E1_pos_ = L_array_(:,ckeep_E_0_(1:tmp_c_rem_cut))*tmp_V_; %<-- original. ;
[E1_gamma_z_(nZR_ij),E1_gsplit_llr_(nZR_ij),E1_gsplit_label_{nZR_ij},~,E1_gsplit_llr__{nZR_ij}] = gsplit_gamma_z(tmp_E1_pos_,n_gamma_z);
...
if (verbose>1); disp(sprintf(' %% max E1_gsplit_llr__: nZR_ij %d/%d ZR_ij %d/%d lp_R_ %0.2f = log(%0.6f)',nZR_ij,n_ZR_ij,ZR_ij,n_iteration,lp_R_(ZR_ij),exp(lp_R_(ZR_ij)))); end;
tmp_r_rem_cut = r_rem_(ZR_ij); tmp_c_rem_cut = c_rem_(ZR_ij); n_rank = 2;
L_array_ = mean_center_0(E_array_base_(E_array_r_ij_,E_array_c_ij_));
[tmp_U_,~,tmp_V_] = svds( L_array_(rkeep_E_0_(1:tmp_r_rem_cut),ckeep_E_0_(1:tmp_c_rem_cut)) ,n_rank);
tmp_E1_pos_ = L_array_(:,ckeep_E_0_(1:tmp_c_rem_cut))*tmp_V_; %<-- original. ;
...
tmp_gamma_z = E1_gamma_z_(nZR_ij);
...
save(str_branch,'trace_','QR_','QC_','ZR_','ZC_','ZR_p_s0000','ZR_p_s0000_','p_set','p_branch','type_code','ZR_ij','ZR_ij_','nZR_ij','branch_A_r_ij_','branch_B_r_ij_','E1_gamma_z_','E1_gsplit_llr_','E1_gsplit_label_','E1_gsplit_llr__','n_gamma_z','gamma_z_');
%%%%%%%%;
...
test_loader_dexcluster_nonbinary_recluster_recursive_0(dir_A_out,prefix_base,E_array_base_,E_array_A_r_ij_,E_array_A_c_ij_,gamma,n_shuffle,p_set);
...
test_loader_dexcluster_nonbinary_recluster_recursive_0(dir_B_out,prefix_base,E_array_base_,E_array_B_r_ij_,E_array_B_c_ij_,gamma,n_shuffle,p_set);
