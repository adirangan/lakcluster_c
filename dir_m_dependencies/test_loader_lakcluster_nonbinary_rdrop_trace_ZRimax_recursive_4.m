...
test_loader_lakcluster_nonbinary_rdrop_trace_ZRimax_recursive_4( ...
 dir_trunk ...
,dir_out ...
,prefix_base ...
,E_array_base_ ...
...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('laknbr_trace_ZRmax',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering test_loader_lakcluster_nonbinary_rdrop_trace_ZRimax_recursive_4]: dir_out %s',dir_out)); end;
...
lpnext = -log(p_next);
if (p_next> p_use);
if (verbose>1); disp(sprintf(' %% %s --> stop (p_branch %0.6f --> p_next %0.6f > p_use %0.6f)',dir_out,p_branch,p_next,p_use)); end;
end;%if (p_branch>p_use);
if (p_next<=p_use);
if (verbose>1); disp(sprintf(' %% %s --> split (p_branch %0.6f --> p_next %0.6f < p_use %0.6f)',dir_out,p_branch,p_next,p_use)); end;
...
[out_xdrop_E_0_,out_trace_E_0_] = lakcluster_nonbinary_AAAA_rdrop_ver0(L_array_,gamma);
...
%%%%%%%%;
QR_E_avg_ = mean(QR_E_(:,2:end),2); QR_E_std_ = std(QR_E_(:,2:end),[],2);
ZR_E_ = (QR_E_ - repmat(QR_E_avg_,1,1+n_shuffle))./repmat(QR_E_std_,1,1+n_shuffle);
QC_E_avg_ = mean(QC_E_(:,2:end),2); QC_E_std_ = std(QC_E_(:,2:end),[],2);
ZC_E_ = (QC_E_ - repmat(QC_E_avg_,1,1+n_shuffle))./repmat(QC_E_std_,1,1+n_shuffle);
%%%%%%%%;
...
if strcmp(type_code,'split');
if (verbose>1); disp(sprintf(' %% %s --> split (lp_R_max %0.2f lp_R_F_max %0.2f --> p_branch %0.6f --> p_next %0.6f)',dir_out,lp_R_max,lp_R_F_max,exp(-lp_R_F_max),p_next)); end;
dir_A_out = sprintf('%s/dir_A',dir_out); if (~exist(dir_A_out,'dir')); disp(sprintf(' %% mkdir %s',dir_A_out)); mkdir(dir_A_out); end;
E_array_A_r_ij_ = E_array_r_ij_(branch_A_r_ij_); E_array_A_c_ij_ = E_array_c_ij_;
if (verbose>1); disp(sprintf(' %% %s <-- calling (n_sample = %d)',dir_A_out,length(E_array_A_r_ij_))); end;
test_loader_lakcluster_nonbinary_rdrop_trace_ZRimax_recursive_4( ...
