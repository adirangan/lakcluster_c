function lisa_prs_collect_ver7(disease_prefix,cl_num_arm1,cl_num_arm2,flag_reverse_use,flag_rerun);
% Assuming that arm1 is cl4. ;
% Using data from /data/rangan/dir_bcc/dir_BIP32_loo_data/dir_icuk_scores ;
% try: ;
%{
  flag_rerun = 1;
  disease_prefix_ = {'SCZ'};%disease_prefix_ = {'MDD','SCZ'};
  for nd=1:length(disease_prefix_); for cl_num_arm1=[4]; for cl_num_arm2=[2,1,3]; for flag_reverse_use=[0:1];
  lisa_prs_collect_ver7(disease_prefix_{nd},cl_num_arm1,cl_num_arm2,flag_reverse_use,flag_rerun);
  end;end;end;end;%for nd=1:length(disease_prefix_); for cl_num_arm1=[4]; for cl_num_arm2=[1,2,3]; for flag_reverse_use=[0:1];
  %}
...
fname = sprintf('%s/%s_imp_%s.hg19.ch.fl.out.dosage_imp_scores.txt.S%s.profile',dir_SCZ_MDD,disease_prefix,tmp_study_name,str_prs_{nprs}); fcheck(fname); 
fid = fopen(fname);
...
%%%%%%%%;
%flag_rerun = 0;
fname = sprintf('%s/prs_collect_ver7_%s_trn%d_tst%d_ni%.3d_ni%.3d.jpg',dir_out_s0000_pca_r0_arm1,disease_prefix,cl_num_arm1,cl_num_arm2,ni_r0_arm2,ni_r1_arm2);
...
if strcmp(disease_prefix,'MDD'); cauclim_ = [0.500,0.625]; end;
if strcmp(disease_prefix,'SCZ'); cauclim_ = [0.500,0.750]; end;
