function test_loader_cluster_wrap_0(dir_trunk,label_A_,E_rank_,I_rank_,prefix_normalization,xeta_,n_rank_xeta_sub);
%{
  % try: ;
  ...
  test_loader_cluster_wrap_0(dir_trunk,label_A_,E_rank_,I_rank_,prefix_normalization,[],[]);
  ...
  %%%%%%%%;
  n_rank_xeta = 16;
  for prefix_covariate_ = {'Phr'};%for prefix_covariate_ = {'127','012','RRR'};%for prefix_covariate_ = {'Phr','127','012','RRR'};
  ...
  %%%%%%%%;
  n_rank_xeta_sub = 9;
  test_loader_cluster_wrap_0(dir_trunk,label_A_,E_rank_,I_rank_,prefix_normalization_{1},xeta_,n_rank_xeta_sub);
  %%%%%%%%;
...
if (nargin<6); n_rank_xeta_sub=9; end;
n_u = size(E_rank_,1);
n_CCOV = size(xeta_.C_rank_,2);
n_I_GENE = xeta_.n_I_GENE;
n_E_GENE = xeta_.n_E_GENE;
for nrank=1:n_rank_xeta_sub;
disp(sprintf(' %% nrank %d/%d',nrank,n_rank_xeta_sub));
...
clear tmp_EI_;
%%%%%%%%;
end;%for nrank=1:n_rank_xeta_sub;
