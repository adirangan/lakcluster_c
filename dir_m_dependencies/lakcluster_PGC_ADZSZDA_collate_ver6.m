function lakcluster_PGC_ADZSZDA_collate_ver6(dir_trunk,prefix,infix,p_threshold,rev_flag,nmds,mds_used,gamma_,B_MLT_,Ireq_,shuffle_num_)
% runs through the directories indexed by PGC_xfix_gen_ver6 and collects output_trace ;
% try with: 
%{
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/?*.[m] ./ ;
  !scp -rp rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/lakcluster_PGC_ADZSZDA_collate_ver6.m ./ ;
  dir_trunk = sprintf('%s',pwd); cl_num=3; p_threshold = 0.10;
  ...
  gamma=0.004;B_MLT=34;Ireq=0;shuffle_num=0;shuffle_num_=[0:64];
  lakcluster_PGC_ADZSZDA_collate_ver6(dir_trunk,prefix,infix,p_threshold,rev_flag,nmds,mds_used,[gamma],[B_MLT],[Ireq],shuffle_num_);
  %catch; disp(' error'); end;%try;
  end;end;%for mds_tab = 0:1;%for rev_flag = 0:1; 
  !scp -p PGC_cl3_maf01_lak_ADZSZDA_p??_?_m?_g???_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
  !scp -p PGC_cl3_maf01_dex_ADZSZDA_p??_?_m?_g???_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
  !scp -p PGC_cl3_maf01_p??_?_m?_g???_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
  !scp -p PGC_cl3_maf01_dex_ADZSZDA_p10_X_m3_g???_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
  !scp -p PGC_cl3_maf01_dex_ADZSZDA_p10_X_m3_g050_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
  !scp -p PGC_cl3_maf01_lak_ADZSZDA_p10_X_m3_g004_B34_n0_sXXXX.mat rangan@access1.cims.nyu.edu:/data/rangan/dir_bcc/dir_code_081717/dir_m/dir_PGC_cl3_maf01/ ;
...
xfix = PGC_xfix_gen_ver6(infix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num);
...
savename = sprintf('%s_%s.mat',prefix,PGC_savename_gen_ver6(infix,p_threshold,rev_flag,nmds,mds_used,gamma_,B_MLT_,Ireq_,shuffle_num_));
...
function [output_string,gamma_d] = PGC_xfix_gen_ver6(infix,p_threshold,rev_flag,nmds,mds_used,gamma,B_MLT,Ireq,shuffle_num);
...
if (length(infix)>0);
output_string = sprintf('%s_%s_%s_m%d_g%.3d_B%.2d_n%d_s%.4d',infix,p_str,rev_str,mds_code,gamma_d,B_MLT,Ireq,shuffle_num);
end;%if (length(infix)>0);
if (length(infix)==0);
output_string = sprintf('%s_%s_m%d_g%.3d_B%.2d_n%d_s%.4d',p_str,rev_str,mds_code,gamma_d,B_MLT,Ireq,shuffle_num);
end;%if (length(infix)==0);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [output_string,gamma_d] = PGC_savename_gen_ver6(infix,p_threshold,rev_flag,nmds,mds_used,gamma_,B_MLT_,Ireq_,shuffle_num_);
p_str = sprintf('p%.2d',floor(100*p_threshold));
if (rev_flag==1); rev_str = 'X'; else rev_str = 'D'; end;
mds_tmp = zeros(1,nmds); mds_tmp(mds_used)=1; mds_code = dot(2.^[0:nmds-1],mds_tmp);
gamma_d = floor(gamma_*1000);
if (length(infix)>0);
output_string = sprintf('%s_%s_%s_m%d',infix,p_str,rev_str,mds_code);
end;%if (length(infix)>0);
if (length(infix)==0);
output_string = sprintf('%s_%s_m%d',p_str,rev_str,mds_code);
end;%if (length(infix)==0);
