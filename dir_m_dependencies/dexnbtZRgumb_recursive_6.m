...
shuffle_num=0;
test_string = sprintf('%s_%s',prefix_base,xxxcluster_uADZSZDA_xfix_gen_ver1('dexnbtZRgumb',rev_flag,A_n_rind_,Z_n_rind_,T_n_cind,GLOBAL_TEST_sparse,gamma,B_MLT,Ireq,shuffle_num));
...
if (verbose>0); disp(sprintf(' %% [entering dexnbtZRgumb_recursive_6]: dir_out %s',dir_out)); end;
...
r_rem_E_ = trace_E_{1}(:,1+1); tmp_E_ij_ = find( (r_rem_E_>=0*n_member_lob) & ((n_u - r_rem_E_)>=0*n_member_lob) );
[~,~,~,tmp_p_opt,tmp_p_emp] = gumbel_fit(max(ZR_(tmp_E_ij_,2:end),[],1),max(ZR_(tmp_E_ij_,1)));
...
%%%%%%%%;
QR_avg_ = mean(QR_(:,2:end),2); QR_std_ = std(QR_(:,2:end),1,2); %<-- match c. ;
ZR_ = (QR_ - repmat(QR_avg_,1,1+n_shuffle))./max(1e-12,repmat(QR_std_,1,1+n_shuffle)); %<-- match c. ;
QC_avg_ = mean(QC_(:,2:end),2); QC_std_ = std(QC_(:,2:end),1,2); %<-- match c. ;
ZC_ = (QC_ - repmat(QC_avg_,1,1+n_shuffle))./max(1e-12,repmat(QC_std_,1,1+n_shuffle)); %<-- match c. ;
%%%%%%%%;
...
if ( exist(tmp_fname_trace_F,'file') &  exist(tmp_fname_xdrop_F,'file') & ~flag_force_create);
out_trace_F_0_ = textread(tmp_fname_trace_F);
out_xdrop_F_0_ = textread(tmp_fname_xdrop_F);
end;%if ( exist(tmp_fname_trace_F,'file') &  exist(tmp_fname_xdrop_F,'file') & ~flag_force_create);
if (~exist(tmp_fname_trace_F,'file') | ~exist(tmp_fname_xdrop_F,'file') |  flag_force_create);
...
for nshuffle=1:n_shuffle;
tmp_fname_trace_F = sprintf('%s/out_trace_F_s%.4d.txt',dir_trace,nshuffle);
if ( exist(tmp_fname_trace_F,'file') & ~flag_force_create);
out_trace_F_0_ = textread(tmp_fname_trace_F);
end;%if ( exist(tmp_fname_trace_F,'file') & ~flag_force_create);
if (~exist(tmp_fname_trace_F,'file') |  flag_force_create);
if (verbose>2); disp(sprintf(' %% %d/%d: not found, creating',nshuffle,n_shuffle)); end;
rseed = cast(nshuffle,'uint32'); rseed = calllib('halfloop_lib','RSEED_adv8',rseed);
tmp_Q_ = zeros(n_u); [tmp_Q_,rseed] = calllib('halfloop_lib','array_orth_f',n_u,n_u,tmp_Q_,rseed); tmp_Q_ = transpose(tmp_Q_);
...
% Now calculate ZR_F. ;
%%%%%%%%;
QR_F_avg_ = mean(QR_F_(:,2:end),2); QR_F_std_ = std(QR_F_(:,2:end),1,2); %<-- match c. ;
ZR_F_ = (QR_F_ - repmat(QR_F_avg_,1,1+n_shuffle))./max(1e-12,repmat(QR_F_std_,1,1+n_shuffle)); %<-- match c. ;
QC_F_avg_ = mean(QC_F_(:,2:end),2); QC_F_std_ = std(QC_F_(:,2:end),1,2); %<-- match c. ;
ZC_F_ = (QC_F_ - repmat(QC_F_avg_,1,1+n_shuffle))./max(1e-12,repmat(QC_F_std_,1,1+n_shuffle)); %<-- match c. ;
%%%%%%%%;
