% Currently assumes that arm2 = cl4 ;
...
figure(n_figure); clf; n_figure = n_figure+1;
imagesc(transpose(prw_S_)*prw_S_,[0,+1]); colormap(colormap_beach());
set(gca,'XTick',1:n_prs,'XTickLabel',log10(prw_t_));
set(gca,'YTick',1:n_prs,'YTickLabel',log10(prw_t_));
xlabel('log10(threshold)'); ylabel('log10(threshold)'); title('dot-product');
colorbar; axis square;
end;%if flag_disp;
 %}
%%%%%%%%;
...
% load bim file. ;
%%%%%%%%%%%%%%%%;
%string_prefix = sprintf('PGC_cl%d_maf01',cl_num_arm2);
%dir__in = sprintf('%s/dir_%s',dir_trunk,string_prefix);
%fcheck(sprintf('%s/%s_bim.ext',dir__in,string_prefix));
%fid = fopen(sprintf('%s/%s_bim.ext',dir__in,string_prefix)); 
%bim_ = textscan(fid,'%d\t%s\t%d\t%d\t%c\t%c\t%s\t%f\t%f\t%f\t%f\n','headerlines',0); 
%fclose(fid);
n_snp = length(bim_{1});
%%%%%%%%;
ni=1;
bim_chr_ = bim_{ni}; ni=ni+1; % chromosome number;
bim_mid_ = bim_{ni}; ni=ni+1; % marker id ;
bim_gdi_ = bim_{ni}; ni=ni+1; % genetic distance ;
bim_pdi_ = bim_{ni}; ni=ni+1; % physical distance ;
bim_al1_ = bim_{ni}; ni=ni+1; % allele 1 ;
bim_al2_ = bim_{ni}; ni=ni+1; % allele 2 ;
bim_alt_ = bim_{ni}; ni=ni+1; % allele type ;
bim_ren_ = bim_{ni}; ni=ni+1; % relative entropy ;
bim_fal_ = bim_{ni}; ni=ni+1; % frequency of allele type ;
bim_fmi_ = bim_{ni}; ni=ni+1; % frequency of missing data ; 
bim_maf_ = bim_{ni}; ni=ni+1; % minor allele frequency ;
%%%%%%%%;
%{
bim_name_ = cell(n_snp,1);
for nsnp=1:n_snp;
if (mod(nsnp,1e5)==0); disp(sprintf(' %% nsnp %d/%d',nsnp,n_snp)); end;
bim_name_{nsnp} = sprintf('%d&%s&%d&s&%s',bim_chr_(nsnp),bim_mid_{nsnp},bim_pdi_(nsnp),bim_al1_(nsnp),bim_al2_(nsnp));
end;%for nsnp=1:n_snp;
 %}
%%%%%%%%%%%%%%%%;
% Note intersection. ;
%%%%%%%%%%%%%%%%;
disp(sprintf(' %% intersection prw_snp_ and bim_{2} %d prw__bp_ and bim_{4} %d',[length(intersect(prw_snp_,bim_{2})),length(intersect(prw__bp_,bim_{4}))]));
%%%%%%%%;
% Now construct approximation to bicluster vector. ;
% Here we assume that the allele types (i.e., nor, and, xor) correspond to the following weights: ;
% nor --> times 0 ?? ;
% xor --> times 1 ?? ;
% and --> times 2 ?? ;
%%%%%%%%;
%{
fname = sprintf('/data/rangan/dir_bcc/dir_PGC_20180304/dir_PGC_cl1_maf01_analyze/dir_PGC_cl1_maf01_dex_p25_D_m2r2_g004/dir_pca/pca_ni375_tst4_k2_B44_V_arm2_.mda'); fcheck(fname);
V_arm2_ = mda_read_r8(fname);
[~,tmp_ij_bim_,tmp_ij_prw_] = intersect(bim_name_,prw_name_);
prw_V_ = zeros(n_prw,1);
for nij=1:length(tmp_ij_prw_);
ij = tmp_ij_prw_(nij);
prw_name = prw_name_(ij);
if (mod(nij,500)==0); disp(sprintf(' %% nij %d/%d ij %d prw_name %s',nij,length(tmp_ij_prw_),ij,prw_name{1})); end;
tmp_ij_ = find(strcmp(bim_name_,prw_name));
for nij2=1:length(tmp_ij_);
ij2 = tmp_ij_(nij2);
if strcmp(bim_alt_(ij2),'nor'); prw_V_(ij) = prw_V_(ij) + 0*V_arm2_(ij2,1); end;
if strcmp(bim_alt_(ij2),'xor'); prw_V_(ij) = prw_V_(ij) + 1*V_arm2_(ij2,1); end;
if strcmp(bim_alt_(ij2),'and'); prw_V_(ij) = prw_V_(ij) + 2*V_arm2_(ij2,1); end;
end;%for nij2=1:length(tmp_ij_);
end;%for nij=1:length(tmp_ij_prw_);
prw_Vz_ = prw_V_/norm(prw_V_,'fro');
%%%%%%%%;
tmp_ij_ = find(prw_Vz_~=0);
for nprs=1:n_prs;
tmp_S_ = zeros(n_prw,1); tmp_S_(tmp_ij_) = prw_S_(tmp_ij_,nprs); tmp_S_ = tmp_S_/norm(tmp_S_,'fro');
disp(sprintf(' %% nprs %d C %0.6f',nprs,dot(tmp_S_,prw_Vz_)));
end;%for nprs=1:n_prs;
 %}
...
ni=1;
fam_fam_ = fam_{ni}; ni=ni+1;
fam_iid_ = fam_{ni}; ni=ni+1;
fam_pat_ = fam_{ni}; ni=ni+1;
fam_mat_ = fam_{ni}; ni=ni+1;
fam_sex_ = fam_{ni}; ni=ni+1;
fam_phe_ = fam_{ni}; ni=ni+1;
fam_dir_ = fam_{ni}; ni=ni+1;
...
%%%%%%%%%%%%%%%%;
disp(sprintf('mr_D error: %0.16f',norm(cast(mr_D_,'double')-cast(fam_phe_==2,'double'))));
disp(sprintf('mr_X error: %0.16f',norm(cast(mr_X_,'double')-cast(fam_phe_==1,'double'))));

%%%%%%%%%%%%%%%%;
% build libraries for cauc p-value ;
%%%%%%%%%%%%%%%%;
[cauc02_avg_,cauc02_min_] = p_cauc_0(mds_sort_(find(mr_X_),1: 2),mds_sort_(find(mr_D_),1: 2),zeros(sum(mr_X_),1),zeros(sum(mr_D_),1),1024*8);
[cauc06_avg_,cauc06_min_] = p_cauc_0(mds_sort_(find(mr_X_),1: 6),mds_sort_(find(mr_D_),1: 6),zeros(sum(mr_X_),1),zeros(sum(mr_D_),1),1024*8);
[cauc19_avg_,cauc19_min_] = p_cauc_0(mds_sort_(find(mr_X_),[1:6,19]),mds_sort_(find(mr_D_),[1:6,19]),zeros(sum(mr_X_),1),zeros(sum(mr_D_),1),1024*8);
[cauc20_avg_,cauc20_min_] = p_cauc_0(mds_sort_(find(mr_X_),1:20),mds_sort_(find(mr_D_),1:20),zeros(sum(mr_X_),1),zeros(sum(mr_D_),1),1024*8);
...
prs_sco_{nprs} = prs_{nprs}{ni}; ni=ni+1;
...
%%%%%%%%%%%%%%%%;
disp(sprintf(' %% nprs %d: %s: intersection %d',nprs,fname,length(intersect(prs_name_{nprs},fam_name_))));
%%%%%%%%%%%%%%%%;
[~,ifam_,iprs_{nprs}] = intersect(fam_name_,prs_name_{nprs},'stable'); if (length(ifam_)<n_patient_F); disp('Warning! fam_name_ not a subset of prs_name_{nprs}'); end; 
disp(sprintf('phe error: %0.16f',norm(cast(fam_phe_,'double')-cast(prs_phe_{nprs}(iprs_{nprs}),'double'))));
disp(sprintf(' c1 error: %0.16f',norm(mds_sort_(:, 1)-prs__c1_{nprs}(iprs_{nprs}))));
disp(sprintf(' c2 error: %0.16f',norm(mds_sort_(:, 2)-prs__c2_{nprs}(iprs_{nprs}))));
disp(sprintf(' c3 error: %0.16f',norm(mds_sort_(:, 3)-prs__c3_{nprs}(iprs_{nprs}))));
disp(sprintf(' c4 error: %0.16f',norm(mds_sort_(:, 4)-prs__c4_{nprs}(iprs_{nprs}))));
disp(sprintf(' c5 error: %0.16f',norm(mds_sort_(:, 5)-prs__c5_{nprs}(iprs_{nprs}))));
disp(sprintf(' c6 error: %0.16f',norm(mds_sort_(:, 6)-prs__c6_{nprs}(iprs_{nprs}))));
disp(sprintf('c19 error: %0.16f',norm(mds_sort_(:,19)-prs_c19_{nprs}(iprs_{nprs}))));
%%%%%%%%%%%%%%%%;
...
auc02_prs_(nprs) = cauc_0(prs_sco_{nprs}(iprs_{nprs}(find(mr_X_))),prs_sco_{nprs}(iprs_{nprs}(find(mr_D_))),mds_sort_(find(mr_X_),1: 2),mds_sort_(find(mr_D_),1: 2));
logp_auc02_prs_(nprs) = log(max(1,length(find(cauc02_avg_>auc02_prs_(nprs)))/length(cauc02_avg_)));
auc06_prs_(nprs) = cauc_0(prs_sco_{nprs}(iprs_{nprs}(find(mr_X_))),prs_sco_{nprs}(iprs_{nprs}(find(mr_D_))),mds_sort_(find(mr_X_),1: 6),mds_sort_(find(mr_D_),1: 6));
logp_auc06_prs_(nprs) = log(max(1,length(find(cauc06_avg_>auc06_prs_(nprs)))/length(cauc06_avg_)));
auc19_prs_(nprs) = cauc_0(prs_sco_{nprs}(iprs_{nprs}(find(mr_X_))),prs_sco_{nprs}(iprs_{nprs}(find(mr_D_))),mds_sort_(find(mr_X_),[1:6,19]),mds_sort_(find(mr_D_),[1:6,19]));
logp_auc19_prs_(nprs) = log(max(1,length(find(cauc19_avg_>auc19_prs_(nprs)))/length(cauc19_avg_)));
auc20_prs_(nprs) = cauc_0(prs_sco_{nprs}(iprs_{nprs}(find(mr_X_))),prs_sco_{nprs}(iprs_{nprs}(find(mr_D_))),mds_sort_(find(mr_X_),1:20),mds_sort_(find(mr_D_),1:20));
...
n_r0_t = 21; n_r1_t = 21;
if (cl_num_arm1==1); ni_r0_arm2_ = [375]; ni_r1_arm2_ = [400]; end;
if (cl_num_arm1==2); ni_r0_arm2_ = [400]; ni_r1_arm2_ = [450]; end;
if (cl_num_arm1==3); ni_r0_arm2_ = [400]; ni_r1_arm2_ = [450]; end;
...
tmp_mds_x_ = mds_sort_(tmp_x_,[1:6,19]); tmp_mds_d_ = mds_sort_(tmp_d_,[1:6,19]);
...
tmp_mds_x_ = mds_sort_(tmp_x_(tmp_xt_),[1:6,19]); tmp_mds_d_ = mds_sort_(tmp_d_(tmp_dt_),[1:6,19]);
...
fname = sprintf('%s/prs_collect_trn%d_tst%d_ni%.3d_ni%.3d.jpg',dir_out_s0000_pca_r0_arm1,cl_num_arm1,cl_num_arm2,ni_r0_arm2,ni_r1_arm2);
print('-djpeg',fname);
