...
npats_A = length(A_{1}); npats_B = length(B_{1}); assert(length(intersect(A_{1},B_{1}))==0);
...
for np=1:npats_A; 
rij_cas(np) = strcmp(A_{1}{np}(1:3),'cas'); 
rij_con(np) = strcmp(A_{1}{np}(1:3),'con'); 
rij_fam(np) = strcmp(A_{1}{np}(1:3),'fam'); 
end;%for np=1:npats_A;
for np=1:npats_B; 
rij_cas(npats_A+np) = strcmp(B_{1}{np}(1:3),'cas'); 
rij_con(npats_A+np) = strcmp(B_{1}{np}(1:3),'con'); 
rij_fam(npats_A+np) = strcmp(B_{1}{np}(1:3),'fam'); 
...
plot3(mds(find(rij_cas),v1),mds(find(rij_cas),v2),mds(find(rij_cas),v3),'r.',mds(find(rij_con),v1),mds(find(rij_con),v2),mds(find(rij_con),v3),'b.'); 
axis vis3d;
