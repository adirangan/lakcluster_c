function error_code = lisa_allele_mismatch_ver0(a1,a2,b1,b2,f_a_and,f_a_xor,f_a_nor,f_b_and,f_b_xor,f_b_nor,ent_cutoff);
...
flag_papb_match = ((p_a_opt-0.5)*(p_b_opt-0.5)>0);
flag_paqb_match = ((p_a_opt-0.5)*(q_b_opt-0.5)>0);
flag_papb_close = (abs(p_a_opt-p_b_opt)<0.05);
flag_paqb_close = (abs(p_a_opt-q_b_opt)<0.05);
flag_paqa_close = (abs(p_a_opt-q_a_opt)<0.05);
flag_pbqb_close = (abs(p_b_opt-q_b_opt)<0.05);
end;%if flag_f;

allele_original_ = zeros(84,1); 
allele_original_(65) = 1; %<-- A ;
allele_original_(67) = 2; %<-- C ;
allele_original_(71) = 3; %<-- G ;
allele_original_(84) = 4; %<-- T ;
allele_complement_ = zeros(84,1); 
allele_complement_(65) = 4; %<-- A<-T ;
allele_complement_(67) = 3; %<-- C<-G ;
allele_complement_(71) = 2; %<-- G<-C ;
allele_complement_(84) = 1; %<-- T<-A ;
%%%%%%%%;
...
if (  flag_paqa_close | flag_pbqb_close ) ; error_code = error_code_ambiguous; 
 else;
if ( flag_papb_match ) ; error_code = error_code_null; end;
if ( flag_paqb_match ) ; error_code = error_code_dominance; end;
 end;% if too close to call;
...
if flag_f==1; if flag_p_hweq;
if (  flag_papb_close ); error_code = error_code_null; end; 
if (  flag_papb_match & ~flag_papb_close ) ; error_code = error_code_null ; end;
if ( ~flag_papb_match & ~flag_papb_close ) ; error_code = error_code_ambiguous ; end;
end;end;%if flag_f==1; if flag_p_hweq;
...
if flag_f==1; if flag_p_hweq;
if (  flag_paqb_close ); error_code = error_code_dominance; end; 
if (  flag_paqb_match & ~flag_paqb_close ) ; error_code = error_code_dominance ; end;
if ( ~flag_paqb_match & ~flag_paqb_close ) ; error_code = error_code_ambiguous ; end;
end;end;%if flag_f==1; if flag_p_hweq;
...
if flag_f==1; if flag_p_hweq;
if (  flag_papb_close ); error_code = error_code_synonym; end; 
if (  flag_papb_match & ~flag_papb_close ) ; error_code = error_code_synonym ; end;
if ( ~flag_papb_match & ~flag_papb_close ) ; error_code = error_code_ambiguous ; end;
end;end;%if flag_f==1; if flag_p_hweq;
...
if flag_f==1; if flag_p_hweq;
if (  flag_paqb_close ); error_code = error_code_both; end; 
if (  flag_paqb_match & ~flag_paqb_close ) ; error_code = error_code_both ; end;
if ( ~flag_paqb_match & ~flag_paqb_close ) ; error_code = error_code_ambiguous ; end;
end;end;%if flag_f==1; if flag_p_hweq;
