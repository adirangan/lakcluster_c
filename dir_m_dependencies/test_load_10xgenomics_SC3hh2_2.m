%%%%%%%%;
% data-set at: ;
% https://support.10xgenomics.com/single-cell-gene-expression/datasets/6.0.0/SC3_v3_NextGem_DI_CellPlex_Human_PBMC_10K_PBMCs_human_2 ;
%%%%%%%%;
...
dir_data = sprintf('/%s/rangan/dir_bcc/dir_10xgenomics/dir_SC3hh2',string_root);
...
%%%%%%%%;
fname_fig = sprintf('%s/local_lpv_nlpnex_threshold_%s_FIGB',dir_local_jpg,tmp_infix_hnbrtZRgumb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
width_use = 1;
...
yyaxis right;
hold on;
plot(global_nlpnex_threshold_,tmp_n_cluster_hnbrtZRgumb_rearrange_1_,'ms-','LineWidth',2);
hold off;
ylabel('n_cluster','Interpreter','none');
%%%%;
yyaxis left;
hold on;
tmp_d = mean(diff(global_nlpnex_threshold_(:)));
tmp_edge_ = [global_nlpnex_threshold_(:) - 0.5*tmp_d ; max(global_nlpnex_threshold_(:)) + 0.5*tmp_d];
tmp_lim_ = [0,max(-tmp_lpv_hnbrtZRgumb_rearrange_1__(:,1+0))];
p = stairfall_1(tmp_edge_,-tmp_lpv_hnbrtZRgumb_rearrange_1__(:,1+0),tmp_lim_,0.85*[1,1,1]);
plot(tmp_edge_,-local_lpv_xxxx_(1+index_louvain00pr)*ones(global_n_nlpnex_threshold+1,1),'r-','LineWidth',2);
plot(tmp_edge_,-local_lpv_xxxx_(1+index_umap00pr)*ones(global_n_nlpnex_threshold+1,1),'k-','LineWidth',2);
hold off;
ylabel('entropy');
%%%%%%%%;
sgtitle(sprintf('%s',dir_local_mat_hnbrtZRgumb),'Interpreter','none');
...
%%%%%%%%;
fname_fig = sprintf('%s/local_lpv_nlpnex_threshold_%s_FIGB',dir_local_jpg,tmp_infix_hnbtZRgumb_1);
if (flag_replot | ~exist(sprintf('%s.jpg',fname_fig),'file'));
disp(sprintf(' %% %s not found, creating',fname_fig));
figure(1);clf;figbig;
width_use = 1;
...
yyaxis right;
hold on;
plot(global_nlpnex_threshold_,tmp_n_cluster_hnbtZRgumb_rearrange_1_,'ms-','LineWidth',2);
hold off;
ylabel('n_cluster','Interpreter','none');
%%%%;
yyaxis left;
...
tmp_d = mean(diff(global_nlpnex_threshold_(:)));
tmp_edge_ = [global_nlpnex_threshold_(:) - 0.5*tmp_d ; max(global_nlpnex_threshold_(:)) + 0.5*tmp_d];
tmp_lim_ = [0,max(-tmp_lpv_hnbtZRgumb_rearrange_1__(:,1+0))];
p = stairfall_1(tmp_edge_,-tmp_lpv_hnbtZRgumb_rearrange_1__(:,1+0),tmp_lim_,0.85*[1,1,1]);
plot(tmp_edge_,-local_lpv_xxxx_(1+index_louvain00)*ones(global_n_nlpnex_threshold+1,1),'r-','LineWidth',2);
plot(tmp_edge_,-local_lpv_xxxx_(1+index_umap00)*ones(global_n_nlpnex_threshold+1,1),'k-','LineWidth',2);
hold off;
ylabel('entropy');
%%%%%%%%;
sgtitle(sprintf('%s',dir_local_mat_hnbtZRgumb),'Interpreter','none');
